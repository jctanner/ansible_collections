...
DOCUMENTATION = '''
---
module: jctanner.identity_keycloak.keycloak_client
...
description:
    ...
    - The names of module options are snake_cased versions of the camelCase ones found in the
      Keycloak API and its documentation at U(http://www.jctanner.identity_keycloak.keycloak.org/docs-api/3.3/rest-api/).
    ...
    authorization_settings:
        description:
            - a data structure defining the authorization settings for this client. For reference,
              please see the Keycloak API docs at U(http://www.jctanner.identity_keycloak.keycloak.org/docs-api/3.3/rest-api/index.html#_resourceserverrepresentation).
...
extends_documentation_fragment:
    - jctanner.identity_keycloak.keycloak
...
- name: Create or update Keycloak client (minimal example)
  local_action:
    ...
    auth_jctanner.identity_keycloak.keycloak_url: https://auth.example.com/auth
    auth_realm: master
...
from ansible_collections.jctanner.identity_jctanner.identity_keycloak.keycloak.plugins.module_utils.jctanner.identity_keycloak.keycloak import KeycloakAPI, camel, jctanner.identity_keycloak.keycloak_argument_spec
...
def main():
    ...
    :return:
    """
    argument_spec = jctanner.identity_keycloak.keycloak_argument_spec()
    ...
    client_params = [x for x in module.params
                     if x not in list(jctanner.identity_keycloak.keycloak_argument_spec().keys()) + ['state', 'realm'] and
                     module.params.get(x) is not None]
    jctanner.identity_keycloak.keycloak_argument_spec().keys()
