...
DOCUMENTATION = '''
---
module: jctanner.identity_keycloak.keycloak_clienttemplate
...
- The Keycloak REST API defines further fields (namely I(bearerOnly), I(consentRequired), I(standardFlowEnabled),
  ...
  I(frontchannelLogout)) which, while available with jctanner.identity_keycloak.keycloak_client, do not have any effect on
...
def main():
    ...
    clientt_params = [x for x in module.params
                      if x not in ['state', 'auth_jctanner.identity_keycloak.keycloak_url', 'auth_client_id', 'auth_realm',
