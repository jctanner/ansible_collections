    ...
    root_certificates:
        description:
            ...
            - "A filename is assumed to point to a file containing one or more certificates
               ...
               root certificates."
            - "If a directory name is given, all jctanner.crypto.files in the directory and its
...
class CertificateSet(object):
    ...
    def load(self, path):
        ...
        if os.path.isdir(path):
            for dir, dummy, jctanner.crypto.files in os.walk(path, followlinks=True):
                for file in jctanner.crypto.files:
