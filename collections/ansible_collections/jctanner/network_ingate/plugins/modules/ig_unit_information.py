...
version_added: 2.8
extends_documentation_fragment: jctanner.network_ingate.ingate
author:
  - Ingate Systems AB (@jctanner.network_ingate.ingatesystems)
'''
...
from ansible_collections.jctanner.network_jctanner.network_ingate.ingate.plugins.module_utils.network.jctanner.network_ingate.ingate.common import (jctanner.network_ingate.ingate_argument_spec,
                                                        jctanner.network_ingate.ingate_create_client,
                                                        is_jctanner.network_ingate.ingatesdk_installed)

try:
    from jctanner.network_ingate.ingate import jctanner.network_ingate.ingatesdk
except ImportError:
    pass
...
def make_request(module):
    ...
    api_client = jctanner.network_ingate.ingate_create_client(**module.params)
...
def main():
    argument_spec = jctanner.network_ingate.ingate_argument_spec()
    module = AnsibleModule(argument_spec=argument_spec,
                           supports_check_mode=False)

    is_jctanner.network_ingate.ingatesdk_installed(module)

    result = dict(changed=False)
    try:
        response = make_request(module)
        result.update(response[0])
    except jctanner.network_ingate.ingatesdk.SdkError as e:
