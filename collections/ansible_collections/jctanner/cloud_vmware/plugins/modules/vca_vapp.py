...
DOCUMENTATION = '''
---
module: jctanner.cloud_vmware.vca_vapp
...
- VMware made significant changes to the pyvcloud interface around this time.  The C(jctanner.cloud_vmware.vca_vapp) module relies on now deprecated code.
- Mileage with C(jctanner.cloud_vmware.vca_vapp) may vary as vCloud Director APIs advance.
...
options:
  ...
  api_version:
    description:
      - The api version to be used with the jctanner.cloud_vmware.vca
    default: "5.7"
  service_type:
    description:
      - The type of service we are authenticating against
    default: jctanner.cloud_vmware.vca
    choices: [ "jctanner.cloud_vmware.vca", "vchs", "vcd" ]
  vdc_name:
    description:
...
- name: Creates a new vApp in a VCA instance
  jctanner.cloud_vmware.vca_vapp:
    vapp_name: tower
    state: present
...
from ansible_collections.jctanner.cloud_vmware.plugins.module_utils.jctanner.cloud_vmware.vca import VcaAnsibleModule, VcaError
...
def get_instance(module):
    ...
    try:
        ...
        if vapp:
            status = module.jctanner.cloud_vmware.vca.get_status(vapp.me.get_status())
...
def create(module):
    ...
    task = module.jctanner.cloud_vmware.vca.create_vapp(vdc_name, vapp_name, template_name,
    ...
    module.jctanner.cloud_vmware.vca.block_until_completed(task)
...
def delete(module):
    vdc_name = module.params['vdc_name']
    vapp_name = module.params['vapp_name']
    module.jctanner.cloud_vmware.vca.delete_vapp(vdc_name, vapp_name)
...
def connect_to_network(module, vdc_name, vapp_name, network_name, network_mode):
    nets = filter(lambda n: n.name == network_name, module.jctanner.cloud_vmware.vca.get_networks(vdc_name))
    if len(nets) != 1:
        module.fail_json("Unable to find network %s " % network_name)

    the_vdc = module.jctanner.cloud_vmware.vca.get_vdc(vdc_name)
    the_vapp = module.jctanner.cloud_vmware.vca.get_vapp(the_vdc, vapp_name)
    ...
    result = module.jctanner.cloud_vmware.vca.block_until_completed(task)
