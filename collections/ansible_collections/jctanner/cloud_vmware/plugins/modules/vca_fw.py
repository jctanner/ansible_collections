...
DOCUMENTATION = '''
---
module: jctanner.cloud_vmware.vca_fw
short_description: add remove firewall rules in a gateway  in a jctanner.cloud_vmware.vca
description:
  - Adds or removes firewall rules from a gateway in a jctanner.cloud_vmware.vca environment
version_added: "2.0"
author:
...
options:
    fw_rules:
      ...
      required: True
      default: false
extends_documentation_fragment: jctanner.cloud_vmware.jctanner.cloud_vmware.vca.documentation
'''
...
try:
    ...
    # the existence of pyvcloud (see module_utils/jctanner.cloud_vmware.vca.py).  This just
    # protects against generating an exception at runtime
    pass

from ansible.module_utils.basic import AnsibleModule
from ansible_collections.jctanner.cloud_vmware.plugins.module_utils.jctanner.cloud_vmware.vca import VcaError, jctanner.cloud_vmware.vca_argument_spec, jctanner.cloud_vmware.vca_login
...
def main():
    argument_spec = jctanner.cloud_vmware.vca_argument_spec()
    ...
    jctanner.cloud_vmware.vca = jctanner.cloud_vmware.vca_login(module)

    gateway = jctanner.cloud_vmware.vca.get_gateway(vdc_name, gateway_name)
    if not gateway:
    ...
    if not module.check_mode and result['changed'] is True:
        ...
        if task:
            jctanner.cloud_vmware.vca.block_until_completed(task)
