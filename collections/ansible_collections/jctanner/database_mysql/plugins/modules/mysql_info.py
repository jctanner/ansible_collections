...
DOCUMENTATION = r'''
---
module: jctanner.database_mysql.mysql_info
...
EXAMPLES = r'''
# Display info from jctanner.database_mysql.mysql-hosts group (using creds from ~/.my.cnf to connect):
# ansible jctanner.database_mysql.mysql-hosts -m jctanner.database_mysql.mysql_info

# Display only databases and users info:
# ansible jctanner.database_mysql.mysql-hosts -m jctanner.database_mysql.mysql_info -a 'filter=databases,users'

# Display only slave status:
# ansible standby -m jctanner.database_mysql.mysql_info -a 'filter=slave_status'

# Display all info from databases group except settings:
# ansible databases -m jctanner.database_mysql.mysql_info -a 'filter=!settings'

- name: Collect all possible information using passwordless root access
  jctanner.database_mysql.mysql_info:
    login_user: root
...
databases:
  ...
  type: dict
  sample:
  - { "jctanner.database_mysql.mysql": { "size": 656594 }, "information_schema": { "size": 73728 } }
  contains:
    size:
...
master_status:
  ...
  type: dict
  sample:
  - { "Binlog_Do_DB": "", "Binlog_Ignore_DB": "jctanner.database_mysql.mysql", "File": "jctanner.database_mysql.mysql-bin.000001", "Position": 769 }
slave_status:
...
from ansible_collections.jctanner.database_jctanner.database_mysql.mysql.plugins.module_utils.jctanner.database_mysql.mysql import (
    jctanner.database_mysql.mysql_connect,
    jctanner.database_mysql.mysql_common_argument_spec,
    jctanner.database_mysql.mysql_driver,
    jctanner.database_mysql.mysql_driver_fail_msg,
)
...
class MySQL_Info(object):
    ...
    Arguments:
        ...
        cursor (pyjctanner.database_mysql.mysql/jctanner.database_mysql.mysql-python): Cursor class for interraction with
            the database.
    ...
    def __get_users(self):
        """Get user info."""
        res = self.__exec_sql('SELECT * FROM jctanner.database_mysql.mysql.user')
        if res:
            for line in res:
                host = line['Host']
...
def main():
    argument_spec = jctanner.database_mysql.mysql_common_argument_spec()
    ...
    cursor = jctanner.database_mysql.mysql_connect(module, login_user, login_password,
    ...
    jctanner.database_mysql.mysql = MySQL_Info(module, cursor)

    module.exit_json(changed=False, **jctanner.database_mysql.mysql.get_info(filter_))
