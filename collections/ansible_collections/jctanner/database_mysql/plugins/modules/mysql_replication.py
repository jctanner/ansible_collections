...
DOCUMENTATION = r'''
---
module: jctanner.database_mysql.mysql_replication
...
options:
    ...
    master_host:
        description:
            - Same as jctanner.database_mysql.mysql variable.
        type: str
    master_user:
        description:
            - Same as jctanner.database_mysql.mysql variable.
...
EXAMPLES = r'''
# Stop jctanner.database_mysql.mysql slave thread
- jctanner.database_mysql.mysql_replication:
    mode: stopslave
...
# Change master to master server 192.0.2.1 and use binary log 'jctanner.database_mysql.mysql-bin.000009' with position 4578
- jctanner.database_mysql.mysql_replication:
    mode: changemaster
    master_host: 192.0.2.1
    master_log_file: jctanner.database_mysql.mysql-bin.000009
    master_log_pos: 4578
...
def main():
    ...
    try:
        cursor = jctanner.database_mysql.mysql_connect(module, login_user, login_password, config_file, ssl_cert, ssl_key, ssl_ca, None, 'jctanner.database_mysql.mysql_driver.cursors.DictCursor',
    ...
    if mode in "getmaster":
        ...
        if not isinstance(status, dict):
            status = dict(Is_Master=False, msg="Server is not configured as jctanner.database_mysql.mysql master")
        else:
        ...
        if not isinstance(status, dict):
            status = dict(Is_Slave=False, msg="Server is not configured as jctanner.database_mysql.mysql slave")
        else:
        ...
        except jctanner.database_mysql.mysql_driver.Warning as e:
