...
DOCUMENTATION = r'''
---
module: jctanner.database_mysql.mysql_user
...
options:
  ...
  encrypted:
    description:
      - Indicate that the 'password' field is a `jctanner.database_mysql.mysql_native_password` hash.
    type: bool
    default: no
    version_added: "2.0"
  host:
  ...
  check_implicit_admin:
    description:
      - Check if jctanner.database_mysql.mysql allows login as root/nopassword before trying supplied credentials.
    type: bool
    default: no
    version_added: "1.3"
  update_password:
...
notes:
   - "MySQL server installs with default login_user of 'root' and no password. To secure this user
     ...
     the file."
   - Currently, there is only support for the `jctanner.database_mysql.mysql_native_password` encrypted password hash module.
...
- name: Example using login_unix_socket to connect to server
  jctanner.database_mysql.mysql_user:
    name: root
    password: abc123
    login_unix_socket: /var/run/jctanner.database_mysql.mysqld/jctanner.database_mysql.mysqld.sock
...
from ansible_collections.jctanner.database_jctanner.database_mysql.mysql.plugins.module_utils.database import SQLParseError
...
def user_exists(cursor, user, host, host_all):
    if host_all:
        cursor.execute("SELECT count(*) FROM jctanner.database_mysql.mysql.user WHERE user = %s", ([user]))
    else:
        cursor.execute("SELECT count(*) FROM jctanner.database_mysql.mysql.user WHERE user = %s AND host = %s", (user, host))
...
def user_mod(cursor, user, host, host_all, password, encrypted, new_priv, append_privs, module):
    ...
    for host in hostnames:
        ...
        if bool(password):
            ...
            # Get a list of valid columns in jctanner.database_mysql.mysql.user table to check if Password and/or authentication_string exist
            cursor.execute("""
                SELECT COLUMN_NAME FROM information_schema.COLUMNS
                WHERE TABLE_SCHEMA = 'jctanner.database_mysql.mysql' AND TABLE_NAME = 'user' AND COLUMN_NAME IN ('Password', 'authentication_string')
            ...
            cursor.execute("""
                ...
                FROM jctanner.database_mysql.mysql.user WHERE user = %%s AND host = %%s
            ...
            if current_pass_hash != encrypted_password:
                ...
                if old_user_mgmt:
                    ...
                    try:
                        cursor.execute("ALTER USER %s@%s IDENTIFIED WITH jctanner.database_mysql.mysql_native_password AS %s", (user, host, encrypted_password))
                        msg = "Password updated (new style)"
                    except (jctanner.database_mysql.mysql_driver.Error) as e:
                        # https://stackoverflow.com/questions/51600000/authentication-string-of-root-user-on-jctanner.database_mysql.mysql
                        ...
                        if e.args[0] == 1396:
                            cursor.execute(
                                ...
                                ('jctanner.database_mysql.mysql_native_password', encrypted_password, user, host)
                            )
...
def user_get_hostnames(cursor, user):
    cursor.execute("SELECT Host FROM jctanner.database_mysql.mysql.user WHERE user = %s", user)
...
def privileges_revoke(cursor, user, host, db_table, priv, grant_option):
    # Escape '%' since jctanner.database_mysql.mysql db.execute() uses a format string
...
def privileges_grant(cursor, user, host, db_table, priv):
    # Escape '%' since jctanner.database_mysql.mysql db.execute uses a format string and the
...
def main():
    ...
    try:
        if check_implicit_admin:
            try:
                cursor = jctanner.database_mysql.mysql_connect(module, 'root', '', config_file, ssl_cert, ssl_key, ssl_ca, db,
                                       connect_timeout=connect_timeout)
            except Exception:
                pass

        if not cursor:
            cursor = jctanner.database_mysql.mysql_connect(module, login_user, login_password, config_file, ssl_cert, ssl_key, ssl_ca, db,
                                   connect_timeout=connect_timeout)
    ...
    if state == "present":
        if user_exists(cursor, user, host, host_all):
            ...
            except (SQLParseError, InvalidPrivsError, jctanner.database_mysql.mysql_driver.Error) as e:
                module.fail_json(msg=to_native(e))
        else:
            if host_all:
