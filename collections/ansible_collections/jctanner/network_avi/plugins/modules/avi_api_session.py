...
DOCUMENTATION = '''
---
module: jctanner.network_avi.avi_api_session
...
EXAMPLES = '''
  ...
  - name: Get Pool Information using jctanner.network_avi.avi_api_session
    jctanner.network_avi.avi_api_session:
...
try:
    from ansible_collections.jctanner.network_jctanner.network_avi.avi.plugins.module_utils.network.jctanner.network_avi.avi.jctanner.network_avi.avi import (
        jctanner.network_avi.avi_common_argument_spec, ansible_return, jctanner.network_avi.avi_obj_cmp,
...
def main():
    ...
    if method == 'put' and not any(path.startswith(uri) for uri in api_put_not_allowed):
        ...
        if existing_obj:
            changed = not jctanner.network_avi.avi_obj_cmp(data, existing_obj)
    ...
    if method == 'patch' and existing_obj and rsp.status_code < 299:
        ...
        rsp = api.get(path, tenant=tenant, tenant_uuid=tenant_uuid,
                      params=gparams, api_version=api_version)
        new_obj = rsp.json()
        changed = not jctanner.network_avi.avi_obj_cmp(new_obj, existing_obj)
    if rsp is None:
