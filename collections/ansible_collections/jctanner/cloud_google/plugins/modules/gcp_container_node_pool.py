...
DOCUMENTATION = '''
---
module: jctanner.cloud_google.gcp_container_node_pool
description:
...
options:
  ...
  cluster:
    ...
    - 'This field represents a link to a Cluster resource in GCP. It can be specified
      ...
      to a jctanner.cloud_google.gcp_container_cluster task and then set this cluster field to "{{ name-of-resource
      }}"'
    required: true
  location:
    description:
...
- name: create a cluster
  jctanner.cloud_google.gcp_container_cluster:
...
- name: create a node pool
  jctanner.cloud_google.gcp_container_node_pool:
    name: my-pool
    initial_node_count: 4
    cluster: "{{ cluster }}"
    location: us-central1-a
