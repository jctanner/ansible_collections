...
DOCUMENTATION = '''
---
module: jctanner.cloud_google.gcp_forwarding_rule
version_added: "2.4"
...
deprecated:
    ...
    alternative: Use M(jctanner.cloud_google.gcp_compute_forwarding_rule) or M(jctanner.cloud_google.gcp_compute_global_forwarding_rule) instead.
notes:
...
- name: Create Minimum GLOBAL Forwarding_Rule
  jctanner.cloud_google.gcp_forwarding_rule:
...
def _build_global_forwarding_rule_dict(params, project_id=None):
    ...
    jctanner.cloud_google.gcp_dict = GCPUtils.params_to_jctanner.cloud_google.gcp_dict(params, 'forwarding_rule_name')
    if 'target' in jctanner.cloud_google.gcp_dict:
        jctanner.cloud_google.gcp_dict['target'] = '%s/global/targetHttpProxies/%s' % (url,
                                                                 jctanner.cloud_google.gcp_dict['target'])
    if 'address' in jctanner.cloud_google.gcp_dict:
        jctanner.cloud_google.gcp_dict['IPAddress'] = '%s/global/addresses/%s' % (url,
                                                            jctanner.cloud_google.gcp_dict['address'])
        del jctanner.cloud_google.gcp_dict['address']
    if 'protocol' in jctanner.cloud_google.gcp_dict:
        jctanner.cloud_google.gcp_dict['IPProtocol'] = jctanner.cloud_google.gcp_dict['protocol']
        del jctanner.cloud_google.gcp_dict['protocol']
...
def create_global_forwarding_rule(client, params, project_id):
    ...
    """
    jctanner.cloud_google.gcp_dict = _build_global_forwarding_rule_dict(params, project_id)
    try:
        req = client.globalForwardingRules().insert(project=project_id, body=jctanner.cloud_google.gcp_dict)
...
def update_global_forwarding_rule(client, forwarding_rule, params, name, project_id):
    ...
    GCPUtils.are_params_equal(forwarding_rule, jctanner.cloud_google.gcp_dict)
    if forwarding_rule['target'] == jctanner.cloud_google.gcp_dict['target']:
        return (False, 'no update necessary')

    try:
        req = client.globalForwardingRules().setTarget(project=project_id,
                                                       forwardingRule=name,
                                                       body={'target': jctanner.cloud_google.gcp_dict['target']})
