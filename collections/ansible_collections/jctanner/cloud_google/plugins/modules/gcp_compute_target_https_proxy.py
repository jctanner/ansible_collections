...
DOCUMENTATION = '''
---
module: jctanner.cloud_google.gcp_compute_target_https_proxy
description:
...
options:
  ...
  ssl_policy:
    ...
    - 'This field represents a link to a SslPolicy resource in GCP. It can be specified
      ...
      to a jctanner.cloud_google.gcp_compute_ssl_policy task and then set this ssl_policy field to "{{ name-of-resource
      }}"'
    required: false
    version_added: 2.8
  url_map:
    ...
    - 'This field represents a link to a UrlMap resource in GCP. It can be specified
      in two ways. First, you can place a dictionary with key ''selfLink'' and value
      of your resource''s selfLink Alternatively, you can add `register: name-of-resource`
      to a jctanner.cloud_google.gcp_compute_url_map task and then set this url_map field to "{{ name-of-resource
      }}"'
    required: true
...
- name: create a target https proxy
  jctanner.cloud_google.gcp_compute_target_https_proxy:
    name: test_object
    ssl_certificates:
    - "{{ sslcert }}"
    url_map: "{{ urlmap }}"
