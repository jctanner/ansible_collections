...
DOCUMENTATION = '''
---
module: jctanner.cloud_google.gcp_healthcheck
version_added: "2.4"
...
deprecated:
    ...
    alternative: >
      Use M(jctanner.cloud_google.gcp_compute_health_check), M(jctanner.cloud_google.gcp_compute_http_health_check) or
      M(jctanner.cloud_google.gcp_compute_https_health_check) instead.
author:
...
- name: Create Minimum HealthCheck
  jctanner.cloud_google.gcp_healthcheck:
...
from ansible_collections.jctanner.cloud_google.plugins.module_utils.jctanner.cloud_google.gcp import get_google_api_client, GCPUtils
...
def _build_healthcheck_dict(params):
    ...
    :rtype ``dict``
    """
    jctanner.cloud_google.gcp_dict = GCPUtils.params_to_jctanner.cloud_google.gcp_dict(params, 'healthcheck_name')
    if 'timeout' in jctanner.cloud_google.gcp_dict:
        jctanner.cloud_google.gcp_dict['timeoutSec'] = jctanner.cloud_google.gcp_dict['timeout']
        del jctanner.cloud_google.gcp_dict['timeout']

    if 'checkInterval' in jctanner.cloud_google.gcp_dict:
        jctanner.cloud_google.gcp_dict['checkIntervalSec'] = jctanner.cloud_google.gcp_dict['checkInterval']
        del jctanner.cloud_google.gcp_dict['checkInterval']

    if 'hostHeader' in jctanner.cloud_google.gcp_dict:
        jctanner.cloud_google.gcp_dict['host'] = jctanner.cloud_google.gcp_dict['hostHeader']
        del jctanner.cloud_google.gcp_dict['hostHeader']

    if 'healthcheckType' in jctanner.cloud_google.gcp_dict:
        del jctanner.cloud_google.gcp_dict['healthcheckType']
    return jctanner.cloud_google.gcp_dict
...
def create_healthcheck(client, params, project_id, resource_type='HTTP'):
    ...
    """
    jctanner.cloud_google.gcp_dict = _build_healthcheck_dict(params)
    try:
        resource, _ = _get_req_resource(client, resource_type)
        args = {'project': project_id, 'body': jctanner.cloud_google.gcp_dict}
...
def update_healthcheck(client, healthcheck, params, name, project_id,
    ...
    """
    jctanner.cloud_google.gcp_dict = _build_healthcheck_dict(params)
    ans = GCPUtils.are_params_equal(healthcheck, jctanner.cloud_google.gcp_dict)
    if ans:
        return (False, 'no update necessary')

    try:
        resource, entity_name = _get_req_resource(client, resource_type)
        args = {'project': project_id, entity_name: name, 'body': jctanner.cloud_google.gcp_dict}
