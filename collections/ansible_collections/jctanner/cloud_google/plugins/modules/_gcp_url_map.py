...
DOCUMENTATION = '''
---
module: jctanner.cloud_google.gcp_url_map
version_added: "2.4"
...
deprecated:
    ...
    alternative: Use M(jctanner.cloud_google.gcp_compute_url_map) instead.
options:
  url_map_name:
    description:
       - Name of the Url_Map.
...
- name: Create Minimal Url_Map
  jctanner.cloud_google.gcp_url_map:
...
from ansible_collections.jctanner.cloud_google.plugins.module_utils.jctanner.cloud_google.gcp import check_params, get_google_api_client, GCPUtils
...
def _build_url_map_dict(params, project_id=None):
    ...
    if project_id:
        url = GCPUtils.build_googleapi_url(project_id)
    jctanner.cloud_google.gcp_dict = GCPUtils.params_to_jctanner.cloud_google.gcp_dict(params, 'url_map_name')
    if 'defaultService' in jctanner.cloud_google.gcp_dict:
        jctanner.cloud_google.gcp_dict['defaultService'] = '%s/global/backendServices/%s' % (url,
                                                                       jctanner.cloud_google.gcp_dict['defaultService'])
    if 'pathMatchers' in jctanner.cloud_google.gcp_dict:
        jctanner.cloud_google.gcp_dict['pathMatchers'] = _build_path_matchers(jctanner.cloud_google.gcp_dict['pathMatchers'], project_id)
...
def create_url_map(client, params, project_id):
    ...
    """
    jctanner.cloud_google.gcp_dict = _build_url_map_dict(params, project_id)
    try:
        req = client.urlMaps().insert(project=project_id, body=jctanner.cloud_google.gcp_dict)
...
def update_url_map(client, url_map, params, name, project_id):
    ...
    ans = GCPUtils.are_params_equal(url_map, jctanner.cloud_google.gcp_dict)
    if ans:
        return (False, 'no update necessary')

    jctanner.cloud_google.gcp_dict['fingerprint'] = url_map['fingerprint']
    try:
        req = client.urlMaps().update(project=project_id,
                                      urlMap=name, body=jctanner.cloud_google.gcp_dict)
