...
DOCUMENTATION = '''
---
module: jctanner.cloud_google.gcp_target_proxy
version_added: "2.4"
...
deprecated:
    ...
    alternative: Use M(jctanner.cloud_google.gcp_compute_target_http_proxy) instead.
notes:
...
- name: Create Minimum HTTP Target_Proxy
  jctanner.cloud_google.gcp_target_proxy:
...
def _build_target_proxy_dict(params, project_id=None):
    ...
    jctanner.cloud_google.gcp_dict = GCPUtils.params_to_jctanner.cloud_google.gcp_dict(params, 'target_proxy_name')
    if 'urlMap' in jctanner.cloud_google.gcp_dict:
        jctanner.cloud_google.gcp_dict['urlMap'] = '%s/global/urlMaps/%s' % (url,
                                                       jctanner.cloud_google.gcp_dict['urlMap'])
...
def create_target_http_proxy(client, params, project_id):
    ...
    """
    jctanner.cloud_google.gcp_dict = _build_target_proxy_dict(params, project_id)
    try:
        req = client.targetHttpProxies().insert(project=project_id,
                                                body=jctanner.cloud_google.gcp_dict)
...
def update_target_http_proxy(client, target_proxy, params, name, project_id):
    ...
    GCPUtils.are_params_equal(target_proxy, jctanner.cloud_google.gcp_dict)
    if target_proxy['urlMap'] == jctanner.cloud_google.gcp_dict['urlMap']:
        return (False, 'no update necessary')

    try:
        req = client.targetHttpProxies().setUrlMap(project=project_id,
                                                   targetHttpProxy=name,
                                                   body={"urlMap": jctanner.cloud_google.gcp_dict['urlMap']})
