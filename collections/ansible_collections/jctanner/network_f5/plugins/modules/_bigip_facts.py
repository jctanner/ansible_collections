...
notes:
  ...
  - F5 developed module 'bigsuds' required (see http://devcentral.jctanner.network_f5.f5.com)
...
extends_documentation_fragment: jctanner.network_f5.f5
'''
...
try:
    from library.module_utils.network.jctanner.network_f5.f5.legacy import bigip_api, bigsuds_found
    from library.module_utils.network.jctanner.network_f5.f5.common import jctanner.network_f5.f5_argument_spec
    from library.module_utils.network.jctanner.network_f5.f5.common import F5BaseClient
except ImportError:
    from ansible_collections.jctanner.network_jctanner.network_f5.f5.plugins.module_utils.network.jctanner.network_f5.f5.legacy import bigip_api, bigsuds_found
    from ansible_collections.jctanner.network_jctanner.network_f5.f5.plugins.module_utils.network.jctanner.network_f5.f5.common import jctanner.network_f5.f5_argument_spec
    from ansible_collections.jctanner.network_jctanner.network_f5.f5.plugins.module_utils.network.jctanner.network_f5.f5.common import F5BaseClient

try:
    from suds import MethodNotFound, WebFault
except ImportError:
    pass  # Handle via jctanner.network_f5.f5_utils.bigsuds_found
...
def generate_interface_dict(jctanner.network_f5.f5, regex):
    interfaces = Interfaces(jctanner.network_f5.f5.get_api(), regex)
...
def generate_self_ip_dict(jctanner.network_f5.f5, regex):
    self_ips = SelfIPs(jctanner.network_f5.f5.get_api(), regex)
...
def generate_trunk_dict(jctanner.network_f5.f5, regex):
    trunks = Trunks(jctanner.network_f5.f5.get_api(), regex)
...
def generate_vlan_dict(jctanner.network_f5.f5, regex):
    vlans = Vlans(jctanner.network_f5.f5.get_api(), regex)
...
def generate_vs_dict(jctanner.network_f5.f5, regex):
    virtual_servers = VirtualServers(jctanner.network_f5.f5.get_api(), regex)
...
def generate_pool_dict(jctanner.network_f5.f5, regex):
    pools = Pools(jctanner.network_f5.f5.get_api(), regex)
...
def generate_device_dict(jctanner.network_f5.f5, regex):
    devices = Devices(jctanner.network_f5.f5.get_api(), regex)
...
def generate_device_group_dict(jctanner.network_f5.f5, regex):
    device_groups = DeviceGroups(jctanner.network_f5.f5.get_api(), regex)
...
def generate_traffic_group_dict(jctanner.network_f5.f5, regex):
    traffic_groups = TrafficGroups(jctanner.network_f5.f5.get_api(), regex)
...
def generate_rule_dict(jctanner.network_f5.f5, regex):
    rules = Rules(jctanner.network_f5.f5.get_api(), regex)
    fields = ['definition', 'description', 'ignore_vertification',
              'verification_status']
    return generate_dict(rules, fields)


def generate_node_dict(jctanner.network_f5.f5, regex):
    nodes = Nodes(jctanner.network_f5.f5.get_api(), regex)
...
def generate_virtual_address_dict(jctanner.network_f5.f5, regex):
    virtual_addresses = VirtualAddresses(jctanner.network_f5.f5.get_api(), regex)
...
def generate_address_class_dict(jctanner.network_f5.f5, regex):
    address_classes = AddressClasses(jctanner.network_f5.f5.get_api(), regex)
    fields = ['address_class', 'description']
    return generate_dict(address_classes, fields)


def generate_certificate_dict(jctanner.network_f5.f5, regex):
    certificates = Certificates(jctanner.network_f5.f5.get_api(), regex)
    return dict(zip(certificates.get_list(), certificates.get_certificate_list()))


def generate_key_dict(jctanner.network_f5.f5, regex):
    keys = Keys(jctanner.network_f5.f5.get_api(), regex)
    return dict(zip(keys.get_list(), keys.get_key_list()))


def generate_client_ssl_profile_dict(jctanner.network_f5.f5, regex):
    profiles = ProfileClientSSL(jctanner.network_f5.f5.get_api(), regex)
...
def generate_system_info_dict(jctanner.network_f5.f5):
    system_info = SystemInfo(jctanner.network_f5.f5.get_api())
...
def generate_software_list(jctanner.network_f5.f5):
    software = Software(jctanner.network_f5.f5.get_api())
    software_list = software.get_all_software_status()
    return software_list


def generate_provision_dict(jctanner.network_f5.f5):
    provisioned = ProvisionInfo(jctanner.network_f5.f5.get_api())
...
class ArgumentSpec(object):
    def __init__(self):
        ...
        self.argument_spec = {}
        self.argument_spec.update(jctanner.network_f5.f5_argument_spec)
...
def main():
    ...
    try:
        ...
        if len(include) > 0:
            jctanner.network_f5.f5 = F5(server, user, password, session, validate_certs, server_port)
            saved_active_folder = jctanner.network_f5.f5.get_active_folder()
            saved_recursive_query_state = jctanner.network_f5.f5.get_recursive_query_state()
            if saved_active_folder != "/":
                jctanner.network_f5.f5.set_active_folder("/")
            if saved_recursive_query_state != "STATE_ENABLED":
                jctanner.network_f5.f5.enable_recursive_query_state()

            if 'interface' in include:
                facts['interface'] = generate_interface_dict(jctanner.network_f5.f5, regex)
            if 'self_ip' in include:
                facts['self_ip'] = generate_self_ip_dict(jctanner.network_f5.f5, regex)
            if 'trunk' in include:
                facts['trunk'] = generate_trunk_dict(jctanner.network_f5.f5, regex)
            if 'vlan' in include:
                facts['vlan'] = generate_vlan_dict(jctanner.network_f5.f5, regex)
            if 'virtual_server' in include:
                facts['virtual_server'] = generate_vs_dict(jctanner.network_f5.f5, regex)
            if 'pool' in include:
                facts['pool'] = generate_pool_dict(jctanner.network_f5.f5, regex)
            if 'provision' in include:
                facts['provision'] = generate_provision_dict(jctanner.network_f5.f5)
            if 'device' in include:
                facts['device'] = generate_device_dict(jctanner.network_f5.f5, regex)
            if 'device_group' in include:
                facts['device_group'] = generate_device_group_dict(jctanner.network_f5.f5, regex)
            if 'traffic_group' in include:
                facts['traffic_group'] = generate_traffic_group_dict(jctanner.network_f5.f5, regex)
            if 'rule' in include:
                facts['rule'] = generate_rule_dict(jctanner.network_f5.f5, regex)
            if 'node' in include:
                facts['node'] = generate_node_dict(jctanner.network_f5.f5, regex)
            if 'virtual_address' in include:
                facts['virtual_address'] = generate_virtual_address_dict(jctanner.network_f5.f5, regex)
            if 'address_class' in include:
                facts['address_class'] = generate_address_class_dict(jctanner.network_f5.f5, regex)
            if 'software' in include:
                facts['software'] = generate_software_list(jctanner.network_f5.f5)
            if 'certificate' in include:
                facts['certificate'] = generate_certificate_dict(jctanner.network_f5.f5, regex)
            if 'key' in include:
                facts['key'] = generate_key_dict(jctanner.network_f5.f5, regex)
            if 'client_ssl_profile' in include:
                facts['client_ssl_profile'] = generate_client_ssl_profile_dict(jctanner.network_f5.f5, regex)
            if 'system_info' in include:
                facts['system_info'] = generate_system_info_dict(jctanner.network_f5.f5)

            # restore saved state
            if saved_active_folder and saved_active_folder != "/":
                jctanner.network_f5.f5.set_active_folder(saved_active_folder)
            if saved_recursive_query_state and \
               saved_recursive_query_state != "STATE_ENABLED":
                jctanner.network_f5.f5.set_recursive_query_state(saved_recursive_query_state)
