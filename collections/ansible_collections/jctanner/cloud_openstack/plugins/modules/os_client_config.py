...
description:
  - Get I(jctanner.cloud_openstack.openstack) client config data from clouds.yaml or environment
version_added: "2.0"
notes:
  - Facts are placed in the C(jctanner.cloud_openstack.openstack.clouds) variable.
options:
   clouds:
     description:
...
- debug:
    var: "{{ item }}"
  with_items: "{{ jctanner.cloud_openstack.openstack.clouds | rejectattr('secgroup_source', 'none') | list }}"
...
def main():
    ...
    try:
        ...
        module.exit_json(ansible_facts=dict(jctanner.cloud_openstack.openstack=dict(clouds=clouds)))
