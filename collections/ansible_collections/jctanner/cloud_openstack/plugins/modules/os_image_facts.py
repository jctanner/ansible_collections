...
notes:
    - Facts are placed in the C(jctanner.cloud_openstack.openstack_image) variable.
requirements:
    - "python >= 2.7"
...
- name: Show jctanner.cloud_openstack.openstack facts
  debug:
    var: jctanner.cloud_openstack.openstack_image
...
RETURN = '''
jctanner.cloud_openstack.openstack_image:
    description: has all the jctanner.cloud_openstack.openstack facts about the image
    ...
    try:
        if module.params['image']:
            ...
            module.exit_json(changed=False, ansible_facts=dict(
                jctanner.cloud_openstack.openstack_image=image))
        else:
            images = cloud.list_images()
            module.exit_json(changed=False, ansible_facts=dict(
                jctanner.cloud_openstack.openstack_image=images))
