...
from ansible_collections.jctanner.cloud_jctanner.cloud_openstack.openstack.plugins.module_utils.jctanner.cloud_openstack.openstack import (
    jctanner.cloud_openstack.openstack_find_nova_addresses, jctanner.cloud_openstack.openstack_cloud_from_module,
    jctanner.cloud_openstack.openstack_full_argument_spec, jctanner.cloud_openstack.openstack_module_kwargs)


def _exit_hostvars(module, cloud, server, changed=True):
    hostvars = cloud.get_jctanner.cloud_openstack.openstack_vars(server)
    module.exit_json(
        changed=changed, server=server, id=server.id, jctanner.cloud_openstack.openstack=hostvars)
...
def _check_ips(module, cloud, server):
    ...
    if floating_ip_pools or floating_ips:
        ips = jctanner.cloud_openstack.openstack_find_nova_addresses(server.addresses, 'floating')
        if not ips:
