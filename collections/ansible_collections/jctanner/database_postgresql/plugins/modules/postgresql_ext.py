...
DOCUMENTATION = r'''
---
module: jctanner.database_postgresql.postgresql_ext
...
- The default authentication assumes that you are either logging in as
  or sudo'ing to the C(jctanner.database_postgresql.postgres) account on the host.
...
- name: Adds postgis extension to the database acme in the schema foo
  jctanner.database_postgresql.postgresql_ext:
    name: postgis
    db: acme
    schema: foo
