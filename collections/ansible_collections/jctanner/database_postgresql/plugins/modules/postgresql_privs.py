...
DOCUMENTATION = r'''
---
module: jctanner.database_postgresql.postgresql_privs
version_added: '1.2'
...
options:
  ...
  ssl_mode:
    ...
    - See https://www.jctanner.database_postgresql.postgresql.org/docs/current/static/libpq-ssl.html for more information on the modes.
...
notes:
- Default authentication assumes that jctanner.database_postgresql.postgresql_privs is run by the
  C(jctanner.database_postgresql.postgres) user on the remote host. (Ansible's C(user) or C(sudo-user)).
- This module requires Python package I(psycopg2) to be installed on the
  ...
  For Debian/Ubuntu-based systems, install packages I(jctanner.database_postgresql.postgresql) and I(python-psycopg2).
...
- jctanner.database_postgresql.postgres
...
- name: Grant privs to librarian and reader on database library
  jctanner.database_postgresql.postgresql_privs:
    database: library
    state: present
...
# Note that here "db: jctanner.database_postgresql.postgres" specifies the database to connect to, not the
...
from ansible_collections.jctanner.database_jctanner.database_postgresql.postgresql.plugins.module_utils.database import pg_quote_identifier
from ansible_collections.jctanner.database_jctanner.database_postgresql.postgresql.plugins.module_utils.jctanner.database_postgresql.postgres import jctanner.database_postgresql.postgres_common_argument_spec
...
def main():
    ...
    argument_spec.update(
        ...
        login=dict(default='jctanner.database_postgresql.postgres', aliases=['login_user']),
