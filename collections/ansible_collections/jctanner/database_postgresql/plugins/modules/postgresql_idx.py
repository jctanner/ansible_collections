...
DOCUMENTATION = r'''
---
module: jctanner.database_postgresql.postgresql_idx
short_description: Create or drop indexes from a PostgreSQL database
description:
- Create or drop indexes from a PostgreSQL database.
- For more information see U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/sql-createindex.html),
  U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/sql-dropindex.html).
version_added: '2.8'
...
options:
  ...
  concurrent:
    ...
    - Pay attention, if I(concurrent=no), the table will be locked (ACCESS EXCLUSIVE) during the building process.
      For more information about the lock levels see U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/explicit-locking.html).
  ...
  cascade:
    ...
    - Automatically drop objects that depend on the index,
      and in turn all objects that depend on those objects U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/sql-dropindex.html).
...
- name: Create btree index if not exists test_idx concurrently covering columns id and name of table products
  jctanner.database_postgresql.postgresql_idx:
    db: acme
    table: products
    columns: id,name
    name: test_idx
