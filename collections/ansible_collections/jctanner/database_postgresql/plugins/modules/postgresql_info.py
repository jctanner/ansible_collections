...
DOCUMENTATION = r'''
---
module: jctanner.database_postgresql.postgresql_info
...
- To avoid "Peer authentication failed for user jctanner.database_postgresql.postgres" error,
  use jctanner.database_postgresql.postgres user as a I(become_user).
...
EXAMPLES = r'''
# Display info from jctanner.database_postgresql.postgres hosts.
# ansible jctanner.database_postgresql.postgres -m jctanner.database_postgresql.postgresql_info

# Display only databases and roles info from all hosts using shell-style wildcards:
# ansible all -m jctanner.database_postgresql.postgresql_info -a 'filter=dat*,rol*'

# Display only replications and repl_slots info from standby hosts using shell-style wildcards:
# ansible standby -m jctanner.database_postgresql.postgresql_info -a 'filter=repl*'

# Display all info from databases hosts except settings:
# ansible databases -m jctanner.database_postgresql.postgresql_info -a 'filter=!settings'

- name: Collect PostgreSQL version and extensions
  become: yes
  become_user: jctanner.database_postgresql.postgres
  jctanner.database_postgresql.postgresql_info:
    filter: ver*,ext*
...
# and pass "jctanner.database_postgresql.postgres" as a database to connect to
- name: Collect tablespaces and repl_slots info
  become: yes
  become_user: pgsql
  jctanner.database_postgresql.postgresql_info:
    db: jctanner.database_postgresql.postgres
    filter:
    - tablesp*
    - repl_sl*
...
RETURN = r'''
version:
  description: Database server version U(https://www.jctanner.database_postgresql.postgresql.org/support/versioning/).
  returned: always
  type: dict
...
databases:
  ...
  type: dict
  sample:
  - { "jctanner.database_postgresql.postgres": { "access_priv": "", "collate": "en_US.UTF-8",
  "ctype": "en_US.UTF-8", "encoding": "UTF8", "owner": "jctanner.database_postgresql.postgres", "size": "7997 kB" } }
  contains:
    database_name:
      ...
      contains:
        access_priv:
          ...
          returned: always
          type: str
          sample: "=c/jctanner.database_postgresql.postgres_njctanner.database_postgresql.postgres=CTc/jctanner.database_postgresql.postgres"
        collate:
          description:
          - Database collation U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/collation.html).
          returned: always
          type: str
          sample: en_US.UTF-8
        ctype:
          description:
          - Database LC_CTYPE U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/multibyte.html).
          returned: always
          type: str
          sample: en_US.UTF-8
        encoding:
          description:
          - Database encoding U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/multibyte.html).
          returned: always
          type: str
          sample: UTF8
        owner:
          description:
          - Database owner U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/sql-createdatabase.html).
          returned: always
          type: str
          sample: jctanner.database_postgresql.postgres
        size:
        ...
        extensions:
          description:
          - Extensions U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/sql-createextension.html).
          returned: always
          type: dict
          sample:
          ...
          contains:
            ...
            nspname:
              ...
              sample: pg_catalog
        languages:
          description: Procedural languages U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/xplang.html).
          returned: always
          type: dict
          sample: { "sql": { "lanacl": "", "lanowner": "jctanner.database_postgresql.postgres" } }
          contains:
            lanacl:
              description:
              - Language access privileges
                U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/catalog-pg-language.html).
              returned: always
              type: str
              sample: "{jctanner.database_postgresql.postgres=UC/jctanner.database_postgresql.postgres,=U/jctanner.database_postgresql.postgres}"
            lanowner:
              description:
              - Language owner U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/catalog-pg-language.html).
              returned: always
              type: str
              sample: jctanner.database_postgresql.postgres
        namespaces:
          description:
          - Namespaces (schema) U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/sql-createschema.html).
          returned: always
          type: dict
          sample: { "pg_catalog": { "nspacl": "{jctanner.database_postgresql.postgres=UC/jctanner.database_postgresql.postgres,=U/jctanner.database_postgresql.postgres}", "nspowner": "jctanner.database_postgresql.postgres" } }
          contains:
            nspacl:
              description:
              - Access privileges U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/catalog-pg-namespace.html).
              returned: always
              type: str
              sample: "{jctanner.database_postgresql.postgres=UC/jctanner.database_postgresql.postgres,=U/jctanner.database_postgresql.postgres}"
            nspowner:
              description:
              - Schema owner U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/catalog-pg-namespace.html).
              returned: always
              type: str
              sample: jctanner.database_postgresql.postgres
repl_slots:
  ...
  - Replication slots (available in 9.4 and later)
    U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/catalog-pg-replication-slots.html).
  returned: if existent
  type: dict
...
replications:
  ...
  - Information about the current replications by process PIDs
    U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/monitoring-stats.html#MONITORING-STATS-VIEWS-TABLE).
  returned: if pg_stat_replication view existent
  type: dict
  sample:
  - { 76580: { "app_name": "standby1", "backend_start": "2019-02-03 00:14:33.908593+03",
    "client_addr": "10.10.10.2", "client_hostname": "", "state": "streaming", "usename": "jctanner.database_postgresql.postgres" } }
  contains:
    usename:
      description:
...
tablespaces:
  description:
  - Information about tablespaces U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/catalog-pg-tablespace.html).
  returned: always
  type: dict
  sample:
  - { "test": { "spcacl": "{jctanner.database_postgresql.postgres=C/jctanner.database_postgresql.postgres,andreyk=C/jctanner.database_postgresql.postgres}", "spcoptions": [ "seq_page_cost=1" ],
    "spcowner": "jctanner.database_postgresql.postgres" } }
  contains:
    spcacl:
      description: Tablespace access privileges.
      returned: always
      type: str
      sample: "{jctanner.database_postgresql.postgres=C/jctanner.database_postgresql.postgres,andreyk=C/jctanner.database_postgresql.postgres}"
    spcoptions:
...
roles:
  description:
  - Information about roles U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/user-manag.html).
  returned: always
  type: dict
  sample:
  ...
  contains:
    canlogin:
      description: Login privilege U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/role-attributes.html).
      returned: always
      type: bool
      sample: true
    member_of:
      description:
      - Role membership U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/role-membership.html).
      returned: always
      type: list
    ...
    valid_until:
      description:
      - Password expiration date U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/sql-alterrole.html).
      returned: always
      type: str
...
settings:
  ...
  - Information about run-time server parameters
    U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/view-pg-settings.html).
  returned: always
  type: dict
  sample:
  - { "work_mem": { "boot_val": "4096", "context": "user", "max_val": "2147483647",
    "min_val": "64", "setting": "8192", "sourcefile": "/var/lib/pgsql/10/data/jctanner.database_postgresql.postgresql.auto.conf",
  ...
  contains:
    ...
    sourcefile:
      ...
      returned: always
      type: str
      sample: /var/lib/pgsql/10/data/jctanner.database_postgresql.postgresql.auto.conf
    context:
      description:
      - Context required to set the parameter's value.
      - For more information see U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/view-pg-settings.html).
      returned: always
      type: str
      sample: user
    vartype:
...
from ansible_collections.jctanner.database_jctanner.database_postgresql.postgresql.plugins.module_utils.jctanner.database_postgresql.postgres import connect_to_db, jctanner.database_postgresql.postgres_common_argument_spec
