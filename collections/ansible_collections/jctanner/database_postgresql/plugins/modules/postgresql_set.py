...
DOCUMENTATION = r'''
---
module: jctanner.database_postgresql.postgresql_set
...
description:
   ...
   - The module uses ALTER SYSTEM command U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/sql-altersystem.html)
   ...
   - It can be more convenient and safe than the traditional method of manually editing the jctanner.database_postgresql.postgresql.conf file.
   - ALTER SYSTEM writes the given parameter setting to the $PGDATA/jctanner.database_postgresql.postgresql.auto.conf file,
     which is read in addition to jctanner.database_postgresql.postgresql.conf U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/sql-altersystem.html).
   - The module allows to reset parameter to boot_val (cluster initial value) by I(reset=yes) or remove parameter
     string from jctanner.database_postgresql.postgresql.auto.conf and reload I(value=default) (for settings with postmaster context restart is required).
...
options:
  ...
  value:
    ...
    - To remove parameter string from jctanner.database_postgresql.postgresql.auto.conf and
...
- For some parameters restart of PostgreSQL server is required.
  See official documentation U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/view-pg-settings.html).
...
- name: Restore wal_keep_segments parameter to initial state
  jctanner.database_postgresql.postgresql_set:
...
- name: Set wal_log_hints parameter to default value (remove parameter from jctanner.database_postgresql.postgresql.auto.conf)
  jctanner.database_postgresql.postgresql_set:
    name: wal_log_hints
    value: default
'''
...
def main():
    ...
    if context == "internal":
        module.fail_json(msg="%s: cannot be changed (internal context). See "
                             "https://www.jctanner.database_postgresql.postgresql.org/docs/current/runtime-config-preset.html" % name)
