...
DOCUMENTATION = r'''
---
module: jctanner.database_postgresql.postgresql_table
...
- Allows to create, drop, rename, truncate a table, or change some table attributes
  U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/sql-createtable.html).
version_added: '2.8'
options:
  table:
    description:
  ...
  cascade:
    ...
    - Automatically drop objects that depend on the table (such as views)
      U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/sql-droptable.html).
...
- If you do not pass db parameter, tables will be created in the database
  named jctanner.database_postgresql.postgres.
...
- Unlogged tables are available from PostgreSQL server version 9.1
  U(https://www.jctanner.database_postgresql.postgresql.org/docs/9.1/sql-createtable.html).
...
- name: Create tbl2 in the acme database with the DDL like tbl1 with testuser as an owner
  jctanner.database_postgresql.postgresql_table:
    db: acme
    name: tbl2
    like: tbl1
    owner: testuser
