...
DOCUMENTATION = r'''
---
module: jctanner.database_postgresql.postgresql_owner
...
- For more information about REASSIGN OWNED BY command
  see U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/sql-reassign-owned.html).
version_added: '2.8'
...
options:
  ...
  reassign_owned_by:
    ...
    - For more information see U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/sql-reassign-owned.html).
...
# ansible -m jctanner.database_postgresql.postgresql_owner -a "db=bar new_owner=alice obj_name=myfunc obj_type=function"

- name: The same as above by playbook
  jctanner.database_postgresql.postgresql_owner:
    db: bar
    new_owner: alice
    obj_name: myfunc
    obj_type: function
