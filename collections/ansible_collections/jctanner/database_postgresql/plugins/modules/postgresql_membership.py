...
DOCUMENTATION = r'''
---
module: jctanner.database_postgresql.postgresql_membership
short_description: Add or remove PostgreSQL roles from groups
description:
- Adds or removes PostgreSQL roles from groups (other roles)
  U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/role-membership.html).
- Users are roles with login privilege (see U(https://www.jctanner.database_postgresql.postgresql.org/docs/current/role-attributes.html) for more information).
- Groups are PostgreSQL roles usually without LOGIN privelege.
- "Common use case:"
- 1) add a new group (groups) by M(jctanner.database_postgresql.postgresql_user) module
  U(https://docs.ansible.com/ansible/latest/modules/jctanner.database_postgresql.postgresql_user_module.html) with I(role_attr_flags=NOLOGIN)
- 2) grant them desired privileges by M(jctanner.database_postgresql.postgresql_privs) module
  U(https://docs.ansible.com/ansible/latest/modules/jctanner.database_postgresql.postgresql_privs_module.html)
...
- For Ubuntu-based systems, install the jctanner.database_postgresql.postgresql, libpq-dev, and python-psycopg2 packages
...
- name: Grant role read_only to alice and bob
  jctanner.database_postgresql.postgresql_membership:
    group: read_only
    target_roles:
    - alice
    - bob
...
from ansible_collections.jctanner.database_jctanner.database_postgresql.postgresql.plugins.module_utils.database import SQLParseError, pg_quote_identifier
