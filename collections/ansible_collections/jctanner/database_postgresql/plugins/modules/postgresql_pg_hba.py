...
'''
This module is used to manage jctanner.database_postgresql.postgres pg_hba files with Ansible.
'''
...
DOCUMENTATION = r'''
---
module: jctanner.database_postgresql.postgresql_pg_hba
...
options:
  ...
  contype:
    description:
      - Type of the rule. If not set, C(jctanner.database_postgresql.postgresql_pg_hba) will only return contents.
    type: str
...
notes:
   ...
   - This module will sort resulting C(pg_hba) files if a rule change is required.
     ...
     After the C(pg_hba) file is rewritten by the M(jctanner.database_postgresql.postgresql_pg_hba) module, the ip specific rule will be sorted above the range rule.
...
- name: Grant users joe and simon access to databases sales and logistics from ipv6 localhost ::1/128 using peer authentication.
  jctanner.database_postgresql.postgresql_pg_hba:
    dest: /var/lib/jctanner.database_postgresql.postgres/data/pg_hba.conf
    contype: host
    users: joe,simon
    source: ::1
...
class PgHba(object):
    ...
    def __init__(self, pg_hba_file=None, order="sdu", backup=False, create=False):
        ...
        self.databases = set(['jctanner.database_postgresql.postgres', 'template0', 'template1'])
        ...
        self.users = set(['jctanner.database_postgresql.postgres'])
