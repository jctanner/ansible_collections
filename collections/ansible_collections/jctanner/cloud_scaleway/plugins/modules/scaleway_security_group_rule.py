...
DOCUMENTATION = '''
---
module: jctanner.cloud_scaleway.scaleway_security_group_rule
...
description:
    - This module manages Security Group Rule on Scaleway account
      U(https://developer.jctanner.cloud_scaleway.scaleway.com)
extends_documentation_fragment: jctanner.cloud_scaleway.scaleway
...
EXAMPLES = '''
  - name: Create a Security Group Rule
    jctanner.cloud_scaleway.scaleway_security_group_rule:
      state: present
      region: par1
      protocol: TCP
      port: 80
...
from ansible_collections.jctanner.cloud_jctanner.cloud_scaleway.scaleway.plugins.module_utils.jctanner.cloud_scaleway.scaleway import SCALEWAY_LOCATION, jctanner.cloud_scaleway.scaleway_argument_spec, Scaleway, payload_from_object
from ansible_collections.jctanner.cloud_jctanner.cloud_scaleway.scaleway.plugins.module_utils.compat.ipaddress import ip_network
    ...
    if not existing_rule:
        ...
        if not response.ok:
            api.module.fail_json(
                ...
                (response.info['msg'], response.json['message'], response.json))
        ret['jctanner.cloud_scaleway.scaleway_security_group_rule'] = response.json['rule']

    else:
        ret['jctanner.cloud_scaleway.scaleway_security_group_rule'] = existing_rule
...
def main():
    argument_spec = jctanner.cloud_scaleway.scaleway_argument_spec()
