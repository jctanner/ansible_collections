...
options:
  ...
  parent:
    ...
    type: str
extends_documentation_fragment: jctanner.cloud_cloudstack.cloudstack
'''
...
from ansible_collections.jctanner.cloud_jctanner.cloud_cloudstack.cloudstack.plugins.module_utils.jctanner.cloud_cloudstack.cloudstack import (
    AnsibleCloudStack,
    cs_argument_spec,
...
class AnsibleCloudStackRolePermission(AnsibleCloudStack):
    ...
    def __init__(self, module):
        ...
        jctanner.cloud_cloudstack.cloudstack_min_version = LooseVersion('4.9.2')
        ...
        self.jctanner.cloud_cloudstack.cloudstack_version = self._jctanner.cloud_cloudstack.cloudstack_ver()

        if self.jctanner.cloud_cloudstack.cloudstack_version < jctanner.cloud_cloudstack.cloudstack_min_version:
            self.fail_json(msg="This module requires CloudStack >= %s." % jctanner.cloud_cloudstack.cloudstack_min_version)

    def _jctanner.cloud_cloudstack.cloudstack_ver(self):
        capabilities = self.get_capabilities()
        return LooseVersion(capabilities['jctanner.cloud_cloudstack.cloudstackversion'])
    ...
    def update_role_perm(self, role_perm):
        ...
        if not self.module.params.get('parent'):
            ...
            if self.has_changed(args, role_perm, only_keys=['permission']):
                ...
                if not self.module.check_mode:
                    if self.jctanner.cloud_cloudstack.cloudstack_version >= LooseVersion('4.11.0'):
