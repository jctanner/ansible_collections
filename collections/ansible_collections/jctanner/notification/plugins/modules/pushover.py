...
class Pushover(object):
    ...
    def run(self, priority, msg, title):
        ...
        jctanner.notification.url = '%s/1/messages.json' % (self.base_uri)
        ...
        data = jctanner.notification.urlencode(options)

        headers = {"Content-type": "application/x-www-form-jctanner.notification.urlencoded"}
        r, info = fetch_jctanner.notification.url(self.module, jctanner.notification.url, method='POST', data=data, headers=headers)
        if info['status'] != 200:
