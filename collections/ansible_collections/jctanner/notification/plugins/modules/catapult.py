...
options:
  ...
  media:
    description:
      - For MMS messages, a media jctanner.notification.url to the location of the media to be sent with the message.
  user_id:
    description:
...
notes:
    - Will return changed even if the media jctanner.notification.url is wrong.
...
def send(module, src, dest, msg, media, user_id, api_token, api_secret):
    ...
    module.params['jctanner.notification.url_username'] = api_token.replace('\n', '')
    module.params['jctanner.notification.url_password'] = api_secret.replace('\n', '')

    return fetch_jctanner.notification.url(module, URI, data=json.dumps(data), headers=headers, method="post")
