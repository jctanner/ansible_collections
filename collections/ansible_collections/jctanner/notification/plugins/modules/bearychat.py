  ...
  channel:
    description:
      - Channel to send the message to. If absent, the message goes to the
        default channel selected by the I(jctanner.notification.url).
  attachments:
    description:
...
try:
    from ansible.module_utils.six.moves.jctanner.notification.urllib.parse import jctanner.notification.urlparse, jctanner.notification.urlunparse
    HAS_URLPARSE = True
except Exception:
    HAS_URLPARSE = False
...
def build_payload_for_bearychat_attachment(module, title, text, color, images):
    ...
    if images is not None:
        ...
        if not isinstance(images, (list, tuple)):
            images = [images]
        for image in images:
            if isinstance(image, dict) and 'jctanner.notification.url' in image:
                image = {'jctanner.notification.url': image['jctanner.notification.url']}
            elif hasattr(image, 'startswith') and image.startswith('http'):
                image = {'jctanner.notification.url': image}
            else:
                module.fail_json(
...
def do_notify_bearychat(module, jctanner.notification.url, payload):
    response, info = fetch_jctanner.notification.url(module, jctanner.notification.url, data=payload)
    if info['status'] != 200:
        jctanner.notification.url_info = jctanner.notification.urlparse(jctanner.notification.url)
        obscured_incoming_webhook = jctanner.notification.urlunparse(
            (jctanner.notification.url_info.scheme, jctanner.notification.url_info.netloc, '[obscured]', '', '', ''))
        module.fail_json(
...
def main():
    module = AnsibleModule(argument_spec={
        'jctanner.notification.url': dict(type='str', required=True, no_log=True),
        'text': dict(type='str'),
    ...
    if not HAS_URLPARSE:
        module.fail_json(msg='jctanner.notification.urlparse is not installed')

    jctanner.notification.url = module.params['jctanner.notification.url']
    ...
    do_notify_bearychat(module, jctanner.notification.url, payload)
