...
options:
  ...
  protocol:
    description:
      - Specify the protocol used to send notification messages before the webhook jctanner.notification.url. (i.e. http or https)
    default: https
    choices:
      - 'http'
      - 'https'
  ...
  icon_emoji:
    description:
      - Emoji for the message sender. The representation for the available emojis can be
        got from Rocket Chat. (for example :thumbsup:) (if I(icon_emoji) is set, I(icon_jctanner.notification.url) will not be used)
  link_names:
    description:
...
def build_payload_for_rocketchat(module, text, channel, username, icon_jctanner.notification.url, icon_emoji, link_names, color, attachments):
    payload = {}
    ...
    if icon_emoji is not None:
        ...
        payload['icon_jctanner.notification.url'] = icon_jctanner.notification.url
...
def do_notify_rocketchat(module, domain, token, protocol, payload):
    ...
    response, info = fetch_jctanner.notification.url(module, rocketchat_incoming_webhook, data=payload)
    if info['status'] != 200:
...
def main():
    ...
    payload = build_payload_for_rocketchat(module, text, channel, username, icon_jctanner.notification.url, icon_emoji, link_names, color, attachments)
