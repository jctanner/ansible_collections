...
options:
  ...
  api:
    description:
      - API jctanner.notification.url if using a self-hosted hipchat server. For Hipchat API version
...
from ansible_collections.jctanner.notification.plugins.module_utils.six.moves.jctanner.notification.urllib.request import pathname2jctanner.notification.url
...
def send_msg_v1(module, token, room, msg_from, msg, msg_format='text',
    ...
    jctanner.notification.url = api + MSG_URI_V1 + "?auth_token=%s" % (token)
    data = jctanner.notification.urlencode(params)

    if module.check_mode:
        # In check mode, exit before actually sending the message
        module.exit_json(changed=False)

    response, info = fetch_jctanner.notification.url(module, jctanner.notification.url, data=data)
    if info['status'] == 200:
...
def send_msg_v2(module, token, room, msg_from, msg, msg_format='text',
    ...
    jctanner.notification.url = POST_URL.replace('{id_or_name}', pathname2jctanner.notification.url(room))
    ...
    response, info = fetch_jctanner.notification.url(module, jctanner.notification.url, data=data, headers=headers, method='POST')
