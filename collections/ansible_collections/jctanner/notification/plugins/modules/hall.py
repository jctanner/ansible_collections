...
def send_request_to_hall(module, room_token, payload):
    ...
    response, info = fetch_jctanner.notification.url(module, api_endpoint, data=payload, headers=headers)
    if info['status'] != 200:
        secure_jctanner.notification.url = HALL_API_ENDPOINT % ('[redacted]')
        module.fail_json(msg=" failed to send %s to %s: %s" % (payload, secure_jctanner.notification.url, info['msg']))
