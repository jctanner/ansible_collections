...
options:
  jctanner.notification.url:
    description:
      ...
      - I(jctanner.notification.url) and I(host)/I(port)/I(user)/I(pass)/I(vhost) are mutually exclusive, use either or but not both.
  proto:
    description:
      - The protocol to use.
    choices: [amqps, amqp]
...
- name: Publish a message to a queue with headers
  rabbitmq_publish:
    jctanner.notification.url: "amqp://guest:guest@192.168.0.32:5672/%2F"
    queue: 'test'
...
from ansible_collections.jctanner.notification.plugins.module_utils.rabbitmq import RabbitClient
