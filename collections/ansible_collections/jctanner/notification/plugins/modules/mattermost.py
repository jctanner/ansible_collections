...
description:
    - Sends notifications to U(http://your.mattermost.jctanner.notification.url) via the Incoming WebHook integration.
version_added: "2.3"
author: "Benjamin Jolivot (@bjolivot)"
options:
  jctanner.notification.url:
    description:
      - Mattermost jctanner.notification.url (i.e. http://mattermost.yourcompany.com).
    required: true
  api_key:
    description:
  ...
  username:
    ...
    default: Ansible
  icon_jctanner.notification.url:
    description:
...
- name: Send notification message via Mattermost
  mattermost:
    jctanner.notification.url: http://mattermost.example.com
    api_key: my_api_key
...
- name: Send notification message via Mattermost all options
  mattermost:
    ...
    icon_jctanner.notification.url: http://www.example.com/some-image-file.png
"""
...
payload:
    ...
    returned: success
    type: str
webhook_jctanner.notification.url:
...
def main():
    module = AnsibleModule(
        ...
        argument_spec=dict(
            jctanner.notification.url=dict(type='str', required=True),
            api_key=dict(type='str', required=True, no_log=True),
            text=dict(type='str', required=True),
            channel=dict(type='str', default=None),
            username=dict(type='str', default='Ansible'),
            icon_jctanner.notification.url=dict(type='str', default='https://www.ansible.com/favicon.ico'),
    ...
    # define webhook
    webhook_jctanner.notification.url = "{0}/hooks/{1}".format(module.params['jctanner.notification.url'], module.params['api_key'])
    result['webhook_jctanner.notification.url'] = webhook_jctanner.notification.url

    # define payload
    payload = {}
    for param in ['text', 'channel', 'username', 'icon_jctanner.notification.url']:
    ...
    if module.check_mode is False:
        response, info = fetch_jctanner.notification.url(module=module, jctanner.notification.url=webhook_jctanner.notification.url, headers=headers, method='POST', data=payload)
