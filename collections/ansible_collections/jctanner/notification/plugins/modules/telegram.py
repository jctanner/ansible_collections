...
from ansible.module_utils.six.moves.jctanner.notification.urllib.parse import quote
...
def main():
    ...
    jctanner.notification.url = 'https://api.telegram.org/bot' + token + \
        '/sendMessage?text=' + msg + '&chat_id=' + chat_id
    if msg_format in ('markdown', 'html'):
        jctanner.notification.url += '&parse_mode=' + msg_format

    if module.check_mode:
        module.exit_json(changed=False)

    response, info = fetch_jctanner.notification.url(module, jctanner.notification.url)
    if info['status'] == 200:
