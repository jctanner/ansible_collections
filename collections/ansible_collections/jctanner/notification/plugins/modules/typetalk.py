...
from ansible.module_utils.jctanner.notification.urls import fetch_jctanner.notification.url, ConnectionError


def do_request(module, jctanner.notification.url, params, headers=None):
    ...
    r, info = fetch_jctanner.notification.url(module, jctanner.notification.url, data=data, headers=headers)
    if info['status'] != 200:
...
def send_message(module, client_id, client_secret, topic, msg):
    ...
    try:
        ...
        jctanner.notification.url = 'https://typetalk.com/api/v1/topics/%d' % topic
        headers = {
            'Authorization': 'Bearer %s' % access_token,
        }
        do_request(module, jctanner.notification.url, {'message': msg}, headers)
