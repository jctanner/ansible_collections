...
from ansible.module_utils.jctanner.notification.urls import fetch_jctanner.notification.url, jctanner.notification.url_argument_spec
...
def send_msg(module):
    ...
    for number in module.params.get('dest'):
        msg['to'] = number
        jctanner.notification.url = "%s?%s" % (NEXMO_API, jctanner.notification.urlencode(msg))

        headers = dict(Accept='application/json')
        response, info = fetch_jctanner.notification.url(module, jctanner.notification.url, headers=headers)
        if info['status'] != 200:
...
def main():
    argument_spec = jctanner.notification.url_argument_spec()
