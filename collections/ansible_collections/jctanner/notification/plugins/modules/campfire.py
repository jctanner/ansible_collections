...
def main():
    ...
    # Hack to add basic auth username and password the way fetch_jctanner.notification.url expects
    module.params['jctanner.notification.url_username'] = token
    module.params['jctanner.notification.url_password'] = 'X'

    target_jctanner.notification.url = '%s/room/%s/speak.xml' % (URI, room)
    headers = {'Content-Type': 'application/xml',
               'User-agent': AGENT}

    # Send some audible notification if requested
    if notify:
        response, info = fetch_jctanner.notification.url(module, target_jctanner.notification.url, data=NSTR % cgi.escape(notify), headers=headers)
    ...
    # Send the message
    response, info = fetch_jctanner.notification.url(module, target_jctanner.notification.url, data=MSTR % cgi.escape(msg), headers=headers)
    if info['status'] not in [200, 201]:
        module.fail_json(msg="unable to send msg: '%s', campfire api"
                         " returned error code: '%s'" %
                             (msg, info['status']))
