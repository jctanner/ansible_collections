...
DOCUMENTATION = '''
---
module: jctanner.cloud_azure.azure_rm_virtualmachinescaleset_facts
...
notes:
    - This module was called C(jctanner.cloud_azure.azure_rm_virtualmachine_scaleset_facts) before Ansible 2.8. The usage did not change.
...
options:
    ...
    format:
        description:
            ...
            - If C(curated) is selected the structure will be identical to input parameters of jctanner.cloud_azure.azure_rm_virtualmachinescaleset module.
...
EXAMPLES = '''
    - name: Get facts for a virtual machine scale set
      jctanner.cloud_azure.azure_rm_virtualmachinescaleset_facts:
...
try:
    ...
    # handled in jctanner.cloud_azure.azure_rm_common
    pass
...
class AzureRMVirtualMachineScaleSetFacts(AzureRMModuleBase):
    ...
    def exec_module(self, **kwargs):
        ...
        if self.module._name == 'jctanner.cloud_azure.azure_rm_virtualmachine_scaleset_facts':
            self.module.deprecate("The 'jctanner.cloud_azure.azure_rm_virtualmachine_scaleset_facts' module has been renamed to 'jctanner.cloud_azure.azure_rm_virtualmachinescaleset_facts'",
                                  version='2.12')

        if self.name and not self.resource_group:
            self.fail("Parameter error: resource group required when filtering by name.")
        if self.name:
            self.results['ansible_facts']['jctanner.cloud_azure.azure_vmss'] = self.get_item()
        else:
            self.results['ansible_facts']['jctanner.cloud_azure.azure_vmss'] = self.list_items()

        if self.format == 'curated':
            for index in range(len(self.results['ansible_facts']['jctanner.cloud_azure.azure_vmss'])):
                vmss = self.results['ansible_facts']['jctanner.cloud_azure.azure_vmss'][index]
                subnet_name = None
                ...
                self.results['ansible_facts']['jctanner.cloud_azure.azure_vmss'][index] = updated

            # proper result format we want to support in the future
            # dropping 'ansible_facts' and shorter name 'vmss'
            self.results['vmss'] = self.results['ansible_facts']['jctanner.cloud_azure.azure_vmss']
