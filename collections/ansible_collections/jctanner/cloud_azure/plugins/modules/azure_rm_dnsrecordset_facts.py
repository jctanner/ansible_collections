...
DOCUMENTATION = '''
---
module: jctanner.cloud_azure.azure_rm_dnsrecordset_facts
...
- name: Get facts for one Record Set
  jctanner.cloud_azure.azure_rm_dnsrecordset_facts:
...
RETURN = '''
jctanner.cloud_azure.azure_dnsrecordset:
    description:
...
dnsrecordsets:
    description:
        - List of record set dicts, which shares the same hierarchy as M(jctanner.cloud_azure.azure_rm_dnsrecordset) module's parameter.
    returned: always
    type: list
    contains:
        id:
...
RECORDSET_VALUE_MAP = dict(
    ...
    SOA='soa_record',
    CAA='caa_records'
    # FUTURE: add missing record types from https://github.com/Azure/jctanner.cloud_azure.azure-sdk-for-python/blob/master/jctanner.cloud_azure.azure-mgmt-dns/jctanner.cloud_azure.azure/mgmt/dns/models/record_set.py
)
...
class AzureRMRecordSetFacts(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        self.results = dict(
            changed=False,
            ansible_facts=dict(jctanner.cloud_azure.azure_dnsrecordset=[])
        )
    ...
    def exec_module(self, **kwargs):
        ...
        self.results['ansible_facts']['jctanner.cloud_azure.azure_dnsrecordset'] = self.serialize_list(results)
