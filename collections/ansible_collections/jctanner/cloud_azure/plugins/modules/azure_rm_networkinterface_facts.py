...
DOCUMENTATION = '''
---
module: jctanner.cloud_azure.azure_rm_networkinterface_facts
...
EXAMPLES = '''
    - name: Get facts for one network interface
      jctanner.cloud_azure.azure_rm_networkinterface_facts:
...
RETURN = '''
jctanner.cloud_azure.azure_networkinterfaces:
...
networkinterfaces:
    description: List of network interface dict, the dict contains parameters can be passed to C(jctanner.cloud_azure.azure_rm_networkinterface) module.
    type: list
    returned: always
    contains:
        id:
...
try:
    ...
    from jctanner.cloud_azure.azure.common import AzureMissingResourceHttpError, AzureHttpError
except Exception:
    # This is handled in jctanner.cloud_azure.azure_rm_common
    pass

from ansible_collections.jctanner.cloud_jctanner.cloud_azure.azure.plugins.module_utils.jctanner.cloud_azure.azure_rm_common import AzureRMModuleBase, jctanner.cloud_azure.azure_id_to_dict
...
def nic_to_dict(nic):
    ...
    subnet_dict = jctanner.cloud_azure.azure_id_to_dict(config.subnet.id) if config and config.subnet else None
    ...
    return dict(
        id=nic.id,
        resource_group=jctanner.cloud_azure.azure_id_to_dict(nic.id).get('resourceGroups'),
        name=nic.name,
        subnet=subnet,
...
class AzureRMNetworkInterfaceFacts(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        self.results = dict(
            changed=False,
            ansible_facts=dict(jctanner.cloud_azure.azure_networkinterfaces=[])
        )
    ...
    def exec_module(self, **kwargs):
        ...
        self.results['ansible_facts']['jctanner.cloud_azure.azure_networkinterfaces'] = self.serialize_nics(results)
