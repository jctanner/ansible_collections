...
DOCUMENTATION = '''
---
module: jctanner.cloud_azure.azure_rm_functionapp_facts
version_added: "2.4"
...
EXAMPLES = '''
    - name: Get facts for one Function App
      jctanner.cloud_azure.azure_rm_functionapp_facts:
...
RETURN = '''
jctanner.cloud_azure.azure_functionapps:
    description:
    ...
    example:
        ...
        state: Running
        host_names:
          - myfunctionapp.jctanner.cloud_azure.azurewebsites.net
        repository_site_name: myfunctionapp
        usage_state: Normal
        enabled: true
        enabled_host_names:
          - myfunctionapp.jctanner.cloud_azure.azurewebsites.net
          - myfunctionapp.scm.jctanner.cloud_azure.azurewebsites.net
        availability_state: Normal
        host_name_ssl_states:
          - name: myfunctionapp.jctanner.cloud_azure.azurewebsites.net
            ssl_state: Disabled
            host_type: Standard
          - name: myfunctionapp.scm.jctanner.cloud_azure.azurewebsites.net
            ssl_state: Disabled
        ...
        resource_group: myResourceGroup
        default_host_name: myfunctionapp.jctanner.cloud_azure.azurewebsites.net
'''
...
class AzureRMFunctionAppFacts(AzureRMModuleBase):
    def __init__(self):
        ...
        self.results = dict(
            changed=False,
            ansible_facts=dict(jctanner.cloud_azure.azure_functionapps=[])
        )
    ...
    def exec_module(self, **kwargs):
        ...
        if self.name:
            self.results['ansible_facts']['jctanner.cloud_azure.azure_functionapps'] = self.get_functionapp()
        elif self.resource_group:
            self.results['ansible_facts']['jctanner.cloud_azure.azure_functionapps'] = self.list_resource_group()
        else:
            self.results['ansible_facts']['jctanner.cloud_azure.azure_functionapps'] = self.list_all()
