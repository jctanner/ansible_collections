...
DOCUMENTATION = '''
---
module: jctanner.cloud_azure.azure_rm_resourcegroup_info
...
EXAMPLES = '''
    - name: Get facts for one resource group
      jctanner.cloud_azure.azure_rm_resourcegroup_info:
...
'''
RETURN = '''
jctanner.cloud_azure.azure_resourcegroups:
...
class AzureRMResourceGroupInfo(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        self.results = dict(
            changed=False,
            ansible_facts=dict(jctanner.cloud_azure.azure_resourcegroups=[]),
            resourcegroups=[]
        )
    ...
    def exec_module(self, **kwargs):
        ...
        if self.name:
            self.results['ansible_facts']['jctanner.cloud_azure.azure_resourcegroups'] = self.get_item()
        else:
            self.results['ansible_facts']['jctanner.cloud_azure.azure_resourcegroups'] = self.list_items()

        if self.list_resources:
            for item in self.results['ansible_facts']['jctanner.cloud_azure.azure_resourcegroups']:
                item['resources'] = self.list_by_rg(item['name'])

        self.results['resourcegroups'] = self.results['ansible_facts']['jctanner.cloud_azure.azure_resourcegroups']
