...
DOCUMENTATION = '''
---
module: jctanner.cloud_azure.azure_rm_virtualmachineimage_facts
...
EXAMPLES = '''
    - name: Get facts for a specific image
      jctanner.cloud_azure.azure_rm_virtualmachineimage_facts:
        location: eastus
        publisher: OpenLogic
        offer: CentOS
        sku: '7.1'
...
RETURN = '''
jctanner.cloud_azure.azure_vmimages:
...
class AzureRMVirtualMachineImageFacts(AzureRMModuleBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.results = dict(
            changed=False,
            ansible_facts=dict(jctanner.cloud_azure.azure_vmimages=[])
        )
    ...
    def exec_module(self, **kwargs):
        ...
        if self.location and self.publisher and self.offer and self.sku and self.version:
            self.results['ansible_facts']['jctanner.cloud_azure.azure_vmimages'] = self.get_item()
        elif self.location and self.publisher and self.offer and self.sku:
            self.results['ansible_facts']['jctanner.cloud_azure.azure_vmimages'] = self.list_images()
        elif self.location and self.publisher:
            self.results['ansible_facts']['jctanner.cloud_azure.azure_vmimages'] = self.list_offers()
        elif self.location:
            self.results['ansible_facts']['jctanner.cloud_azure.azure_vmimages'] = self.list_publishers()
