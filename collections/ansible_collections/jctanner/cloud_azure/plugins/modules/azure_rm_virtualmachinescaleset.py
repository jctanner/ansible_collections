...
DOCUMENTATION = '''
---
module: jctanner.cloud_azure.azure_rm_virtualmachinescaleset
...
notes:
    - This module was called C(jctanner.cloud_azure.azure_rm_virtualmachine_scaleset) before Ansible 2.8. The usage did not change.
...
- name: Create VMSS
  jctanner.cloud_azure.azure_rm_virtualmachinescaleset:
...
RETURN = '''
jctanner.cloud_azure.azure_vmss:
...
from ansible_collections.jctanner.cloud_jctanner.cloud_azure.azure.plugins.module_utils.jctanner.cloud_azure.azure_rm_common import AzureRMModuleBase, jctanner.cloud_azure.azure_id_to_dict, format_resource_id
...
class AzureRMVirtualMachineScaleSet(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        self.results = dict(
            changed=False,
            actions=[],
            ansible_facts=dict(jctanner.cloud_azure.azure_vmss=None)
        )
    ...
    def exec_module(self, **kwargs):
        ...
        if self.module._name == 'jctanner.cloud_azure.azure_rm_virtualmachine_scaleset':
            self.module.deprecate("The 'jctanner.cloud_azure.azure_rm_virtualmachine_scaleset' module has been renamed to 'jctanner.cloud_azure.azure_rm_virtualmachinescaleset'", version='2.12')
        ...
        self.results['ansible_facts']['jctanner.cloud_azure.azure_vmss'] = results
        ...
        if changed:
            if self.state == 'present':
                ...
                self.results['ansible_facts']['jctanner.cloud_azure.azure_vmss'] = self.serialize_vmss(self.get_vmss())

            elif self.state == 'absent':
                # delete the VM
                self.log("Delete virtual machine scale set {0}".format(self.name))
                self.results['ansible_facts']['jctanner.cloud_azure.azure_vmss'] = None
    ...
    def parse_nsg(self):
        ...
        name = jctanner.cloud_azure.azure_id_to_dict(id).get('name')
