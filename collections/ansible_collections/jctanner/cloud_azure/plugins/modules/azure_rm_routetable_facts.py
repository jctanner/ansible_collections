...
DOCUMENTATION = '''
---
module: jctanner.cloud_azure.azure_rm_routetable_facts
...
EXAMPLES = '''
    - name: Get facts for one route table
      jctanner.cloud_azure.azure_rm_routetable_facts:
        name: Testing
...
def route_to_dict(route):
    id_dict = jctanner.cloud_azure.azure_id_to_dict(route.id)
    return dict(
        id=route.id,
        name=route.name,
...
def instance_to_dict(table):
    return dict(
        id=table.id,
        name=table.name,
        resource_group=jctanner.cloud_azure.azure_id_to_dict(table.id).get('resourceGroups'),
