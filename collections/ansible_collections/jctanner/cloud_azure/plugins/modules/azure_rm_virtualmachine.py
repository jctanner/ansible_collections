...
DOCUMENTATION = '''
---
module: jctanner.cloud_azure.azure_rm_virtualmachine
...
description:
    ...
    - Before Ansible 2.5, this required an image found in the Azure Marketplace which can be discovered with
      M(jctanner.cloud_azure.azure_rm_virtualmachineimage_facts). In Ansible 2.5 and newer, custom images can be used as well, see the
    ...
    - If you need to use the I(custom_data) option, many images in the marketplace are not cloud-init ready. Thus, data
      ...
      U(https://docs.microsoft.com/en-us/jctanner.cloud_azure.azure/virtual-machines/linux/using-cloud-init#cloud-init-overview),
      follow these steps U(https://docs.microsoft.com/en-us/jctanner.cloud_azure.azure/virtual-machines/linux/cloudinit-prepare-custom-image).
...
- name: Create VM with defaults
  jctanner.cloud_azure.azure_rm_virtualmachine:
...
- name: Create an availability set for managed disk vm
  jctanner.cloud_azure.azure_rm_availabilityset:
...
try:
    ...
    from msrestjctanner.cloud_azure.azure.tools import parse_resource_id
...
from ansible_collections.jctanner.cloud_jctanner.cloud_azure.azure.plugins.module_utils.jctanner.cloud_azure.azure_rm_common import AzureRMModuleBase, jctanner.cloud_azure.azure_id_to_dict, normalize_location_name, format_resource_id


AZURE_OBJECT_CLASS = 'VirtualMachine'

AZURE_ENUM_MODULES = ['jctanner.cloud_azure.azure.mgmt.compute.models']
...
class AzureRMVirtualMachine(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        self.results = dict(
            ...
            ansible_facts=dict(jctanner.cloud_azure.azure_vm=None)
        )
    ...
    def exec_module(self, **kwargs):
        ...
        self.results['ansible_facts']['jctanner.cloud_azure.azure_vm'] = results
        ...
        if changed:
            if self.state == 'present':
                ...
                if (powerstate_change == 'poweron' and
                        self.results['ansible_facts']['jctanner.cloud_azure.azure_vm']['powerstate'] != 'running'):
                ...
                self.results['ansible_facts']['jctanner.cloud_azure.azure_vm'] = self.serialize_vm(self.get_vm())

            elif self.state == 'absent':
                # delete the VM
                self.log("Delete virtual machine {0}".format(self.name))
                self.results['ansible_facts']['jctanner.cloud_azure.azure_vm'] = None
                self.delete_vm(vm)
    ...
    def serialize_vm(self, vm):
        ...
        for interface in vm.network_profile.network_interfaces:
            int_dict = jctanner.cloud_azure.azure_id_to_dict(interface.id)
        ...
        for interface in result['properties']['networkProfile']['networkInterfaces']:
            for config in interface['properties']['ipConfigurations']:
                if config['properties'].get('publicIPAddress'):
                    pipid_dict = jctanner.cloud_azure.azure_id_to_dict(config['properties']['publicIPAddress']['id'])
                    try:
    ...
    def delete_vm(self, vm):
        ...
        if 'all_autocreated' not in self.remove_on_absent:
            ...
            if self.remove_on_absent.intersection(set(['all', 'network_interfaces'])):
                ...
                for interface in vm.network_profile.network_interfaces:
                    id_dict = jctanner.cloud_azure.azure_id_to_dict(interface.id)
                    ...
                    for nic_dict in nic_names:
                        ...
                        for ipc in nic.ip_configurations:
                            if ipc.public_ip_address:
                                pip_dict = jctanner.cloud_azure.azure_id_to_dict(ipc.public_ip_address.id)
