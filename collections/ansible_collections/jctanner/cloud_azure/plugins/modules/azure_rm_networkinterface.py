...
DOCUMENTATION = '''
---
module: jctanner.cloud_azure.azure_rm_networkinterface
...
options:
    ...
    location:
        description:
            - Valid jctanner.cloud_azure.azure location. Defaults to location of the resource group.
    virtual_network:
        description:
...
EXAMPLES = '''
    - name: Create a network interface with minimal parameters
      jctanner.cloud_azure.azure_rm_networkinterface:
        name: nic001
...
try:
    from msrestjctanner.cloud_azure.azure.tools import parse_resource_id, resource_id, is_valid_resource_id
...
def subnet_to_dict(subnet):
    dic = jctanner.cloud_azure.azure_id_to_dict(subnet.id)
    return dict(
        id=subnet.id,
...
def nic_to_dict(nic):
    ip_configurations = [
        dict(
            ...
            public_ip_address=dict(
                ...
                name=jctanner.cloud_azure.azure_id_to_dict(config.public_ip_address.id).get('publicIPAddresses'),
    ...
    return dict(
        ...
        network_security_group=dict(
            ...
            name=jctanner.cloud_azure.azure_id_to_dict(nic.network_security_group.id).get('networkSecurityGroups')
