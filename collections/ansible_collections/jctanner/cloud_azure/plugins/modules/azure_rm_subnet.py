...
DOCUMENTATION = '''
---
module: jctanner.cloud_azure.azure_rm_subnet
version_added: "2.1"
...
description:
    - Create, update or delete a subnet within a given virtual network. Allows setting and updating the address
      prefix CIDR, which must be valid within the context of the virtual network. Use the jctanner.cloud_azure.azure_rm_networkinterface
...
EXAMPLES = '''
    - name: Create a subnet
      jctanner.cloud_azure.azure_rm_subnet:
...
from ansible_collections.jctanner.cloud_jctanner.cloud_azure.azure.plugins.module_utils.jctanner.cloud_azure.azure_rm_common import AzureRMModuleBase, CIDR_PATTERN, jctanner.cloud_azure.azure_id_to_dict, format_resource_id
...
def subnet_to_dict(subnet):
    ...
    if subnet.network_security_group:
        id_keys = jctanner.cloud_azure.azure_id_to_dict(subnet.network_security_group.id)
    ...
    if subnet.route_table:
        id_keys = jctanner.cloud_azure.azure_id_to_dict(subnet.route_table.id)
