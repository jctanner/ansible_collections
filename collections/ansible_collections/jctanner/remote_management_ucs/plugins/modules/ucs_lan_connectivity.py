...
DOCUMENTATION = r'''
---
module: jctanner.remote_management_ucs.ucs_lan_connectivity
...
- name: Configure LAN Connectivity Policy
  jctanner.remote_management_ucs.ucs_lan_connectivity:
...
def configure_lan_connectivity(jctanner.remote_management_ucs.ucs, module, dn):
    from jctanner.remote_management_ucs.ucsmsdk.mometa.vnic.VnicLanConnPolicy import VnicLanConnPolicy
    from jctanner.remote_management_ucs.ucsmsdk.mometa.vnic.VnicEther import VnicEther
    from jctanner.remote_management_ucs.ucsmsdk.mometa.vnic.VnicIScsiLCP import VnicIScsiLCP
    from jctanner.remote_management_ucs.ucsmsdk.mometa.vnic.VnicVlan import VnicVlan
            ...
            if module.params.get('vnic_list'):
                for vnic in module.params['vnic_list']:
                    if vnic['state'] == 'absent':
                        ...
                        if mo_1:
                            jctanner.remote_management_ucs.ucs.login_handle.remove_mo(mo_1)
                    else:  # state == 'present'
                        mo_1 = VnicEther(
                            addr='derived',
                            parent_mo_or_dn=mo,
...
def check_vnic_props(jctanner.remote_management_ucs.ucs, module, dn):
    props_match = True
...
def check_iscsi_vnic_props(jctanner.remote_management_ucs.ucs, module, dn):
    props_match = True
...
def check_lan_connecivity_props(jctanner.remote_management_ucs.ucs, module, mo, dn):
    props_match = False
    ...
    if (mo.check_prop_match(**kwargs)):
        ...
        # check vnic 1st
        props_match = check_vnic_props(jctanner.remote_management_ucs.ucs, module, dn)

        if props_match:
            props_match = check_iscsi_vnic_props(jctanner.remote_management_ucs.ucs, module, dn)
...
def main():
    ...
    if mo:
        if module.params['state'] == 'absent':
            ...
            props_match = check_lan_connecivity_props(jctanner.remote_management_ucs.ucs, module, mo, dn)

    if module.params['state'] == 'present' and not props_match:
        configure_lan_connectivity(jctanner.remote_management_ucs.ucs, module, dn)
