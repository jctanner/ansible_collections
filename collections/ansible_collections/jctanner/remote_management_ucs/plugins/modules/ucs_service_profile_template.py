...
DOCUMENTATION = r'''
---
module: jctanner.remote_management_ucs.ucs_service_profile_template
...
- name: Configure Service Profile Template with LAN/SAN Connectivity and all other options defaulted
  jctanner.remote_management_ucs.ucs_service_profile_template:
...
def configure_service_profile_template(jctanner.remote_management_ucs.ucs, module):
    from jctanner.remote_management_ucs.ucsmsdk.mometa.ls.LsServer import LsServer
    from jctanner.remote_management_ucs.ucsmsdk.mometa.vnic.VnicConnDef import VnicConnDef
    from jctanner.remote_management_ucs.ucsmsdk.mometa.vnic.VnicIScsiNode import VnicIScsiNode
    from jctanner.remote_management_ucs.ucsmsdk.mometa.ls.LsRequirement import LsRequirement
    from jctanner.remote_management_ucs.ucsmsdk.mometa.ls.LsPower import LsPower
    from jctanner.remote_management_ucs.ucsmsdk.mometa.lstorage.LstorageProfileBinding import LstorageProfileBinding
    from jctanner.remote_management_ucs.ucsmsdk.mometa.mgmt.MgmtInterface import MgmtInterface
    from jctanner.remote_management_ucs.ucsmsdk.mometa.mgmt.MgmtVnet import MgmtVnet
    from jctanner.remote_management_ucs.ucsmsdk.mometa.vnic.VnicIpV4MgmtPooledAddr import VnicIpV4MgmtPooledAddr
...
def check_storage_profile_props(jctanner.remote_management_ucs.ucs, module, dn):
    props_match = False

    child_dn = dn + '/profile-binding'
    mo_1 = jctanner.remote_management_ucs.ucs.login_handle.query_dn(child_dn)
    if mo_1:
...
def check_connectivity_policy_props(jctanner.remote_management_ucs.ucs, module, dn):
    props_match = False
...
def check_iqn_pool_props(jctanner.remote_management_ucs.ucs, module, dn):
    props_match = False
...
def check_inband_management_props(jctanner.remote_management_ucs.ucs, module, dn):
    props_match = False
    ...
    if mo_1:
        ...
        if mo_1.check_prop_match(**kwargs):
            ...
            mo_2 = jctanner.remote_management_ucs.ucs.login_handle.query_dn(child_dn)
            if mo_2:
                kwargs = dict(name=module.params['mgmt_vnet_name'])
                if mo_2.check_prop_match(**kwargs):
                    child_dn = child_dn + '/ipv4-pooled-addr'
                    mo_3 = jctanner.remote_management_ucs.ucs.login_handle.query_dn(child_dn)
                    if mo_3:
...
def check_power_props(jctanner.remote_management_ucs.ucs, module, dn):
    props_match = False
...
def check_server_pool(jctanner.remote_management_ucs.ucs, module, dn):
    props_match = False
...
def check_serivce_profile_templates_props(jctanner.remote_management_ucs.ucs, module, mo, dn):
    props_match = False
    ...
    if mo.check_prop_match(**kwargs):
        ...
        props_match = check_storage_profile_props(jctanner.remote_management_ucs.ucs, module, dn)

        if props_match:
            props_match = check_connectivity_policy_props(jctanner.remote_management_ucs.ucs, module, dn)

        if props_match:
            props_match = check_iqn_pool_props(jctanner.remote_management_ucs.ucs, module, dn)

        if props_match:
            props_match = check_inband_management_props(jctanner.remote_management_ucs.ucs, module, dn)

        if props_match:
            props_match = check_power_props(jctanner.remote_management_ucs.ucs, module, dn)

        if props_match:
            props_match = check_server_pool(jctanner.remote_management_ucs.ucs, module, dn)
...
def main():
    ...
    # UCSModule creation above verifies jctanner.remote_management_ucs.ucsmsdk is present and exits on failure.
    ...
    if mo:
        if module.params['state'] == 'absent':
            ...
            props_match = check_serivce_profile_templates_props(jctanner.remote_management_ucs.ucs, module, mo, dn)

    if module.params['state'] == 'present' and not props_match:
        configure_service_profile_template(jctanner.remote_management_ucs.ucs, module)
