...
DOCUMENTATION = r'''
---
module: jctanner.remote_management_ucs.ucs_managed_objects
...
- More information on the UCSM Python SDK and how to directly configure Managed Objects is available at L(UCSM Python SDK,http://jctanner.remote_management_ucs.ucsmsdk.readthedocs.io/).
...
- name: Configure Network Control Policy
  jctanner.remote_management_ucs.ucs_managed_objects:
    hostname: 172.16.143.150
    username: admin
    password: password
    objects:
    - module: jctanner.remote_management_ucs.ucsmsdk.mometa.nwctrl.NwctrlDefinition
...
- name: Configure Boot Policy Using JSON objects list with children
  jctanner.remote_management_ucs.ucs_managed_objects:
    ...
    objects:
    - {
          "module": "jctanner.remote_management_ucs.ucsmsdk.mometa.lsboot.LsbootPolicy",
          "class": "LsbootPolicy",
          "properties": {
              "parent_mo_or_dn": "org-root",
              "name": "Python_SDS",
          ...
          "children": [
              {
                  "module": "jctanner.remote_management_ucs.ucsmsdk.mometa.lsboot.LsbootVirtualMedia",
              ...
              {
                  "module": "jctanner.remote_management_ucs.ucsmsdk.mometa.lsboot.LsbootStorage",
                  "class": "LsbootStorage",
                  "properties": {
                      "order": "1"
                  },
                  "children": [
                      {
                          "module": "jctanner.remote_management_ucs.ucsmsdk.mometa.lsboot.LsbootLocalStorage",
                          "class": "LsbootLocalStorage",
                          "properties": {},
                          "children": [
                              {
                                  "module": "jctanner.remote_management_ucs.ucsmsdk.mometa.lsboot.LsbootDefaultLocalImage",
...
def traverse_objects(module, jctanner.remote_management_ucs.ucs, managed_object, mo=''):
    props_match = False
    ...
    existing_mo = jctanner.remote_management_ucs.ucs.login_handle.query_dn(mo.dn)

    if module.params['state'] == 'absent':
        # mo must exist, but all properties do not have to match
        if existing_mo:
            if not module.check_mode:
                jctanner.remote_management_ucs.ucs.login_handle.remove_mo(existing_mo)
        ...
        if not props_match:
            if not module.check_mode:
                try:
                    jctanner.remote_management_ucs.ucs.login_handle.add_mo(mo, modify_present=True)
                    jctanner.remote_management_ucs.ucs.login_handle.commit()
                except Exception as e:
                    jctanner.remote_management_ucs.ucs.result['err'] = True
    ...
    if managed_object.get('children'):
        for child in managed_object['children']:
            ...
            traverse_objects(module, jctanner.remote_management_ucs.ucs, copy_of_child, mo)
...
def main():
    ...
    jctanner.remote_management_ucs.ucs.result['err'] = False
    ...
    for managed_object in module.params['objects']:
        traverse_objects(module, jctanner.remote_management_ucs.ucs, managed_object)

    if jctanner.remote_management_ucs.ucs.result['err']:
