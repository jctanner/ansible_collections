...
DOCUMENTATION = r'''
---
module: jctanner.remote_management_ucs.ucs_disk_group_policy
...
- name: Configure Disk Group Policy
  jctanner.remote_management_ucs.ucs_disk_group_policy:
...
def configure_disk_policy(jctanner.remote_management_ucs.ucs, module, dn):
    from jctanner.remote_management_ucs.ucsmsdk.mometa.lstorage.LstorageDiskGroupConfigPolicy import LstorageDiskGroupConfigPolicy
    from jctanner.remote_management_ucs.ucsmsdk.mometa.lstorage.LstorageDiskGroupQualifier import LstorageDiskGroupQualifier
    from jctanner.remote_management_ucs.ucsmsdk.mometa.lstorage.LstorageLocalDiskConfigRef import LstorageLocalDiskConfigRef
...
def check_disk_policy_props(jctanner.remote_management_ucs.ucs, module, mo, dn):
    props_match = True
    ...
    if mo.check_prop_match(**kwargs):
        ...
        if props_match:
            if module.params['virtual_drive']:
                props_match = check_virtual_drive_props(jctanner.remote_management_ucs.ucs, module, dn)
    else:
        props_match = False
    return props_match


def check_virtual_drive_props(jctanner.remote_management_ucs.ucs, module, dn):
...
def _configure_virtual_drive(module, mo):
    from jctanner.remote_management_ucs.ucsmsdk.mometa.lstorage.LstorageVirtualDriveDef import LstorageVirtualDriveDef
...
def main():
    ...
    if mo:
        if module.params['state'] == 'absent':
            ...
            props_match = check_disk_policy_props(jctanner.remote_management_ucs.ucs, module, mo, dn)

    if module.params['state'] == 'present' and not props_match:
        configure_disk_policy(jctanner.remote_management_ucs.ucs, module, dn)
