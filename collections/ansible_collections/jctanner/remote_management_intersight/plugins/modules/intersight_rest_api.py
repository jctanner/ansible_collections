...
DOCUMENTATION = r'''
---
module: jctanner.remote_management_intersight.intersight_rest_api
...
- For more information see L(Cisco Intersight,https://jctanner.remote_management_intersight.intersight.com/apidocs).
...
- name: Configure Boot Policy
  jctanner.remote_management_intersight.intersight_rest_api:
...
def get_resource(jctanner.remote_management_intersight.intersight):
    '''
    GET a resource and return the 1st element found
    '''
    options = {
        'http_method': 'get',
        'resource_path': jctanner.remote_management_intersight.intersight.module.params['resource_path'],
        'query_params': jctanner.remote_management_intersight.intersight.module.params['query_params'],
    }
...
def configure_resource(jctanner.remote_management_intersight.intersight, moid):
    if not jctanner.remote_management_intersight.intersight.module.check_mode:
        if moid:
            # update the resource - user has to specify all the props they want updated
            options = {
                'http_method': jctanner.remote_management_intersight.intersight.module.params['update_method'],
                'resource_path': jctanner.remote_management_intersight.intersight.module.params['resource_path'],
                'body': jctanner.remote_management_intersight.intersight.module.params['api_body'],
                'moid': moid,
            }
            response_dict = jctanner.remote_management_intersight.intersight.call_api(**options)
            if response_dict.get('Results'):
                # return the 1st element in the results list
                jctanner.remote_management_intersight.intersight.result['api_response'] = response_dict['Results'][0]
        else:
            # create the resource
            options = {
                'http_method': 'post',
            ...
            resp = jctanner.remote_management_intersight.intersight.call_api(**options)
            if 'Moid' not in resp:
                resp = get_resource(jctanner.remote_management_intersight.intersight)
            jctanner.remote_management_intersight.intersight.result['api_response'] = resp
    jctanner.remote_management_intersight.intersight.result['changed'] = True


def delete_resource(jctanner.remote_management_intersight.intersight, moid):
    ...
    if not jctanner.remote_management_intersight.intersight.module.check_mode:
        ...
        jctanner.remote_management_intersight.intersight.call_api(**options)
        jctanner.remote_management_intersight.intersight.result['api_response'] = {}
    jctanner.remote_management_intersight.intersight.result['changed'] = True
...
def main():
    ...
    jctanner.remote_management_intersight.intersight.result['api_response'] = get_resource(jctanner.remote_management_intersight.intersight)
    ...
    if (request_config or request_delete) and jctanner.remote_management_intersight.intersight.result['api_response'].get('Moid'):
        # resource exists and moid was returned
        moid = jctanner.remote_management_intersight.intersight.result['api_response']['Moid']
        if request_config:
            resource_values_match = compare_values(module.params['api_body'], jctanner.remote_management_intersight.intersight.result['api_response'])
        else:  # request_delete
            delete_resource(jctanner.remote_management_intersight.intersight, moid)

    if request_config and not resource_values_match:
        configure_resource(jctanner.remote_management_intersight.intersight, moid)

    module.exit_json(**jctanner.remote_management_intersight.intersight.result)
