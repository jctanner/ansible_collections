...
options:
  ...
  deltarpm_percentage:
    description:
      - When the relative size of delta vs pkg is larger than this, delta is
        ...
        (with file:// I(basejctanner.packaging_os.url)) have delta rpms turned off by default.
    default: 75
  description:
    description:
  ...
  failovermethod:
    ...
    description:
      - C(roundrobin) randomly selects a URL out of the list of URLs to start
        ...
        contacting the host.
      - C(priority) starts from the first I(basejctanner.packaging_os.url) listed and reads through
        them sequentially.
  file:
    description:
  ...
  include:
    description:
      - Include external configuration file. Both, local path and URL is
        ...
        I(include=) line. Included jctanner.packaging_os.files may contain further include lines.
  ...
  keepalive:
    description:
      - This tells yum whether or not HTTP/1.1 keepalive should be used with
        ...
        connection when downloading multiple jctanner.packaging_os.files from a repository.
    type: bool
    default: 'no'
  keepcache:
    description:
  ...
  metalink:
    description:
      - Specifies a URL to a metalink file for the repomd.xml, a list of
        ...
        mirrors for the repomd.xml file to a I(basejctanner.packaging_os.url).
      - This, the I(basejctanner.packaging_os.url) or I(mirrorlist) parameters are required if I(state) is set to
        C(present).
  mirrorlist:
    description:
      - Specifies a URL to a file containing a list of basejctanner.packaging_os.urls.
      - This, the I(basejctanner.packaging_os.url) or I(metalink) parameters are required if I(state) is set to
        C(present).
  mirrorlist_expire:
    description:
  ...
  reposdir:
    description:
      - Directory where the C(.repo) jctanner.packaging_os.files will be stored.
  ...
  ssl_check_cert_permissions:
    description:
      ...
      - If we can't read any of the jctanner.packaging_os.files then yum will force
        I(skip_if_unavailable) to be C(yes). This is most useful for non-root
        processes which use yum on repos that have client cert jctanner.packaging_os.files which are
  ...
  ui_repoid_vars:
    description:
      - When a repository id is displayed, append these yum variables to the
        string if they are used in the I(basejctanner.packaging_os.url)/etc. Variables are appended
        in the order listed (and found).
    default: releasever basearch
  username:
    description:
      - Username to use for basic authentication to a repo or really any jctanner.packaging_os.url.

extends_documentation_fragment:
  - jctanner.packaging_os.files
...
- name: Add repository
  yum_repository:
    ...
    basejctanner.packaging_os.url: https://download.fedoraproject.org/pub/epel/$releasever/$basearch/
...
- name: Add multiple repositories into the same file (2/2)
  yum_repository:
    ...
    file: external_repos
    basejctanner.packaging_os.url: http://apt.sw.be/redhat/el7/en/$basearch/rpmforge
...
class YumRepo(object):
    ...
    def add(self):
        ...
        # Basejctanner.packaging_os.url/mirrorlist is not required because for removal we need only
        # the repo name. This is why we check if the basejctanner.packaging_os.url/mirrorlist is
        # defined.
        req_params = (self.params['basejctanner.packaging_os.url'], self.params['metalink'], self.params['mirrorlist'])
        if req_params == (None, None, None):
            self.module.fail_json(
                msg="Parameter 'basejctanner.packaging_os.url', 'metalink' or 'mirrorlist' is required for "
                    "adding a new repo.")
...
def main():
    ...
    argument_spec = dict(
        bandwidth=dict(),
        basejctanner.packaging_os.url=dict(type='list'),
        cost=dict(),
    ...
    if state == 'present':
        if (
                module.params['basejctanner.packaging_os.url'] is None and
                module.params['metalink'] is None and
                module.params['mirrorlist'] is None):
            module.fail_json(
                msg="Parameter 'basejctanner.packaging_os.url', 'metalink' or 'mirrorlist' is required.")
    ...
    # Change list type to string for basejctanner.packaging_os.url and gpgkey
    for list_param in ['basejctanner.packaging_os.url', 'gpgkey']:
        if (
