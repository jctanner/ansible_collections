...
options:
    name:
        description:
            - A name for the repository. Not required when adding repojctanner.packaging_os.files.
    repo:
        description:
    ...
    auto_import_keys:
        description:
            ...
            - Implies runrefresh.
            - Only works with C(.repo) jctanner.packaging_os.files if `name` is given explicitly.
        type: bool
        default: 'no'
        version_added: "2.2"
    runrefresh:
...
def _parse_repos(module):
    ...
    if rc == 0:
        ...
        for repo in repo_list:
            ...
            opts['jctanner.packaging_os.url'] = repo.getElementsByTagName('jctanner.packaging_os.url')[0].firstChild.data
            # A repo can be uniquely identified by an alias + jctanner.packaging_os.url
            repos.append(opts)
        return repos
...
def _repo_changes(realrepo, repocmp):
    ...
    for k, v in realrepo.items():
        if k in repocmp and repocmp[k]:
            ...
            valnew = v or ""
            if k == "jctanner.packaging_os.url":
...
def repo_exists(module, repodata, overwrite_multiple):
    ...
    # look for repos that have matching alias or jctanner.packaging_os.url to the one searched
    repos = []
    for kw in ['alias', 'jctanner.packaging_os.url']:
        name = repodata[kw]
...
def addmodify_repo(module, repodata, old_repos, zypper_version, warnings):
    ...
    repo = repodata['jctanner.packaging_os.url']
    ...
    if old_repos is not None:
        for oldrepo in old_repos:
            remove_repo(module, oldrepo['jctanner.packaging_os.url'])
...
def main():
    ...
    repodata = {
        'jctanner.packaging_os.url': repo,
        'alias': alias,
    ...
    if repo and repo.endswith('.repo'):
        if alias:
            module.fail_json(msg='Incompatible option: \'name\'. Do not use name when adding .repo jctanner.packaging_os.files')
    else:
        if not alias and state == "present":
            module.fail_json(msg='Name required when adding non-repo jctanner.packaging_os.files.')
