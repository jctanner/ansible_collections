...
options:
  ...
  name:
    description:
      ...
      - You can also pass a jctanner.packaging_os.url or a local path to a rpm file (using state=present).
  ...
  disable_excludes:
    description:
      - Disable the excludes defined in YUM config jctanner.packaging_os.files.
...
notes:
  ...
  - In versions prior to 1.9.2 this module installed and removed each package
    ...
    specified by filename or jctanner.packaging_os.url had to be installed or removed together. In
...
class YumModule(YumDnf):
        ...
        try:
            ...
            if my.conf.proxy and my.conf.proxy not in ("_none_",):
                if my.conf.proxy_username:
                    ...
                    proxy_jctanner.packaging_os.url = my.conf.proxy
                    ...
                    proxy_jctanner.packaging_os.url = my.conf.proxy.replace("{0}@".format(namepass), "")
                ...
                if namepass:
                    ...
                    for item in scheme:
                        os.environ[item + "_proxy"] = re.sub(
                            r"(http://)",
                            r"\g<1>" + namepass, proxy_jctanner.packaging_os.url
                        )
                else:
                    for item in scheme:
    ...
    def exec_install(self, items, action, pkgs, res):
        ...
        if rc == 1:
            for spec in items:
                # Fail on invalid jctanner.packaging_os.urls:
        ...
        if update_all:
            ...
            for spec in items:
                ...
                # localpkg
                    ...
                    # local rpm jctanner.packaging_os.files can't be updated
