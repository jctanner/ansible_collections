...
- homebrew_tap:
    ...
    jctanner.packaging_os.url: 'https://bitbucket.org/telemachus/brew'
'''
...
def add_tap(module, brew_path, tap, jctanner.packaging_os.url=None):
    '''Adds a single tap.'''
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            jctanner.packaging_os.url=dict(default=None, required=False),
    ...
    if module.params['state'] == 'present':
        if jctanner.packaging_os.url is None:
            ...
            if len(taps) > 1:
                msg = "List of multiple taps may not be provided with 'jctanner.packaging_os.url' option."
                module.fail_json(msg=msg)
            else:
                failed, changed, msg = add_tap(module, brew_path, taps[0], jctanner.packaging_os.url)
