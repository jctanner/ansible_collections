...
options:
  contentjctanner.packaging_os.url:
    description:
  ...
  jctanner.packaging_os.url:
    description:
      - Overrides both I(contentjctanner.packaging_os.url) and I(versionjctanner.packaging_os.url).
  verify:
    description:
      - Verify content for OS version.
    type: bool
  versionjctanner.packaging_os.url:
    description:
...
- name: Check integrity of jctanner.packaging_os.filesystem
  swupd:
    verify: yes
...
class Swupd(object):
    FILES_NOT_MATCH = "jctanner.packaging_os.files did not match"
    FILES_REPLACED = "missing jctanner.packaging_os.files were replaced"
    FILES_FIXED = "jctanner.packaging_os.files were fixed"
    FILES_DELETED = "jctanner.packaging_os.files were deleted"
    ...
    def _get_cmd(self, command):
        ...
        if self.jctanner.packaging_os.url:
            cmd += " --jctanner.packaging_os.url=%s" % self.jctanner.packaging_os.url
        else:
            if self.contentjctanner.packaging_os.url and command != "check-update":
                cmd += " --contentjctanner.packaging_os.url=%s" % self.contentjctanner.packaging_os.url
            if self.versionjctanner.packaging_os.url:
                cmd += " --versionjctanner.packaging_os.url=%s" % self.versionjctanner.packaging_os.url
    ...
    def _needs_verify(self):
        ...
        if self.rc != 0:
            self.failed = True
            self.msg = "Failed to check for jctanner.packaging_os.filesystem inconsistencies."
    ...
    def verify_os(self):
        """Verifies jctanner.packaging_os.filesystem against specified or current version"""
        if self.module.check_mode:
            self.module.exit_json(changed=self._needs_verify())

        if not self._needs_verify():
            self.msg = "No jctanner.packaging_os.files where changed"
            return
