...
options:
    ...
    mode:
        description:
            - The octal mode for newly created jctanner.packaging_os.files in sources.list.d
        default: '0644'
        version_added: "1.6"
    update_cache:
        description:
    ...
    filename:
        description:
            - Sets the name of the source list file in sources.list.d.
              Defaults to a file name based on the repository source jctanner.packaging_os.url.
...
class SourcesList(object):
    def __init__(self, module):
        self.module = module
        self.jctanner.packaging_os.files = {}  # group sources by file
    ...
    def __iter__(self):
        ...
        for file, sources in self.jctanner.packaging_os.files.items():
    ...
    @staticmethod
    def _apt_cfg_file(jctanner.packaging_os.filespec):
        '''
        Wrapper for `apt_pkg` module for running with Python 2.5
        '''
        try:
            result = apt_pkg.config.find_file(jctanner.packaging_os.filespec)
        except AttributeError:
            result = apt_pkg.Config.FindFile(jctanner.packaging_os.filespec)
        return result
    ...
    def load(self, file):
        ...
        self.jctanner.packaging_os.files[file] = group

    def save(self):
        for filename, sources in list(self.jctanner.packaging_os.files.items()):
            if sources:
                ...
                del self.jctanner.packaging_os.files[filename]
                if os.path.exists(filename):
                    os.remove(filename)

    def dump(self):
        dumpstruct = {}
        for filename, sources in self.jctanner.packaging_os.files.items():
            if sources:
                lines = []
                for n, valid, enabled, source, comment in sources:
                    chunks = []
    ...
    def modify(self, file, n, enabled=None, source=None, comment=None):
        ...
        '''
        valid, enabled_old, source_old, comment_old = self.jctanner.packaging_os.files[file][n][1:]
        self.jctanner.packaging_os.files[file][n] = (n, valid, self._choice(enabled, enabled_old), self._choice(source, source_old), self._choice(comment, comment_old))
    ...
    def _add_valid_source(self, source_new, comment_new, file):
        ...
        if not found:
            ...
            if file not in self.jctanner.packaging_os.files:
                self.jctanner.packaging_os.files[file] = []

            jctanner.packaging_os.files = self.jctanner.packaging_os.files[file]
            jctanner.packaging_os.files.append((len(jctanner.packaging_os.files), True, True, source_new, comment_new))
            self.new_repos.add(file)

    def add_source(self, line, comment='', file=None):
        source = self._parse(line, raise_if_invalid_or_disabled=True)[2]

        # Prefer separate jctanner.packaging_os.files for new sources.
    ...
    def _remove_valid_source(self, source):
        ...
        for filename, n, enabled, src, comment in self:
            if source == src and enabled:
                self.jctanner.packaging_os.files[filename].pop(n)
...
class UbuntuSourcesList(SourcesList):
    ...
    def _get_ppa_info(self, owner_name, ppa_name):
        ...
        response, info = fetch_jctanner.packaging_os.url(self.module, lp_api, headers=headers)
        if info['status'] != 200:
    ...
    def add_source(self, line, comment='', file=None):
        if line.startswith('ppa:'):
            ...
            if source in self.repos_jctanner.packaging_os.urls:
    ...
    @property
    def repos_jctanner.packaging_os.urls(self):
        _repositories = []
        for parsed_repos in self.jctanner.packaging_os.files.values():
...
def main():
    ...
    if changed and not module.check_mode:
        try:
            ...
            # Revert the sourcelist jctanner.packaging_os.files to their previous state.
            # First remove any new jctanner.packaging_os.files that were created:
            for filename in set(sources_after.keys()).difference(sources_before.keys()):
                if os.path.exists(filename):
                    os.remove(filename)
            # Now revert the existing jctanner.packaging_os.files to their former state:
