...
options:
  ...
  generate_sqlite:
    description:
      - Boolean flag to indicate whether sqlite jctanner.packaging_os.files should be generated during
  ...
  proxy_host:
    description:
      - Proxy jctanner.packaging_os.url setting for the pulp repository importer. This is in the
        format scheme://host.
    required: false
    default: null
  proxy_port:
  ...
  relative_jctanner.packaging_os.url:
    description:
  ...
  repoview:
    description:
      - Whether to generate repoview jctanner.packaging_os.files for a published repository. Setting
  ...
  jctanner.packaging_os.url_password:
    description:
      - The password for use in HTTP basic authentication to the pulp API.
        If the I(jctanner.packaging_os.url_username) parameter is not specified, the I(jctanner.packaging_os.url_password)
        parameter will not be used.
  jctanner.packaging_os.url_username:
    description:
...
- name: Create a new repo with name 'my_repo'
  pulp_repo:
    name: my_repo
    relative_jctanner.packaging_os.url: my/repo
    state: present
...
- name: Create a repo with a feed and a relative URL
  pulp_repo:
    ...
    relative_jctanner.packaging_os.url: centos/6/updates
    jctanner.packaging_os.url_username: admin
    jctanner.packaging_os.url_password: admin
...
from ansible.module_utils.jctanner.packaging_os.urls import jctanner.packaging_os.url_argument_spec
...
class pulp_server(object):
    ...
    def create_repo(
        ...
        jctanner.packaging_os.url = "%s/pulp/api/v2/repositories/" % self.host
        data = dict()
        data['id'] = repo_id
        data['distributors'] = []
        ...
        if self.repo_type == 'rpm':
            ...
            yum_distributor['distributor_config']['relative_jctanner.packaging_os.url'] = relative_jctanner.packaging_os.url
            ...
            if add_export_distributor:
                ...
                export_distributor['distributor_config']['relative_jctanner.packaging_os.url'] = relative_jctanner.packaging_os.url
        ...
        response, info = fetch_jctanner.packaging_os.url(
            self.module,
            jctanner.packaging_os.url,
        ...
        if info['status'] != 201:
            self.module.fail_json(
                ...
                response=info['msg'],
                jctanner.packaging_os.url=jctanner.packaging_os.url)
        else:
            return True

    def delete_repo(self, repo_id):
        jctanner.packaging_os.url = "%s/pulp/api/v2/repositories/%s/" % (self.host, repo_id)
        response, info = fetch_jctanner.packaging_os.url(self.module, jctanner.packaging_os.url, data='', method='DELETE')
    ...
    def publish_repo(self, repo_id, publish_distributor):
        jctanner.packaging_os.url = "%s/pulp/api/v2/repositories/%s/actions/publish/" % (self.host, repo_id)
    ...
    def sync_repo(self, repo_id):
        jctanner.packaging_os.url = "%s/pulp/api/v2/repositories/%s/actions/sync/" % (self.host, repo_id)
        response, info = fetch_jctanner.packaging_os.url(self.module, jctanner.packaging_os.url, data='', method='POST')
    ...
    def update_repo_distributor_config(self, repo_id, **kwargs):
        jctanner.packaging_os.url = "%s/pulp/api/v2/repositories/%s/distributors/" % (self.host, repo_id)
        repo_config = self.get_repo_config_by_id(repo_id)

        for distributor in repo_config['distributors']:
            distributor_jctanner.packaging_os.url = "%s%s/" % (jctanner.packaging_os.url, distributor['id'])
            data = dict()
            ...
            response, info = fetch_jctanner.packaging_os.url(
                self.module,
                distributor_jctanner.packaging_os.url,
                data=json.dumps(data),
                method='PUT')

            if info['status'] != 202:
                self.module.fail_json(
                    msg="Failed to set the relative jctanner.packaging_os.url for the repository.",
                    status_code=info['status'],
                    response=info['msg'],
                    jctanner.packaging_os.url=jctanner.packaging_os.url)

    def update_repo_importer_config(self, repo_id, **kwargs):
        jctanner.packaging_os.url = "%s/pulp/api/v2/repositories/%s/importers/" % (self.host, repo_id)
        data = dict()
    ...
    def set_repo_list(self):
        jctanner.packaging_os.url = "%s/pulp/api/v2/repositories/?details=true" % self.host
        response, info = fetch_jctanner.packaging_os.url(self.module, jctanner.packaging_os.url, method='GET')
    ...
    def verify_tasks_completed(self, response_dict):
        for task in response_dict['spawned_tasks']:
            task_jctanner.packaging_os.url = "%s%s" % (self.host, task['_href'])

            while True:
                response, info = fetch_jctanner.packaging_os.url(
                    self.module,
                    task_jctanner.packaging_os.url,
                    data='',
                    method='GET')
                ...
                if info['status'] != 200:
                    self.module.fail_json(
                        ...
                        status_code=info['status'],
                        response=info['msg'],
                        jctanner.packaging_os.url=task_jctanner.packaging_os.url)
...
def main():
    argument_spec = jctanner.packaging_os.url_argument_spec()
    argument_spec.update(
        ...
        relative_jctanner.packaging_os.url=dict(),
    ...
    relative_jctanner.packaging_os.url = module.params['relative_jctanner.packaging_os.url']
    ...
    if (state == 'present') and (not relative_jctanner.packaging_os.url):
        module.fail_json(msg="When state is present, relative_jctanner.packaging_os.url is required.")
    ...
    if state == 'present':
        if not repo_exists:
            if not module.check_mode:
                server.create_repo(
                    repo_id=repo,
                    relative_jctanner.packaging_os.url=relative_jctanner.packaging_os.url,
                    feed=feed,
            ...
            if relative_jctanner.packaging_os.url is not None:
