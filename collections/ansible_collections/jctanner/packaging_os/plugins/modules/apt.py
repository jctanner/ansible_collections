...
options:
  ...
  purge:
    description:
     - Will force purging of configuration jctanner.packaging_os.files if the module state is set to I(absent).
    type: bool
    default: 'no'
  default_release:
    description:
  ...
  autoclean:
    description:
      - If C(yes), cleans the local repository of retrieved package jctanner.packaging_os.files that can no longer be downloaded.
    type: bool
    default: 'no'
    version_added: "2.4"
  policy_rc_d:
...
from ansible.module_utils.jctanner.packaging_os.urls import fetch_file
...
def package_status(m, pkgname, version, cache, state):
            ...
            try:
                ...
                if provided_packages:
                    ...
                    if cache.is_virtual_package(pkgname) and len(provided_packages) == 1:
                        ...
                        installed, version_ok, upgradable, has_jctanner.packaging_os.files = package_status(m, package.name, version, cache, state='install')
                        if installed:
                            is_installed = True
    ...
    try:
        has_jctanner.packaging_os.files = len(pkg.installed_jctanner.packaging_os.files) > 0
    except UnicodeDecodeError:
        has_jctanner.packaging_os.files = True
    except AttributeError:
        has_jctanner.packaging_os.files = False  # older python-apt cannot be used to determine non-purged
    ...
    return package_is_installed, version_is_installed, package_is_upgradable, has_jctanner.packaging_os.files
...
def install(m, pkgspec, cache, upgrade=False, default_release=None,
    ...
    for package in pkgspec:
        ...
        installed, installed_version, upgradable, has_jctanner.packaging_os.files = package_status(m, name, version, cache, state='install')
...
def remove(m, pkgspec, cache, purge=False, force=False,
    ...
    for package in pkgspec:
        name, version = package_split(package)
        installed, installed_version, upgradable, has_jctanner.packaging_os.files = package_status(m, name, version, cache, state='remove')
        if installed_version or (has_jctanner.packaging_os.files and purge):
            pkg_list.append("'%s'" % package)
    packages = ' '.join(pkg_list)
...
def get_cache(module):
    ...
    try:
        ...
        if '/var/lib/apt/lists/' in to_native(e).lower():
            # update cache until jctanner.packaging_os.files are fixed or retries exceeded
            retries = 0
            while retries < 2:
...
def main():
    ...
    try:
        ...
        module.fail_json(msg="Could not fetch updated apt jctanner.packaging_os.files")
