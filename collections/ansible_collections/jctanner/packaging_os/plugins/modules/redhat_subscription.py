...
notes:
    ...
    - Since 2.5 values for I(server_hostname), I(server_insecure), I(rhsm_basejctanner.packaging_os.url),
...
options:
    ...
    rhsm_basejctanner.packaging_os.url:
        description:
            - Specify CDN basejctanner.packaging_os.url
    rhsm_repo_ca_cert:
        description:
...
class Rhsm(RegistrationBase):
    ...
    def register(self, username, password, auto_attach, activationkey, org_id,
                 ...
                 rhsm_basejctanner.packaging_os.url, server_insecure, server_hostname, server_proxy_hostname,
        ...
        if rhsm_basejctanner.packaging_os.url:
            args.extend(['--basejctanner.packaging_os.url', rhsm_basejctanner.packaging_os.url])

        if server_insecure:
            args.extend(['--insecure'])

        if server_hostname:
            args.extend(['--serverjctanner.packaging_os.url', server_hostname])
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            server_insecure=dict(default=None,
                                 required=False),
            rhsm_basejctanner.packaging_os.url=dict(default=None,
                              required=False),
    ...
    rhsm_basejctanner.packaging_os.url = module.params['rhsm_basejctanner.packaging_os.url']
        ...
        if rhsm.is_registered and not force_register:
            ...
            try:
                ...
                rhsm.register(username, password, auto_attach, activationkey, org_id,
                              ...
                              environment, rhsm_basejctanner.packaging_os.url, server_insecure, server_hostname,
