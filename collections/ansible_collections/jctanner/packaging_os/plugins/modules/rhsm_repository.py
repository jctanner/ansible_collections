...
def get_repository_list(module, list_parameter):
    ...
    repo_jctanner.packaging_os.url_re = re.compile(r'Repo URL:\s+(.*)')
    repo_enabled_re = re.compile(r'Enabled:\s+(.*)')

    repo_id = ''
    repo_name = ''
    repo_jctanner.packaging_os.url = ''
    repo_enabled = ''
    ...
    for line in out.splitlines():
        ...
        repo_jctanner.packaging_os.url_match = repo_jctanner.packaging_os.url_re.match(line)
        if repo_jctanner.packaging_os.url_match:
            repo_jctanner.packaging_os.url = repo_jctanner.packaging_os.url_match.group(1)
            continue
        ...
        if repo_enabled_match:
            ...
            repo = {
                "id": repo_id,
                "name": repo_name,
                "jctanner.packaging_os.url": repo_jctanner.packaging_os.url,
