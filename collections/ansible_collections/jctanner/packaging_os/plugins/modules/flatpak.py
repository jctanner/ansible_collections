...
import subprocess
from ansible.module_utils.six.moves.jctanner.packaging_os.urllib.parse import jctanner.packaging_os.urlparse
...
def _match_installed_flat_name(module, binary, name, method):
    # This is a difficult function, since if the user supplies a flatpakref jctanner.packaging_os.url,
...
def _parse_flatpak_name(name):
    if name.startswith('http://') or name.startswith('https://'):
        file_name = jctanner.packaging_os.urlparse(name).path.split('/')[-1]
