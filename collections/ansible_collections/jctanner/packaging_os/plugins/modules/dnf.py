...
options:
  name:
    description:
      - "A package name or package specifier with version, like C(name-1.0).
        ...
        You can also pass a jctanner.packaging_os.url or a local path to a rpm file.
  ...
  disable_excludes:
    description:
      - Disable the excludes defined in DNF config jctanner.packaging_os.files.
  ...
  validate_certs:
    description:
      - This only applies if using a https jctanner.packaging_os.url as the source of the rpm. e.g. for localinstall. If set to C(no), the SSL certificates will not be validated.
...
from ansible_collections.jctanner.packaging_os.plugins.module_utils.yumdnf import YumDnf, yumdnf_argument_spec

# 64k.  Number of bytes to read at a time when manually downloading pkgs via a jctanner.packaging_os.url
BUFSIZE = 65536
...
class DnfModule(YumDnf):
    ...
    #   http://yum.basejctanner.packaging_os.url.org/gitweb?p=yum.git;a=blob;f=rpmUtils/miscutils.py
    ...
    def fetch_rpm_from_jctanner.packaging_os.url(self, spec):
        ...
        try:
            rsp, info = fetch_jctanner.packaging_os.url(self.module, spec)
            if not rsp:
                self.module.fail_json(
    ...
    def _configure_base(self, base, conf_file, disable_gpg_check, installroot='/'):
        ...
        # Load substitutions from the jctanner.packaging_os.filesystem
    ...
    def _parse_spec_group_file(self):
        ...
        for name in self.names:
            if '://' in name:
                name = self.fetch_rpm_from_jctanner.packaging_os.url(name)
    ...
    def ensure(self):
        ...
        if self.names == ['*'] and self.state == 'latest':
            ...
            if self.state in ['installed', 'present']:
                # Install jctanner.packaging_os.files.
