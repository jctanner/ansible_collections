...
options:
  ...
  state:
    ...
    default: present
notes:
- If the package is already installed, even the package/jctanner.packaging_os.fileset is new, the module will not install it.
'''
...
def _check_new_pkg(module, package, repository_path):
    """
    Check if the package of jctanner.packaging_os.fileset is correct name and repository path.

    :param module: Ansible module arguments spec.
    :param package: Package/jctanner.packaging_os.fileset name.
...
def _check_installed_pkg(module, package, repository_path):
    ...
    for line in full_pkg_data:
        pkg_name, jctanner.packaging_os.fileset, level = line.split(':')[0:3]
        pkg_data[pkg_name] = jctanner.packaging_os.fileset, level
    ...
    for package in packages:
        ...
        if pkg_check:
            ...
            if pkg_check_current:
                # Check if package is a package and not a jctanner.packaging_os.fileset, get version
                ...
                if package in pkg_info.keys():
                    ...
                    # If the package is not a package but a jctanner.packaging_os.fileset, confirm
                    # and add the jctanner.packaging_os.fileset/package into already installed list
