...
options:
  executable:
    ...
    default: flatpak
  flatpakrepo_jctanner.packaging_os.url:
    description:
    - The URL to the I(flatpakrepo) file representing the repository remote to add.
    - When used with I(state=present), the flatpak remote specified under the I(flatpakrepo_jctanner.packaging_os.url)
...
- name: Add the Gnome flatpak remote to the system installation
  flatpak_remote:
    name: gnome
    state: present
    flatpakrepo_jctanner.packaging_os.url: https://sdk.gnome.org/gnome-apps.flatpakrepo

- name: Add the flathub flatpak repository remote to the user installation
  flatpak_remote:
    name: flathub
    state: present
    flatpakrepo_jctanner.packaging_os.url: https://dl.flathub.org/repo/flathub.flatpakrepo
    method: user
...
def add_remote(module, binary, name, flatpakrepo_jctanner.packaging_os.url, method):
    """Add a new remote."""
    global result
    command = "{0} remote-add --{1} {2} {3}".format(
        binary, method, name, flatpakrepo_jctanner.packaging_os.url)
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            flatpakrepo_jctanner.packaging_os.url=dict(type='str'),
    ...
    flatpakrepo_jctanner.packaging_os.url = module.params['flatpakrepo_jctanner.packaging_os.url']
    method = module.params['method']
    state = module.params['state']
    executable = module.params['executable']
    binary = module.get_bin_path(executable, None)

    if flatpakrepo_jctanner.packaging_os.url is None:
        flatpakrepo_jctanner.packaging_os.url = ''
    ...
    if state == 'present' and not remote_already_exists:
        add_remote(module, binary, name, flatpakrepo_jctanner.packaging_os.url, method)
