...
def install_packages(module, pacman_path, state, packages, package_jctanner.packaging_os.files):
    install_c = 0
    package_err = []
    message = ""
    data = []
    ...
    to_install_repos = []
    to_install_jctanner.packaging_os.files = []
    for i, package in enumerate(packages):
        ...
        if package_jctanner.packaging_os.files[i]:
            to_install_jctanner.packaging_os.files.append(package_jctanner.packaging_os.files[i])
        else:
    ...
    if to_install_jctanner.packaging_os.files:
        cmd = "%s --upgrade --noconfirm --noprogressbar --needed %s %s" % (pacman_path, module.params["extra_args"], " ".join(to_install_jctanner.packaging_os.files))
        rc, stdout, stderr = module.run_command(cmd, check_rc=False)

        if rc != 0:
            module.fail_json(msg="failed to install %s: %s" % (" ".join(to_install_jctanner.packaging_os.files), stderr))
        ...
        install_c += len(to_install_jctanner.packaging_os.files)
...
def main():
    ...
    if p['name']:
        ...
        pkg_jctanner.packaging_os.files = []
        for i, pkg in enumerate(pkgs):
            if not pkg:  # avoid empty strings
                ...
                pkg_jctanner.packaging_os.files.append(pkg)
                pkgs[i] = re.sub(r'-[0-9].*$', '', pkgs[i].split('/')[-1])
            else:
                pkg_jctanner.packaging_os.files.append(None)

        if module.check_mode:
            check_packages(module, pacman_path, pkgs, p['state'])

        if p['state'] in ['present', 'latest']:
            install_packages(module, pacman_path, p['state'], pkgs, pkg_jctanner.packaging_os.files)
        elif p['state'] == 'absent':
