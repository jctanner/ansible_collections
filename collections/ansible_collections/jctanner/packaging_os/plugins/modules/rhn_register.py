...
options:
    ...
    password:
        ...
        type: str
    server_jctanner.packaging_os.url:
        description:
...
- name: Register as user with password against a satellite server
  rhn_register:
    ...
    username: joe_user
    password: somepass
    server_jctanner.packaging_os.url: https://xmlrpc.my.satellite/XMLRPC
...
from ansible.module_utils.six.moves import jctanner.packaging_os.urllib, xmlrpc_client
...
class Rhn(redhat.RegistrationBase):
    ...
    @property
    def server_jctanner.packaging_os.url(self):
    ...
    def hostname(self):
        '''
            ...
            Returns: str
        '''
        jctanner.packaging_os.url = jctanner.packaging_os.urllib.parse.jctanner.packaging_os.urlparse(self.server_jctanner.packaging_os.url)
        return jctanner.packaging_os.url[1].replace('xmlrpc.', '')
    ...
    def configure_server_jctanner.packaging_os.url(self, server_jctanner.packaging_os.url):
        '''
            Configure server_jctanner.packaging_os.url for registration
        '''

        self.config.set('serverURL', server_jctanner.packaging_os.url)
        self.config.save()
    ...
    def register(self, enable_eus=False, activationkey=None, profilename=None, sslcacert=None, systemorgid=None, nopackages=False):
        ...
        if self.server_jctanner.packaging_os.url:
            register_cmd.extend(['--serverUrl', self.server_jctanner.packaging_os.url])
        if enable_eus:
    ...
    def api(self, method, *args):
        ...
        if self.server is None:
            if self.hostname != 'rhn.redhat.com':
                jctanner.packaging_os.url = "https://%s/rpc/api" % self.hostname
            else:
                jctanner.packaging_os.url = "https://xmlrpc.%s/rpc/api" % self.hostname
            self.server = xmlrpc_client.ServerProxy(jctanner.packaging_os.url)
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            server_jctanner.packaging_os.url=dict(type='str'),
    ...
    server_jctanner.packaging_os.url = module.params['server_jctanner.packaging_os.url']
    ...
    # use the provided server jctanner.packaging_os.url and persist it to the rhn config.
    if server_jctanner.packaging_os.url:
        rhn.configure_server_jctanner.packaging_os.url(server_jctanner.packaging_os.url)

    if not rhn.server_jctanner.packaging_os.url:
        module.fail_json(
            msg="No serverURL was found (from either the 'server_jctanner.packaging_os.url' module arg or the config file option 'serverURL' in /etc/sysconfig/rhn/up2date)"
        )
