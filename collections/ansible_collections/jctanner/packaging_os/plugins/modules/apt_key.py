...
options:
    ...
    validate_certs:
        description:
            - If C(no), SSL certificates for the target jctanner.packaging_os.url will not be validated. This should only be used
...
- name: Add an Apt signing key, uses whichever key is at the URL
  apt_key:
    jctanner.packaging_os.url: https://ftp-master.debian.org/keys/archive-key-6.0.asc
    state: present
...
def download_key(module, jctanner.packaging_os.url):
    # FIXME: move get_jctanner.packaging_os.url code to common, allow for in-memory D/L, support proxies
    # and reuse here
    if jctanner.packaging_os.url is None:
        module.fail_json(msg="needed a URL but was not specified")

    try:
        rsp, info = fetch_jctanner.packaging_os.url(module, jctanner.packaging_os.url)
        if info['status'] != 200:
            module.fail_json(msg="Failed to download key at %s: %s" % (jctanner.packaging_os.url, info['msg']))

        return rsp.read()
    except Exception:
        module.fail_json(msg="error getting key id from jctanner.packaging_os.url: %s" % jctanner.packaging_os.url, traceback=format_exc())
...
def main():
    module = AnsibleModule(
        ...
        mutually_exclusive=(('data', 'filename', 'keyserver', 'jctanner.packaging_os.url'),),
    )

    key_id = module.params['id']
    jctanner.packaging_os.url = module.params['jctanner.packaging_os.url']
    ...
    if state == 'present':
        if fingerprint and fingerprint in keys:
            ...
            if not filename and not data and not keyserver:
                data = download_key(module, jctanner.packaging_os.url)
