...
def get_want_state(names, remove=False):
    packages = []
    jctanner.packaging_os.urls = []
    for name in names:
        if '://' in name or name.endswith('.rpm'):
            jctanner.packaging_os.urls.append(name)
        else:
    ...
    return packages, jctanner.packaging_os.urls
...
def package_present(m, name, want_latest):
    ...
    packages, jctanner.packaging_os.urls = get_want_state(name)
    ...
    if not packages and not jctanner.packaging_os.urls:
    ...
    cmd.append('--')
    cmd.extend(jctanner.packaging_os.urls)
...
def package_absent(m, name):
    ...
    retvals = {'rc': 0, 'stdout': '', 'stderr': ''}
    # Get package state
    packages, jctanner.packaging_os.urls = get_want_state(name, remove=True)
    if any(p.prefix == '+' for p in packages):
        m.fail_json(msg="Can not combine '+' prefix with state=remove/absent.")
    if jctanner.packaging_os.urls:
