...
options:
  name:
    ...
    required: true
  list_jctanner.packaging_os.url:
    description:
...
- layman:
    name: cvut
    list_jctanner.packaging_os.url: 'http://raw.github.com/cvut/gentoo-overlay/master/overlay.xml'
...
def download_jctanner.packaging_os.url(module, jctanner.packaging_os.url, dest):
    '''
    :param jctanner.packaging_os.url: the URL to download
    ...
    # Hack to add params in the form that fetch_jctanner.packaging_os.url expects
    module.params['http_agent'] = USERAGENT
    response, info = fetch_jctanner.packaging_os.url(module, jctanner.packaging_os.url)
    if info['status'] != 200:
        raise ModuleError("Failed to get %s: %s" % (jctanner.packaging_os.url, info['msg']))
...
def install_overlay(module, name, list_jctanner.packaging_os.url=None):
    '''Installs the overlay repository. If not on the central overlays list,
    then :list_jctanner.packaging_os.url of an alternative list must be provided. The list will be
    ...
    :param list_jctanner.packaging_os.url: the URL of the remote repositories list to look for the overlay
    ...
    if not layman.is_repo(name):
        if not list_jctanner.packaging_os.url:
        ...
        download_jctanner.packaging_os.url(module, list_jctanner.packaging_os.url, dest)
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            list_jctanner.packaging_os.url=dict(aliases=['jctanner.packaging_os.url']),
    ...
    state, name, jctanner.packaging_os.url = (module.params[key] for key in ['state', 'name', 'list_jctanner.packaging_os.url'])

    changed = False
    try:
        if state == 'present':
            changed = install_overlay(module, name, jctanner.packaging_os.url)

        elif state == 'updated':
            if name == 'ALL':
                sync_overlays()
            elif install_overlay(module, name, jctanner.packaging_os.url):
                changed = True
            else:
                sync_overlay(name)
        else:
