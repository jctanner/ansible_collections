...
DOCUMENTATION = r'''
---
module: jctanner.network_meraki.meraki_switchport
...
def sort_vlans(jctanner.network_meraki.meraki, vlans):
    converted = set()
    for vlan in vlans:
...
def main():
    ...
    jctanner.network_meraki.meraki = MerakiModule(module, function='switchport')
    jctanner.network_meraki.meraki.params['follow_redirects'] = 'all'

    if jctanner.network_meraki.meraki.params['type'] == 'trunk':
        if not jctanner.network_meraki.meraki.params['allowed_vlans']:
            jctanner.network_meraki.meraki.params['allowed_vlans'] = ['all']  # Backdoor way to set default without conflicting on access
    ...
    if jctanner.network_meraki.meraki.params['state'] == 'query':
        if jctanner.network_meraki.meraki.params['number']:
            path = jctanner.network_meraki.meraki.construct_path('get_one', custom={'serial': jctanner.network_meraki.meraki.params['serial'],
            ...
            path = jctanner.network_meraki.meraki.construct_path('get_all', custom={'serial': jctanner.network_meraki.meraki.params['serial']})
            response = jctanner.network_meraki.meraki.request(path, method='GET')
            jctanner.network_meraki.meraki.result['data'] = response
    elif jctanner.network_meraki.meraki.params['state'] == 'present':
        payload = dict()

        for k, v in jctanner.network_meraki.meraki.params.items():
            try:
        ...
        if jctanner.network_meraki.meraki.params['allowed_vlans'][0] == 'all':
            allowed.add('all')
        else:
            for vlan in jctanner.network_meraki.meraki.params['allowed_vlans']:
                allowed.add(str(vlan))
            if jctanner.network_meraki.meraki.params['vlan'] is not None:
                allowed.add(str(jctanner.network_meraki.meraki.params['vlan']))
        if len(allowed) > 1:  # Convert from list to comma separated
            payload['allowedVlans'] = sort_vlans(jctanner.network_meraki.meraki, allowed)
        else:
            payload['allowedVlans'] = next(iter(allowed))

        # Exceptions need to be made for idempotency check based on how Meraki returns
        if jctanner.network_meraki.meraki.params['type'] == 'access':
            if not jctanner.network_meraki.meraki.params['vlan']:  # VLAN needs to be specified in access ports, but can't default to it
                payload['vlan'] = 1

        proposed = payload.copy()
        query_path = jctanner.network_meraki.meraki.construct_path('get_one', custom={'serial': jctanner.network_meraki.meraki.params['serial'],
                                                              'number': jctanner.network_meraki.meraki.params['number'],
                                                              })
        original = jctanner.network_meraki.meraki.request(query_path, method='GET')
        if jctanner.network_meraki.meraki.params['type'] == 'trunk':
            proposed['voiceVlan'] = original['voiceVlan']  # API shouldn't include voice VLAN on a trunk port
        if jctanner.network_meraki.meraki.is_update_required(original, proposed, optional_ignore=['number']):
            path = jctanner.network_meraki.meraki.construct_path('update', custom={'serial': jctanner.network_meraki.meraki.params['serial'],
                                                           'number': jctanner.network_meraki.meraki.params['number'],
                                                           })
