...
DOCUMENTATION = r'''
---
module: jctanner.network_meraki.meraki_ssid
...
options:
    ...
    auth_mode:
        ...
        type: str
        choices: [open, psk, open-with-radius, 8021x-jctanner.network_meraki.meraki, 8021x-radius]
    encryption_mode:
        description:
...
def main():
    ...
    argument_spec.update(state=dict(type='str', choices=['absent', 'present', 'query'], default='present'),
                         ...
                         auth_mode=dict(type='str', choices=['open', 'psk', 'open-with-radius', '8021x-jctanner.network_meraki.meraki', '8021x-radius']),
    ...
    jctanner.network_meraki.meraki = MerakiModule(module, function='ssid')
    ...
    if jctanner.network_meraki.meraki.params['psk']:
        if jctanner.network_meraki.meraki.params['auth_mode'] != 'psk':
            jctanner.network_meraki.meraki.fail_json(msg='PSK is only allowed when auth_mode is set to psk')
        if jctanner.network_meraki.meraki.params['encryption_mode'] != 'wpa':
            jctanner.network_meraki.meraki.fail_json(msg='PSK requires encryption_mode be set to wpa')
    if jctanner.network_meraki.meraki.params['radius_servers']:
        if jctanner.network_meraki.meraki.params['auth_mode'] not in ('open-with-radius', '8021x-radius'):
            jctanner.network_meraki.meraki.fail_json(msg='radius_servers requires auth_mode to be open-with-radius or 8021x-radius')
    if jctanner.network_meraki.meraki.params['radius_accounting_enabled'] is True:
        if jctanner.network_meraki.meraki.params['auth_mode'] not in ('open-with-radius', '8021x-radius'):
            jctanner.network_meraki.meraki.fails_json(msg='radius_accounting_enabled is only allowed when auth_mode is open-with-radius or 8021x-radius')
    if jctanner.network_meraki.meraki.params['radius_accounting_servers'] is True:
        if jctanner.network_meraki.meraki.params['auth_mode'] not in ('open-with-radius', '8021x-radius') or jctanner.network_meraki.meraki.params['radius_accounting_enabled'] is False:
            jctanner.network_meraki.meraki.fail_json(msg='radius_accounting_servers is only allowed when auth_mode is open_with_radius or 8021x-radius and \
    ...
    if jctanner.network_meraki.meraki.params['state'] == 'query':
        if jctanner.network_meraki.meraki.params['name']:
            ssid_id = get_ssid_number(jctanner.network_meraki.meraki.params['name'], get_ssids(jctanner.network_meraki.meraki, net_id))
            path = jctanner.network_meraki.meraki.construct_path('get_one', net_id=net_id) + str(ssid_id)
            jctanner.network_meraki.meraki.result['data'] = jctanner.network_meraki.meraki.request(path, method='GET')
        elif jctanner.network_meraki.meraki.params['number']:
            path = jctanner.network_meraki.meraki.construct_path('get_one', net_id=net_id) + jctanner.network_meraki.meraki.params['number']
            jctanner.network_meraki.meraki.result['data'] = jctanner.network_meraki.meraki.request(path, method='GET')
        else:
            jctanner.network_meraki.meraki.result['data'] = get_ssids(jctanner.network_meraki.meraki, net_id)
    elif jctanner.network_meraki.meraki.params['state'] == 'present':
        payload = dict()
        for k, v in param_map.items():
            if jctanner.network_meraki.meraki.params[v] is not None:
                payload[k] = jctanner.network_meraki.meraki.params[v]
        ssids = get_ssids(jctanner.network_meraki.meraki, net_id)
        number = jctanner.network_meraki.meraki.params['number']
        if number is None:
            number = get_ssid_number(jctanner.network_meraki.meraki.params['name'], ssids)
        original = ssids[number]
        if jctanner.network_meraki.meraki.is_update_required(original, payload):
            ssid_id = jctanner.network_meraki.meraki.params['number']
            if ssid_id is None:  # Name should be used to lookup number
                ssid_id = get_ssid_number(jctanner.network_meraki.meraki.params['name'], ssids)
                if ssid_id is False:
                    ssid_id = get_available_number(ssids)
                    if ssid_id is False:
                        jctanner.network_meraki.meraki.fail_json(msg='No unconfigured SSIDs are available. Specify a number.')
            path = jctanner.network_meraki.meraki.construct_path('update', net_id=net_id) + str(ssid_id)
            result = jctanner.network_meraki.meraki.request(path, 'PUT', payload=json.dumps(payload))
            jctanner.network_meraki.meraki.result['data'] = result
        ...
        if ssid_id is None:  # Name should be used to lookup number
            ...
            if ssid_id is False:
                ssid_id = get_available_number(ssids)
                if ssid_id is False:
                    jctanner.network_meraki.meraki.fail_json(msg='No SSID found by specified name and no number was referenced.')
        path = jctanner.network_meraki.meraki.construct_path('update', net_id=net_id) + str(ssid_id)
