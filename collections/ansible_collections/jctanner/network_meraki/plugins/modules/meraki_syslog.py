...
DOCUMENTATION = r'''
---
module: jctanner.network_meraki.meraki_syslog
...
- name: Query syslog configurations on network named MyNet in the YourOrg organization
  jctanner.network_meraki.meraki_syslog:
    auth_key: abc12345
    status: query
    org_name: YourOrg
    net_name: MyNet
...
def is_net_valid(jctanner.network_meraki.meraki, net_name, data):
    for n in data:
        if n['name'] == net_name:
            return True
    return False
...
# def validate_roles(jctanner.network_meraki.meraki, data):
...
#                 # jctanner.network_meraki.meraki.fail_json(msg="Heck yes")
#                 jctanner.network_meraki.meraki.fail_json(msg='{0} is not a valid Syslog role.'.format(role))
...
def main():
    ...
    jctanner.network_meraki.meraki = MerakiModule(module, function='syslog')
    module.params['follow_redirects'] = 'all'
    payload = None

    syslog_urls = {'syslog': '/networks/{net_id}/syslogServers'}
    jctanner.network_meraki.meraki.url_catalog['query_update'] = syslog_urls
    ...
    if jctanner.network_meraki.meraki.params['state'] != 'query':
        if not jctanner.network_meraki.meraki.params['net_name'] or jctanner.network_meraki.meraki.params['net_id']:
    ...
    if jctanner.network_meraki.meraki.params['state'] == 'query':
        path = jctanner.network_meraki.meraki.construct_path('query_update', net_id=net_id)
        ...
        # Construct payload
        payload = dict()
        payload['servers'] = jctanner.network_meraki.meraki.params['servers']
        ...
        if jctanner.network_meraki.meraki.is_update_required(original, payload):
            path = jctanner.network_meraki.meraki.construct_path('query_update', net_id=net_id)
            r = jctanner.network_meraki.meraki.request(path, method='PUT', payload=json.dumps(payload))
            if jctanner.network_meraki.meraki.status == 200:
                jctanner.network_meraki.meraki.result['data'] = r
