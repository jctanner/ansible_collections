...
DOCUMENTATION = r'''
---
module: jctanner.network_meraki.meraki_mx_l3_firewall
...
- name: Query firewall rules
  jctanner.network_meraki.meraki_mx_l3_firewall:
    auth_key: abc123
    org_name: YourOrg
    net_name: YourNet
    state: query
...
def get_rules(jctanner.network_meraki.meraki, net_id):
...
def main():
    ...
    jctanner.network_meraki.meraki = MerakiModule(module, function='mx_l3_firewall')
    ...
    if jctanner.network_meraki.meraki.params['state'] == 'query':
        jctanner.network_meraki.meraki.result['data'] = get_rules(jctanner.network_meraki.meraki, net_id)
    elif jctanner.network_meraki.meraki.params['state'] == 'present':
        rules = get_rules(jctanner.network_meraki.meraki, net_id)
        path = jctanner.network_meraki.meraki.construct_path('get_all', net_id=net_id)
        if jctanner.network_meraki.meraki.params['rules']:
            ...
            payload = dict()
        update = False
        if jctanner.network_meraki.meraki.params['syslog_default_rule'] is not None:
            payload['syslogDefaultRule'] = jctanner.network_meraki.meraki.params['syslog_default_rule']
        try:
            if len(rules) - 1 != len(payload['rules']):  # Quick and simple check to avoid more processing
                update = True
            if jctanner.network_meraki.meraki.params['syslog_default_rule'] is not None:
                if rules[len(rules) - 1]['syslogEnabled'] != jctanner.network_meraki.meraki.params['syslog_default_rule']:
                    update = True
            if update is False:
                default_rule = rules[len(rules) - 1].copy()
                del rules[len(rules) - 1]  # Remove default rule for comparison
                for r in range(len(rules) - 1):
                    if jctanner.network_meraki.meraki.is_update_required(rules[r], payload['rules'][r]) is True:
                        update = True
