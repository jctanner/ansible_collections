...
DOCUMENTATION = r'''
---
module: jctanner.network_meraki.meraki_vlan
...
- name: Query all VLANs in a network.
  jctanner.network_meraki.meraki_vlan:
    auth_key: abc12345
    org_name: YourOrg
    net_name: YourNet
    state: query
...
def get_vlans(jctanner.network_meraki.meraki, net_id):
...
def is_vlan_valid(jctanner.network_meraki.meraki, net_id, vlan_id):
    vlans = get_vlans(jctanner.network_meraki.meraki, net_id)
    for vlan in vlans:
        if vlan_id == vlan['id']:
            return True
    return False
...
def main():
    ...
    jctanner.network_meraki.meraki = MerakiModule(module, function='vlan')
    ...
    jctanner.network_meraki.meraki.url_catalog['create'] = create_url
    jctanner.network_meraki.meraki.url_catalog['update'] = update_url
    jctanner.network_meraki.meraki.url_catalog['delete'] = delete_url
    ...
    if jctanner.network_meraki.meraki.params['state'] == 'query':
        if not jctanner.network_meraki.meraki.params['vlan_id']:
            jctanner.network_meraki.meraki.result['data'] = get_vlans(jctanner.network_meraki.meraki, net_id)
        else:
            path = jctanner.network_meraki.meraki.construct_path('get_one', net_id=net_id, custom={'vlan_id': jctanner.network_meraki.meraki.params['vlan_id']})
        ...
        payload = {'id': jctanner.network_meraki.meraki.params['vlan_id'],
                   'name': jctanner.network_meraki.meraki.params['name'],
                   'subnet': jctanner.network_meraki.meraki.params['subnet'],
                   'applianceIp': jctanner.network_meraki.meraki.params['appliance_ip'],
                   }
        if is_vlan_valid(jctanner.network_meraki.meraki, net_id, jctanner.network_meraki.meraki.params['vlan_id']) is False:
            path = jctanner.network_meraki.meraki.construct_path('create', net_id=net_id)
            response = jctanner.network_meraki.meraki.request(path, method='POST', payload=json.dumps(payload))
            jctanner.network_meraki.meraki.result['changed'] = True
            jctanner.network_meraki.meraki.result['data'] = response
        else:
            path = jctanner.network_meraki.meraki.construct_path('get_one', net_id=net_id, custom={'vlan_id': jctanner.network_meraki.meraki.params['vlan_id']})
            original = jctanner.network_meraki.meraki.request(path, method='GET')
            if jctanner.network_meraki.meraki.params['dns_nameservers']:
                if jctanner.network_meraki.meraki.params['dns_nameservers'] not in ('opendns', 'google_dns', 'upstream_dns'):
                    payload['dnsNameservers'] = format_dns(jctanner.network_meraki.meraki.params['dns_nameservers'])
                else:
                    payload['dnsNameservers'] = jctanner.network_meraki.meraki.params['dns_nameservers']
            if jctanner.network_meraki.meraki.params['fixed_ip_assignments']:
                payload['fixedIpAssignments'] = fixed_ip_factory(jctanner.network_meraki.meraki, jctanner.network_meraki.meraki.params['fixed_ip_assignments'])
            if jctanner.network_meraki.meraki.params['reserved_ip_range']:
                payload['reservedIpRanges'] = jctanner.network_meraki.meraki.params['reserved_ip_range']
            if jctanner.network_meraki.meraki.params['vpn_nat_subnet']:
                payload['vpnNatSubnet'] = jctanner.network_meraki.meraki.params['vpn_nat_subnet']
            ignored = ['networkId']
            if jctanner.network_meraki.meraki.is_update_required(original, payload, optional_ignore=ignored):
                path = jctanner.network_meraki.meraki.construct_path('update', net_id=net_id) + str(jctanner.network_meraki.meraki.params['vlan_id'])
        ...
        if is_vlan_valid(jctanner.network_meraki.meraki, net_id, jctanner.network_meraki.meraki.params['vlan_id']):
            path = jctanner.network_meraki.meraki.construct_path('delete', net_id=net_id) + str(jctanner.network_meraki.meraki.params['vlan_id'])
