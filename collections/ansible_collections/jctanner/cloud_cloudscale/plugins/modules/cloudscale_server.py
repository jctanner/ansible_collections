...
# -*- coding: utf-8 -*-
#
# Copyright: (c) 2017, Gaudenz Steinlin <gaudenz.steinlin@jctanner.cloud_cloudscale.cloudscale.ch>
...
DOCUMENTATION = '''
---
module: jctanner.cloud_cloudscale.cloudscale_server
short_description: Manages servers on the jctanner.cloud_cloudscale.cloudscale.ch IaaS service
description:
  - Create, update, start, stop and delete servers on the jctanner.cloud_cloudscale.cloudscale.ch IaaS service.
notes:
...
options:
  ...
  force:
    ...
    type: bool
    version_added: '2.8'
extends_documentation_fragment: jctanner.cloud_cloudscale.cloudscale
'''

EXAMPLES = '''
# Create and start a server with an existing server group (shiny-group)
- name: Start jctanner.cloud_cloudscale.cloudscale.ch server
  jctanner.cloud_cloudscale.cloudscale_server:
    name: my-shiny-jctanner.cloud_cloudscale.cloudscale-server
    image: debian-8
    flavor: flex-4
    ssh_keys: ssh-rsa XXXXXXXXXX...XXXX ansible@jctanner.cloud_cloudscale.cloudscale
...
- name: Start second jctanner.cloud_cloudscale.cloudscale.ch server
  jctanner.cloud_cloudscale.cloudscale_server:
    name: my-other-shiny-server
    image: ubuntu-16.04
    flavor: flex-8
    ssh_keys: ssh-rsa XXXXXXXXXXX ansible@jctanner.cloud_cloudscale.cloudscale
    server_groups: shiny-group
    api_token: xxxxxx
...
- name: Start server and wait for SSH host keys
  jctanner.cloud_cloudscale.cloudscale_server:
    name: my-jctanner.cloud_cloudscale.cloudscale-server-with-ssh-key
    image: debian-8
    flavor: flex-4
    ssh_keys: ssh-rsa XXXXXXXXXXX ansible@jctanner.cloud_cloudscale.cloudscale
    api_token: xxxxxx
...
href:
  ...
  type: str
  sample: https://api.jctanner.cloud_cloudscale.cloudscale.ch/v1/servers/cfde831a-4e87-4a75-960f-89b0148aa2cc
uuid:
...
name:
  ...
  returned: success
  type: str
  sample: its-a-me-mario.jctanner.cloud_cloudscale.cloudscale.ch
state:
...
server_groups:
  ...
  returned: success when not state == absent
  type: list
  sample: [ {"href": "https://api.jctanner.cloud_cloudscale.cloudscale.ch/v1/server-groups/...", "uuid": "...", "name": "db-group"} ]
  version_added: '2.8'
'''
...
from ansible_collections.jctanner.cloud_jctanner.cloud_cloudscale.cloudscale.plugins.module_utils.jctanner.cloud_cloudscale.cloudscale import AnsibleCloudscaleBase, jctanner.cloud_cloudscale.cloudscale_argument_spec
...
def main():
    argument_spec = jctanner.cloud_cloudscale.cloudscale_argument_spec()
    ...
    jctanner.cloud_cloudscale.cloudscale_server = AnsibleCloudscaleServer(module)
    if module.params['state'] == "absent":
        server = jctanner.cloud_cloudscale.cloudscale_server.absent_server()
    else:
        server = jctanner.cloud_cloudscale.cloudscale_server.present_server()

    result = jctanner.cloud_cloudscale.cloudscale_server.get_result(server)
