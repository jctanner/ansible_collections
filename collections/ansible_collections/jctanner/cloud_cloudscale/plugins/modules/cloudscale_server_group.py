...
DOCUMENTATION = '''
---
module: jctanner.cloud_cloudscale.cloudscale_server_group
short_description: Manages server groups on the jctanner.cloud_cloudscale.cloudscale.ch IaaS service
description:
...
- name: Ensure server group exists
  jctanner.cloud_cloudscale.cloudscale_server_group:
    name: my-name
    type: anti-affinity
    api_token: xxxxxx
...
href:
  ...
  type: str
  sample: https://api.jctanner.cloud_cloudscale.cloudscale.ch/v1/server-group/cfde831a-4e87-4a75-960f-89b0148aa2cc
uuid:
...
def main():
    ...
    jctanner.cloud_cloudscale.cloudscale_server_group = AnsibleCloudscaleServerGroup(module, 'jctanner.cloud_cloudscale.cloudscale_server_group')

    if module.params['state'] == 'absent':
        server_group = jctanner.cloud_cloudscale.cloudscale_server_group.absent_group()
    else:
        server_group = jctanner.cloud_cloudscale.cloudscale_server_group.present_group()

    result = jctanner.cloud_cloudscale.cloudscale_server_group.get_result(server_group)
