...
DOCUMENTATION = '''
module: jctanner.cloud_amazon.aws_waf_web_acl
...
EXAMPLES = '''
  - name: create web ACL
    jctanner.cloud_amazon.aws_waf_web_acl:
      name: my_web_acl
      rules:
        - name: my_rule
          priority: 1
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import boto3_conn, get_jctanner.cloud_amazon.aws_connection_info, jctanner.cloud_amazon.ec2_argument_spec, camel_dict_to_snake_dict
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.aws.waf import list_rules_with_backoff, list_web_acls_with_backoff, list_regional_web_acls_with_backoff, \
...
def create_rule_lookup(client, module):
    if client.__class__.__name__ == 'WAF':
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not list rules')
    elif client.__class__.__name__ == 'WAFRegional':
        try:
            rules = list_regional_rules_with_backoff(client)
            return dict((rule['Name'], rule) for rule in rules)
        except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not list regional rules')
...
def get_web_acl(client, module, web_acl_id):
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not get Web ACL with id %s' % web_acl_id)
...
def list_web_acls(client, module,):
    if client.__class__.__name__ == 'WAF':
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not get Web ACLs')
    elif client.__class__.__name__ == 'WAFRegional':
        try:
...
def find_and_update_web_acl(client, module, web_acl_id):
    ...
    if deletions:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not update Web ACL')
    if insertions:
        try:
...
def remove_rules_from_web_acl(client, module, web_acl_id):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not remove rule')
...
def ensure_web_acl_present(client, module):
    ...
    if web_acl_id:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not create Web ACL')
...
def ensure_web_acl_absent(client, module):
    ...
    if web_acl_id:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not delete Web ACL')
    return False, {}
