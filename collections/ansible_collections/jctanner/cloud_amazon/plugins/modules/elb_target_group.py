...
description:
    - Manage an AWS Elastic Load Balancer target group. See
      U(https://docs.jctanner.cloud_amazon.aws.amazon.com/elasticloadbalancing/latest/application/load-balancer-target-groups.html) or
      U(https://docs.jctanner.cloud_amazon.aws.amazon.com/elasticloadbalancing/latest/network/load-balancer-target-groups.html) for details.
version_added: "2.4"
requirements: [ boto3 ]
...
target_group_arn:
    ...
    returned: when state present
    type: str
    sample: "arn:jctanner.cloud_amazon.aws:elasticloadbalancing:ap-southeast-2:01234567890:targetgroup/mytargetgroup/aabbccddee0044332211"
target_group_name:
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import (camel_dict_to_snake_dict, boto3_tag_list_to_ansible_dict, jctanner.cloud_amazon.ec2_argument_spec,
                                      compare_jctanner.cloud_amazon.aws_tags, ansible_dict_to_boto3_tag_list)
...
def get_target_group_tags(connection, module, target_group_arn):
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get target group tags")
...
def get_target_group(connection, module):
    try:
        ...
        if e.response['Error']['Code'] == 'TargetGroupNotFound':
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get target group")
...
def wait_for_status(connection, module, target_group_arn, targets, status):
    ...
    for x in range(0, max_retries):
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't describe target health")
...
def create_or_update_target_group(connection, module):
    ...
    if tg:
        ...
        if health_options:
            ...
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't update target group")
        ...
        if module.params.get("modify_targets"):
            if module.params.get("targets"):
                ...
                try:
                    ...
                    module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get target group health")
                ...
                if add_instances:
                    ...
                    try:
                        ...
                        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't register targets")
                ...
                if remove_instances:
                    ...
                    try:
                        ...
                        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't remove targets")
                ...
                try:
                    ...
                    module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get target health")
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't create target group")
    ...
    if update_attributes:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't delete target group")
    ...
    if tags:
        ...
        tags_need_modify, tags_to_delete = compare_jctanner.cloud_amazon.aws_tags(boto3_tag_list_to_ansible_dict(current_tags), tags, purge_tags)
        if tags_to_delete:
            try:
                connection.remove_tags(ResourceArns=[tg['TargetGroupArn']], TagKeys=tags_to_delete)
            except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't delete tags from target group")
            changed = True
        ...
        if tags_need_modify:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't add tags to target group")
            changed = True
