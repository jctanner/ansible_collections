...
- cloudwatchlogs_log_group:
    ...
    log_group_name: test-log-group
    tags: { "Name": "test-log-group", "Env" : "QA" }
    kms_key_id: arn:jctanner.cloud_amazon.aws:kms:region:account-id:key/key-id
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import HAS_BOTO3, camel_dict_to_snake_dict, boto3_conn, jctanner.cloud_amazon.ec2_argument_spec, get_jctanner.cloud_amazon.aws_connection_info
    ...
    if 'logGroups' in desc_log_group:
        for i in desc_log_group['logGroups']:
            if log_group_name == i['logGroupName']:
                return i
    module.fail_json(msg="The jctanner.cloud_amazon.aws CloudWatchLogs log group was not created. \n please try again!")
...
def main():
    ...
    logs = boto3_conn(module, conn_type='client', resource='logs', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
