...
options:
  ...
  encryption:
    ...
    choices: [ 'none', 'AES256', 'jctanner.cloud_amazon.aws:kms' ]
    version_added: "2.9"
  encryption_key_id:
    description: KMS master key ID to use for the default encryption. This parameter is allowed if encryption is jctanner.cloud_amazon.aws:kms. If
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.aws.core import AnsibleAWSModule, is_boto3_error_code
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import compare_policies, jctanner.cloud_amazon.ec2_argument_spec, boto3_tag_list_to_ansible_dict, ansible_dict_to_boto3_tag_list
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import get_jctanner.cloud_amazon.aws_connection_info, boto3_conn, AWSRetry
...
def create_or_update_bucket(s3_client, module, location):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Invalid endpoint provided: %s" % to_text(e))
    except (BotoCoreError, ClientError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to check bucket presence")
    ...
    if not bucket_is_present:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='An error occurred waiting for the bucket to become available')
        except (BotoCoreError, ClientError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while creating bucket")

    # Versioning
    try:
        versioning_status = get_bucket_versioning(s3_client, name)
    except BotoCoreError as exp:
        module.fail_json_jctanner.cloud_amazon.aws(exp, msg="Failed to get bucket versioning")
        ...
        if versioning is not None:
            ...
            if required_versioning:
                try:
                    ...
                    module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to update bucket versioning")
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(exp, msg="Failed to get bucket request payment")
    except ClientError as exp:
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(exp, msg="Failed to get bucket policy")
    except ClientError as exp:
        ...
        if policy is not None:
            ...
            if not policy and current_policy:
                try:
                    ...
                    module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to delete bucket policy")
                ...
                try:
                    ...
                    module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to update bucket policy")
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(exp, msg="Failed to get bucket tags")
    except ClientError as exp:
            ...
            if current_tags_dict != tags:
                if tags:
                    try:
                        ...
                        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to update bucket tags")
                else:
                    try:
                        delete_bucket_tagging(s3_client, name)
                    except (BotoCoreError, ClientError) as e:
                        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to delete bucket tags")
    ...
    if hasattr(s3_client, "get_bucket_encryption"):
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to get bucket encryption")
    ...
    if encryption is not None:
        ...
        if encryption == 'none' and current_encryption_algorithm is not None:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to delete bucket encryption")
            current_encryption = wait_encryption_is_applied(module, s3_client, name, None)
            changed = True
        elif encryption != 'none' and (encryption != current_encryption_algorithm) or (encryption == 'jctanner.cloud_amazon.aws:kms' and current_encryption_key != encryption_key_id):
            expected_encryption = {'SSEAlgorithm': encryption}
            if encryption == 'jctanner.cloud_amazon.aws:kms':
                expected_encryption.update({'KMSMasterKeyID': encryption_key_id})
            try:
                put_bucket_encryption(s3_client, name, expected_encryption)
            except (BotoCoreError, ClientError) as e:
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to set bucket encryption")
...
def wait_policy_is_applied(module, s3_client, bucket_name, expected_policy, should_fail=True):
    for dummy in range(0, 12):
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to get bucket policy")
...
def wait_payer_is_applied(module, s3_client, bucket_name, expected_payer, should_fail=True):
    for dummy in range(0, 12):
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to get bucket request payment")
...
def wait_encryption_is_applied(module, s3_client, bucket_name, expected_encryption):
    for dummy in range(0, 12):
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to get updated encryption for bucket")
...
def wait_versioning_is_applied(module, s3_client, bucket_name, required_versioning):
    for dummy in range(0, 24):
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to get updated versioning for bucket")
...
def destroy_bucket(s3_client, module):
    ...
    if force:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while deleting bucket")

    try:
        delete_bucket(s3_client, name)
        s3_client.get_waiter('bucket_not_exists').wait(Bucket=name)
    except WaiterError as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg='An error occurred waiting for the bucket to be deleted.')
    except (BotoCoreError, ClientError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to delete bucket")
...
def get_s3_client(module, jctanner.cloud_amazon.aws_connect_kwargs, location, ceph, s3_url):
    if s3_url and ceph:  # TODO - test this
        ceph = urlparse(s3_url)
        params = dict(module=module, conn_type='client', resource='s3', use_ssl=ceph.scheme == 'https', region=location, endpoint=s3_url, **jctanner.cloud_amazon.aws_connect_kwargs)
        ...
        params = dict(module=module, conn_type='client', resource='s3', region=location,
                      ...
                      use_ssl=fakes3.scheme == 'fakes3s', **jctanner.cloud_amazon.aws_connect_kwargs)
    else:
        params = dict(module=module, conn_type='client', resource='s3', region=location, endpoint=s3_url, **jctanner.cloud_amazon.aws_connect_kwargs)
...
def main():
    ...
    argument_spec.update(
        dict(
            ...
            encryption=dict(choices=['none', 'AES256', 'jctanner.cloud_amazon.aws:kms']),
    ...
    module = AnsibleAWSModule(
        ...
        required_if=[['encryption', 'jctanner.cloud_amazon.aws:kms', ['encryption_key_id']]]
    )
    ...
    if s3_url:
        for key in ['validate_certs', 'security_token', 'profile_name']:
            jctanner.cloud_amazon.aws_connect_kwargs.pop(key, None)
    s3_client = get_s3_client(module, jctanner.cloud_amazon.aws_connect_kwargs, location, ceph, s3_url)
    ...
    elif encryption_key_id is not None and encryption != 'jctanner.cloud_amazon.aws:kms':
        module.fail_json(msg="Only 'jctanner.cloud_amazon.aws:kms' is a valid option for encryption parameter when you specify encryption_key_id.")
