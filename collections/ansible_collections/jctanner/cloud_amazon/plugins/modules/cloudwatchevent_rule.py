...
- cloudwatchevent_rule:
    ...
    targets:
      - id: MyTargetId
        arn: arn:jctanner.cloud_amazon.aws:lambda:us-east-1:123456789012:function:MyFunction
...
rule:
    ...
    type: dict
    sample:
      arn: 'arn:jctanner.cloud_amazon.aws:events:us-east-1:123456789012:rule/MyCronTask'
      description: 'Run my scheduled task'
      name: 'MyCronTask'
      schedule_expression: 'cron(0 20 * * ? *)'
      state: 'ENABLED'
targets:
    ...
    returned: success
    type: list
    sample: "[{ 'arn': 'arn:jctanner.cloud_amazon.aws:lambda:us-east-1:123456789012:function:MyFunction', 'id': 'MyTargetId' }]"
'''
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import boto3_conn, camel_dict_to_snake_dict
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import jctanner.cloud_amazon.ec2_argument_spec, get_jctanner.cloud_amazon.aws_connection_info
...
class CloudWatchEventRule(object):
    ...
    def describe(self):
        ...
        try:
            ...
            if error_code == 'ResourceNotFoundException':
                return {}
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not describe rule %s" % self.name)
    ...
    def put(self, enabled=True):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not create/update rule %s" % self.name)
        self.changed = True
        return response
    ...
    def delete(self):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not delete rule %s" % self.name)
        self.changed = True
        return response
    ...
    def enable(self):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not enable rule %s" % self.name)
        self.changed = True
        return response
    ...
    def disable(self):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not disable rule %s" % self.name)
        self.changed = True
        return response
    ...
    def list_targets(self):
        ...
        try:
            ...
            if error_code == 'ResourceNotFoundException':
                return []
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not find target for rule %s" % self.name)
        except botocore.exceptions.BotoCoreError as e:
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not find target for rule %s" % self.name)
    ...
    def put_targets(self, targets):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not create/update rule targets for rule %s" % self.name)
        self.changed = True
        return response
    ...
    def remove_targets(self, target_ids):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not remove rule targets from rule %s" % self.name)
        self.changed = True
        return response
...
class CloudWatchEventRuleManager(object):
    ...
    def fetch_jctanner.cloud_amazon.aws_state(self):
        """Retrieves rule and target state from AWS"""
        jctanner.cloud_amazon.aws_state = {
            'rule': {},
            'targets': [],
        ...
        rule_description = self.rule.describe()
        if not rule_description:
            return jctanner.cloud_amazon.aws_state

        # Don't need to include response metadata noise in response
        del rule_description['response_metadata']

        jctanner.cloud_amazon.aws_state['rule'] = rule_description
        jctanner.cloud_amazon.aws_state['targets'].extend(self.rule.list_targets())
        return jctanner.cloud_amazon.aws_state

    def _sync_rule(self, enabled=True):
        """Syncs local rule state with AWS"""
        if not self._rule_matches_jctanner.cloud_amazon.aws():
    ...
    def _rule_matches_jctanner.cloud_amazon.aws(self):
        """Checks if the local rule data matches AWS"""
        jctanner.cloud_amazon.aws_rule_data = self.rule.describe()

        # The rule matches AWS only if all rule data fields are equal
        # to their corresponding local value defined in the task
        return all([
            getattr(self.rule, field) == jctanner.cloud_amazon.aws_rule_data.get(field, None)
...
def get_cloudwatchevents_client(module):
    ...
    region, jctanner.cloud_amazon.ec2_url, jctanner.cloud_amazon.aws_conn_kwargs = get_jctanner.cloud_amazon.aws_connection_info(module, boto3=True)
    return boto3_conn(module, conn_type='client',
                      resource='events',
                      region=region, endpoint=jctanner.cloud_amazon.ec2_url,
                      **jctanner.cloud_amazon.aws_conn_kwargs)
...
def main():
    ...
    module.exit_json(**cwe_rule_manager.fetch_jctanner.cloud_amazon.aws_state())
