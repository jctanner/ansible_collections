...
options:
  ...
  runtime:
    description:
      ...
      - For supported list of runtimes, see U(https://docs.jctanner.cloud_amazon.aws.amazon.com/lambda/latest/dg/lambda-runtimes.html).
  role:
    description:
...
- name: looped creation
  lambda:
    ...
    runtime: 'python2.7'
    role: 'arn:jctanner.cloud_amazon.aws:iam::987654321012:role/lambda_basic_execution'
    ...
    sample:
      {
        ...
        'function_arn': 'arn:jctanner.cloud_amazon.aws:lambda:us-east-1:123456789012:function:myFunction:1',
        'function_name': 'myFunction',
        'handler': 'index.handler',
        'last_modified': '2017-08-01T00:00:00.000+0000',
        'memory_size': 128,
        'role': 'arn:jctanner.cloud_amazon.aws:iam::123456789012:role/lambda_basic_execution',
        'runtime': 'nodejs6.10',
        'timeout': 3,
        'version': '1',
        'vpc_config': {
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import get_jctanner.cloud_amazon.aws_connection_info, boto3_conn, camel_dict_to_snake_dict
...
def get_account_id(module, region=None, endpoint=None, **jctanner.cloud_amazon.aws_connect_kwargs):
    ...
    try:
        sts_client = boto3_conn(module, conn_type='client', resource='sts',
                                region=region, endpoint=endpoint, **jctanner.cloud_amazon.aws_connect_kwargs)
        ...
        try:
            ...
            if (e.response['Error']['Code'] == 'AccessDenied'):
                ...
                account_id = except_msg.search(r"arn:jctanner.cloud_amazon.aws:iam::([0-9]{12,32}):\w+/").group(1)
            if account_id is None:
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="getting account information")
...
def set_tag(client, module, tags, function):
    ...
    tags_to_add, tags_to_remove = compare_jctanner.cloud_amazon.aws_tags(current_tags, tags, purge_tags=True)
...
def main():
    ...
    try:
        client = boto3_conn(module, conn_type='client', resource='lambda',
                            region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
    except (ClientError, ValidationError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Trying to connect to AWS")

    if state == 'present':
        if role.startswith('arn:jctanner.cloud_amazon.aws:iam'):
            role_arn = role
        else:
            # get account ID and assemble ARN
            account_id = get_account_id(module, region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
            role_arn = 'arn:jctanner.cloud_amazon.aws:iam::{0}:role/{1}'.format(account_id, role)
    ...
    if state == 'present' and current_function:
        ...
        if len(func_kwargs) > 1:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Trying to update lambda configuration")
        ...
        if len(code_kwargs) > 2:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Trying to upload new code")
    ...
    # Function doesn't exists, create new Lambda function
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Trying to create function")
    ...
    if state == 'absent' and current_function:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Trying to delete Lambda function")
