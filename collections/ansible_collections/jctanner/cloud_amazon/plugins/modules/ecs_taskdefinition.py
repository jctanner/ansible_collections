...
options:
    ...
    network_mode:
        description:
            ...
            - C(jctanner.cloud_amazon.awsvpc) mode was added in Ansible 2.5
        required: false
        default: bridge
        choices: [ 'bridge', 'host', 'none', 'jctanner.cloud_amazon.awsvpc' ]
        version_added: 2.3
    task_role_arn:
        description:
  ...
  ecs_taskdefinition:
    ...
    - name: simple-app
      ...
      - containerPort: 80
        hostPort: 80
      logConfiguration:
        logDriver: jctanner.cloud_amazon.awslogs
        options:
          jctanner.cloud_amazon.awslogs-group: /ecs/test-cluster-taskdef
          jctanner.cloud_amazon.awslogs-region: us-west-2
          jctanner.cloud_amazon.awslogs-stream-prefix: ecs
    - name: busybox
      command:
        - >
...
- name: Create task definition
  ecs_taskdefinition:
    ...
    memory: 1024
    state: present
    network_mode: jctanner.cloud_amazon.awsvpc
'''
RETURN = '''
taskdefinition:
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import boto3_conn, camel_dict_to_snake_dict, jctanner.cloud_amazon.ec2_argument_spec, get_jctanner.cloud_amazon.aws_connection_info
...
class EcsTaskManager:
    ...
    def __init__(self, module):
        ...
        self.ecs = boto3_conn(module, conn_type='client', resource='ecs', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
        ...
        for container in container_definitions:
            ...
            if 'portMappings' in container:
                for port_mapping in container['portMappings']:
                    ...
                    if network_mode == 'jctanner.cloud_amazon.awsvpc' and 'hostPort' in port_mapping:
                        if port_mapping['hostPort'] != port_mapping.get('containerPort'):
                            self.module.fail_json(msg="In jctanner.cloud_amazon.awsvpc network mode, host port must be set to the same as "
...
def main():
    ...
    argument_spec.update(dict(
        ...
        network_mode=dict(required=False, default='bridge', choices=['bridge', 'host', 'none', 'jctanner.cloud_amazon.awsvpc'], type='str'),
    ...
    if module.params['state'] == 'present':
        ...
        if launch_type == 'FARGATE' and network_mode != 'jctanner.cloud_amazon.awsvpc':
            module.fail_json(msg="To use FARGATE launch type, network_mode must be jctanner.cloud_amazon.awsvpc")
