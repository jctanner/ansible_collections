...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_eni
...
interface:
  ...
  contains:
    ...
    owner_id:
      description: jctanner.cloud_amazon.aws account id
      type: str
      sample: 812381371
    private_ip_address:
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import (AnsibleAWSError, connect_to_jctanner.cloud_amazon.aws,
                                      jctanner.cloud_amazon.ec2_argument_spec, get_jctanner.cloud_amazon.aws_connection_info,
                                      get_jctanner.cloud_amazon.ec2_security_group_ids_from_names)
...
def create_eni(connection, vpc_id, module):
    ...
    security_groups = get_jctanner.cloud_amazon.ec2_security_group_ids_from_names(module.params.get('security_groups'), connection, vpc_id=vpc_id, boto3=False)
...
def modify_eni(connection, vpc_id, module, eni):
    ...
    try:
        ...
        if len(security_groups) > 0:
            groups = get_jctanner.cloud_amazon.ec2_security_group_ids_from_names(security_groups, connection, vpc_id=vpc_id, boto3=False)
...
def main():
    ...
    if region:
        try:
            connection = connect_to_jctanner.cloud_amazon.aws(boto.jctanner.cloud_amazon.ec2, region, **jctanner.cloud_amazon.aws_connect_params)
            vpc_connection = connect_to_jctanner.cloud_amazon.aws(boto.vpc, region, **jctanner.cloud_amazon.aws_connect_params)
