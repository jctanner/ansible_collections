...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_snapshot
...
def create_snapshot(module, jctanner.cloud_amazon.ec2, state=None, description=None, wait=None,
    ...
    if instance_id:
        try:
            volumes = jctanner.cloud_amazon.ec2.get_all_volumes(filters={'attachment.instance-id': instance_id, 'attachment.device': device_name})
    ...
    if state == 'absent':
        ...
        try:
            jctanner.cloud_amazon.ec2.delete_snapshot(snapshot_id)
        except boto.exception.BotoServerError as e:
    ...
    if last_snapshot_min_age > 0:
        try:
            current_snapshots = jctanner.cloud_amazon.ec2.get_all_snapshots(filters={'volume_id': volume_id})
        except boto.exception.BotoServerError as e:
            module.fail_json(msg="%s: %s" % (e.error_code, e.error_message))
    ...
    try:
        ...
        if snapshot is None:
            snapshot = jctanner.cloud_amazon.ec2.create_snapshot(volume_id, description=description)
            changed = True
        if wait:
...
def main():
    ...
    create_snapshot(
        ...
        jctanner.cloud_amazon.ec2=jctanner.cloud_amazon.ec2,
        volume_id=volume_id,
