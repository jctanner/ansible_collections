...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_vpc_route_table
...
- name: Set up public subnet route table
  jctanner.cloud_amazon.ec2_vpc_route_table:
    vpc_id: vpc-1245678
    region: us-west-1
    tags:
      Name: Public
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.aws.waiters import get_waiter
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import jctanner.cloud_amazon.ec2_argument_spec, boto3_conn, get_jctanner.cloud_amazon.aws_connection_info
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import ansible_dict_to_boto3_filter_list
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import camel_dict_to_snake_dict, snake_dict_to_camel_dict
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import ansible_dict_to_boto3_tag_list, boto3_tag_list_to_ansible_dict
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import compare_jctanner.cloud_amazon.aws_tags, AWSRetry
...
def find_subnets(connection, module, vpc_id, identified_subnets):
    ...
    Note that this function is duplicated in other jctanner.cloud_amazon.ec2 modules, and should
    ...
    if subnet_ids:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't find subnet with id %s" % subnet_ids)
    ...
    if subnet_cidrs:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't find subnet with cidr %s" % subnet_cidrs)
    ...
    if subnet_names:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't find subnet with names %s" % subnet_names)
...
def find_igw(connection, module, vpc_id):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg='No IGW found for VPC {0}'.format(vpc_id))
    if len(igw) == 1:
...
def get_route_table_by_id(connection, module, route_table_id):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get route table")
    if route_tables:
        ...
        if match is None:
            if route_spec.get('DestinationCidrBlock'):
                ...
                module.warn("Skipping creating {0} because it has no destination cidr block. "
                            "To add VPC endpoints to route tables use the jctanner.cloud_amazon.ec2_vpc_endpoint module.".format(route_spec))
        else:
            if match[0] == "replace":
                if route_spec.get('DestinationCidrBlock'):
        ...
        for r in routes_to_match:
            if not r.get('DestinationCidrBlock'):
                module.warn("Skipping purging route {0} because it has no destination cidr block. "
                            "To remove VPC endpoints from route tables use the jctanner.cloud_amazon.ec2_vpc_endpoint module.".format(r))
                continue
    ...
    if changed and not check_mode:
        for route in routes_to_delete:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't delete route")
        ...
        for route_spec in route_specs_to_recreate:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't recreate route")
        ...
        for route_spec in route_specs_to_create:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't create route")
...
def ensure_subnet_association(connection=None, module=None, vpc_id=None, route_table_id=None, subnet_id=None,
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get route tables")
            ...
            if a['SubnetId'] == subnet_id:
                if route_table['RouteTableId'] == route_table_id:
                    ...
                    try:
                        ...
                        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't disassociate subnet from route table")

    try:
        association_id = connection.associate_route_table(RouteTableId=route_table_id, SubnetId=subnet_id)
    except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't associate subnet with route table")
        ...
        if not check_mode:
            for vgw_id in to_add:
                try:
                    ...
                    module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't enable route propagation")
...
def ensure_route_table_absent(connection, module):
    ...
    if not module.check_mode:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error deleting route table")
...
def get_route_table_info(connection, module, route_table):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get tags for route table")
    ...
    if lookup == 'tag':
        if tags is not None:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error finding route table with lookup 'tag'")
        else:
            route_table = None
    elif lookup == 'id':
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error finding route table with lookup 'id'")
    ...
    if route_table is None:
        ...
        if not module.check_mode:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error creating route table")
        else:
...
def main():
    ...
    connection = boto3_conn(module, conn_type='client', resource='jctanner.cloud_amazon.ec2',
