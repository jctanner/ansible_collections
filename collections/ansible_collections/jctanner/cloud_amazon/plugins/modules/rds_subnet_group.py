...
def main():
    ...
    if not region:
        module.fail_json(msg=str("Either region or AWS_REGION or EC2_REGION environment variable or boto config jctanner.cloud_amazon.aws_region or jctanner.cloud_amazon.ec2_region must be set."))

    try:
        conn = connect_to_jctanner.cloud_amazon.aws(boto.rds, region, **jctanner.cloud_amazon.aws_connect_kwargs)
