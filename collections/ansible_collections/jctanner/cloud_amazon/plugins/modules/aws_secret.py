...
DOCUMENTATION = r'''
---
module: jctanner.cloud_amazon.aws_secret
...
- name: Add string to AWS Secrets Manager
  jctanner.cloud_amazon.aws_secret:
...
secret:
  ...
  contains:
    arn:
      ...
      returned: always
      type: string
      sample: arn:jctanner.cloud_amazon.aws:secretsmanager:eu-west-1:xxxxxxxxxx:secret:xxxxxxxxxxx
    last_accessed_date:
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import boto3_tag_list_to_ansible_dict, compare_jctanner.cloud_amazon.aws_tags, ansible_dict_to_boto3_tag_list
...
class SecretsManagerInterface(object):
    ...
    def get_secret(self, name):
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to describe secret")
        return secret
    ...
    def create_secret(self, secret):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to create secret")
    ...
    def update_secret(self, secret):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to update secret")
        return response
    ...
    def restore_secret(self, name):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to restore secret")
        return response
    ...
    def delete_secret(self, name, recovery_window):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to delete secret")
        return response
    ...
    def update_rotation(self, secret):
        if secret.rotation_enabled:
            try:
                ...
                self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to rotate secret secret")
        else:
            try:
                response = self.client.cancel_rotate_secret(SecretId=secret.name)
            except (BotoCoreError, ClientError) as e:
                self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to cancel rotation")
        return response
    ...
    def tag_secret(self, secret_name, tags):
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to add tag(s) to secret")

    def untag_secret(self, secret_name, tag_keys):
        try:
            self.client.untag_resource(SecretId=secret_name, TagKeys=tag_keys)
        except (BotoCoreError, ClientError) as e:
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to remove tag(s) from secret")
...
def main():
    ...
    if state == 'present':
        if current_secret is None:
            ...
            tags_to_add, tags_to_remove = compare_jctanner.cloud_amazon.aws_tags(current_tags, secret.tags)
            if tags_to_add:
