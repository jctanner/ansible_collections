...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.aws_direct_connect_virtual_interface
...
- name: create an association between a LAG and connection
  jctanner.cloud_amazon.aws_direct_connect_virtual_interface:
    state: present
    name: "{{ name }}"
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.aws.direct_connect import DirectConnectError, delete_virtual_interface
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import (AWSRetry, HAS_BOTO3, boto3_conn,
...
def main():
    ...
    connection = boto3_conn(module, conn_type='client', resource='directconnect', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)

    try:
        changed, latest_state = ensure_state(connection, module)
    except DirectConnectError as e:
        if e.exception:
            module.fail_json_jctanner.cloud_amazon.aws(exception=e.exception, msg=e.msg)
        else:
