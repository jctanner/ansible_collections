...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_vpc_egress_igw
...
jctanner.cloud_amazon.ec2_vpc_egress_igw:
  vpc_id: vpc-abcdefgh
  state: present
register: eigw
...
def delete_eigw(module, conn, eigw_id):
    ...
    try:
        ...
        if e.response.get('Error', {}).get('Code') == "DryRunOperation":
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not delete Egress-Only Internet Gateway {0} from VPC {1}".format(eigw_id, module.vpc_id))
...
def create_eigw(module, conn, vpc_id):
    ...
    try:
        ...
        if e.response.get('Error', {}).get('Code') == "DryRunOperation":
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="invalid vpc ID '{0}' provided".format(vpc_id))
        else:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not create Egress-Only Internet Gateway for vpc ID {0}".format(vpc_id))
    except botocore.exceptions.BotoCoreError as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not create Egress-Only Internet Gateway for vpc ID {0}".format(vpc_id))
...
def describe_eigws(module, conn, vpc_id):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not get list of existing Egress-Only Internet Gateways")
