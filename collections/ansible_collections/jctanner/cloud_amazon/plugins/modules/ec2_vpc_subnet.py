...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_vpc_subnet
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import (ansible_dict_to_boto3_filter_list, ansible_dict_to_boto3_tag_list,
                                      jctanner.cloud_amazon.ec2_argument_spec, camel_dict_to_snake_dict, get_jctanner.cloud_amazon.aws_connection_info,
                                      boto3_conn, boto3_tag_list_to_ansible_dict, compare_jctanner.cloud_amazon.aws_tags, AWSRetry)
...
def handle_waiter(conn, module, waiter_name, params, start_time):
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, "Failed to wait for updates to complete")
    except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, "An exception happened while trying to wait for updates")
...
def create_subnet(conn, module, vpc_id, cidr, ipv6_cidr=None, az=None, start_time=None):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't create subnet")
    ...
    if wait and subnet.get('state') != 'available':
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, "Create subnet action timed out waiting for subnet to become available")
...
def ensure_map_public(conn, module, subnet, map_public, check_mode, start_time):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't modify subnet attribute")
...
def disassociate_ipv6_cidr(conn, module, subnet, start_time):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't disassociate ipv6 cidr block id {0} from subnet {1}"
...
def ensure_ipv6_cidr_block(conn, module, subnet, ipv6_cidr, check_mode, start_time):
    ...
    if ipv6_cidr:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get subnet info")
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't associate ipv6 cidr {0} to {1}".format(ipv6_cidr, subnet['id']))
        else:
...
def get_matching_subnet(conn, module, vpc_id, cidr):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get matching subnet")
...
def ensure_subnet_absent(conn, module):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't delete subnet")
