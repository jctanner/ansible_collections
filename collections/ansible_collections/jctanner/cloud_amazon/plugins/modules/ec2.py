...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2
short_description: create, terminate, start or stop an instance in jctanner.cloud_amazon.ec2
description:
    - Creates or terminates jctanner.cloud_amazon.ec2 instances.
version_added: "0.9"
options:
  key_name:
    description:
  ...
  id:
    ...
    description:
      ...
      - For details, see the description of client token at U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/UserGuide/Run_Instance_Idempotency.html).
    type: str
  group:
    description:
  ...
  region:
    version_added: "1.2"
    description:
      - The AWS region to use.  Must be specified if jctanner.cloud_amazon.ec2_url is not used.
      - If not specified then the value of the C(AWS_REGION) or C(EC2_REGION) environment variable, if any, is used.
      - See U(https://docs.jctanner.cloud_amazon.aws.amazon.com/general/latest/gr/rande.html#jctanner.cloud_amazon.ec2_region).
    aliases: [ 'jctanner.cloud_amazon.aws_region', 'jctanner.cloud_amazon.ec2_region' ]
    type: str
  zone:
    version_added: "1.2"
    description:
      - AWS availability zone in which to launch the instance.
    aliases: [ 'jctanner.cloud_amazon.aws_zone', 'jctanner.cloud_amazon.ec2_zone' ]
    type: str
  instance_type:
    description:
      - Instance type to use for the instance, see U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/UserGuide/instance-types.html).
    required: true
    type: str
  tenancy:
    version_added: "1.9"
  ...
  ebs_optimized:
    version_added: "1.6"
    description:
      - whether instance is using optimized EBS volumes, see U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/UserGuide/EBSOptimized.html).
    default: 'no'
    type: bool
  exact_count:
    version_added: "1.5"
  ...
  spot_launch_group:
    version_added: "2.1"
    description:
      - Launch group for spot request, see U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/UserGuide/how-spot-instances-work.html#spot-launch-group).
    type: str
author:
    - "Tim Gerla (@tgerla)"
    - "Lester Wade (@lwade)"
    - "Seth Vidal (@skvidal)"
extends_documentation_fragment: jctanner.cloud_amazon.aws
'''
  ...
  tasks:
    - name: Launch instance
      jctanner.cloud_amazon.ec2:
         ...
         vpc_subnet_id: subnet-29e63245
         assign_public_ip: yes
      register: jctanner.cloud_amazon.ec2

    - name: Add new instance to host group
      add_host:
        hostname: "{{ item.public_ip }}"
        groupname: launched
      loop: "{{ jctanner.cloud_amazon.ec2.instances }}"
...
- name: Terminate instances
  ...
  tasks:
    - name: Terminate instances that were previously launched
      jctanner.cloud_amazon.ec2:
        state: 'absent'
        instance_ids: '{{ jctanner.cloud_amazon.ec2.instance_ids }}'
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import get_jctanner.cloud_amazon.aws_connection_info, jctanner.cloud_amazon.ec2_argument_spec, jctanner.cloud_amazon.ec2_connect
from ansible.module_utils.six import get_function_code, string_types
from ansible.module_utils._text import to_bytes, to_text

try:
    import boto.jctanner.cloud_amazon.ec2
    from boto.jctanner.cloud_amazon.ec2.blockdevicemapping import BlockDeviceType, BlockDeviceMapping
    from boto.exception import EC2ResponseError
    from boto import connect_jctanner.cloud_amazon.ec2_endpoint
...
def find_running_instances_by_count_tag(module, jctanner.cloud_amazon.ec2, vpc, count_tag, zone=None):

    # get reservations for instances that match tag(s) and are in the desired state
    state = module.params.get('state')
    if state not in ['running', 'stopped']:
        state = None
    reservations = get_reservations(module, jctanner.cloud_amazon.ec2, vpc, tags=count_tag, state=state, zone=zone)
...
def get_reservations(module, jctanner.cloud_amazon.ec2, vpc, tags=None, state=None, zone=None):
    ...
    if state:
        # http://stackoverflow.com/questions/437511/what-are-the-valid-instancestates-for-the-amazon-jctanner.cloud_amazon.ec2-api
    ...
    results = jctanner.cloud_amazon.ec2.get_all_instances(filters=filters)
...
def boto_supports_associate_public_ip_address(jctanner.cloud_amazon.ec2):
    """
    Check if Boto library has associate_public_ip_address in the NetworkInterfaceSpecification
    class. Added in Boto 2.13.0

    jctanner.cloud_amazon.ec2: authenticated jctanner.cloud_amazon.ec2 connection object
    ...
    try:
        network_interface = boto.jctanner.cloud_amazon.ec2.networkinterface.NetworkInterfaceSpecification()
...
def boto_supports_profile_name_arg(jctanner.cloud_amazon.ec2):
    """
    ...
    """
    run_instances_method = getattr(jctanner.cloud_amazon.ec2, 'run_instances')
...
def create_block_device(module, jctanner.cloud_amazon.ec2, volume):
    # Not aware of a way to determine this programatically
    # http://jctanner.cloud_amazon.aws.amazon.com/about-jctanner.cloud_amazon.aws/whats-new/2013/10/09/ebs-provisioned-iops-maximum-iops-gb-ratio-increased-to-30-1/
    ...
    if 'snapshot' in volume:
        ...
        if 'iops' in volume:
            snapshot = jctanner.cloud_amazon.ec2.get_all_snapshots(snapshot_ids=[volume['snapshot']])[0]
...
def boto_supports_param_in_spot_request(jctanner.cloud_amazon.ec2, param):
    """
    ...
    Returns:
        True if boto library has the named param as an argument on the request_spot_instances method, else False
    """
    method = getattr(jctanner.cloud_amazon.ec2, 'request_spot_instances')
    return param in get_function_code(method).co_varnames


def await_spot_requests(module, jctanner.cloud_amazon.ec2, spot_requests, count):
    """
    Wait for a group of spot requests to be fulfilled, or fail.

    module: Ansible module object
    jctanner.cloud_amazon.ec2: authenticated jctanner.cloud_amazon.ec2 connection object
    spot_requests: boto.jctanner.cloud_amazon.ec2.spotinstancerequest.SpotInstanceRequest object returned by jctanner.cloud_amazon.ec2.request_spot_instances
    ...
    while time.time() < wait_complete:
        reqs = jctanner.cloud_amazon.ec2.get_all_spot_instance_requests()
        for sirb in spot_requests:
            ...
            for sir in reqs:
                ...
                if sir.instance_id is not None:
                    ...
                    # https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/UserGuide/spot-bid-status.html
...
def enforce_count(module, jctanner.cloud_amazon.ec2, vpc):
    ...
    reservations, instances = find_running_instances_by_count_tag(module, jctanner.cloud_amazon.ec2, vpc, count_tag, zone)
    ...
    if len(instances) == exact_count:
        ...
        if not checkmode:
            (instance_dict_array, changed_instance_ids, changed) \
                = create_instances(module, jctanner.cloud_amazon.ec2, vpc, override_count=to_create)
        ...
        if not checkmode:
            ...
            (changed, instance_dict_array, changed_instance_ids) \
                = terminate_instances(module, jctanner.cloud_amazon.ec2, remove_ids)
            terminated_list = []
            for inst in instance_dict_array:
...
def create_instances(module, jctanner.cloud_amazon.ec2, vpc, override_count=None):
    """
    ...
    try:
        ...
        if group_name:
            if vpc_id:
                grp_details = jctanner.cloud_amazon.ec2.get_all_security_groups(filters={'vpc_id': vpc_id})
            else:
                grp_details = jctanner.cloud_amazon.ec2.get_all_security_groups()
        ...
        # Now we try to lookup the group id testing if group exists.
            ...
            if isinstance(group_id, string_types):
                group_id = [group_id]
            grp_details = jctanner.cloud_amazon.ec2.get_all_security_groups(group_ids=group_id)
    ...
    if id is not None:
        ...
        previous_reservations = jctanner.cloud_amazon.ec2.get_all_instances(None, filter_dict)
    ...
    if count_remaining == 0:
        ...
        try:
            ...
            if boto_supports_profile_name_arg(jctanner.cloud_amazon.ec2):
            ...
            if assign_public_ip is not None:
                if not boto_supports_associate_public_ip_address(jctanner.cloud_amazon.ec2):
                    module.fail_json(
                    ...
                    if private_ip:
                        interface = boto.jctanner.cloud_amazon.ec2.networkinterface.NetworkInterfaceSpecification(
                        ...
                        interface = boto.jctanner.cloud_amazon.ec2.networkinterface.NetworkInterfaceSpecification(
                            ...
                            groups=group_id,
                            associate_public_ip_address=assign_public_ip)
                    interfaces = boto.jctanner.cloud_amazon.ec2.networkinterface.NetworkInterfaceCollection(interface)
                ...
                if network_interfaces:
                    ...
                    params['network_interfaces'] = \
                        boto.jctanner.cloud_amazon.ec2.networkinterface.NetworkInterfaceCollection(*interfaces)
                else:
            ...
            if volumes:
                ...
                for volume in volumes:
                    ...
                    if 'volume_size' not in volume or int(volume['volume_size']) > 0:
                        bdm[volume['device_name']] = create_block_device(module, jctanner.cloud_amazon.ec2, volume)
            ...
            if not spot_price:
                ...
                try:
                    res = jctanner.cloud_amazon.ec2.run_instances(**params)
                ...
                while True:
                    try:
                        jctanner.cloud_amazon.ec2.get_all_instances(instids)
                        break
                    except boto.exception.EC2ResponseError as e:
                ...
                # The instances returned through jctanner.cloud_amazon.ec2.run_instances above can be in
                ...
                if boto_supports_param_in_spot_request(jctanner.cloud_amazon.ec2, 'placement_group'):
                ...
                res = jctanner.cloud_amazon.ec2.request_spot_instances(spot_price, **params)

                # Now we have to do the intermediate waiting
                if wait:
                    instids = await_spot_requests(module, jctanner.cloud_amazon.ec2, res, count)
                else:
                    instids = []
        ...
        while wait_timeout > time.time() and num_running < len(instids):
            try:
                res_list = jctanner.cloud_amazon.ec2.get_all_instances(instids)
            except boto.exception.BotoServerError as e:
        ...
        if instance_tags and instids:
            try:
                jctanner.cloud_amazon.ec2.create_tags(instids, instance_tags)
            except boto.exception.EC2ResponseError as e:
...
def terminate_instances(module, jctanner.cloud_amazon.ec2, instance_ids):
    """
    ...
    for res in jctanner.cloud_amazon.ec2.get_all_instances(instance_ids):
        for inst in res.instances:
            if inst.state == 'running' or inst.state == 'stopped':
                terminated_instance_ids.append(inst.id)
                instance_dict_array.append(get_instance_info(inst))
                try:
                    jctanner.cloud_amazon.ec2.terminate_instances([inst.id])
    ...
    if wait:
        ...
        while wait_timeout > time.time() and num_terminated < len(terminated_instance_ids):
            response = jctanner.cloud_amazon.ec2.get_all_instances(instance_ids=terminated_instance_ids,
        ...
        instance_dict_array = []
        for res in jctanner.cloud_amazon.ec2.get_all_instances(instance_ids=terminated_instance_ids, filters={'instance-state-name': 'terminated'}):
            for inst in res.instances:
                instance_dict_array.append(get_instance_info(inst))
...
def startstop_instances(module, jctanner.cloud_amazon.ec2, instance_ids, state, instance_tags):
    """
    ...
    for res in jctanner.cloud_amazon.ec2.get_all_instances(instance_ids, filters=filters):
        for inst in res.instances:

            warn_if_public_ip_assignment_changed(module, inst)

            changed = (check_source_dest_attr(module, inst, jctanner.cloud_amazon.ec2) or
                       check_termination_protection(module, inst) or changed)

            # Check security groups and if we're using jctanner.cloud_amazon.ec2-vpc; jctanner.cloud_amazon.ec2-classic security groups may not be modified
            if inst.vpc_id and group_name:
                grp_details = jctanner.cloud_amazon.ec2.get_all_security_groups(filters={'vpc_id': inst.vpc_id})
                if isinstance(group_name, string_types):
                    group_name = [group_name]
...
def restart_instances(module, jctanner.cloud_amazon.ec2, instance_ids, state, instance_tags):
    """
...
def check_source_dest_attr(module, inst, jctanner.cloud_amazon.ec2):
    """
            ...
            if exc.code == 'InvalidInstanceID':
                for interface in inst.interfaces:
                    if interface.source_dest_check != source_dest_check:
                        jctanner.cloud_amazon.ec2.modify_network_interface_attribute(interface.id, "sourceDestCheck", source_dest_check)
                        return True
            else:
...
def main():
    ...
    argument_spec.update(
        dict(
            ...
            zone=dict(aliases=['jctanner.cloud_amazon.aws_zone', 'jctanner.cloud_amazon.ec2_zone']),
    ...
    try:
        region, jctanner.cloud_amazon.ec2_url, jctanner.cloud_amazon.aws_connect_kwargs = get_jctanner.cloud_amazon.aws_connection_info(module)
        if module.params.get('region') or not module.params.get('jctanner.cloud_amazon.ec2_url'):
            jctanner.cloud_amazon.ec2 = jctanner.cloud_amazon.ec2_connect(module)
        elif module.params.get('jctanner.cloud_amazon.ec2_url'):
            jctanner.cloud_amazon.ec2 = connect_jctanner.cloud_amazon.ec2_endpoint(jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)

        if 'region' not in jctanner.cloud_amazon.aws_connect_kwargs:
            jctanner.cloud_amazon.aws_connect_kwargs['region'] = jctanner.cloud_amazon.ec2.region

        vpc = connect_vpc(**jctanner.cloud_amazon.aws_connect_kwargs)
    except boto.exception.NoAuthHandlerFound as e:
    ...
    if state == 'absent':
        ...
        (changed, instance_dict_array, new_instance_ids) = terminate_instances(module, jctanner.cloud_amazon.ec2, instance_ids)
        ...
        (changed, instance_dict_array, new_instance_ids) = startstop_instances(module, jctanner.cloud_amazon.ec2, instance_ids, state, instance_tags)
        ...
        (changed, instance_dict_array, new_instance_ids) = restart_instances(module, jctanner.cloud_amazon.ec2, instance_ids, state, instance_tags)
        ...
        if module.params.get('exact_count') is None:
            (instance_dict_array, new_instance_ids, changed) = create_instances(module, jctanner.cloud_amazon.ec2, vpc)
        else:
            (tagged_instances, instance_dict_array, new_instance_ids, changed) = enforce_count(module, jctanner.cloud_amazon.ec2, vpc)
