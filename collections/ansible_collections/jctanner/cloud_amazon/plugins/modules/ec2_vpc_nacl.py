...
DOCUMENTATION = '''
module: jctanner.cloud_amazon.ec2_vpc_nacl
short_description: create and delete Network ACLs.
description:
  - Read the AWS documentation for Network ACLS
    U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AmazonVPC/latest/UserGuide/VPC_ACLs.html)
version_added: "2.2"
options:
  name:
    description:
...
- name: "Create and associate production DMZ network ACL with DMZ subnets"
  jctanner.cloud_amazon.ec2_vpc_nacl:
    vpc_id: vpc-12345678
    name: prod-dmz-nacl
...
def rules_changed(jctanner.cloud_amazon.aws_rules, param_rules, Egress, nacl_id, client, module):
    changed = False
    rules = list()
    for entry in param_rules:
        rules.append(process_rule_entry(entry, Egress))
    if rules == jctanner.cloud_amazon.aws_rules:
        return changed
    else:
        removed_rules = [x for x in jctanner.cloud_amazon.aws_rules if x not in rules]
        if removed_rules:
            params = dict()
            ...
            changed = True
        added_rules = [x for x in rules if x not in jctanner.cloud_amazon.aws_rules]
        if added_rules:
...
def main():
    ...
    try:
        ...
        client = boto3_conn(module, conn_type='client', resource='jctanner.cloud_amazon.ec2', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
