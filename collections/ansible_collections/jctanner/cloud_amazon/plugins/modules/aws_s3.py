...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.aws_s3
...
notes:
   - In 2.4, this module has been renamed from C(s3) into M(jctanner.cloud_amazon.aws_s3).
version_added: "1.1"
options:
  jctanner.cloud_amazon.aws_access_key:
    description:
      - AWS access key id. If not set then the value of the AWS_ACCESS_KEY environment variable is used.
    aliases: [ 'jctanner.cloud_amazon.ec2_access_key', 'access_key' ]
  jctanner.cloud_amazon.aws_secret_key:
    description:
      - AWS secret key. If not set then the value of the AWS_SECRET_KEY environment variable is used.
    aliases: ['jctanner.cloud_amazon.ec2_secret_key', 'secret_key']
  bucket:
    description:
      - Bucket name.
    required: true
  ...
  permission:
    description:
      - This option lets the user set the canned permissions on the object/bucket that are created.
        ...
        'private', 'public-read', 'public-read-write', 'jctanner.cloud_amazon.aws-exec-read', 'authenticated-read', 'bucket-owner-read',
  ...
  overwrite:
    description:
      - Force overwrite either locally on the filesystem or remotely with the object/key. Used with PUT and GET operations.
        ...
        U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AmazonS3/latest/API/RESTCommonResponseHeaders.html)
    default: 'always'
    aliases: ['force']
    version_added: "1.2"
  region:
    description:
     - "AWS region to create the bucket in. If not set then the value of the AWS_REGION and EC2_REGION environment variables
       are checked, followed by the jctanner.cloud_amazon.aws_region and jctanner.cloud_amazon.ec2_region settings in the Boto config file. If none of those are set the
  ...
  encryption_kms_key_id:
    description:
      - KMS key id to use when encrypting objects using C(jctanner.cloud_amazon.aws:kms) encryption. Ignored if encryption is not C(jctanner.cloud_amazon.aws:kms)
    version_added: "2.7"
...
url:
  ...
  type: str
  sample: https://my-bucket.s3.amazonjctanner.cloud_amazon.aws.com/my-key.txt?AWSAccessKeyId=<access-key>&Expires=1506888865&Signature=<signature>
expiry:
...
def key_check(module, s3, bucket, obj, version=None, validate=True):
    ...
    try:
        ...
        if error_code == 404:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while looking up object (during key check) %s." % obj)
...
def bucket_check(module, s3, bucket, validate=True):
    ...
    try:
        ...
        if error_code == 404:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while looking up bucket (during bucket_check) %s." % bucket)
    except botocore.exceptions.EndpointConnectionError as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Invalid endpoint provided")
    except botocore.exceptions.BotoCoreError as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while looking up bucket (during bucket_check) %s." % bucket)
    return exists
...
def create_bucket(module, s3, bucket, location=None):
    ...
    try:
        ...
        if e.response['Error']['Code'] in IGNORE_S3_DROP_IN_EXCEPTIONS:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while creating bucket or setting acl (check that you have CreateBucket and PutBucketAcl permission).")
    except botocore.exceptions.BotoCoreError as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while creating bucket or setting acl (check that you have CreateBucket and PutBucketAcl permission).")
...
def list_keys(module, s3, bucket, prefix, marker, max_keys):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while listing the keys in the bucket {0}".format(bucket))
...
def delete_bucket(module, s3, bucket):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while deleting bucket %s." % bucket)
...
def delete_key(module, s3, bucket, obj):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while trying to delete %s." % obj)
...
def create_dirkey(module, s3, bucket, obj, encrypt):
    ...
    try:
        ...
        if module.params['encryption_kms_key_id'] and module.params['encryption_mode'] == 'jctanner.cloud_amazon.aws:kms':
        ...
        if e.response['Error']['Code'] in IGNORE_S3_DROP_IN_EXCEPTIONS:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while creating object %s." % obj)
    except botocore.exceptions.BotoCoreError as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while creating object %s." % obj)
...
def upload_s3file(module, s3, bucket, obj, src, expiry, metadata, encrypt, headers):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to complete PUT operation.")
    try:
        for acl in module.params.get('permission'):
            s3.put_object_acl(ACL=acl, Bucket=bucket, Key=obj)
    except botocore.exceptions.ClientError as e:
        if e.response['Error']['Code'] in IGNORE_S3_DROP_IN_EXCEPTIONS:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to set object ACL")
    except botocore.exceptions.BotoCoreError as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to set object ACL")
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to generate presigned URL")
...
def download_s3file(module, s3, bucket, obj, dest, retries, version=None):
    ...
    try:
        ...
        if e.response['Error']['Code'] == 'InvalidArgument' and 'require AWS Signature Version 4' in to_text(e):
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not find the key %s." % obj)
    except botocore.exceptions.BotoCoreError as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not find the key %s." % obj)
    ...
    for x in range(0, retries + 1):
        try:
            ...
            if x >= retries:
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while downloading %s." % obj)
            # otherwise, try again, this may be a transient timeout.
        except SSLError as e:  # will ClientError catch SSLError?
            # actually fail on last pass through the loop.
            if x >= retries:
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="s3 download failed")
            # otherwise, try again, this may be a transient timeout.
...
def download_s3str(module, s3, bucket, obj, version=None, validate=True):
    ...
    try:
        ...
        if e.response['Error']['Code'] == 'InvalidArgument' and 'require AWS Signature Version 4' in to_text(e):
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while getting contents of object %s as a string." % obj)
    except botocore.exceptions.BotoCoreError as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while getting contents of object %s as a string." % obj)
...
def get_download_url(module, s3, bucket, obj, expiry, changed=True):
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed while getting download url.")
...
def get_s3_connection(module, jctanner.cloud_amazon.aws_connect_kwargs, location, rgw, s3_url, sig_4=False):
    if s3_url and rgw:  # TODO - test this
        rgw = urlparse(s3_url)
        params = dict(module=module, conn_type='client', resource='s3', use_ssl=rgw.scheme == 'https', region=location, endpoint=s3_url, **jctanner.cloud_amazon.aws_connect_kwargs)
        ...
        if module.params['mode'] == 'put' and module.params['encryption_mode'] == 'jctanner.cloud_amazon.aws:kms':
...
def main():
    ...
    argument_spec.update(
        dict(
            ...
            encryption_mode=dict(choices=['AES256', 'jctanner.cloud_amazon.aws:kms'], default='AES256'),
    ...
    object_canned_acl = ["private", "public-read", "public-read-write", "jctanner.cloud_amazon.aws-exec-read", "authenticated-read", "bucket-owner-read", "bucket-owner-full-control"]
    ...
    if dualstack and s3_url is not None and 'amazonjctanner.cloud_amazon.aws.com' not in s3_url:
    ...
    s3 = get_s3_connection(module, jctanner.cloud_amazon.aws_connect_kwargs, location, rgw, s3_url)
            ...
            if keysum_compare(module, dest, s3, bucket, obj, version=version):
                ...
                if overwrite == 'always':
                    try:
                        ...
                        s3 = get_s3_connection(module, jctanner.cloud_amazon.aws_connect_kwargs, location, rgw, s3_url, sig_4=True)
                        download_s3file(module, s3, bucket, obj, dest, retries, version=version)
                else:
