...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_vol
...
- jctanner.cloud_amazon.ec2_vol:
  ...
  register: jctanner.cloud_amazon.ec2_vol
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import (HAS_BOTO, AnsibleAWSError, connect_to_jctanner.cloud_amazon.aws, jctanner.cloud_amazon.ec2_argument_spec,
                                      get_jctanner.cloud_amazon.aws_connection_info)


def get_volume(module, jctanner.cloud_amazon.ec2):
    ...
    if id:
        volume_ids = [id]
    try:
        vols = jctanner.cloud_amazon.ec2.get_all_volumes(volume_ids=volume_ids, filters=filters)
...
def get_volumes(module, jctanner.cloud_amazon.ec2):

    instance = module.params.get('instance')

    try:
        if not instance:
            vols = jctanner.cloud_amazon.ec2.get_all_volumes()
        else:
            vols = jctanner.cloud_amazon.ec2.get_all_volumes(filters={'attachment.instance-id': instance})
    except boto.exception.BotoServerError as e:
        module.fail_json(msg="%s: %s" % (e.error_code, e.error_message))
    return vols


def delete_volume(module, jctanner.cloud_amazon.ec2):
    volume_id = module.params['id']
    try:
        jctanner.cloud_amazon.ec2.delete_volume(volume_id)
        module.exit_json(changed=True)
    except boto.exception.EC2ResponseError as jctanner.cloud_amazon.ec2_error:
        if jctanner.cloud_amazon.ec2_error.code == 'InvalidVolume.NotFound':
            module.exit_json(changed=False)
        module.fail_json(msg=jctanner.cloud_amazon.ec2_error.message)
...
def create_volume(module, jctanner.cloud_amazon.ec2, zone):
    changed = False
    name = module.params.get('name')
    ...
    volume = get_volume(module, jctanner.cloud_amazon.ec2)
    if volume is None:
        try:
            if boto_supports_volume_encryption():
                if kms_key_id is not None:
                    volume = jctanner.cloud_amazon.ec2.create_volume(volume_size, zone, snapshot, volume_type, iops, encrypted, kms_key_id)
                else:
                    volume = jctanner.cloud_amazon.ec2.create_volume(volume_size, zone, snapshot, volume_type, iops, encrypted)
                changed = True
            else:
                volume = jctanner.cloud_amazon.ec2.create_volume(volume_size, zone, snapshot, volume_type, iops)
                changed = True
            ...
            if name:
                tags["Name"] = name
            if tags:
                jctanner.cloud_amazon.ec2.create_tags([volume.id], tags)
        except boto.exception.BotoServerError as e:
            module.fail_json(msg="%s: %s" % (e.error_code, e.error_message))
...
def attach_volume(module, jctanner.cloud_amazon.ec2, volume, instance):
    ...
    # https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/UserGuide/block-device-mapping-concepts.html
    ...
    if device_name is None:
        try:
            if not jctanner.cloud_amazon.ec2.get_password_data(instance.id):
                device_name = '/dev/sdf'
            else:
                device_name = '/dev/xvdf'
        except boto.exception.BotoServerError as e:
    ...
    if volume.attachment_state() is not None:
        ...
        if adata.instance_id != instance.id:
            ...
            changed = modify_dot_attribute(module, jctanner.cloud_amazon.ec2, instance, device_name)
    else:
        try:
        ...
        modify_dot_attribute(module, jctanner.cloud_amazon.ec2, instance, device_name)

    return volume, changed


def modify_dot_attribute(module, jctanner.cloud_amazon.ec2, instance, device_name):
    ...
    if delete_on_termination != dot:
        try:
            ...
            jctanner.cloud_amazon.ec2.modify_instance_attribute(instance_id=instance.id, attribute='blockDeviceMapping', value=bdm)
...
def detach_volume(module, jctanner.cloud_amazon.ec2, volume):
...
def main():
    ...
    argument_spec.update(dict(
        ...
        zone=dict(aliases=['availability_zone', 'jctanner.cloud_amazon.aws_zone', 'jctanner.cloud_amazon.ec2_zone']),
        snapshot=dict(),
    ...
    region, jctanner.cloud_amazon.ec2_url, jctanner.cloud_amazon.aws_connect_params = get_jctanner.cloud_amazon.aws_connection_info(module)

    if region:
        try:
            jctanner.cloud_amazon.ec2 = connect_to_jctanner.cloud_amazon.aws(boto.jctanner.cloud_amazon.ec2, region, **jctanner.cloud_amazon.aws_connect_params)
    ...
    if state == 'list':
        returned_volumes = []
        vols = get_volumes(module, jctanner.cloud_amazon.ec2)
    ...
    if instance:
        try:
            reservation = jctanner.cloud_amazon.ec2.get_all_instances(instance_ids=instance)
    ...
    if state == 'present':
        volume, changed = create_volume(module, jctanner.cloud_amazon.ec2, zone)
        if detach_vol_flag:
            volume, changed = detach_volume(module, jctanner.cloud_amazon.ec2, volume)
        elif inst is not None:
            volume, changed = attach_volume(module, jctanner.cloud_amazon.ec2, volume, inst)
        ...
        if module.params.get('delete_on_termination'):
            for attempt in range(0, 8):
                if volume_info['attachment_set'].get('deleteOnTermination') == 'true':
                    break
                time.sleep(5)
                volume = jctanner.cloud_amazon.ec2.get_all_volumes(volume_ids=volume.id)[0]
                volume_info = get_volume_info(volume, state)
        ...
        delete_volume(module, jctanner.cloud_amazon.ec2)
