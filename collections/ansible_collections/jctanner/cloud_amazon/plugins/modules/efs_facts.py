...
mount_point:
    ...
    returned: always
    type: str
    sample: .fs-xxxxxxxx.efs.us-west-2.amazonjctanner.cloud_amazon.aws.com:/
filesystem_address:
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import boto3_conn, get_jctanner.cloud_amazon.aws_connection_info, jctanner.cloud_amazon.ec2_argument_spec, AWSRetry
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import camel_dict_to_snake_dict, boto3_tag_list_to_ansible_dict
...
class EFSConnection(object):
        ...
        for item in file_systems:
            if item['life_cycle_state'] == self.STATE_AVAILABLE:
                try:
                    ...
                    self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get EFS targets")
                ...
                for target in item['mount_targets']:
                    if target['life_cycle_state'] == self.STATE_AVAILABLE:
                        try:
                            ...
                            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get EFS security groups")
                    else:
    ...
    def get_file_systems(self, file_system_id=None, creation_token=None):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get EFS file systems")
        ...
        for item in file_systems:
            ...
            U(https://docs.jctanner.cloud_amazon.aws.amazon.com/efs/latest/ug/gs-step-three-connect-to-jctanner.cloud_amazon.ec2-instance.html)
            """
            item['MountPoint'] = '.%s.efs.%s.amazonjctanner.cloud_amazon.aws.com:/' % (item['FileSystemId'], self.region)
            item['FilesystemAddress'] = '%s.efs.%s.amazonjctanner.cloud_amazon.aws.com:/' % (item['FileSystemId'], self.region)
            ...
            if result['life_cycle_state'] == self.STATE_AVAILABLE:
                try:
                    ...
                    self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get EFS tags")
...
def main():
    ...
    region, _, jctanner.cloud_amazon.aws_connect_params = get_jctanner.cloud_amazon.aws_connection_info(module, boto3=True)
    connection = EFSConnection(module, region, **jctanner.cloud_amazon.aws_connect_params)
