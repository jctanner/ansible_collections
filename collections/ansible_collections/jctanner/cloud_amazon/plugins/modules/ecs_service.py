...
notes:
  - the service role specified must be assumable (i.e. have a trust relationship for the ecs service, ecs.amazonjctanner.cloud_amazon.aws.com)
...
options:
    ...
    role:
        description:
          - The name or full Amazon Resource Name (ARN) of the IAM role that allows your Amazon ECS container agent to make calls to your load balancer
            on your behalf. This parameter is only required if you are using a load balancer with your service, in a network mode other than `jctanner.cloud_amazon.awsvpc`.
        required: false
    delay:
        description:
    ...
    network_configuration:
        description:
          - network configuration of the service. Only applicable for task definitions created with C(jctanner.cloud_amazon.awsvpc) I(network_mode).
...
service:
    ...
    contains:
        ...
        runningCount:
            ...
            type: int
        serviceArn:
            description: The Amazon Resource Name (ARN) that identifies the service. The ARN contains the arn:jctanner.cloud_amazon.aws:ecs namespace, followed by the region
                         of the service, the AWS account ID of the service owner, the service namespace, and then the service name. For example,
                         arn:jctanner.cloud_amazon.aws:ecs:region :012345678910 :service/my-service .
            returned: always
            type: str
        serviceName:
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import snake_dict_to_camel_dict, map_complex_type, get_jctanner.cloud_amazon.ec2_security_group_ids_from_names
...
class EcsServiceManager:
    ...
    def __init__(self, module):
        ...
        self.jctanner.cloud_amazon.ec2 = module.client('jctanner.cloud_amazon.ec2')
    ...
    def format_network_configuration(self, network_config):
        ...
        if network_config['security_groups'] is not None:
            ...
            if any(not sg.startswith('sg-') for sg in groups):
                try:
                    vpc_id = self.jctanner.cloud_amazon.ec2.describe_subnets(SubnetIds=[result['subnets'][0]])['Subnets'][0]['VpcId']
                    groups = get_jctanner.cloud_amazon.ec2_security_group_ids_from_names(groups, self.jctanner.cloud_amazon.ec2, vpc_id)
                except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
                    self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't look up security groups")
        ...
        return dict(jctanner.cloud_amazon.awsvpcConfiguration=result)
            ...
            if not module.check_mode:
                ...
                if update:
                    ...
                    try:
                        ...
                        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't create service")
            ...
            if 'status' in existing and existing['status'] == "INACTIVE":
                ...
                if not module.check_mode:
                    try:
                        ...
                        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't delete service")
                results['changed'] = True
