...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.aws_kms
...
options:
  ...
  alias:
    description: An alias for a key. For safety, even though KMS does not require keys
      ...
      referred to by I(key_id). Use M(jctanner.cloud_amazon.aws_kms_info) to find key ids. Required
  ...
  grants:
    description:
      ...
      - Constraints is a dict containing C(encryption_context_subset) or C(encryption_context_equals),
        ...
        See U(https://docs.jctanner.cloud_amazon.aws.amazon.com/kms/latest/APIReference/API_GrantConstraints.html)
      - I(grantee_principal) and I(retiring_principal) must be ARNs
    version_added: 2.8
  policy:
    description:
      - policy to apply to the KMS key
      - See U(https://docs.jctanner.cloud_amazon.aws.amazon.com/kms/latest/developerguide/key-policies.html)
    version_added: 2.8
author:
  - Ted Timmons (@tedder)
...
- jctanner.cloud_amazon.aws_kms:
    ...
    grants:
      - name: billing_prod
        grantee_principal: arn:jctanner.cloud_amazon.aws:iam::1234567890123:role/billing_prod
        constraints:
...
key_arn:
  ...
  type: str
  returned: always
  sample: arn:jctanner.cloud_amazon.aws:kms:ap-southeast-2:123456789012:key/abcd1234-abcd-1234-5678-ef1234567890
key_state:
...
origin:
  ...
  returned: always
  sample: AWS_KMS
jctanner.cloud_amazon.aws_account_id:
...
aliases:
  ...
  returned: always
  sample:
    - jctanner.cloud_amazon.aws/acm
    - jctanner.cloud_amazon.aws/ebs
policies:
    ...
    - Sid: "Allow access through EBS for all principals in the account that are authorized to use EBS"
      ...
      Condition:
        StringEquals:
          ...
          kms:ViaService: "jctanner.cloud_amazon.ec2.ap-southeast-2.amazonjctanner.cloud_amazon.aws.com"
    - Sid: "Allow direct access to key metadata to the account"
      Effect: "Allow"
      Principal:
        AWS: "arn:jctanner.cloud_amazon.aws:iam::111111111111:root"
      Action:
      - "kms:Describe*"
      - "kms:Get*"
      - "kms:List*"
  ...
  contains:
    constraints:
      description: Constraints on the encryption context that the grant allows.
        See U(https://docs.jctanner.cloud_amazon.aws.amazon.com/kms/latest/APIReference/API_GrantConstraints.html) for further details
      type: dict
      returned: always
      sample:
        encryption_context_equals:
           "jctanner.cloud_amazon.aws:lambda:_function_arn": "arn:jctanner.cloud_amazon.aws:lambda:ap-southeast-2:012345678912:function:xyz"
    creation_date:
    ...
    grantee_principal:
      ...
      type: str
      returned: always
      sample: arn:jctanner.cloud_amazon.aws:sts::0123456789012:assumed-role/lambda_xyz/xyz
    issuing_account:
      description: The AWS account under which the grant was issued
      type: str
      returned: always
      sample: arn:jctanner.cloud_amazon.aws:iam::01234567890:root
    key_id:
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import jctanner.cloud_amazon.ec2_argument_spec
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import AWSRetry, camel_dict_to_snake_dict
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import boto3_tag_list_to_ansible_dict, ansible_dict_to_boto3_tag_list
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import compare_jctanner.cloud_amazon.aws_tags
...
def get_kms_tags(connection, module, key_id):
    ...
    while more:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to obtain key tags")
...
def get_kms_policies(connection, module, key_id):
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to obtain key policies")
...
def get_kms_facts(connection, module):
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to obtain keys")
...
def ensure_enabled_disabled(connection, module, key):
    ...
    if key['key_state'] == 'Disabled' and module.params['enabled']:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to enable key")
    ...
    if key['key_state'] == 'Enabled' and not module.params['enabled']:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to disable key")
    return changed
...
def update_key(connection, module, key):
    ...
    if key_id:
        ...
        if alias not in [_alias['AliasName'] for _alias in aliases]:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(msg="Failed create key alias")
    ...
    if key['key_state'] == 'PendingDeletion':
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to cancel key deletion")
    ...
    if description and key['description'] != description:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to update key description")

    desired_tags = module.params.get('tags')
    to_add, to_remove = compare_jctanner.cloud_amazon.aws_tags(key['tags'], desired_tags,
    ...
    if to_remove:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to remove or update tag")
    if to_add:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to add tag to key")
    ...
    if to_remove:
        for grant in to_remove:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to retire grant")
    ...
    if to_add:
        for grant in to_add:
            ...
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to create grant")
...
def delete_key(connection, module, key):
    ...
    if key['key_state'] != 'PendingDeletion':
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to schedule key for deletion")
        ...
        for granttype in ['role', 'role grant', 'admin']:
            ...
            if mode == 'grant' and statement['Sid'] == statement_label[granttype]:
                ...
                if granttype in granttypes:
                    invalid_entries = [item for item in statement['Principal']['AWS'] if not item.startswith('arn:jctanner.cloud_amazon.aws:iam::')]
                    if clean_invalid_entries and invalid_entries:
                        # we have bad/invalid entries. These are roles that were deleted.
                        # prune the list.
                        valid_entries = [item for item in statement['Principal']['AWS'] if item.startswith('arn:jctanner.cloud_amazon.aws:iam::')]
