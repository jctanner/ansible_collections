...
sns_arn:
    ...
    type: str
    returned: always
    sample: "arn:jctanner.cloud_amazon.aws:sns:us-east-2:111111111111:my_topic_name"
sns_topic:
  ...
  contains:
    ...
    policy:
      ...
      type: str
      sample: >
        {"Version":"2012-10-17","Id":"SomePolicyId","Statement":[{"Sid":"ANewSid","Effect":"Allow","Principal":{"AWS":"arn:jctanner.cloud_amazon.aws:iam::111111111111:root"},
        "Action":"sns:Subscribe","Resource":"arn:jctanner.cloud_amazon.aws:sns:us-east-2:111111111111:ansible-test-dummy-topic","Condition":{"StringEquals":{"sns:Protocol":"email"}}}]}
    state:
    ...
    topic_arn:
      ...
      returned: when topic is owned by this AWS account
      type: str
      sample: arn:jctanner.cloud_amazon.aws:sns:us-east-2:111111111111:ansible-test-dummy-topic
    topic_created:
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import compare_policies, AWSRetry, camel_dict_to_snake_dict
...
class SnsTopicManager(object):
    ...
    def _list_topics(self):
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get topic list")
    ...
    def _create_topic(self):
        if not self.check_mode:
            try:
                ...
                self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't create topic %s" % self.name)
    ...
    def _set_topic_attrs(self):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get topic attributes for topic %s" % self.topic_arn)
        ...
        if self.display_name and self.display_name != topic_attributes['DisplayName']:
            ...
            if not self.check_mode:
                try:
                    ...
                    self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't set display name")
        ...
        if self.policy and compare_policies(self.policy, json.loads(topic_attributes['Policy'])):
            ...
            if not self.check_mode:
                try:
                    ...
                    self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't set topic policy")
        ...
        if self.delivery_policy and ('DeliveryPolicy' not in topic_attributes or
            ...
            if not self.check_mode:
                try:
                    ...
                    self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't set topic delivery policy")
        return changed
            ...
            if (self.purge_subscriptions and sub_key not in desired_subscriptions and
                ...
                if not self.check_mode:
                    try:
                        ...
                        self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't unsubscribe from topic")
        ...
        for protocol, endpoint in set(desired_subscriptions).difference(subscriptions_existing_list):
            ...
            if not self.check_mode:
                try:
                    ...
                    self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't subscribe to topic %s" % self.topic_arn)
        return changed
    ...
    def _list_topic_subscriptions(self):
        try:
            ...
            try:
                ...
                self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get subscriptions list for topic %s" % self.topic_arn)
        except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:  # pylint: disable=duplicate-except
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get subscriptions list for topic %s" % self.topic_arn)

    def _delete_subscriptions(self):
        # NOTE: subscriptions in 'PendingConfirmation' timeout in 3 days
        #       https://forums.jctanner.cloud_amazon.aws.amazon.com/thread.jspa?threadID=85993
    ...
    def _delete_topic(self):
        ...
        if not self.check_mode:
            try:
                ...
                self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't delete topic %s" % self.topic_arn)
        return True
