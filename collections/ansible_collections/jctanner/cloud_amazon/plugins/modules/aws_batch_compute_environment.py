...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.aws_batch_compute_environment
...
description:
    - This module allows the management of AWS Batch Compute Environments.
      It is idempotent and supports "Check" mode.  Use module M(jctanner.cloud_amazon.aws_batch_compute_environment) to manage the compute
      environment, M(jctanner.cloud_amazon.aws_batch_job_queue) to manage job queues, M(jctanner.cloud_amazon.aws_batch_job_definition) to manage job definitions.
...
options:
  ...
  jctanner.cloud_amazon.ec2_key_pair:
    description:
...
- hosts: localhost
  ...
  - name: My Batch Compute Environment
    jctanner.cloud_amazon.aws_batch_compute_environment:
      ...
      security_group_ids:
        - my-sg1
        - my-sg2
      instance_role: arn:jctanner.cloud_amazon.aws:iam::<account>:instance-profile/<role>
      tags:
        tag1: value1
        tag2: value2
      service_role: arn:jctanner.cloud_amazon.aws:iam::<account>:role/service-role/<role>

  - name: show results
    debug: var=jctanner.cloud_amazon.aws_batch_compute_environment_action
'''
  ...
  sample:
    ...
    invocation:
      module_args:
        ...
        desiredv_cpus: 0
        jctanner.cloud_amazon.ec2_key_pair: ~
        jctanner.cloud_amazon.ec2_url: ~
        image_id: ~
        instance_role: "arn:jctanner.cloud_amazon.aws:iam::..."
        instance_types:
          - optimal
        maxv_cpus: 8
        minv_cpus: 0
        ...
        security_group_ids:
          - "*******"
        security_token: ~
        service_role: "arn:jctanner.cloud_amazon.aws:iam::...."
        spot_iam_fleet_role: ~
        state: present
        subnets:
          - "******"
        ...
        validate_certs: true
    response:
      computeEnvironmentArn: "arn:jctanner.cloud_amazon.aws:batch:...."
      computeEnvironmentName: <name>
      computeResources:
        desiredvCpus: 0
        instanceRole: "arn:jctanner.cloud_amazon.aws:iam::..."
        instanceTypes:
          - optimal
        maxvCpus: 8
        minvCpus: 0
        ...
        tags:
          ...
          Name: <name>
        type: EC2
      ecsClusterArn: "arn:jctanner.cloud_amazon.aws:ecs:....."
      serviceRole: "arn:jctanner.cloud_amazon.aws:iam::..."
      state: ENABLED
      status: VALID
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.aws.batch import AWSConnection
from ansible.module_utils.basic import AnsibleModule
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import jctanner.cloud_amazon.ec2_argument_spec, HAS_BOTO3
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import snake_dict_to_camel_dict, camel_dict_to_snake_dict
import re
import traceback
...
def validate_params(module, jctanner.cloud_amazon.aws):
    """
    Performs basic parameter validation.

    :param module:
    :param jctanner.cloud_amazon.aws:
    :return:
    """
    ...
    if not compute_environment_name.startswith('arn:jctanner.cloud_amazon.aws:batch:'):
...
def create_compute_environment(module, jctanner.cloud_amazon.aws):
    """
    ...
    client = jctanner.cloud_amazon.aws.client('batch')
    changed = False
    ...
    compute_resources_param_list = ('minv_cpus', 'maxv_cpus', 'desiredv_cpus', 'instance_types', 'image_id', 'subnets',
                                    'security_group_ids', 'jctanner.cloud_amazon.ec2_key_pair', 'instance_role', 'tags', 'bid_percentage',
                                    'spot_iam_fleet_role')
...
def remove_compute_environment(module, jctanner.cloud_amazon.aws):
    """
...
def manage_state(module, jctanner.cloud_amazon.aws):
    changed = False
    ...
    current_compute_environment = get_current_compute_environment(module, jctanner.cloud_amazon.aws)
    ...
    if state == 'present':
        if current_state == 'present':
            ...
            if updates:
                try:
                    if not check_mode:
                        update_env_response = jctanner.cloud_amazon.aws.client().update_compute_environment(**compute_kwargs)
            ...
            changed = create_compute_environment(module, jctanner.cloud_amazon.aws)
            # Describe compute environment
            action_taken = 'added'
        response = get_current_compute_environment(module, jctanner.cloud_amazon.aws)
        if not response:
            module.fail_json(msg='Unable to get compute environment information after creating')
    else:
        if current_state == 'present':
            # remove the compute environment
            changed = remove_compute_environment(module, jctanner.cloud_amazon.aws)
...
def main():
    ...
    argument_spec.update(
        dict(
            ...
            jctanner.cloud_amazon.ec2_key_pair=dict(),
            instance_role=dict(required=True),
            tags=dict(type='dict'),
            bid_percentage=dict(type='int'),
            spot_iam_fleet_role=dict(),
            region=dict(aliases=['jctanner.cloud_amazon.aws_region', 'jctanner.cloud_amazon.ec2_region'])
        )
    )
    ...
    jctanner.cloud_amazon.aws = AWSConnection(module, ['batch'])

    validate_params(module, jctanner.cloud_amazon.aws)

    results = manage_state(module, jctanner.cloud_amazon.aws)
