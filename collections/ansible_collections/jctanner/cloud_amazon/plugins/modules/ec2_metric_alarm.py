...
DOCUMENTATION = """
module: jctanner.cloud_amazon.ec2_metric_alarm
...
EXAMPLES = '''
  - name: create alarm
    jctanner.cloud_amazon.ec2_metric_alarm:
      state: present
  ...
  - name: Create an alarm to recover a failed instance
    jctanner.cloud_amazon.ec2_metric_alarm:
      ...
      dimensions: {"InstanceId":'i-XXX'}
      alarm_actions: ["arn:jctanner.cloud_amazon.aws:automate:us-west-1:jctanner.cloud_amazon.ec2:recover"]

'''

try:
    import boto.jctanner.cloud_amazon.ec2.cloudwatch
    from boto.jctanner.cloud_amazon.ec2.cloudwatch import MetricAlarm
...
def main():
    ...
    if region:
        try:
            connection = connect_to_jctanner.cloud_amazon.aws(boto.jctanner.cloud_amazon.ec2.cloudwatch, region, **jctanner.cloud_amazon.aws_connect_params)
