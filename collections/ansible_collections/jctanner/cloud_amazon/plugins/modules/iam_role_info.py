...
iam_roles:
  ...
  contains:
    arn:
      ...
      returned: always
      type: str
      sample: arn:jctanner.cloud_amazon.aws:iam::123456789012:role/AnsibleTestRole
    ...
    managed_policies:
      ...
      contains:
        policy_arn:
          ...
          returned: always
          type: str
          sample: arn:jctanner.cloud_amazon.aws:iam::123456789012:policy/AnsibleTestEC2Policy
        policy_name:
    ...
    instance_profiles:
      ...
      contains:
        arn:
          ...
          returned: always
          type: str
          sample: arn:jctanner.cloud_amazon.aws:iam::123456789012:instance-profile/AnsibleTestEC2Policy
        create_date:
...
def describe_iam_role(module, client, role):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get inline policies for role %s" % name)
    try:
        role['ManagedPolicies'] = list_iam_attached_role_policies_with_backoff(client, name)
    except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get managed  policies for role %s" % name)
    try:
        role['InstanceProfiles'] = list_iam_instance_profiles_for_role_with_backoff(client, name)
    except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get instance profiles for role %s" % name)
    return role
    ...
    if name:
        try:
            ...
            if e.response['Error']['Code'] == 'NoSuchEntity':
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get IAM role %s" % name)
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't list IAM roles")
