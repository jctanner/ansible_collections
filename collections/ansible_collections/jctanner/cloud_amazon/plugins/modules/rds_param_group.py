...
options:
  ...
  engine:
    description:
      ...
      - '# jctanner.cloud_amazon.aws rds describe-db-engine-versions --query "DBEngineVersions[].DBParameterGroupFamily"'
  immediate:
    description:
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import camel_dict_to_snake_dict, HAS_BOTO3, compare_jctanner.cloud_amazon.aws_tags
...
def main():
    ...
    try:
        conn = boto3_conn(module, conn_type='client', resource='rds', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
