...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_instance
...
options:
  ...
  user_data:
    description:
      - Opaque blob of data which is made available to the jctanner.cloud_amazon.ec2 instance
  tower_callback:
    description:
  ...
  image:
    description:
      - An image to use for the instance. The M(jctanner.cloud_amazon.ec2_ami_info) module may be used to retrieve images.
  ...
  vpc_subnet_id:
    description:
      - The subnet ID in which to launch the instance (VPC)
        If none is provided, jctanner.cloud_amazon.ec2_instance will chose the default zone of the default VPC.
    aliases: ['subnet_id']
  network:
    description:
      ...
      - Use the jctanner.cloud_amazon.ec2_eni to create ENIs with special settings.
  volumes:
    description:
    ...
    - For more information about each parameter, see U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/APIReference/API_BlockDeviceMapping.html).
  launch_template:
    description:
  ...
  cpu_options:
    description:
      ...
      - See U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/UserGuide/instance-optimize-cpu.html) for combinations available.
  ...
  ebs_optimized:
    description:
      - Whether instance is should use optimized EBS volumes, see U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/UserGuide/EBSOptimized.html).
    type: bool
  filters:
    description:
      - A dict of filters to apply when deciding whether existing instances match and should be altered. Each dict item
        ...
        U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeInstances.html).
  ...
  instance_role:
    description:
      - The ARN or name of an EC2-enabled instance role to be used. If a name is not provided in arn format
        ...
        U(https://docs.jctanner.cloud_amazon.aws.amazon.com/IAM/latest/APIReference/API_ListInstanceProfiles.html) If no full ARN is provided,
...
import ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 as jctanner.cloud_amazon.ec2_utils
...
def manage_tags(match, new_tags, purge_tags, jctanner.cloud_amazon.ec2):
    changed = False
    old_tags = boto3_tag_list_to_ansible_dict(match['Tags'])
    tags_to_set, tags_to_delete = compare_jctanner.cloud_amazon.aws_tags(
        old_tags, new_tags,
    ...
    if tags_to_delete:
        ...
        jctanner.cloud_amazon.ec2.delete_tags(
            Resources=[match['InstanceId']],
...
def build_volume_spec(params):
    ...
    return [jctanner.cloud_amazon.ec2_utils.snake_dict_to_camel_dict(v, capitalize_first=True) for v in volumes]
...
def add_or_update_instance_profile(instance, desired_profile_name):
    ...
    if instance_profile_setting and desired_profile_name:
        ...
        try:
            association = jctanner.cloud_amazon.ec2.describe_iam_instance_profile_associations(Filters=[{'Name': 'instance-id', 'Values': [instance['InstanceId']]}])
        except botocore.exceptions.ClientError as e:
            # check for InvalidAssociationID.NotFound
            module.fail_json_jctanner.cloud_amazon.aws(e, "Could not find instance profile association")
        try:
            resp = jctanner.cloud_amazon.ec2.replace_iam_instance_profile_association(
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, "Could not associate instance profile")

    if not instance_profile_setting and desired_profile_name:
        # create association
        jctanner.cloud_amazon.ec2 = module.client('jctanner.cloud_amazon.ec2')
        try:
            resp = jctanner.cloud_amazon.ec2.associate_iam_instance_profile(
                IamInstanceProfile={'Arn': determine_iam_role(desired_profile_name)},
                InstanceId=instance['InstanceId']
            )
            return True
        except botocore.exceptions.ClientError as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, "Could not associate new instance profile")

    return False


def build_network_spec(params, jctanner.cloud_amazon.ec2=None):
    """
    ...
    """
    if jctanner.cloud_amazon.ec2 is None:
        jctanner.cloud_amazon.ec2 = module.client('jctanner.cloud_amazon.ec2')
    ...
    if not network.get('interfaces'):
        ...
        if params.get('vpc_subnet_id'):
            ...
            default_vpc = get_default_vpc(jctanner.cloud_amazon.ec2)
            if default_vpc is None:
                ...
                sub = get_default_subnet(jctanner.cloud_amazon.ec2, default_vpc)
...
def discover_security_groups(group, groups, parent_vpc_id=None, subnet_id=None, jctanner.cloud_amazon.ec2=None):
    if jctanner.cloud_amazon.ec2 is None:
        jctanner.cloud_amazon.ec2 = module.client('jctanner.cloud_amazon.ec2')

    if subnet_id is not None:
        try:
            sub = jctanner.cloud_amazon.ec2.describe_subnets(SubnetIds=[subnet_id])
        except botocore.exceptions.ClientError as e:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error while searching for subnet {0} parent VPC.".format(subnet_id))
    ...
    for f_set in (id_filters, name_filters):
        if len(f_set) > 1:
            found_groups.extend(jctanner.cloud_amazon.ec2.get_paginator(
...
def build_run_instance_spec(params, jctanner.cloud_amazon.ec2=None):
    if jctanner.cloud_amazon.ec2 is None:
        jctanner.cloud_amazon.ec2 = module.client('jctanner.cloud_amazon.ec2')
    ...
    # network parameters
    spec['NetworkInterfaces'] = build_network_spec(params, jctanner.cloud_amazon.ec2)
...
def await_instances(ids, state='OK'):
    ...
    waiter = module.client('jctanner.cloud_amazon.ec2').get_waiter(state_opts[state])
    try:
        waiter.wait(
            InstanceIds=ids,
            WaiterConfig={
...
def diff_instance_and_params(instance, params, jctanner.cloud_amazon.ec2=None, skip=None):
    ...
    for mapping in param_mappings:
        if params.get(mapping.param_key) is not None and mapping.instance_key not in skip:
            value = jctanner.cloud_amazon.ec2.describe_instance_attribute(Attribute=mapping.attribute_name, InstanceId=id_)
...
def change_network_attachments(instance, params, jctanner.cloud_amazon.ec2):
    if (params.get('network') or {}).get('interfaces') is not None:
        ...
        for eni_id in to_attach:
            jctanner.cloud_amazon.ec2.attach_network_interface(
...
def find_instances(jctanner.cloud_amazon.ec2, ids=None, filters=None):
    paginator = jctanner.cloud_amazon.ec2.get_paginator('describe_instances')
    if ids:
...
@AWSRetry.jittered_backoff()
def get_default_vpc(jctanner.cloud_amazon.ec2):
    vpcs = jctanner.cloud_amazon.ec2.describe_vpcs(Filters=ansible_dict_to_boto3_filter_list({'isDefault': 'true'}))
...
@AWSRetry.jittered_backoff()
def get_default_subnet(jctanner.cloud_amazon.ec2, vpc, availability_zone=None):
    subnets = jctanner.cloud_amazon.ec2.describe_subnets(
...
def ensure_instance_state(state, jctanner.cloud_amazon.ec2=None):
    if jctanner.cloud_amazon.ec2 is None:
        module.client('jctanner.cloud_amazon.ec2')
...
@AWSRetry.jittered_backoff()
def change_instance_state(filters, desired_state, jctanner.cloud_amazon.ec2=None):
    """Takes STOPPED/RUNNING/TERMINATED"""
    if jctanner.cloud_amazon.ec2 is None:
        jctanner.cloud_amazon.ec2 = module.client('jctanner.cloud_amazon.ec2')

    changed = set()
    instances = find_instances(jctanner.cloud_amazon.ec2, filters=filters)
    ...
    for inst in instances:
        try:
            if desired_state == 'TERMINATED':
                ...
                # https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/APIReference/Run_Instance_Idempotency.html
                resp = jctanner.cloud_amazon.ec2.terminate_instances(InstanceIds=[inst['InstanceId']])
            ...
            if desired_state == 'STOPPED':
                ...
                resp = jctanner.cloud_amazon.ec2.stop_instances(InstanceIds=[inst['InstanceId']])
            ...
            if desired_state == 'RUNNING':
                ...
                resp = jctanner.cloud_amazon.ec2.start_instances(InstanceIds=[inst['InstanceId']])
    ...
    instances = find_instances(jctanner.cloud_amazon.ec2, ids=list(i['InstanceId'] for i in instances))
...
def handle_existing(existing_matches, changed, jctanner.cloud_amazon.ec2, state):
    ...
    for c in changes:
        jctanner.cloud_amazon.ec2.modify_instance_attribute(**c)
    changed |= bool(changes)
    changed |= add_or_update_instance_profile(existing_matches[0], module.params.get('instance_role'))
    changed |= change_network_attachments(existing_matches[0], module.params, jctanner.cloud_amazon.ec2)
    altered = find_instances(jctanner.cloud_amazon.ec2, ids=[i['InstanceId'] for i in existing_matches])
    module.exit_json(
...
def ensure_present(existing_matches, changed, jctanner.cloud_amazon.ec2, state):
    if len(existing_matches):
        try:
            handle_existing(existing_matches, changed, jctanner.cloud_amazon.ec2, state)
    ...
    try:
        ...
        instance_response = run_instances(jctanner.cloud_amazon.ec2, **instance_spec)
        ...
        for ins in instances:
            ...
            for c in changes:
                try:
                    AWSRetry.jittered_backoff()(jctanner.cloud_amazon.ec2.modify_instance_attribute)(**c)
                except botocore.exceptions.ClientError as e:
                    module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not apply change {0} to new instance.".format(str(c)))
        ...
        instances = jctanner.cloud_amazon.ec2.get_paginator('describe_instances').paginate(
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to create new EC2 instance")


@AWSRetry.jittered_backoff()
def run_instances(jctanner.cloud_amazon.ec2, **instance_spec):
    try:
        return jctanner.cloud_amazon.ec2.run_instances(**instance_spec)
    except botocore.exceptions.ClientError as e:
        if e.response['Error']['Code'] == 'InvalidParameterValue' and "Invalid IAM Instance Profile ARN" in e.response['Error']['Message']:
            # If the instance profile has just been created, it takes some time to be visible by jctanner.cloud_amazon.ec2
...
def main():
        ...
        if isinstance(module.params.get('instance_ids'), string_types):
            ...
            if not module.params.get('vpc_subnet_id'):
                if module.params.get('network'):
                    ...
                    sub = get_default_subnet(jctanner.cloud_amazon.ec2, get_default_vpc(jctanner.cloud_amazon.ec2), availability_zone=module.params.get('availability_zone'))
    ...
    existing_matches = find_instances(jctanner.cloud_amazon.ec2, filters=module.params.get('filters'))
    changed = False
    ...
    if state not in ('terminated', 'absent') and existing_matches:
        for match in existing_matches:
            ...
            if name:
                tags['Name'] = name
            changed |= manage_tags(match, tags, module.params.get('purge_tags', False), jctanner.cloud_amazon.ec2)

    if state in ('present', 'running', 'started'):
        ensure_present(existing_matches=existing_matches, changed=changed, jctanner.cloud_amazon.ec2=jctanner.cloud_amazon.ec2, state=state)
    elif state in ('restarted', 'rebooted', 'stopped', 'absent', 'terminated'):
        if existing_matches:
            ensure_instance_state(state, jctanner.cloud_amazon.ec2)
        else:
            module.exit_json(
