...
DOCUMENTATION = """
---
module: jctanner.cloud_amazon.ec2_elb
...
description:
  - This module de-registers or registers an AWS EC2 instance from the ELBs
    that it belongs to.
  - Returns fact "jctanner.cloud_amazon.ec2_elbs" which is a list of elbs attached to the instance
...
options:
  ...
  instance_id:
    description:
      - EC2 Instance ID
    required: true
  jctanner.cloud_amazon.ec2_elbs:
    description:
      - List of ELB names, required for registration. The jctanner.cloud_amazon.ec2_elbs fact should be used if there was a previous de-register.
...
pre_tasks:
  - name: Gathering jctanner.cloud_amazon.ec2 facts
    action: jctanner.cloud_amazon.ec2_facts
  - name: Instance De-register
    local_action:
      module: jctanner.cloud_amazon.ec2_elb
      instance_id: "{{ ansible_jctanner.cloud_amazon.ec2_instance_id }}"
      state: absent
roles:
  - myrole
post_tasks:
  - name: Instance Register
    local_action:
      module: jctanner.cloud_amazon.ec2_elb
      instance_id: "{{ ansible_jctanner.cloud_amazon.ec2_instance_id }}"
      jctanner.cloud_amazon.ec2_elbs: "{{ item }}"
      state: present
    loop: "{{ jctanner.cloud_amazon.ec2_elbs }}"
"""
...
try:
    ...
    import boto.jctanner.cloud_amazon.ec2.autoscale
    import boto.jctanner.cloud_amazon.ec2.elb
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import (AnsibleAWSError, HAS_BOTO, connect_to_jctanner.cloud_amazon.aws, jctanner.cloud_amazon.ec2_argument_spec,
...
class ElbManager:
    ...
    def __init__(self, module, instance_id=None, jctanner.cloud_amazon.ec2_elbs=None,
                 region=None, **jctanner.cloud_amazon.aws_connect_params):
        self.module = module
        self.instance_id = instance_id
        self.region = region
        self.jctanner.cloud_amazon.aws_connect_params = jctanner.cloud_amazon.aws_connect_params
        self.lbs = self._get_instance_lbs(jctanner.cloud_amazon.ec2_elbs)
        self.changed = False
    ...
    def _get_instance_lbs(self, jctanner.cloud_amazon.ec2_elbs=None):
        """Returns a list of ELBs attached to self.instance_id
        jctanner.cloud_amazon.ec2_elbs: an optional list of elb names that will be used
                  for elb lookup instead of returning what elbs
                  are attached to self.instance_id"""

        if not jctanner.cloud_amazon.ec2_elbs:
            jctanner.cloud_amazon.ec2_elbs = self._get_auto_scaling_group_lbs()

        try:
            elb = connect_to_jctanner.cloud_amazon.aws(boto.jctanner.cloud_amazon.ec2.elb, self.region, **self.jctanner.cloud_amazon.aws_connect_params)
        ...
        if jctanner.cloud_amazon.ec2_elbs:
            lbs = sorted(lb for lb in elbs if lb.name in jctanner.cloud_amazon.ec2_elbs)
        else:
            lbs = []
            for lb in elbs:
    ...
    def _get_auto_scaling_group_lbs(self):
        ...
        try:
            asg = connect_to_jctanner.cloud_amazon.aws(boto.jctanner.cloud_amazon.ec2.autoscale, self.region, **self.jctanner.cloud_amazon.aws_connect_params)
        except (boto.exception.NoAuthHandlerFound, AnsibleAWSError) as e:
            self.module.fail_json(msg=str(e))
    ...
    def _get_instance(self):
        """Returns a boto.jctanner.cloud_amazon.ec2.InstanceObject for self.instance_id"""
        try:
            jctanner.cloud_amazon.ec2 = connect_to_jctanner.cloud_amazon.aws(boto.jctanner.cloud_amazon.ec2, self.region, **self.jctanner.cloud_amazon.aws_connect_params)
        except (boto.exception.NoAuthHandlerFound, AnsibleAWSError) as e:
            self.module.fail_json(msg=str(e))
        return jctanner.cloud_amazon.ec2.get_only_instances(instance_ids=[self.instance_id])[0]
...
def main():
    ...
    argument_spec.update(dict(
        ...
        jctanner.cloud_amazon.ec2_elbs={'default': None, 'required': False, 'type': 'list'},
    ...
    jctanner.cloud_amazon.ec2_elbs = module.params['jctanner.cloud_amazon.ec2_elbs']
    wait = module.params['wait']
    enable_availability_zone = module.params['enable_availability_zone']
    timeout = module.params['wait_timeout']

    if module.params['state'] == 'present' and 'jctanner.cloud_amazon.ec2_elbs' not in module.params:
        module.fail_json(msg="ELBs are required for registration")

    instance_id = module.params['instance_id']
    elb_man = ElbManager(module, instance_id, jctanner.cloud_amazon.ec2_elbs, region=region, **jctanner.cloud_amazon.aws_connect_params)

    if jctanner.cloud_amazon.ec2_elbs is not None:
        for elb in jctanner.cloud_amazon.ec2_elbs:
    ...
    ansible_facts = {'jctanner.cloud_amazon.ec2_elbs': [lb.name for lb in elb_man.lbs]}
    jctanner.cloud_amazon.ec2_facts_result = dict(changed=elb_man.changed, ansible_facts=ansible_facts)

    module.exit_json(**jctanner.cloud_amazon.ec2_facts_result)
