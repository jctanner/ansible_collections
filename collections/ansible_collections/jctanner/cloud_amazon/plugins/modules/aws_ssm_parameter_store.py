...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.aws_ssm_parameter_store
short_description: Manage key-value pairs in jctanner.cloud_amazon.aws parameter store.
description:
  - Manage key-value pairs in jctanner.cloud_amazon.aws parameter store.
version_added: "2.5"
options:
  name:
    description:
  ...
  key_id:
    description:
      - jctanner.cloud_amazon.aws KMS key to decrypt the secrets.
    required: false
    default: jctanner.cloud_amazon.aws/ssm (this key is automatically generated at the first parameter created).
  overwrite_value:
    description:
...
EXAMPLES = '''
- name: Create or update key/value pair in jctanner.cloud_amazon.aws parameter store
  jctanner.cloud_amazon.aws_ssm_parameter_store:
    name: "Hello"
...
- name: Create or update secure key/value pair with default kms key (jctanner.cloud_amazon.aws/ssm)
  jctanner.cloud_amazon.aws_ssm_parameter_store:
    name: "Hello"
    description: "This is your first key"
...
- name: recommend to use with jctanner.cloud_amazon.aws_ssm lookup plugin
  debug: msg="{{ lookup('jctanner.cloud_amazon.aws_ssm', 'hello') }}"
'''
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import boto3_conn, get_jctanner.cloud_amazon.aws_connection_info
...
def update_parameter(client, module, args):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="setting parameter")
        ...
        if (module.params.get('overwrite_value') == 'always'):
            ...
            if args.get('Description'):
                ...
                try:
                    ...
                    module.fail_json_jctanner.cloud_amazon.aws(e, msg="getting description value")
...
def delete_parameter(client, module):
    ...
    try:
        ...
        if e.response['Error']['Code'] == 'ParameterNotFound':
            return False, {}
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="deleting parameter")
...
def setup_client(module):
    ...
    connection = boto3_conn(module, conn_type='client', resource='ssm', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_params)
    return connection
...
def setup_module_object():
    argument_spec = dict(
        ...
        key_id=dict(default="alias/jctanner.cloud_amazon.aws/ssm"),
