...
- name: Encrypt Kinesis Stream test-stream.
  kinesis_stream:
    ...
    encryption_type: KMS
    key_id: alias/jctanner.cloud_amazon.aws/kinesis
    wait: yes
    wait_timeout: 600
  register: test_stream
...
stream_arn:
  ...
  returned: when state == present.
  type: str
  sample: "arn:jctanner.cloud_amazon.aws:kinesis:east-side:123456789:stream/test-stream"
stream_status:
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import HAS_BOTO3, boto3_conn, jctanner.cloud_amazon.ec2_argument_spec, get_jctanner.cloud_amazon.aws_connection_info
...
def make_tags_in_jctanner.cloud_amazon.aws_format(tags):
    """Take a dictionary of tags and convert them into the AWS Tags format.
    Args:
...
def get_tags(client, stream_name, check_mode=False):
    ...
    Kwargs:
        check_mode (bool): This will pass DryRun as one of the parameters to the jctanner.cloud_amazon.aws api.
            default=False
    ...
    try:
        if not check_mode:
            ...
            results = {
                ...
                'StreamName': stream_name,
                'StreamARN': 'arn:jctanner.cloud_amazon.aws:kinesis:east-side:123456789:stream/{0}'.format(stream_name),
                'StreamStatus': 'ACTIVE',
                'EncryptionType': 'NONE'
            }
        success = True
...
def tags_action(client, stream_name, tags, action='create', check_mode=False):
    ...
    Basic Usage:
        >>> client = boto3.client('jctanner.cloud_amazon.ec2')
...
def update_tags(client, stream_name, tags, check_mode=False):
    ...
    if current_tags:
        tags = make_tags_in_jctanner.cloud_amazon.aws_format(tags)
        current_tags_set = (
            set(
                reduce(
                    lambda x, y: x + y,
...
def stream_encryption_action(client, stream_name, action='start_encryption', encryption_type='', key_id='',
    ...
    Basic Usage:
        ...
        >>> stream_name = 'test-stream'
        >>> shard_count = 20
        >>> stream_action(client, stream_name, shard_count, action='create', encryption_type='KMS',key_id='alias/jctanner.cloud_amazon.aws')
...
def update(client, current_stream, stream_name, number_of_shards=1, retention_period=None,
    ...
    Basic Usage:
        ...
        >>> current_stream = {
            ...
            'StreamARN': 'arn:jctanner.cloud_amazon.aws:kinesis:us-west-2:123456789:stream/test-stream',
            'StreamStatus': "ACTIVE'
        }
        >>> stream_name = 'test-stream'
        >>> retention_period = 48
...
def start_stream_encryption(client, stream_name, encryption_type='', key_id='',
    ...
    Basic Usage:
        >>> client = boto3.client('kinesis')
        >>> stream_name = 'test-stream'
        >>> key_id = 'alias/jctanner.cloud_amazon.aws'
        >>> encryption_type = 'KMS'
...
def main():
    ...
    try:
        region, jctanner.cloud_amazon.ec2_url, jctanner.cloud_amazon.aws_connect_kwargs = (
            get_jctanner.cloud_amazon.aws_connection_info(module, boto3=True)
        )
        client = (
            boto3_conn(
