  ...
  duration_seconds:
    description:
      - The duration, in seconds, of the session token.
        See U(https://docs.jctanner.cloud_amazon.aws.amazon.com/STS/latest/APIReference/API_GetSessionToken.html#API_GetSessionToken_RequestParameters)
...
# Get a session token (more details: https://docs.jctanner.cloud_amazon.aws.amazon.com/STS/latest/APIReference/API_GetSessionToken.html)
sts_session_token:
...
jctanner.cloud_amazon.ec2_tag:
  jctanner.cloud_amazon.aws_access_key: "{{ session_credentials.sts_creds.access_key }}"
  jctanner.cloud_amazon.aws_secret_key: "{{ session_credentials.sts_creds.secret_key }}"
...
def main():
    ...
    if region:
        connection = boto3_conn(module, conn_type='client', resource='sts', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
    else:
