...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.aws_config_aggregator
...
options:
  ...
  account_sources:
    ...
    suboptions:
      ...
      jctanner.cloud_amazon.aws_regions:
        description:
        - A list of source regions being aggregated.
      all_jctanner.cloud_amazon.aws_regions:
        description:
...
- name: Create cross-account aggregator
  jctanner.cloud_amazon.aws_config_aggregator:
    ...
    account_sources:
      ...
      - 0123456789
      - 9012345678
      all_jctanner.cloud_amazon.aws_regions: yes
'''
...
def create_resource(client, module, params, result):
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't create AWS Config configuration aggregator")
...
def delete_resource(client, module, resource_type, params, result):
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't delete AWS Config configuration aggregator")
...
def main():
    ...
    if module.params.get('account_sources'):
        ...
        for i in module.params.get('account_sources'):
            ...
            if i.get('jctanner.cloud_amazon.aws_regions'):
                tmp_dict['AwsRegions'] = i.get('jctanner.cloud_amazon.aws_regions')
            if i.get('all_jctanner.cloud_amazon.aws_regions') is not None:
                tmp_dict['AllAwsRegions'] = i.get('all_jctanner.cloud_amazon.aws_regions')
    ...
    if module.params.get('organization_source'):
        ...
        if module.params.get('organization_source').get('jctanner.cloud_amazon.aws_regions'):
            params['OrganizationAggregationSource'].update({
                'AwsRegions': module.params.get('organization_source').get('jctanner.cloud_amazon.aws_regions')
            })
        if module.params.get('organization_source').get('all_jctanner.cloud_amazon.aws_regions') is not None:
            params['OrganizationAggregationSourcep'].update({
                'AllAwsRegions': module.params.get('organization_source').get('all_jctanner.cloud_amazon.aws_regions')
            })
