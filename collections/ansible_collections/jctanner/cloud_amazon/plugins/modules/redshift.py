...
options:
  ...
  availability_zone:
    ...
    aliases: ['zone', 'jctanner.cloud_amazon.aws_zone']
  number_of_nodes:
    description:
...
cluster:
    ...
    contains:
        ...
        url:
            ...
            returned: success
            type: str
            sample: "new-redshift_cluster.jfkdjfdkj.us-east-1.redshift.amazonjctanner.cloud_amazon.aws.com"
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import jctanner.cloud_amazon.ec2_argument_spec, snake_dict_to_camel_dict
...
def create_cluster(module, redshift):
    ...
    try:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to create cluster")
    except (botocore.exceptions.BotoCoreError, botocore.exceptions.ClientError) as e:  # pylint: disable=duplicate-except
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to describe cluster")
    if wait:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Timeout waiting for the cluster creation")
    try:
...
def describe_cluster(module, redshift):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error describing cluster")
...
def delete_cluster(module, redshift):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to delete cluster")
    ...
    if wait:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Timeout deleting the cluster")
...
def modify_cluster(module, redshift):
    ...
    if module.params.get('enhanced_vpc_routing') is not None:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't modify redshift cluster %s " % identifier)
    if wait:
        attempts = wait_timeout // 60
        waiter = redshift.get_waiter('cluster_available')
        try:
    ...
    if wait:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Timeout waiting for cluster modification")
    try:
        resource = redshift.describe_clusters(ClusterIdentifier=identifier)['Clusters'][0]
    except (botocore.exceptions.BotoCoreError, botocore.exceptions.ClientError) as e:
...
def main():
    ...
    argument_spec.update(dict(
        ...
        availability_zone=dict(aliases=['jctanner.cloud_amazon.aws_zone', 'zone']),
