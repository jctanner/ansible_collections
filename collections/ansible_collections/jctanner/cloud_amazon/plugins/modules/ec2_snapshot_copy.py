...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_snapshot_copy
...
EXAMPLES = '''
# Basic Snapshot Copy
- jctanner.cloud_amazon.ec2_snapshot_copy:
...
- jctanner.cloud_amazon.ec2_snapshot_copy:
    ...
    source_snapshot_id: snap-xxxxxxx
    encrypted: yes
    kms_key_id: arn:jctanner.cloud_amazon.aws:kms:eu-central-1:XXXXXXXXXXXX:key/746de6ea-50a4-4bcb-8fbc-e3b29f2d367b
'''
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import (boto3_conn, jctanner.cloud_amazon.ec2_argument_spec, get_jctanner.cloud_amazon.aws_connection_info, camel_dict_to_snake_dict)
...
def copy_snapshot(module, jctanner.cloud_amazon.ec2):
    """
    Copies an EC2 Snapshot to another region

    module : AnsibleModule object
    jctanner.cloud_amazon.ec2: jctanner.cloud_amazon.ec2 connection object
    """
    ...
    try:
        snapshot_id = jctanner.cloud_amazon.ec2.copy_snapshot(**params)['SnapshotId']
        if module.params.get('wait'):
            ...
            jctanner.cloud_amazon.ec2.get_waiter('snapshot_completed').wait(
                SnapshotIds=[snapshot_id],
                WaiterConfig=dict(Delay=delay, MaxAttempts=max_attempts)
            )
        if module.params.get('tags'):
            jctanner.cloud_amazon.ec2.create_tags(
...
def main():
    ...
    client = boto3_conn(module, conn_type='client', resource='jctanner.cloud_amazon.ec2',
