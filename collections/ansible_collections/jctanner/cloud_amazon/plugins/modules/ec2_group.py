...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_group
author: "Andrew de Quincey (@adq)"
version_added: "1.3"
requirements: [ boto3 ]
short_description: maintain an jctanner.cloud_amazon.ec2 VPC security group.
description:
    - maintains jctanner.cloud_amazon.ec2 security groups. This module has a dependency on python-boto >= 2.5
options:
  name:
    description:
...
- name: example using security group rule descriptions
  jctanner.cloud_amazon.ec2_group:
    ...
    vpc_id: vpc-xxxxxxxx
    profile: "{{ jctanner.cloud_amazon.aws_profile }}"
    region: us-east-1
    rules:
      - proto: tcp
        ports:
...
- name: example jctanner.cloud_amazon.ec2 group
  jctanner.cloud_amazon.ec2_group:
    name: example
    description: an example EC2 group
    vpc_id: 12345
    region: eu-west-1
    jctanner.cloud_amazon.aws_secret_key: SECRET
    jctanner.cloud_amazon.aws_access_key: ACCESS
    rules:
      - proto: tcp
        from_port: 80
        to_port: 80
...
- name: example2 jctanner.cloud_amazon.ec2 group
  jctanner.cloud_amazon.ec2_group:
    name: example2
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.aws.iam import get_jctanner.cloud_amazon.aws_account_id
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.aws.waiters import get_waiter
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import AWSRetry, camel_dict_to_snake_dict, compare_jctanner.cloud_amazon.aws_tags
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import ansible_dict_to_boto3_filter_list, boto3_tag_list_to_ansible_dict, ansible_dict_to_boto3_tag_list
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.common.network import to_ipv6_subnet, to_subnet
...
def update_rules_description(module, client, rule_type, group_id, ip_permissions):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to update rule description for group %s" % group_id)
...
def revoke(client, module, ip_permissions, group_id, rule_type):
    if not module.check_mode:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, "Unable to revoke {0}: {1}".format(rules, ip_permissions))
...
def authorize(client, module, ip_permissions, group_id, rule_type):
    if not module.check_mode:
        try:
            ...
            rules = 'ingress rules' if rule_type == 'in' else 'egress rules'
            module.fail_json_jctanner.cloud_amazon.aws(e, "Unable to authorize {0}: {1}".format(rules, ip_permissions))
...
def update_tags(client, module, group_id, current_tags, tags, purge_tags):
    tags_need_modify, tags_to_delete = compare_jctanner.cloud_amazon.aws_tags(current_tags, tags, purge_tags)
    ...
    if not module.check_mode:
        if tags_to_delete:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to delete tags {0}".format(tags_to_delete))
...
def update_rule_descriptions(module, group_id, present_ingress, named_tuple_ingress_list, present_egress, named_tuple_egress_list):
    changed = False
    client = module.client('jctanner.cloud_amazon.ec2')
...
def create_security_group(client, module, name, description, vpc_id):
    if not module.check_mode:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to create security group")
    ...
    def await_rules(group, desired_rules, purge, rule_key):
        for i in range(tries):
            ...
            if purge and len(current_rules ^ set(desired_rules)) == 0:
                ...
                return group
            sleep(10)
            group = get_security_groups_with_backoff(module.client('jctanner.cloud_amazon.ec2'), GroupIds=[group_id])['SecurityGroups'][0]
...
def group_exists(client, module, vpc_id, group_id, name):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error in describe_security_groups")
...
def get_diff_final_resource(client, module, security_group):
    ...
    def get_final_tags(security_group_tags, specified_tags, purge_tags):
        ...
        tags_need_modify, tags_to_delete = compare_jctanner.cloud_amazon.aws_tags(security_group_tags, specified_tags, purge_tags)
...
def main():
    ...
    current_account_id = get_jctanner.cloud_amazon.aws_account_id(module)
    ...
    if state == 'absent':
        if group:
            ...
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to delete security group '%s'" % group)
            else:
                group = None
                changed = True
        else:
