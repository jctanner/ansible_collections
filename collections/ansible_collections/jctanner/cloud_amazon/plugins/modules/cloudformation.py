...
options:
  ...
  stack_policy:
    description:
      - the path of the cloudformation stack policy. A policy cannot be removed once placed, but it can be modified.
        for instance, allow all updates U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSCloudFormation/latest/UserGuide/protect-stack-resources.html#d0e9051)
    version_added: "1.9"
  tags:
    description:
  ...
  create_changeset:
    description:
      - "If stack already exists create a changeset instead of directly applying changes.
        See the AWS Change Sets docs U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSCloudFormation/latest/UserGuide/using-cfn-updating-stacks-changesets.html).
  ...
  role_arn:
    ...
    - The role that AWS CloudFormation assumes to create the stack. See the AWS CloudFormation Service Role
      docs U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSCloudFormation/latest/UserGuide/using-iam-servicerole.html)
    version_added: "2.3"
...
- name: create a stack, specify role that cloudformation assumes
  cloudformation:
    ...
    role_arn: 'arn:jctanner.cloud_amazon.aws:iam::123456789012:role/cloudformation-iam-role'
...
- name: create a stack, pass in the template via an URL
  cloudformation:
    ...
    region: us-east-1
    disable_rollback: true
    template_url: https://s3.amazonjctanner.cloud_amazon.aws.com/my-bucket/cloudformation.template
    template_parameters:
      KeyName: jmartin
      DiskType: ephemeral
      InstanceType: m1.small
...
- cloudformation:
    ...
    template_parameters:
      DBSnapshotIdentifier:
        ...
        value: arn:jctanner.cloud_amazon.aws:rds:es-east-1:000000000000:snapshot:rds:my-db-snapshot
      DBName:
        use_previous_value: True
    tags:
      Stack: "ansible-cloudformation"
...
import ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2
# import a class, otherwise we'll use a fully qualified path
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import AWSRetry, boto_exception
    ...
    try:
        ...
        if changeset_name in pending_changesets:
            ...
            result['warnings'] = ['Created changeset named %s for stack %s' % (changeset_name, stack_params['StackName']),
                                  'You can execute it using: jctanner.cloud_amazon.aws cloudformation execute-change-set --change-set-name %s' % cs['Id'],
...
def stack_operation(cfn, stack_name, operation, events_limit, op_token=None):
    ...
    while True:
        ...
        # Possible states: https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSCloudFormation/latest/UserGuide/using-cfn-describing-stacks.html#w1ab2c15c17c21c13
...
def main():
    ...
    if isinstance(module.params.get('tags'), dict):
        stack_params['Tags'] = ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2.ansible_dict_to_boto3_tag_list(module.params['tags'])
    ...
    try:
        region, jctanner.cloud_amazon.ec2_url, jctanner.cloud_amazon.aws_connect_kwargs = ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2.get_jctanner.cloud_amazon.aws_connection_info(module, boto3=True)
        cfn = ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2.boto3_conn(module, conn_type='client', resource='cloudformation', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
