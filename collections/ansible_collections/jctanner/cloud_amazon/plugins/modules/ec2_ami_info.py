...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_ami_info
version_added: '2.5'
short_description: Gather information about jctanner.cloud_amazon.ec2 AMIs
description:
  - Gather information about jctanner.cloud_amazon.ec2 AMIs
  - This module was called C(jctanner.cloud_amazon.ec2_ami_facts) before Ansible 2.9. The usage did not change.
author:
...
options:
  ...
  filters:
    description:
      ...
      - See U(https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeImages.html) for possible filters.
      - Filter names and values are case sensitive.
  owners:
    description:
      - Filter the images by the owner. Valid options are an AWS account ID, self,
      - or an AWS owner alias ( amazon | jctanner.cloud_amazon.aws-marketplace | microsoft ).
    aliases: [owner]
  executable_users:
    description:
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import (boto3_conn, jctanner.cloud_amazon.ec2_argument_spec, get_jctanner.cloud_amazon.aws_connection_info, ansible_dict_to_boto3_filter_list,
                                      camel_dict_to_snake_dict, boto3_tag_list_to_ansible_dict)


def list_jctanner.cloud_amazon.ec2_images(jctanner.cloud_amazon.ec2_client, module):
    ...
    for owner in owners:
        if owner.isdigit():
            ...
            # self not a valid owner-alias filter (https://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/APIReference/API_DescribeImages.html)
    ...
    try:
        images = jctanner.cloud_amazon.ec2_client.describe_images(ImageIds=image_ids, Filters=filters, Owners=owner_param, ExecutableUsers=executable_users)
        images = [camel_dict_to_snake_dict(image) for image in images["Images"]]
    except (ClientError, BotoCoreError) as err:
        module.fail_json_jctanner.cloud_amazon.aws(err, msg="error describing images")
    for image in images:
        try:
            image['tags'] = boto3_tag_list_to_ansible_dict(image.get('tags', []))
            if module.params.get("describe_image_attributes"):
                launch_permissions = jctanner.cloud_amazon.ec2_client.describe_image_attribute(Attribute='launchPermission', ImageId=image['image_id'])['LaunchPermissions']
...
def main():
    ...
    if module._module._name == 'jctanner.cloud_amazon.ec2_ami_facts':
        module._module.deprecate("The 'jctanner.cloud_amazon.ec2_ami_facts' module has been renamed to 'jctanner.cloud_amazon.ec2_ami_info'", version='2.13')
    ...
    list_jctanner.cloud_amazon.ec2_images(jctanner.cloud_amazon.ec2_client, module)
