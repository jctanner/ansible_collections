...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_customer_gateway
...
- jctanner.cloud_amazon.ec2_customer_gateway:
    bgp_asn: 12345
    ip_address: 1.2.3.4
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import (boto3_conn, AWSRetry, camel_dict_to_snake_dict,
...
class Ec2CustomerGatewayManager:
    ...
    def __init__(self, module):
        ...
        try:
            ...
            self.jctanner.cloud_amazon.ec2 = boto3_conn(module, conn_type='client', resource='jctanner.cloud_amazon.ec2', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
    ...
    def ensure_cgw_absent(self, gw_id):
        response = self.jctanner.cloud_amazon.ec2.delete_customer_gateway(
            DryRun=False,
    ...
    def ensure_cgw_present(self, bgp_asn, ip_address):
        if not bgp_asn:
            bgp_asn = 65000
        response = self.jctanner.cloud_amazon.ec2.create_customer_gateway(
            DryRun=False,
            Type='ipsec.1',
            PublicIp=ip_address,
            BgpAsn=bgp_asn,
    ...
    def tag_cgw_name(self, gw_id, name):
        response = self.jctanner.cloud_amazon.ec2.create_tags(
            DryRun=False,
            Resources=[
                gw_id,
            ],
    ...
    def describe_gateways(self, ip_address):
        response = self.jctanner.cloud_amazon.ec2.describe_customer_gateways(
            DryRun=False,
            Filters=[
                {
                    'Name': 'state',
