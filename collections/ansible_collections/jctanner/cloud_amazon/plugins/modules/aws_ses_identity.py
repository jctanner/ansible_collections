...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.aws_ses_identity
...
- name: Ensure example@example.com email identity exists
  jctanner.cloud_amazon.aws_ses_identity:
...
identity_arn:
    ...
    returned: success
    type: str
    sample: arn:jctanner.cloud_amazon.aws:ses:us-east-1:12345678:identity/example@example.com
...
notification_attributes:
    ...
    type: complex
    sample: {
        "bounce_topic": "arn:jctanner.cloud_amazon.aws:sns:....",
        "complaint_topic": "arn:jctanner.cloud_amazon.aws:sns:....",
        "delivery_topic": "arn:jctanner.cloud_amazon.aws:sns:....",
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import camel_dict_to_snake_dict, AWSRetry, get_jctanner.cloud_amazon.aws_connection_info
...
def get_verification_attributes(connection, module, identity, retries=0, retryDelay=10):
    ...
    for attempt in range(0, retries + 1):
        try:
            response = connection.get_identity_verification_attributes(Identities=[identity], jctanner.cloud_amazon.aws_retry=True)
        except (BotoCoreError, ClientError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Failed to retrieve identity verification attributes for {identity}'.format(identity=identity))
...
def get_identity_notifications(connection, module, identity, retries=0, retryDelay=10):
    ...
    for attempt in range(0, retries + 1):
        try:
            response = connection.get_identity_notification_attributes(Identities=[identity], jctanner.cloud_amazon.aws_retry=True)
        except (BotoCoreError, ClientError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Failed to retrieve identity notification attributes for {identity}'.format(identity=identity))
    ...
    if current != required:
        try:
            if not module.check_mode:
                connection.set_identity_notification_topic(Identity=identity, NotificationType=notification_type, SnsTopic=required, jctanner.cloud_amazon.aws_retry=True)
        except (BotoCoreError, ClientError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Failed to set identity notification topic for {identity} {notification_type}'.format(
                identity=identity,
...
def update_notification_topic_headers(connection, module, identity, identity_notifications, notification_type):
    ...
    if current != required:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Failed to set identity headers in notification for {identity} {notification_type}'.format(
                identity=identity,
                notification_type=notification_type,
            ))
        return True
...
def update_feedback_forwarding(connection, module, identity, identity_notifications):
    ...
    if current != required:
        try:
            if not module.check_mode:
                connection.set_identity_feedback_forwarding_enabled(Identity=identity, ForwardingEnabled=required, jctanner.cloud_amazon.aws_retry=True)
        except (BotoCoreError, ClientError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Failed to set identity feedback forwarding for {identity}'.format(identity=identity))
        return True
    return False
...
def create_or_update_identity(connection, module, region, account_id):
    ...
    if verification_attributes is None:
        try:
            if not module.check_mode:
                if '@' in identity:
                    connection.verify_email_identity(EmailAddress=identity, jctanner.cloud_amazon.aws_retry=True)
                else:
                    connection.verify_domain_identity(Domain=identity, jctanner.cloud_amazon.aws_retry=True)
        except (BotoCoreError, ClientError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Failed to verify identity {identity}'.format(identity=identity))
        if module.check_mode:
            verification_attributes = {
    ...
    identity_arn = 'arn:jctanner.cloud_amazon.aws:ses:' + region + ':' + account_id + ':identity/' + identity
...
def destroy_identity(connection, module):
    ...
    if verification_attributes is not None:
        try:
            if not module.check_mode:
                connection.delete_identity(Identity=identity, jctanner.cloud_amazon.aws_retry=True)
        except (BotoCoreError, ClientError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Failed to delete identity {identity}'.format(identity=identity))
        changed = True
...
def get_account_id(module):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg='Failed to retrieve caller identity')
