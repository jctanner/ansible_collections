...
arn:
    ...
    returned: success
    type: str
    sample: "arn:jctanner.cloud_amazon.aws:iam::911277865346:server-certificate/server-cert-name"
path:
...
def get_server_certs(iam, name=None):
    ...
    Basic Usage:
        ...
        >>> results = get_server_certs(iam, name)
        {
            ...
            "path": "/",
            "arn": "arn:jctanner.cloud_amazon.aws:iam::911277865346:server-certificate/server-cert-name"
        }
    """
    results = dict()
    try:
...
def main():
    ...
    try:
        ...
        iam = boto3_conn(module, conn_type='client', resource='iam', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
