...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_launch_template
version_added: "2.8"
...
description:
  ...
  - The I(jctanner.cloud_amazon.ec2_instance) and I(jctanner.cloud_amazon.ec2_asg) modules can, instead of specifying all
...
options:
  ...
  cpu_options:
    ...
    - For more information, see U(http://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/UserGuide/instance-optimize-cpu.html)
    suboptions:
      core_count:
  ...
  elastic_gpu_specifications:
    description: Settings for Elastic GPU attachments. See U(https://jctanner.cloud_amazon.aws.amazon.com/jctanner.cloud_amazon.ec2/elastic-gpus/) for details.
    suboptions:
      type:
  ...
  instance_type:
    description: >
      ...
      http://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/UserGuide/instance-types.html
  kernel_id:
    description: >
      The ID of the kernel. We recommend that you use PV-GRUB instead of
      kernels and RAM disks. For more information, see
      U(http://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/UserGuide/UserProvidedkernels.html)
  key_name:
    description:
  ...
  tags:
    ...
    - "Tag key constraints: Tag keys are case-sensitive and accept a maximum of 127 Unicode characters. May not begin with I(jctanner.cloud_amazon.aws:)"
  ...
  user_data:
    description: >
      ...
      U(http://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/UserGuide/user-data.html) and Windows
      U(http://docs.jctanner.cloud_amazon.aws.amazon.com/AWSEC2/latest/WindowsGuide/jctanner.cloud_amazon.ec2-instance-metadata.html#instancedata-add-user-data)
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.aws.core import AnsibleAWSModule, is_boto3_error_code, get_boto3_client_method_parameters
from ansible.module_utils.common.dict_transformations import camel_dict_to_snake_dict, snake_dict_to_camel_dict
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import ansible_dict_to_boto3_tag_list, AWSRetry, boto3_tag_list_to_ansible_dict, ansible_dict_to_boto3_tag_list
...
def determine_iam_role(module, name_or_arn):
    if re.match(r'^arn:jctanner.cloud_amazon.aws:iam::\d+:instance-profile/[\w+=/,.@-]+$', name_or_arn):
        return name_or_arn
    iam = module.client('iam', retry_decorator=AWSRetry.jittered_backoff())
    try:
        role = iam.get_instance_profile(InstanceProfileName=name_or_arn, jctanner.cloud_amazon.aws_retry=True)
        return {'arn': role['InstanceProfile']['Arn']}
    except is_boto3_error_code('NoSuchEntity') as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not find instance_role {0}".format(name_or_arn))
    except (BotoCoreError, ClientError) as e:  # pylint: disable=duplicate-except
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="An error occurred while searching for instance_role {0}. Please try supplying the full ARN.".format(name_or_arn))


def existing_templates(module):
    jctanner.cloud_amazon.ec2 = module.client('jctanner.cloud_amazon.ec2', retry_decorator=AWSRetry.jittered_backoff())
    matches = None
    try:
        if module.params.get('template_id'):
            matches = jctanner.cloud_amazon.ec2.describe_launch_templates(LaunchTemplateIds=[module.params.get('template_id')])
        elif module.params.get('template_name'):
            matches = jctanner.cloud_amazon.ec2.describe_launch_templates(LaunchTemplateNames=[module.params.get('template_name')])
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg='Launch template with ID {0} is not a valid ID. It should start with `lt-....`'.format(
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not check existing launch templates. This may be an IAM permission problem.')
    else:
        ...
        try:
            return template, jctanner.cloud_amazon.ec2.describe_launch_template_versions(LaunchTemplateId=template_id)['LaunchTemplateVersions']
        except (ClientError, BotoCoreError, WaiterError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not find launch template versions for {0} (ID: {1}).'.format(template['LaunchTemplateName'], template_id))
    ...
    if template or template_versions:
        ...
        if non_default_versions:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not delete existing versions of the launch template {0}".format(template['LaunchTemplateId']))
        try:
            resp = jctanner.cloud_amazon.ec2.delete_launch_template(
                LaunchTemplateId=template['LaunchTemplateId'],
            )
        except (ClientError, BotoCoreError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not delete launch template {0}".format(template['LaunchTemplateId']))
        return {
...
def create_or_update(module, template_options):
    ...
    if not (template or template_versions):
        ...
        try:
            resp = jctanner.cloud_amazon.ec2.create_launch_template(
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't create launch template")
        template, template_versions = existing_templates(module)
        out['changed'] = True
        ...
        if lt_data == most_recent['LaunchTemplateData']:
            ...
            return out
        try:
            resp = jctanner.cloud_amazon.ec2.create_launch_template_version(
                LaunchTemplateId=template['LaunchTemplateId'],
                LaunchTemplateData=lt_data,
                ClientToken=uuid4().hex,
                jctanner.cloud_amazon.aws_retry=True,
            ...
            if module.params.get('default_version') in (None, ''):
                ...
                set_default = jctanner.cloud_amazon.ec2.modify_launch_template(
                    LaunchTemplateId=template['LaunchTemplateId'],
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't create subsequent launch template version")
        template, template_versions = existing_templates(module)
        out['changed'] = True
    return out
...
def main():
    ...
    if not module.boto3_at_least('1.6.0'):
        module.fail_json(msg="jctanner.cloud_amazon.ec2_launch_template requires boto3 >= 1.6.0")
