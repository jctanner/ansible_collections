...
- hosts: localhost
  ...
  - name: DynamoDB stream event mapping
    lambda_event:
      ...
      alias: Dev
      source_params:
        source_arn: arn:jctanner.cloud_amazon.aws:dynamodb:us-east-1:123456789012:table/tableName/stream/2016-03-19T19:51:37.457
        enabled: True
        batch_size: 100
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import (HAS_BOTO3, boto3_conn, camel_dict_to_snake_dict, jctanner.cloud_amazon.ec2_argument_spec,
...
class AWSConnection:
    ...
    def __init__(self, ansible_obj, resources, use_boto3=True):
        ...
        try:
            self.region, self.endpoint, jctanner.cloud_amazon.aws_connect_kwargs = get_jctanner.cloud_amazon.aws_connection_info(ansible_obj, boto3=use_boto3)
            ...
            for resource in resources:
                jctanner.cloud_amazon.aws_connect_kwargs.update(dict(region=self.region,
                                               endpoint=self.endpoint,
                                               conn_type='client',
                                               resource=resource
                                               ))
                self.resource_client[resource] = boto3_conn(ansible_obj, **jctanner.cloud_amazon.aws_connect_kwargs)
...
def validate_params(module, jctanner.cloud_amazon.aws):
    ...
    if len(function_name) > 64 and not function_name.startswith('arn:jctanner.cloud_amazon.aws:lambda:'):
        module.fail_json(msg='Function name "{0}" exceeds 64 character limit'.format(function_name))

    elif len(function_name) > 140 and function_name.startswith('arn:jctanner.cloud_amazon.aws:lambda:'):
    ...
    if not module.params['lambda_function_arn'].startswith('arn:jctanner.cloud_amazon.aws:lambda:'):
        function_name = module.params['lambda_function_arn']
        module.params['lambda_function_arn'] = 'arn:jctanner.cloud_amazon.aws:lambda:{0}:{1}:function:{2}'.format(jctanner.cloud_amazon.aws.region, jctanner.cloud_amazon.aws.account_id, function_name)
...
def lambda_event_stream(module, jctanner.cloud_amazon.aws):
    """
    ...
    client = jctanner.cloud_amazon.aws.client('lambda')
    facts = dict()
    changed = False
...
def main():
    ...
    jctanner.cloud_amazon.aws = AWSConnection(module, ['lambda'])

    validate_params(module, jctanner.cloud_amazon.aws)

    if module.params['event_source'].lower() in ('stream', 'sqs'):
        results = lambda_event_stream(module, jctanner.cloud_amazon.aws)
    else:
