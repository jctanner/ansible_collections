...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.aws_ses_identity_policy
...
- name: add sending authorization policy to domain identity
  jctanner.cloud_amazon.aws_ses_identity_policy:
...
- name: add sending authorization policy to identity using ARN
  jctanner.cloud_amazon.aws_ses_identity_policy:
    identity: "arn:jctanner.cloud_amazon.aws:ses:us-east-1:12345678:identity/example.com"
    policy_name: ExamplePolicy
    policy: "{{ lookup('template', 'policy.json.j2') }}"
    state: present
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import compare_policies, AWSRetry
...
def get_identity_policy(connection, module, identity, policy_name):
    try:
        response = connection.get_identity_policies(Identity=identity, PolicyNames=[policy_name], jctanner.cloud_amazon.aws_retry=True)
    except (BotoCoreError, ClientError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg='Failed to retrieve identity policy {policy}'.format(policy=policy_name))
...
def create_or_update_identity_policy(connection, module):
    ...
    if compare_policies(policy_dict, required_policy_dict):
        ...
        try:
            if not module.check_mode:
                connection.put_identity_policy(Identity=identity, PolicyName=policy_name, Policy=required_policy, jctanner.cloud_amazon.aws_retry=True)
        except (BotoCoreError, ClientError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Failed to put identity policy {policy}'.format(policy=policy_name))
    ...
    try:
        policies_present = connection.list_identity_policies(Identity=identity, jctanner.cloud_amazon.aws_retry=True)['PolicyNames']
    except (BotoCoreError, ClientError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg='Failed to list identity policies')
...
def delete_identity_policy(connection, module):
    ...
    if policy_name in policies_present:
        try:
            if not module.check_mode:
                connection.delete_identity_policy(Identity=identity, PolicyName=policy_name, jctanner.cloud_amazon.aws_retry=True)
        except (BotoCoreError, ClientError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Failed to delete identity policy {policy}'.format(policy=policy_name))
        changed = True
        policies_present = list(policies_present)
