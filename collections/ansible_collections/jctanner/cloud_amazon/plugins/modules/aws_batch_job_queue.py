...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.aws_batch_job_queue
...
output:
  ...
  sample:
    ...
    changed: false
    response:
      job_queue_arn: "arn:jctanner.cloud_amazon.aws:batch:...."
      job_queue_name: <name>
      priority: 1
      state: DISABLED
      status: UPDATING
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.aws.batch import AWSConnection, cc, set_api_params
from ansible.module_utils.basic import AnsibleModule
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import jctanner.cloud_amazon.ec2_argument_spec, get_jctanner.cloud_amazon.aws_connection_info, boto3_conn, HAS_BOTO3
...
def create_job_queue(module, jctanner.cloud_amazon.aws):
    """
...
def remove_job_queue(module, jctanner.cloud_amazon.aws):
    """
...
def manage_state(module, jctanner.cloud_amazon.aws):
    ...
    current_job_queue = get_current_job_queue(module, jctanner.cloud_amazon.aws)
    ...
    if state == 'present':
        if current_state == 'present':
            ...
            if updates:
                try:
                    if not check_mode:
                        jctanner.cloud_amazon.aws.client().update_job_queue(**job_kwargs)
                    changed = True
            ...
            # Create Job Queue
            changed = create_job_queue(module, jctanner.cloud_amazon.aws)
            action_taken = 'added'

        # Describe job queue
        response = get_current_job_queue(module, jctanner.cloud_amazon.aws)
        if not response:
            module.fail_json(msg='Unable to get job queue information after creating/updating')
    else:
        if current_state == 'present':
            # remove the Job Queue
            changed = remove_job_queue(module, jctanner.cloud_amazon.aws)
