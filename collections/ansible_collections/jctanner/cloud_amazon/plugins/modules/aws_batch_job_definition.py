...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.aws_batch_job_definition
...
output:
  ...
  sample:
    ...
    changed: false
    response:
      job_definition_arn: "arn:jctanner.cloud_amazon.aws:batch:...."
...
def create_job_definition(module, jctanner.cloud_amazon.aws):
    """
...
def remove_job_definition(module, jctanner.cloud_amazon.aws):
    """
...
def manage_state(module, jctanner.cloud_amazon.aws):
    ...
    current_job_definition = get_current_job_definition(module, jctanner.cloud_amazon.aws)
    ...
    if state == 'present':
        if current_state == 'present':
            ...
            if not job_definition_equal(module, current_job_definition):
                create_job_definition(module, jctanner.cloud_amazon.aws)
                action_taken = 'updated with new version'
                changed = True
        else:
            # Create Job definition
            changed = create_job_definition(module, jctanner.cloud_amazon.aws)
            action_taken = 'added'

        response = get_current_job_definition(module, jctanner.cloud_amazon.aws)
        if not response:
            module.fail_json(msg='Unable to get job definition information after creating/updating')
    else:
        if current_state == 'present':
            # remove the Job definition
            changed = remove_job_definition(module, jctanner.cloud_amazon.aws)
