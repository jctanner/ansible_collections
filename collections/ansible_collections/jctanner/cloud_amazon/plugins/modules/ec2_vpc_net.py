...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_vpc_net
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.network.common.utils import to_subnet
...
def vpc_exists(module, vpc, name, cidr_block, multi):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to describe VPCs")
...
def get_vpc(module, connection, vpc_id):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to wait for VPC {0} to be available.".format(vpc_id))
...
def update_vpc_tags(connection, module, vpc_id, tags, name):
    ...
    try:
        ...
        tags_to_update, dummy = compare_jctanner.cloud_amazon.aws_tags(current_tags, tags, False)
        if tags_to_update:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to update tags")
    ...
    if vpc_obj['DhcpOptionsId'] != dhcp_id:
        if not module.check_mode:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Failed to associate DhcpOptionsId {0}".format(dhcp_id))
...
def create_vpc(connection, module, cidr_block, tenancy):
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, "Failed to create the VPC")

    # wait for vpc to exist
    try:
        connection.get_waiter('vpc_exists').wait(VpcIds=[vpc_obj['Vpc']['VpcId']])
    except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to wait for VPC {0} to be created.".format(vpc_obj['Vpc']['VpcId']))
    ...
    if state == 'present':
        ...
        if purge_cidrs:
            for association_id in to_remove:
                ...
                try:
                    ...
                    module.fail_json_jctanner.cloud_amazon.aws(e, "Unable to disassociate {0}. You must detach or delete all gateways and resources that "
        ...
        if dhcp_id is not None:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, "Failed to update DHCP options")
        ...
        current_dns_enabled = connection.describe_vpc_attribute(Attribute='enableDnsSupport', VpcId=vpc_id, jctanner.cloud_amazon.aws_retry=True)['EnableDnsSupport']['Value']
        current_dns_hostnames = connection.describe_vpc_attribute(Attribute='enableDnsHostnames', VpcId=vpc_id, jctanner.cloud_amazon.aws_retry=True)['EnableDnsHostnames']['Value']
        if current_dns_enabled != dns_support:
            ...
            if not module.check_mode:
                try:
                    ...
                    module.fail_json_jctanner.cloud_amazon.aws(e, "Failed to update enabled dns support attribute")
        if current_dns_hostnames != dns_hostnames:
            ...
            if not module.check_mode:
                try:
                    ...
                    module.fail_json_jctanner.cloud_amazon.aws(e, "Failed to update enabled dns hostnames attribute")
        ...
        if to_add or to_remove:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, "Failed to wait for CIDRs to update")
