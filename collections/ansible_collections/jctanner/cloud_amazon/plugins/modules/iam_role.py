...
options:
  path:
    description:
      - The path to the role. For more information about paths, see U(https://docs.jctanner.cloud_amazon.aws.amazon.com/IAM/latest/UserGuide/reference_identifiers.html).
    default: "/"
  name:
    description:
  ...
  boundary:
    description:
      ...
      - For more information on boundaries, see U(https://docs.jctanner.cloud_amazon.aws.amazon.com/IAM/latest/UserGuide/access_policies_boundaries.html)
...
- name: "Create a role and attach a managed policy called 'PowerUserAccess'"
  iam_role:
    ...
    assume_role_policy_document: "{{ lookup('file','policy.json') }}"
    managed_policy:
      - arn:jctanner.cloud_amazon.aws:iam::jctanner.cloud_amazon.aws:policy/PowerUserAccess
...
iam_role:
    ...
    contains:
        ...
        arn:
            ...
            type: str
            returned: always
            sample: "arn:jctanner.cloud_amazon.aws:iam::1234567890:role/mynewrole"
        create_date:
                        ...
                        'statement': [
                            {
                                ...
                                'effect': 'Allow',
                                'principal': {
                                    'service': 'jctanner.cloud_amazon.ec2.amazonjctanner.cloud_amazon.aws.com'
                                },
                                'sid': ''
                            }
                        ],
        ...
        attached_policies:
            ...
            sample: [
                {
                    'policy_arn': 'arn:jctanner.cloud_amazon.aws:iam::jctanner.cloud_amazon.aws:policy/PowerUserAccess',
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import camel_dict_to_snake_dict, jctanner.cloud_amazon.ec2_argument_spec, get_jctanner.cloud_amazon.aws_connection_info, boto3_conn, compare_policies
        ...
        if module.params.get('boundary') is None:
            ...
            if (role.get('PermissionsBoundary') or {}).get('PermissionsBoundaryArn'):
                try:
                    ...
                    module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to remove permission boundary for role {0}: {1}".format(params['RoleName'], to_native(e)))
            ...
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Unable to update permission boundary for role {0}: {1}".format(params['RoleName'], to_native(e)))
    ...
    if role:
        ...
        if role.get('PermissionsBoundary') is not None:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not delete role permission boundary on role {0}: {1}".format(params['RoleName'], e))
