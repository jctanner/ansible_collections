...
DOCUMENTATION = """
---
module: jctanner.cloud_amazon.aws_ses_rule_set
short_description: Manages SES inbound receipt rule sets
description:
    - The M(jctanner.cloud_amazon.aws_ses_rule_set) module allows you to create, delete, and manage SES receipt rule sets
version_added: 2.8
author:
...
- name: Create default rule set and activate it if not already
  jctanner.cloud_amazon.aws_ses_rule_set:
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import camel_dict_to_snake_dict, AWSRetry
...
def list_rule_sets(client, module):
    try:
        response = client.list_receipt_rule_sets(jctanner.cloud_amazon.aws_retry=True)
    except (BotoCoreError, ClientError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't list rule sets.")
...
def ruleset_active(client, module, name):
    try:
        active_rule_set = client.describe_active_receipt_rule_set(jctanner.cloud_amazon.aws_retry=True)
    except (BotoCoreError, ClientError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get the active rule set.")
...
def deactivate_rule_set(client, module):
    try:
        ...
        client.set_active_receipt_rule_set(jctanner.cloud_amazon.aws_retry=True)
    except (BotoCoreError, ClientError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't set active rule set to None.")
...
def update_active_rule_set(client, module, name, desired_active):
    ...
    if desired_active is not None:
        if desired_active and not active:
            if not check_mode:
                try:
                    client.set_active_receipt_rule_set(RuleSetName=name, jctanner.cloud_amazon.aws_retry=True)
                except (BotoCoreError, ClientError) as e:
                    module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't set active rule set to {0}.".format(name))
            changed = True
            active = True
...
def create_or_update_rule_set(client, module):
    ...
    if not rule_set_in(name, rule_sets):
        if not check_mode:
            try:
                client.create_receipt_rule_set(RuleSetName=name, jctanner.cloud_amazon.aws_retry=True)
            except (BotoCoreError, ClientError) as e:
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't create rule set {0}.".format(name))
        changed = True
    ...
    if rule_set_in(name, rule_sets):
        ...
        if not check_mode:
            if active and module.params.get('force'):
                deactivate_rule_set(client, module)
            try:
                client.delete_receipt_rule_set(RuleSetName=name, jctanner.cloud_amazon.aws_retry=True)
            except (BotoCoreError, ClientError) as e:
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't delete rule set {0}.".format(name))
        changed = True
