  ...
  region:
    description:
     - >
       ...
       followed by the jctanner.cloud_amazon.aws_region and jctanner.cloud_amazon.ec2_region settings in the Boto config file.  If none of those are set the region defaults to the
...
def main():
    ...
    if region:
        client_connection = boto3_conn(module, conn_type='client', resource='s3', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_params)
        resource_connection = boto3_conn(module, conn_type='resource', resource='s3', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_params)
    else:
