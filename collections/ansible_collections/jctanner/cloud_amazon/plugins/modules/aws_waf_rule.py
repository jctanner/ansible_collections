...
DOCUMENTATION = '''
module: jctanner.cloud_amazon.aws_waf_rule
...
options:
    ...
    conditions:
        description: >
          ...
          the name of the existing condition. M(jctanner.cloud_amazon.aws_waf_condition) can be used to
...
EXAMPLES = '''
  ...
  - name: create WAF rule
    jctanner.cloud_amazon.aws_waf_rule:
      name: my_waf_rule
      conditions:
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.aws.waf import run_func_with_change_token_backoff, list_rules_with_backoff, list_regional_rules_with_backoff, MATCH_LOOKUP
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.aws.waf import get_web_acl_with_backoff, list_web_acls_with_backoff, list_regional_web_acls_with_backoff
...
def get_rule(client, module, rule_id):
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not get WAF rule')
...
def list_rules(client, module):
    if client.__class__.__name__ == 'WAF':
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not list WAF rules')
    elif client.__class__.__name__ == 'WAFRegional':
        try:
            return list_regional_rules_with_backoff(client)
        except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not list WAF Regional rules')
...
def find_and_update_rule(client, module, rule_id):
    ...
    for condition_type in MATCH_LOOKUP:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not list %s conditions' % condition_type)
    ...
    if changed:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not update rule conditions')
...
def remove_rule_conditions(client, module, rule_id):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not remove rule conditions')
...
def ensure_rule_present(client, module):
    ...
    if rule_id:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not create rule')
...
def find_rule_in_web_acls(client, module, rule_id):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not list Web ACLs')
    for web_acl in all_web_acls:
        try:
            web_acl_details = get_web_acl_with_backoff(client, web_acl['WebACLId'])
        except (botocore.exceptions.ClientError, botocore.exceptions.BotoCoreError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not get Web ACL details')
...
def ensure_rule_absent(client, module):
    ...
    if rule_id:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg='Could not delete rule')
    return False, {}
