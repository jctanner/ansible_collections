...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import AnsibleAWSError, jctanner.cloud_amazon.ec2_argument_spec, get_jctanner.cloud_amazon.aws_connection_info
...
def main():
    ...
    if region in ('us-east-1', '', None):
        ...
        location = region
    try:
        connection = boto.s3.connect_to_region(location, is_secure=True, calling_format=OrdinaryCallingFormat(), **jctanner.cloud_amazon.aws_connect_params)
        # use this as fallback because connect_to_region seems to fail in boto + non 'classic' jctanner.cloud_amazon.aws accounts in some cases
        if connection is None:
            connection = boto.connect_s3(**jctanner.cloud_amazon.aws_connect_params)
