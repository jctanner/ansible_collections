...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_ami
version_added: "1.3"
short_description: create or destroy an image in jctanner.cloud_amazon.ec2
description:
     - Registers or deregisters jctanner.cloud_amazon.ec2 images.
options:
  instance_id:
    description:
  ...
  billing_products:
    description:
      - A list of valid billing codes. To be used with valid accounts by jctanner.cloud_amazon.aws marketplace vendors.
    version_added: "2.5"
  ramdisk_id:
    description:
...
import time
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import get_jctanner.cloud_amazon.aws_connection_info, jctanner.cloud_amazon.ec2_argument_spec, boto3_conn, camel_dict_to_snake_dict
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import ansible_dict_to_boto3_tag_list, boto3_tag_list_to_ansible_dict, compare_jctanner.cloud_amazon.aws_tags
...
def deregister_image(module, connection):
    ...
    if 'ImageId' in image:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error deregistering image")
    else:
...
def get_image_by_id(module, connection, image_id):
    try:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error retrieving image %s" % image_id)
        ...
        if no_images == 1:
            ...
            try:
                ...
                if e.response['Error']['Code'] != 'InvalidAMIID.Unavailable':
                    module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error retrieving image attributes for image %s" % image_id)
            except botocore.exceptions.BotoCoreError as e:
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error retrieving image attributes for image %s" % image_id)
            return result
        module.fail_json(msg="Invalid number of instances (%s) found for image_id: %s." % (str(len(images)), image_id))
    except (botocore.exceptions.BotoCoreError, botocore.exceptions.ClientError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error retrieving image by image_id")
...
def main():
    ...
    try:
        ...
        connection = boto3_conn(module, conn_type='client', resource='jctanner.cloud_amazon.ec2', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
