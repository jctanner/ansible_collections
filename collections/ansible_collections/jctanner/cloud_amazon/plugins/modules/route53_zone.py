...
def find_zones(module, client, zone_in, private_zone):
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not list current hosted zones")
    zones = []
...
def create_or_update_private(module, client, matching_zones, record):
    for z in matching_zones:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not get details about hosted zone %s" % z['Id'])
            ...
            if 'Comment' in zone_details['Config'] and zone_details['Config']['Comment'] != record['comment']:
                if not module.check_mode:
                    try:
                        ...
                        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not update comment for hosted zone %s" % zone_details['Id'])
                return True, record
            else:
    ...
    if not module.check_mode:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not create hosted zone")
...
def create_or_update_public(module, client, matching_zones, record):
    ...
    for matching_zone in matching_zones:
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not get details about hosted zone %s" % matching_zone['Id'])
        if 'Comment' in zone_details['Config'] and zone_details['Config']['Comment'] != record['comment']:
            if not module.check_mode:
                try:
            ...
            if vpc_details['VPC']['VPCId'] == vpc_id and vpc_region == vpc_details['VPC']['VPCRegion']:
                if not module.check_mode:
                    try:
                        ...
                        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not delete hosted zone %s" % z['Id'])
    ...
    if len(matching_zones) > 1:
        ...
        if not module.check_mode:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not get delete hosted zone %s" % matching_zones[0]['Id'])
        changed = True
    ...
    if hosted_zone_id == "all":
        ...
        if not module.check_mode:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, msg="Could not delete hosted zone %s" % hosted_zone_id)
        changed = True
