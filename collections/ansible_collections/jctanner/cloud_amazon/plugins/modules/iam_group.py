...
- iam_group:
    name: testgroup1
    managed_policy:
      - arn:jctanner.cloud_amazon.aws:iam::jctanner.cloud_amazon.aws:policy/AmazonSNSFullAccess
    state: present
...
group:
    ...
    contains:
        arn:
            ...
            type: str
            sample: "arn:jctanner.cloud_amazon.aws:iam::1234567890:group/testgroup1"
        create_date:
...
users:
    ...
    contains:
        arn:
            ...
            type: str
            sample: "arn:jctanner.cloud_amazon.aws:iam::1234567890:user/test_user1"
        create_date:
...
def create_or_update_group(connection, module):
    ...
    try:
        ...
        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't get group")
    ...
    if group is None:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't create group")
            ...
            for policy_arn in list(set(current_attached_policies_arn_list) - set(managed_policies)):
                ...
                if not module.check_mode:
                    try:
                        ...
                        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't detach policy from group %s" % params['GroupName'])
            ...
            if managed_policies != [None] and not module.check_mode:
                for policy_arn in managed_policies:
                    try:
                        ...
                        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't attach policy to group %s" % params['GroupName'])

    # Manage group memberships
    try:
        current_group_members = get_group(connection, module, params['GroupName'])['Users']
    except (BotoCoreError, ClientError) as e:
        module.fail_json_jctanner.cloud_amazon.aws(e, "Couldn't get group %s" % params['GroupName'])
            ...
            for user in list(set(current_group_members_list) - set(users)):
                ...
                if not module.check_mode:
                    try:
                        ...
                        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't remove user %s from group %s" % (user, params['GroupName']))
            ...
            if users != [None] and not module.check_mode:
                for user in users:
                    try:
                        ...
                        module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't add user %s to group %s" % (user, params['GroupName']))
    if module.check_mode:
...
def destroy_group(connection, module):
    ...
    if group:
        ...
        try:
            ...
            module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't remove policy from group %s" % params['GroupName'])
        ...
        for user in current_group_members_list:
            try:
                ...
                module.fail_json_jctanner.cloud_amazon.aws(e, "Couldn't remove user %s from group %s" % (user, params['GroupName']))

        try:
            connection.delete_group(**params)
        except (BotoCoreError, ClientError) as e:
            module.fail_json_jctanner.cloud_amazon.aws(e, "Couldn't delete group %s" % params['GroupName'])
