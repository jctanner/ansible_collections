...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.ec2_lc

short_description: Create or delete AWS Autoscaling Launch Configurations

description:
  - Can create or delete AWS Autoscaling Configurations
  - Works with the jctanner.cloud_amazon.ec2_asg module to manage Autoscaling Groups
...
options:
  ...
  user_data:
    description:
      - Opaque blob of data which is made available to the jctanner.cloud_amazon.ec2 instance. Mutually exclusive with I(user_data_path).
  user_data_path:
    description:
      - Path to the file that contains userdata for the jctanner.cloud_amazon.ec2 instances. Mutually exclusive with I(user_data).
    version_added: "2.3"
  kernel_id:
    description:
...
arn:
  ...
  type: str
  sample: arn:jctanner.cloud_amazon.aws:autoscaling:us-east-1:148830907657:launchConfiguration:888d9b58-d93a-40c4-90cf-759197a2621a:launchConfigurationName/launch_config_name
changed:
...
import traceback
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import (get_jctanner.cloud_amazon.aws_connection_info, jctanner.cloud_amazon.ec2_argument_spec, jctanner.cloud_amazon.ec2_connect, camel_dict_to_snake_dict, get_jctanner.cloud_amazon.ec2_security_group_ids_from_names,
...
def create_launch_config(connection, module):
    ...
    try:
        ...
        jctanner.cloud_amazon.ec2_connection = boto3_conn(module, 'client', 'jctanner.cloud_amazon.ec2', region, jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
        security_groups = get_jctanner.cloud_amazon.ec2_security_group_ids_from_names(module.params.get('security_groups'), jctanner.cloud_amazon.ec2_connection, vpc_id=vpc_id, boto3=True)
...
def main():
    ...
    try:
        region, jctanner.cloud_amazon.ec2_url, jctanner.cloud_amazon.aws_connect_kwargs = get_jctanner.cloud_amazon.aws_connection_info(module, boto3=True)
        connection = boto3_conn(module, conn_type='client', resource='autoscaling', region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
