...
short_description: create invalidations for jctanner.cloud_amazon.aws cloudfront distributions
description:
...
location:
  ...
  returned: always
  type: str
  sample: https://cloudfront.amazonjctanner.cloud_amazon.aws.com/2017-03-25/distribution/E1ZID6KZJECZY7/invalidation/I2G9MOWJZFV622
'''
...
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import jctanner.cloud_amazon.ec2_argument_spec, boto3_conn
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import snake_dict_to_camel_dict
from ansible_collections.jctanner.cloud_amazon.plugins.module_utils.jctanner.cloud_amazon.ec2 import camel_dict_to_snake_dict
...
class CloudFrontInvalidationServiceManager(object):
    ...
    def create_client(self, resource):
        region, jctanner.cloud_amazon.ec2_url, jctanner.cloud_amazon.aws_connect_kwargs = get_jctanner.cloud_amazon.aws_connection_info(self.module, boto3=True)
        self.client = boto3_conn(self.module, conn_type='client', resource=resource, region=region, endpoint=jctanner.cloud_amazon.ec2_url, **jctanner.cloud_amazon.aws_connect_kwargs)
    ...
    def create_invalidation(self, distribution_id, invalidation_batch):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error creating CloudFront invalidations.")
    ...
    def get_invalidation(self, distribution_id, caller_reference):
        ...
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error listing CloudFront invalidations.")
        ...
        for inv_id in invalidation_ids:
            try:
                ...
                self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error getting Cloudfront invalidation {0}".format(inv_id))
...
class CloudFrontInvalidationValidationManager(object):
    ...
    def validate_distribution_id(self, distribution_id, alias):
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error validating parameters.")

    def create_jctanner.cloud_amazon.aws_list(self, invalidation_batch):
        jctanner.cloud_amazon.aws_list = {}
        jctanner.cloud_amazon.aws_list["Quantity"] = len(invalidation_batch)
        jctanner.cloud_amazon.aws_list["Items"] = invalidation_batch
        return jctanner.cloud_amazon.aws_list
    ...
    def validate_invalidation_batch(self, invalidation_batch, caller_reference):
        try:
            ...
            valid_invalidation_batch = {
                'paths': self.create_jctanner.cloud_amazon.aws_list(invalidation_batch),
                'caller_reference': valid_caller_reference
            }
            return valid_invalidation_batch
        except (ClientError, BotoCoreError) as e:
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Error validating invalidation batch.")
