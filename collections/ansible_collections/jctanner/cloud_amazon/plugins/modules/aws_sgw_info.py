...
DOCUMENTATION = '''
---
module: jctanner.cloud_amazon.aws_sgw_info
short_description: Fetch AWS Storage Gateway information
description:
    - Fetch AWS Storage Gateway information
    - This module was called C(jctanner.cloud_amazon.aws_sgw_facts) before Ansible 2.9. The usage did not change.
version_added: "2.6"
requirements: [ boto3 ]
...
gateways:
  ...
  contains:
    gateway_arn:
      ...
      returned: always
      type: str
      sample: "arn:jctanner.cloud_amazon.aws:storagegateway:eu-west-1:367709993819:gateway/sgw-9999F888"
    gateway_id:
    ...
    file_shares:
      ...
      contains:
        file_share_arn:
          ...
          returned: always
          type: str
          sample: "arn:jctanner.cloud_amazon.aws:storagegateway:eu-west-1:399805793479:share/share-AF999C88"
        file_share_id:
    ...
    tapes:
        ...
        contains:
          tape_arn:
            ...
            returned: always
            type: str
            sample: "arn:jctanner.cloud_amazon.aws:storagegateway:eu-west-1:399805793479:tape/tape-AF999C88"
          tape_barcode:
            description: "Tape ARN"
            returned: always
            type: str
...
- name: "Get AWS storage gateway information"
  jctanner.cloud_amazon.aws_sgw_info:
...
class SGWInformationManager(object):
    ...
    def list_gateways(self):
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't list storage gateways")
    ...
    """
    @staticmethod
    def _read_gateway_fileshare_response(fileshares, jctanner.cloud_amazon.aws_reponse):
        for share in jctanner.cloud_amazon.aws_reponse["FileShareInfoList"]:
            share_obj = camel_dict_to_snake_dict(share)
            if "gateway_arn" in share_obj:
                del share_obj["gateway_arn"]
            fileshares.append(share_obj)

        return jctanner.cloud_amazon.aws_reponse["NextMarker"] if "NextMarker" in jctanner.cloud_amazon.aws_reponse else None
    ...
    def list_gateway_file_shares(self, gateway):
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't list gateway file shares")
    ...
    def list_local_disks(self, gateway):
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't list storage gateway local disks")
    ...
    """
    @staticmethod
    def _read_gateway_tape_response(tapes, jctanner.cloud_amazon.aws_response):
        for tape in jctanner.cloud_amazon.aws_response["TapeInfos"]:
            tape_obj = camel_dict_to_snake_dict(tape)
            if "gateway_arn" in tape_obj:
                del tape_obj["gateway_arn"]
            tapes.append(tape_obj)

        return jctanner.cloud_amazon.aws_response["Marker"] if "Marker" in jctanner.cloud_amazon.aws_response else None
    ...
    def list_gateway_vtl(self, gateway):
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't list storage gateway tapes")
    ...
    def list_gateway_volumes(self, gateway):
        try:
            ...
            self.module.fail_json_jctanner.cloud_amazon.aws(e, msg="Couldn't list storage gateway volumes")
...
def main():
    ...
    if module._name == 'jctanner.cloud_amazon.aws_sgw_facts':
        module.deprecate("The 'jctanner.cloud_amazon.aws_sgw_facts' module has been renamed to 'jctanner.cloud_amazon.aws_sgw_info'", version='2.13')
