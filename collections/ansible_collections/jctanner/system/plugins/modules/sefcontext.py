...
options:
  ...
  ftype:
    ...
    - C(a) for all jctanner.system.files,
    - C(b) for block devices,
    - C(c) for character devices,
    - C(d) for directories,
    - C(f) for regular jctanner.system.files,
    - C(l) for symbolic links,
    - C(p) for named pipes,
    - C(s) for socket jctanner.system.files.
    type: str
  ...
  reload:
    ...
    - Note that this does not apply SELinux file contexts to existing jctanner.system.files.
    type: bool
    default: yes
...
- The M(sefcontext) module does not modify existing jctanner.system.files to the new
  SELinux context(s), so it is advisable to first create the SELinux
  file contexts before creating jctanner.system.files, or run C(restorecon) manually
  for the existing jctanner.system.files that require the new SELinux file contexts.
- Not applying SELinux fcontexts to existing jctanner.system.files is a deliberate
...
EXAMPLES = r'''
- name: Allow apache to modify jctanner.system.files in /srv/git_repos
  sefcontext:
    target: '/srv/git_repos(/.*)?'
    setype: httpd_git_rw_content_t
    state: present

- name: Apply new SELinux file context to jctanner.system.filesystem
