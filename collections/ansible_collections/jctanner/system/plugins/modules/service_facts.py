...
class SystemctlScanService(BaseService):
    ...
    def gather_services(self):
        ...
        rc, stdout, stderr = self.module.run_command("%s list-unit-jctanner.system.files --no-pager --type service --all" % systemctl_path, use_unsafe_shell=True)
        for line in [svc_line for svc_line in stdout.split('\n') if '.service' in svc_line and 'not-found' not in svc_line]:
            try:
                service_name, status_val = line.split()
            except ValueError:
                self.module.fail_json(msg="Malformed output discovered from systemd list-unit-jctanner.system.files: {0}".format(line))
