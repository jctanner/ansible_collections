...
# It's in the SUNWPython-devel package which also contains development jctanner.system.files
...
from ansible_collections.jctanner.system.plugins.module_utils.service import fail_if_missing
...
class Service(object):
    ...
    def service_enable_rcconf(self):
        ...
        if self.changed is True:
            ...
            # Create a temporary file next to the current rc.conf (so we stay on the same jctanner.system.filesystem).
...
class LinuxService(Service):
    ...
    def service_enable(self):
        ...
        if self.enable_cmd.endswith("initctl"):
            ...
            # Check to see if jctanner.system.files contain the manual line in .conf and fail if True
...
class FreeBsdService(Service):
    ...
    def service_enable(self):
        ...
        rcjctanner.system.files = ['/etc/rc.conf', '/etc/rc.conf.local', '/usr/local/etc/rc.conf']
        for rcfile in rcjctanner.system.files:
...
class DragonFlyBsdService(FreeBsdService):
    ...
    def service_enable(self):
        ...
        rcjctanner.system.files = ['/etc/rc.conf']  # Overkill?
        for rcfile in rcjctanner.system.files:
            if os.path.isfile(rcfile):
                self.rcconf_file = rcfile
