...
from ansible_collections.jctanner.system.plugins.module_utils.facts.system.chroot import is_chroot
from ansible_collections.jctanner.system.plugins.module_utils.service import sysv_exists, sysv_is_enabled, fail_if_missing
...
def main():
    ...
    if unit:
        ...
        if request_was_ignored(out) or request_was_ignored(err):
            # fallback list-unit-jctanner.system.files as show does not work on some systems (chroot)
            # not used as primary as it skips some services (like those using init.d) and requires .service/etc notation
            (rc, out, err) = module.run_command("%s list-unit-jctanner.system.files '%s'" % (systemctl, unit))
            if rc == 0:
                is_systemd = True
