...
class NosystemdTimezone(Timezone):
    ...
    conf_jctanner.system.files = dict(
        name=None,  # To be set in __init__
        hwclock=None,  # To be set in __init__
        adjtime='/etc/adjtime'
    )

    # It's fine if all tree config jctanner.system.files don't exist
    allow_no_file = dict(
        name=True,
        hwclock=True,
        adjtime=True
    ...
    def __init__(self, module):
        ...
        if self.module.get_bin_path('dpkg-reconfigure') is not None:
            ...
            self.conf_jctanner.system.files['name'] = '/etc/timezone'
            self.conf_jctanner.system.files['hwclock'] = '/etc/default/rcS'
            ...
            self.conf_jctanner.system.files['name'] = '/etc/sysconfig/clock'
            self.conf_jctanner.system.files['hwclock'] = '/etc/sysconfig/clock'
            try:
                f = open(self.conf_jctanner.system.files['name'], 'r')
                ...
                if self._allow_ioerror(err, 'name'):
                    ...
                    self.abort('could not read configuration file "%s"' % self.conf_jctanner.system.files['name'])
            else:
    ...
    def _get_value_from_config(self, key, phase):
        filename = self.conf_jctanner.system.files[key]
        try:
            ...
            if value == planned:
                ...
                if os.path.islink('/etc/localtime'):
                    ...
                    if os.path.exists('/etc/localtime'):
                        # We use readlink() because on some distros zone jctanner.system.files are symlinks
                        # to other zone jctanner.system.files, so it's hard to get which TZ is actually set
    ...
    def set_timezone(self, value):
        self._edit_file(filename=self.conf_jctanner.system.files['name'],
    ...
    def set_hwclock(self, value):
        if value == 'local':
            ...
            option = '--utc'
            utc = 'yes'
        if self.conf_jctanner.system.files['hwclock'] is not None:
            self._edit_file(filename=self.conf_jctanner.system.files['hwclock'],
...
class BSDTimezone(Timezone):
    ...
    def __get_timezone(self):
        ...
        #   Check all jctanner.system.files in /usr/share/zoneinfo and return first non-link match.
