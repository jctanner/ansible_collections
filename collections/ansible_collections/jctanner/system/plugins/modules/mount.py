...
options:
  path:
    description:
      - Path to the mount point (e.g. C(/mnt/jctanner.system.files)).
  ...
  fstab:
    description:
      ...
      - OpenBSD does not allow specifying alternate fstab jctanner.system.files with mount so do not
        use this on OpenBSD with any state that operates on the live jctanner.system.filesystem.
      - This parameter defaults to /etc/fstab or /etc/vfstab on Solaris.
    type: str
  boot:
    description:
      - Determines if the jctanner.system.filesystem should be mounted on boot.
...
def mount(module, args):
    ...
    if get_platform().lower() == 'openbsd':
        ...
        if module.params['fstab'] is not None:
            module.fail_json(
                msg=(
                    'OpenBSD does not support alternate fstab jctanner.system.files. Do not '
...
def is_bind_mounted(module, linux_mounts, dest, src=None, fstype=None):
    ...
    :kwarg fstype: The jctanner.system.filesystem type. If specified this is also used to
