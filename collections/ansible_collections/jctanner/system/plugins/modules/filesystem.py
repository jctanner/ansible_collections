...
module: jctanner.system.filesystem
short_description: Makes a jctanner.system.filesystem
description:
  - This module creates a jctanner.system.filesystem.
version_added: "1.2"
options:
  fstype:
  ...
  force:
    description:
    - If C(yes), allows to create new jctanner.system.filesystem on devices that already has jctanner.system.filesystem.
    type: bool
    default: 'no'
  resizefs:
    description:
    - If C(yes), if the block device and jctanner.system.filesystem size differ, grow the jctanner.system.filesystem into the space.
    - Supported for C(ext2), C(ext3), C(ext4), C(ext4dev), C(f2fs), C(lvm), C(xfs), C(vfat), C(swap) jctanner.system.filesystems.
...
notes:
  - Potential jctanner.system.filesystem on I(dev) are checked using C(blkid), in case C(blkid) isn't able to detect an existing jctanner.system.filesystem,
    this jctanner.system.filesystem is overwritten even if I(force) is C(no).
'''

EXAMPLES = '''
- name: Create a ext2 jctanner.system.filesystem on /dev/sdb1
  jctanner.system.filesystem:
    fstype: ext2
    dev: /dev/sdb1

- name: Create a ext4 jctanner.system.filesystem on /dev/sdb1 and check disk blocks
  jctanner.system.filesystem:
    fstype: ext4
    dev: /dev/sdb1
    opts: -cc
...
class Filesystem(object):
    ...
    def get_fs_size(self, dev):
        """ Return size in bytes of jctanner.system.filesystem on device. Returns int """
    ...
    def grow(self, dev):
        ...
        try:
            ...
            self.module.fail_json(changed=False, msg="module does not support resizing %s jctanner.system.filesystem yet." % self.fstype)

        if not fssize_in_bytes < devsize_in_bytes:
            self.module.exit_json(changed=False, msg="%s jctanner.system.filesystem is using the whole device %s" % (self.fstype, dev))
        elif self.module.check_mode:
            self.module.exit_json(changed=True, msg="Resizing jctanner.system.filesystem %s on device %s" % (self.fstype, dev))
        else:
...
class F2fs(Filesystem):
    ...
    def MKFS_FORCE_FLAGS(self):
        ...
        if match is not None:
            # Since 1.9.0, mkfs.f2fs check overwrite before make jctanner.system.filesystem
...
class VFAT(Filesystem):
    ...
    def get_fs_size(self, dev):
        ...
        self.module.fail_json(msg="fatresize failed to provide jctanner.system.filesystem size for %s" % dev)
...
def main():
    ...
    # There is no "single command" to manipulate jctanner.system.filesystems, so we map them all out and their options
    ...
    try:
        ...
        module.fail_json(changed=False, msg="module does not support this jctanner.system.filesystem (%s) yet." % fstype)
    ...
    # In case blkid isn't able to identify an existing jctanner.system.filesystem, device is considered as empty,
    # then this existing jctanner.system.filesystem would be overwritten even if force isn't enabled.
    fs = raw_fs.strip()

    jctanner.system.filesystem = klass(module)
    ...
    if same_fs and not resizefs and not force:
        ...
        if not jctanner.system.filesystem.GROW:
            module.fail_json(changed=False, msg="module does not support resizing %s jctanner.system.filesystem yet." % fstype)

        out = jctanner.system.filesystem.grow(dev)
    ...
    # create fs
    jctanner.system.filesystem.create(opts, dev)
    changed = True
