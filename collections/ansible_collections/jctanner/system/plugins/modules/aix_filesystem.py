...
module: aix_jctanner.system.filesystem
...
description:
  - This module creates, removes, mount and unmount LVM and NFS file system for
    AIX using C(/etc/jctanner.system.filesystems).
...
options:
  ...
  attributes:
    description:
      - Specifies attributes for jctanner.system.files system separated by comma.
    type: list
...
EXAMPLES = r'''
- name: Create jctanner.system.filesystem in a previously defined logical volume.
  aix_jctanner.system.filesystem:
    device: testlv
    jctanner.system.filesystem: /testfs
    state: present

- name: Creating NFS jctanner.system.filesystem from nfshost.
  aix_jctanner.system.filesystem:
    device: /home/ftp
    nfs_server: nfshost
...
- name: Resizing /mksysb to +512M.
  aix_jctanner.system.filesystem:
    jctanner.system.filesystem: /mksysb
    size: +512M
    state: present
...
RETURN = r'''
changed:
  description: Return changed for aix_jctanner.system.filesystems actions as true or false.
  returned: always
  type: bool
msg:
...
from ansible_collections.jctanner.system.plugins.module_utils.ismount import ismount
import re


def _fs_exists(module, jctanner.system.filesystem):
    """
    Check if file system already exists on /etc/jctanner.system.filesystems.

    :param module: Ansible module.
    :param jctanner.system.filesystem: jctanner.system.filesystem name.
    :return: True or False.
    """
    lsfs_cmd = module.get_bin_path('lsfs', True)
    rc, lsfs_out, err = module.run_command("%s -l %s" % (lsfs_cmd, jctanner.system.filesystem))
    if rc == 1:
...
def resize_fs(module, jctanner.system.filesystem, size):
    """ Resize LVM file system. """

    chfs_cmd = module.get_bin_path('chfs', True)
    if not module.check_mode:
        rc, chfs_out, err = module.run_command('%s -a size="%s" %s' % (chfs_cmd, size, jctanner.system.filesystem))
        ...
        if rc == 28:
            ...
            if re.findall('The jctanner.system.filesystem size is already', chfs_out):
                changed = False
            else:
                changed = True
...
def create_fs(
        module, fs_type, jctanner.system.filesystem, vg, device, size, mount_group, auto_mount,
    ...
    if nfs_server is not None:
        ...
        if not module.check_mode:
            rc, mknfsmnt_out, err = module.run_command('%s -f "%s" %s -h "%s" -t "%s" "%s" -w "bg"' % (
                mknfsmnt_cmd, jctanner.system.filesystem, device, nfs_server, permissions, auto_mount))
            if rc != 0:
                module.fail_json(msg="Failed to run mknfsmnt.", rc=rc, err=err)
            else:
                changed = True
                msg = "NFS file system %s created." % jctanner.system.filesystem
        ...
        if not module.check_mode:
            rc, crfs_out, err = module.run_command(
                "%s -v %s -m %s %s %s %s %s %s -p %s %s -a %s" % (
                    crfs_cmd, fs_type, jctanner.system.filesystem, vg, device, mount_group, auto_mount, account_subsystem, permissions, size, attributes))
...
def remove_fs(module, jctanner.system.filesystem, rm_mount_point):
    ...
    if not module.check_mode:
        rc, rmfs_out, err = module.run_command("%s -r %s %s" % (rmfs_cmd, rm_mount_point, jctanner.system.filesystem))
        if rc != 0:
            ...
            msg = rmfs_out
            if not rmfs_out:
                msg = "File system %s removed." % jctanner.system.filesystem
...
def mount_fs(module, jctanner.system.filesystem):
    """ Mount a file system. """
    mount_cmd = module.get_bin_path('mount', True)

    if not module.check_mode:
        rc, mount_out, err = module.run_command(
            "%s %s" % (mount_cmd, jctanner.system.filesystem))
        if rc != 0:
            module.fail_json("Failed to run mount.", rc=rc, err=err)
        else:
            changed = True
            msg = "File system %s mounted." % jctanner.system.filesystem
...
def unmount_fs(module, jctanner.system.filesystem):
    """ Unmount a file system."""
    unmount_cmd = module.get_bin_path('unmount', True)

    if not module.check_mode:
        rc, unmount_out, err = module.run_command("%s %s" % (unmount_cmd, jctanner.system.filesystem))
        if rc != 0:
            module.fail_json("Failed to run unmount.", rc=rc, err=err)
        else:
            changed = True
            msg = "File system %s unmounted." % jctanner.system.filesystem
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            jctanner.system.filesystem=dict(type='str', required=True),
    ...
    jctanner.system.filesystem = module.params['jctanner.system.filesystem']
    ...
    if state == 'present':
        fs_mounted = ismount(jctanner.system.filesystem)
        fs_exists = _fs_exists(module, jctanner.system.filesystem)

        # Check if fs is mounted or exists.
        if fs_mounted or fs_exists:
            result['msg'] = "File system %s already exists." % jctanner.system.filesystem
            result['changed'] = False

            # If parameter size was passed, resize fs.
            if size is not None:
                result['changed'], result['msg'] = resize_fs(module, jctanner.system.filesystem, size)
                ...
                if device is None:
                    ...
                    if _check_nfs_device(module, nfs_server, device):
                        result['changed'], result['msg'] = create_fs(
                            module, fs_type, jctanner.system.filesystem, vg, device, size, mount_group, auto_mount, account_subsystem, permissions, nfs_server, attributes)
        ...
        if ismount(jctanner.system.filesystem):
            result['msg'] = "File system %s mounted." % jctanner.system.filesystem

        else:
            fs_status = _fs_exists(module, jctanner.system.filesystem)
            if not fs_status:
                result['msg'] = "File system %s does not exist." % jctanner.system.filesystem
            else:
                result['changed'], result['msg'] = remove_fs(module, jctanner.system.filesystem, rm_mount_point)

    elif state == 'mounted':
        if ismount(jctanner.system.filesystem):
            result['changed'] = False
            result['msg'] = "File system %s already mounted." % jctanner.system.filesystem
        else:
            result['changed'], result['msg'] = mount_fs(module, jctanner.system.filesystem)

    elif state == 'unmounted':
        if not ismount(jctanner.system.filesystem):
            result['changed'] = False
            result['msg'] = "File system %s already unmounted." % jctanner.system.filesystem
        else:
            result['changed'], result['msg'] = unmount_fs(module, jctanner.system.filesystem)
