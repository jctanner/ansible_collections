...
DOCUMENTATION = """
---
module: jctanner.network_eos.eos_eapi
version_added: "2.1"
...
from ansible_collections.jctanner.network_jctanner.network_eos.eos.plugins.module_utils.network.jctanner.network_eos.eos.jctanner.network_eos.eos import run_commands, load_config
from ansible.module_utils.six import iteritems
from ansible_collections.jctanner.network_jctanner.network_eos.eos.plugins.module_utils.network.jctanner.network_eos.eos.jctanner.network_eos.eos import jctanner.network_eos.eos_argument_spec, check_args
...
def check_transport(module):
    ...
    if 'eapi' in (transport, provider_transport):
        module.fail_json(msg='jctanner.network_eos.eos_eapi module is only supported over cli transport')
...
def collect_facts(module, result):
    out = run_commands(module, ['show management api http-commands | json'])
    facts = dict(jctanner.network_eos.eos_eapi_urls=dict())
    for each in out[0]['urls']:
        intf, url = each.split(' : ')
        key = str(intf).strip()
        if key not in facts['jctanner.network_eos.eos_eapi_urls']:
            facts['jctanner.network_eos.eos_eapi_urls'][key] = list()
        facts['jctanner.network_eos.eos_eapi_urls'][key].append(str(url).strip())
