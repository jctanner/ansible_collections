...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_report_setting
...
- hosts: localhost
  ...
  - name: Report setting configuration.
    jctanner.network_fortios.fortios_report_setting:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_report(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_report(module.params, fos)
