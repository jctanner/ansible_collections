...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_log_fortianalyzer2_setting
...
- hosts: localhost
  ...
  - name: Global FortiAnalyzer settings.
    jctanner.network_fortios.fortios_log_fortianalyzer2_setting:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_log_fortianalyzer2(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_log_fortianalyzer2(module.params, fos)
