...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_log_syslogd2_setting
...
- hosts: localhost
  ...
  - name: Global settings for remote syslog server.
    jctanner.network_fortios.fortios_log_syslogd2_setting:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_log_syslogd2(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_log_syslogd2(module.params, fos)
