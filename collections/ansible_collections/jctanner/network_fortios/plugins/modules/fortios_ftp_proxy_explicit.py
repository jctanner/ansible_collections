...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_ftp_proxy_explicit
...
- hosts: localhost
  ...
  - name: Configure explicit FTP proxy settings.
    jctanner.network_fortios.fortios_ftp_proxy_explicit:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_ftp_proxy(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_ftp_proxy(module.params, fos)
