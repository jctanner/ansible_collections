...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_extender_controller_extender
...
- hosts: localhost
  ...
  - name: Extender controller configuration.
    jctanner.network_fortios.fortios_extender_controller_extender:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_extender_controller(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_extender_controller(module.params, fos)
