...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_wireless_controller_wids_profile
...
- hosts: localhost
  ...
  - name: Configure wireless intrusion detection system (WIDS) profiles.
    jctanner.network_fortios.fortios_wireless_controller_wids_profile:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_wireless_controller(data, fos):
    login(data, fos)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_wireless_controller(module.params, fos)
