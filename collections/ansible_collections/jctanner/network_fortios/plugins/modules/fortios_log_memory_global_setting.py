...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_log_memory_global_setting
...
- hosts: localhost
  ...
  - name: Global settings for memory logging.
    jctanner.network_fortios.fortios_log_memory_global_setting:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_log_memory(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_log_memory(module.params, fos)
