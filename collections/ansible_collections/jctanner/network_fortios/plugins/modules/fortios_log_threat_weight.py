...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_log_threat_weight
...
- hosts: localhost
  ...
  - name: Configure threat weight settings.
    jctanner.network_fortios.fortios_log_threat_weight:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_log(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_log(module.params, fos)
