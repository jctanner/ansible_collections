...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_webfilter_ips_urlfilter_cache_setting
...
- hosts: localhost
  ...
  - name: Configure IPS URL filter cache settings.
    jctanner.network_fortios.fortios_webfilter_ips_urlfilter_cache_setting:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_webfilter(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_webfilter(module.params, fos)
