...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_firewall_ssl_setting
...
- hosts: localhost
  ...
  - name: SSL proxy settings.
    jctanner.network_fortios.fortios_firewall_ssl_setting:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_firewall_ssl(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_firewall_ssl(module.params, fos)
