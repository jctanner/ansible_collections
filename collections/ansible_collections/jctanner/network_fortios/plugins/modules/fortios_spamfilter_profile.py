...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_spamfilter_profile
...
- hosts: localhost
  ...
  - name: Configure AntiSpam profiles.
    jctanner.network_fortios.fortios_spamfilter_profile:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_spamfilter(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_spamfilter(module.params, fos)
