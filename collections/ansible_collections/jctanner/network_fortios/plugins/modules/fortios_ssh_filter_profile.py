...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_ssh_filter_profile
...
- hosts: localhost
  ...
  - name: SSH filter profile.
    jctanner.network_fortios.fortios_ssh_filter_profile:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_ssh_filter(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_ssh_filter(module.params, fos)
