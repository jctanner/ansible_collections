...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_wanopt_settings
...
- hosts: localhost
  ...
  - name: Configure WAN optimization settings.
    jctanner.network_fortios.fortios_wanopt_settings:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_wanopt(data, fos):
    login(data, fos)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_wanopt(module.params, fos)
