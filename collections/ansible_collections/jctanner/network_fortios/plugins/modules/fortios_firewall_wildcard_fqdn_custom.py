...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_firewall_wildcard_fqdn_custom
...
- hosts: localhost
  ...
  - name: Config global/VDOM Wildcard FQDN address.
    jctanner.network_fortios.fortios_firewall_wildcard_fqdn_custom:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_firewall_wildcard_fqdn(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_firewall_wildcard_fqdn(module.params, fos)
