...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_dnsfilter_domain_filter
...
- hosts: localhost
  ...
  - name: Configure DNS domain filters.
    jctanner.network_fortios.fortios_dnsfilter_domain_filter:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_dnsfilter(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_dnsfilter(module.params, fos)
