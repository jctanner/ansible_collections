...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_user_tacacsplus
...
- hosts: localhost
  ...
  - name: Configure TACACS+ server entries.
    jctanner.network_fortios.fortios_user_tacacsplus:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_user(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_user(module.params, fos)
