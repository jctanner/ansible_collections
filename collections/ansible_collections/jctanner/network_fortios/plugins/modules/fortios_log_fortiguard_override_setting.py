...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_log_fortiguard_override_setting
...
- hosts: localhost
  ...
  - name: Override global FortiCloud logging settings for this VDOM.
    jctanner.network_fortios.fortios_log_fortiguard_override_setting:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_log_fortiguard(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_log_fortiguard(module.params, fos)
