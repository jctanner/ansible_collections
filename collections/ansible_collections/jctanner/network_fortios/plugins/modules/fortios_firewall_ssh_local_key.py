...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_firewall_ssh_local_key
...
- hosts: localhost
  ...
  - name: SSH proxy local keys.
    jctanner.network_fortios.fortios_firewall_ssh_local_key:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_firewall_ssh(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_firewall_ssh(module.params, fos)
