...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_firewall_ipmacbinding_setting
...
- hosts: localhost
  ...
  - name: Configure IP to MAC binding settings.
    jctanner.network_fortios.fortios_firewall_ipmacbinding_setting:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_firewall_ipmacbinding(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_firewall_ipmacbinding(module.params, fos)
