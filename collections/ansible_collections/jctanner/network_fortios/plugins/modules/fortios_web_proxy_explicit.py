...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_web_proxy_explicit
...
- hosts: localhost
  ...
  - name: Configure explicit Web proxy settings.
    jctanner.network_fortios.fortios_web_proxy_explicit:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_web_proxy(data, fos):
    login(data, fos)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_web_proxy(module.params, fos)
