...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_dlp_fp_sensitivity
...
- hosts: localhost
  ...
  - name: Create self-explanatory DLP sensitivity levels to be used when setting sensitivity under config fp-doc-source.
    jctanner.network_fortios.fortios_dlp_fp_sensitivity:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_dlp(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_dlp(module.params, fos)
