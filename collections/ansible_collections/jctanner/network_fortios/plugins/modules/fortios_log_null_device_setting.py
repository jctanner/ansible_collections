...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_log_null_device_setting
...
- hosts: localhost
  ...
  - name: Settings for null device logging.
    jctanner.network_fortios.fortios_log_null_device_setting:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_log_null_device(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_log_null_device(module.params, fos)
