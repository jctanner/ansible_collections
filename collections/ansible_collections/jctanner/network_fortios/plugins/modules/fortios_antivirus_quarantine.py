...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_antivirus_quarantine
...
- hosts: localhost
  ...
  - name: Configure quarantine options.
    jctanner.network_fortios.fortios_antivirus_quarantine:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_antivirus(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_antivirus(module.params, fos)
