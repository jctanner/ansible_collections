...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_log_syslogd_override_setting
...
- hosts: localhost
  ...
  - name: Override settings for remote syslog server.
    jctanner.network_fortios.fortios_log_syslogd_override_setting:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_log_syslogd(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_log_syslogd(module.params, fos)
