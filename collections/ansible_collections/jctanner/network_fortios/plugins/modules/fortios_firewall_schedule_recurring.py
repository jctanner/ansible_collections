...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_firewall_schedule_recurring
...
- hosts: localhost
  ...
  - name: Recurring schedule configuration.
    jctanner.network_fortios.fortios_firewall_schedule_recurring:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_firewall_schedule(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_firewall_schedule(module.params, fos)
