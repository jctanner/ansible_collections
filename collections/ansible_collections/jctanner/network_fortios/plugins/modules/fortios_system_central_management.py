...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_system_central_management
...
- hosts: localhost
  ...
  - name: Configure central management.
    jctanner.network_fortios.fortios_system_central_management:
      host:  "{{  host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{  vdom }}"
...
def jctanner.network_fortios.fortios_system(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_system(module.params, fos)
