...
DOCUMENTATION = '''
---
module: jctanner.network_fortios.fortios_vpn_ipsec_manualkey_interface
...
- hosts: localhost
  ...
  - name: Configure IPsec manual keys.
    jctanner.network_fortios.fortios_vpn_ipsec_manualkey_interface:
      host:  "{{ host }}"
      username: "{{ username }}"
      password: "{{ password }}"
      vdom:  "{{ vdom }}"
...
def jctanner.network_fortios.fortios_vpn_ipsec(data, fos):
    login(data)
...
def main():
    ...
    is_error, has_changed, result = jctanner.network_fortios.fortios_vpn_ipsec(module.params, fos)
