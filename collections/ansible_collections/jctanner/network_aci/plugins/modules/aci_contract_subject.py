...
description:
- Manage initial Contract Subjects on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.4'
options:
  tenant:
    description:
  ...
  reverse_filter:
    ...
    - Determines if the APIC should reverse the src and dst ports to allow the
      return traffic back, since jctanner.network_jctanner.network_aci.aci.ACI is stateless filter.
...
- The C(tenant) and C(contract) used must exist before using this module in your playbook.
  The M(jctanner.network_aci.aci_tenant) and M(jctanner.network_aci.aci_contract) modules can be used for this.
seealso:
...
- name: Add a new contract subject
  jctanner.network_aci.aci_contract_subject:
    host: apic
    username: admin
...
def main():
    ...
    reverse_filter = jctanner.network_aci.aci.boolean(module.params['reverse_filter'])
    ...
    if directive is not None or filter_name is not None:
        module.fail_json(msg="Managing Contract Subjects to Filter bindings has been moved to module 'jctanner.network_aci.aci_subject_bind_filter'")
    ...
    jctanner.network_aci.aci.construct_url(
        ...
        subclass_2=dict(
            jctanner.network_aci.aci_class='vzSubj',
            jctanner.network_aci.aci_rn='subj-{0}'.format(subject),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='vzSubj')
