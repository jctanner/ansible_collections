...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_interface_policy_leaf_policy_group
short_description: Manage fabric interface policy leaf policy groups (infra:AccBndlGrp, infra:AccPortGrp)
description:
- Manage fabric interface policy leaf policy groups on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.5'
options:
  policy_group:
    description:
...
- name: Create a Port Channel (PC) Interface Policy Group
  jctanner.network_aci.aci_interface_policy_leaf_policy_group:
    host: apic
    username: admin
...
def main():
    ...
    if lag_type == 'leaf':
        jctanner.network_aci.aci_class_name = 'infraAccPortGrp'
        dn_name = 'accportgrp'
        ...
        jctanner.network_aci.aci_class_name = 'infraAccBndlGrp'
        dn_name = 'accbundle'
        class_config_dict = dict(
            name=policy_group,
            descr=description,
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            jctanner.network_aci.aci_class=jctanner.network_aci.aci_class_name,
            jctanner.network_aci.aci_rn='infra/funcprof/{0}-{1}'.format(dn_name, policy_group),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class=jctanner.network_aci.aci_class_name)
