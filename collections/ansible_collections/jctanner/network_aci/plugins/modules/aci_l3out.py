...
description:
- Manage Layer 3 Outside (L3Out) on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.6'
options:
  tenant:
    description:
...
- The C(tenant) and C(domain) and C(vrf) used must exist before using this module in your playbook.
  The M(jctanner.network_aci.aci_tenant) and M(jctanner.network_aci.aci_domain) and M(jctanner.network_aci.aci_vrf) modules can be used for this.
seealso:
...
- name: Add a new L3Out
  jctanner.network_aci.aci_l3out:
    host: apic
    username: admin
...
def main():
    ...
    if enforceRtctrl is not None:
        if len(enforceRtctrl) == 1 and enforceRtctrl[0] == 'import':
            jctanner.network_aci.aci.fail_json(
    ...
    jctanner.network_aci.aci.construct_url(
        ...
        subclass_1=dict(
            jctanner.network_aci.aci_class='l3extOut',
            jctanner.network_aci.aci_rn='out-{0}'.format(l3out),
            module_object=l3out,
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='l3extOut')
