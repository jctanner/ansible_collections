...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_filter_entry
short_description: Manage filter entries (vz:Entry)
description:
- Manage filter entries for a filter on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.4'
options:
  arp_flag:
    description:
...
- The C(tenant) and C(filter) used must exist before using this module in your playbook.
  The M(jctanner.network_aci.aci_tenant) and M(jctanner.network_aci.aci_filter) modules can be used for this.
seealso:
...
EXAMPLES = r'''
- jctanner.network_aci.aci_filter_entry:
...
def main():
    ...
    stateful = jctanner.network_aci.aci.boolean(module.params['stateful'])
    ...
    jctanner.network_aci.aci.construct_url(
        ...
        subclass_1=dict(
            jctanner.network_aci.aci_class='vzFilter',
            jctanner.network_aci.aci_rn='flt-{0}'.format(filter_name),
            module_object=filter_name,
            target_filter={'name': filter_name},
        ),
        subclass_2=dict(
            jctanner.network_aci.aci_class='vzEntry',
            jctanner.network_aci.aci_rn='e-{0}'.format(entry),
            module_object=entry,
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='vzEntry')
