...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_interface_selector_to_switch_policy_leaf_profile
short_description: Bind interface selector profiles to switch policy leaf profiles (infra:RsAccPortP)
description:
- Bind interface selector profiles to switch policy leaf profiles on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.5'
options:
  leaf_profile:
    description:
...
notes:
- This module requires an existing leaf profile, the module M(jctanner.network_aci.aci_switch_policy_leaf_profile) can be used for this.
seealso:
- module: jctanner.network_aci.aci_switch_policy_leaf_profile
...
- name: Associating an interface selector profile to a switch policy leaf profile
  jctanner.network_aci.aci_interface_selector_to_switch_policy_leaf_profile:
    host: apic
    username: admin
...
def main():
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            jctanner.network_aci.aci_class='infraNodeP',
            jctanner.network_aci.aci_rn='infra/nprof-{0}'.format(leaf_profile),
            module_object=leaf_profile,
            target_filter={'name': leaf_profile},
        ),
        subclass_1=dict(
            jctanner.network_aci.aci_class='infraRsAccPortP',
            jctanner.network_aci.aci_rn='rsaccPortP-[{0}]'.format(interface_selector_tDn),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='infraRsAccPortP')
