...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_encap_pool_range
short_description: Manage encap ranges assigned to pools (fvns:EncapBlk, fvns:VsanEncapBlk)
description:
- Manage vlan, vxlan, and vsan ranges that are assigned to pools on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.5'
options:
  allocation_mode:
    description:
...
seealso:
- module: jctanner.network_aci.aci_encap_pool
- module: jctanner.network_aci.aci_vlan_pool_encap_block
...
- name: Add a new VLAN pool range
  jctanner.network_aci.aci_encap_pool_range:
    host: apic
    username: admin
...
jctanner.network_jctanner.network_aci.aci.ACI_POOL_MAPPING = dict(
    vlan=dict(
        jctanner.network_aci.aci_class='fvnsVlanInstP',
        jctanner.network_aci.aci_mo='infra/vlanns-',
    ),
    vxlan=dict(
        jctanner.network_aci.aci_class='fvnsVxlanInstP',
        jctanner.network_aci.aci_mo='infra/vxlanns-',
    ),
    vsan=dict(
        jctanner.network_aci.aci_class='fvnsVsanInstP',
        jctanner.network_aci.aci_mo='infra/vsanns-',
    ),
)
...
def main():
    ...
    jctanner.network_jctanner.network_aci.aci.ACI_RANGE_MAPPING = dict(
        vlan=dict(
            jctanner.network_aci.aci_class='fvnsEncapBlk',
            jctanner.network_aci.aci_mo='from-[{0}]-to-[{1}]'.format(encap_start, encap_end),
        ),
        vxlan=dict(
            jctanner.network_aci.aci_class='fvnsEncapBlk',
            jctanner.network_aci.aci_mo='from-[{0}]-to-[{1}]'.format(encap_start, encap_end),
        ...
        vsan=dict(
            jctanner.network_aci.aci_class='fvnsVsanEncapBlk',
            jctanner.network_aci.aci_mo='vsanfrom-[{0}]-to-[{1}]'.format(encap_start, encap_end),
        ),
    )

    # Collect proper class and mo information based on pool_type
    jctanner.network_aci.aci_range_class = jctanner.network_jctanner.network_aci.aci.ACI_RANGE_MAPPING[pool_type]["jctanner.network_aci.aci_class"]
    jctanner.network_aci.aci_range_mo = jctanner.network_jctanner.network_aci.aci.ACI_RANGE_MAPPING[pool_type]["jctanner.network_aci.aci_mo"]
    jctanner.network_aci.aci_pool_class = jctanner.network_jctanner.network_aci.aci.ACI_POOL_MAPPING[pool_type]["jctanner.network_aci.aci_class"]
    jctanner.network_aci.aci_pool_mo = jctanner.network_jctanner.network_aci.aci.ACI_POOL_MAPPING[pool_type]["jctanner.network_aci.aci_mo"]
    pool_name = pool
    ...
    if range_end is not None and range_start is not None:
        ...
        if range_name is None:
            # Reset range managed object to None for jctanner.network_aci.aci util to properly handle query
            jctanner.network_aci.aci_range_mo = None
    ...
    # jctanner.network_jctanner.network_aci.aci.ACI Pool URL requires the allocation mode for vlan and vsan pools (ex: uni/infra/vlanns-[poolname]-static)
    if pool_type != 'vxlan' and pool is not None:
        if pool_allocation_mode is not None:
            pool_name = '[{0}]-{1}'.format(pool, pool_allocation_mode)
        else:
            module.fail_json(msg='jctanner.network_jctanner.network_aci.aci.ACI requires the "pool_allocation_mode" for "pool_type" of "vlan" and "vsan" when the "pool" is provided')
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            jctanner.network_aci.aci_class=jctanner.network_aci.aci_pool_class,
            jctanner.network_aci.aci_rn='{0}{1}'.format(jctanner.network_aci.aci_pool_mo, pool_name),
            module_object=pool,
            target_filter={'name': pool},
        ),
        subclass_1=dict(
            jctanner.network_aci.aci_class=jctanner.network_aci.aci_range_class,
            jctanner.network_aci.aci_rn='{0}'.format(jctanner.network_aci.aci_range_mo),
            module_object=jctanner.network_aci.aci_range_mo,
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class=jctanner.network_aci.aci_range_class)
