...
- Connect to external virtual and physical domains by using
  attachable Access Entity Profiles (AEP) on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.4'
options:
  aep:
    description:
...
EXAMPLES = r'''
- name: Add a new AEP
  jctanner.network_aci.aci_aep:
    host: apic
    username: admin
    password: SomeSecretPassword
    aep: jctanner.network_jctanner.network_aci.aci.ACI-AEP
    description: default
    state: present
...
def main():
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='infraAttEntityP')
