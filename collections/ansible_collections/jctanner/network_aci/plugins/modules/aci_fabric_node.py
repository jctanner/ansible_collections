...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_fabric_node
short_description: Manage Fabric Node Members (fabric:NodeIdentP)
description:
- Manage Fabric Node Members on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.5'
options:
  pod_id:
    description:
...
- name: Add fabric node
  jctanner.network_aci.aci_fabric_node:
    host: apic
    username: admin
...
def main():
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            jctanner.network_aci.aci_class='fabricNodeIdentP',
            jctanner.network_aci.aci_rn='controller/nodeidentpol/nodep-{0}'.format(serial),
    ...
    if state == 'present':
        jctanner.network_aci.aci.payload(
            ...
            class_config=dict(
                ...
                # FIXME: Did this change with jctanner.network_jctanner.network_aci.aci.ACI version ?
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='fabricNodeIdentP')
