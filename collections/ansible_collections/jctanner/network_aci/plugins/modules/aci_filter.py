...
description:
- Manages top level filter objects on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
- This modules does not manage filter entries, see M(jctanner.network_aci.aci_filter_entry) for this functionality.
version_added: '2.4'
options:
  filter:
    description:
...
- name: Add a new filter to a tenant
  jctanner.network_aci.aci_filter:
    host: apic
    username: admin
...
def main():
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='vzFilter')
