...
description:
- Manage AAA users on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
requirements:
- python-dateutil
version_added: '2.5'
options:
...
EXAMPLES = r'''
- name: Add a user
  jctanner.network_aci.aci_aaa_user:
    host: apic
    username: admin
...
def main():
    ...
    aaa_password_update_required = jctanner.network_aci.aci.boolean(module.params['aaa_password_update_required'])
    aaa_user = module.params['aaa_user']
    clear_password_history = jctanner.network_aci.aci.boolean(module.params['clear_password_history'], 'yes', 'no')
    description = module.params['description']
    email = module.params['email']
    enabled = jctanner.network_aci.aci.boolean(module.params['enabled'], 'active', 'inactive')
    expires = jctanner.network_aci.aci.boolean(module.params['expires'])
    ...
    if expiration is not None and expiration != 'never':
        try:
            expiration = jctanner.network_aci.aci.iso8601_format(dateutil.parser.parse(expiration).replace(tzinfo=tzutc()))
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            ...
            jctanner.network_aci.aci_rn='userext/user-{0}'.format(aaa_user),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='aaaUser')
