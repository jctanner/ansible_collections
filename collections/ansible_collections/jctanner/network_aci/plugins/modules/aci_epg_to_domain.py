...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_epg_to_domain
short_description: Bind EPGs to Domains (fv:RsDomAtt)
description:
- Bind EPGs to Physical and Virtual Domains on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.4'
options:
  allow_useg:
    description:
...
- The C(tenant), C(ap), C(epg), and C(domain) used must exist before using this module in your playbook.
  The M(jctanner.network_aci.aci_tenant) M(jctanner.network_aci.aci_ap), M(jctanner.network_aci.aci_epg) M(jctanner.network_aci.aci_domain) modules can be used for this.
...
- name: Add a new physical domain to EPG binding
  jctanner.network_aci.aci_epg_to_domain:
    host: apic
    username: admin
...
def main():
    ...
    netflow = jctanner.network_aci.aci.boolean(module.params['netflow'], 'enabled', 'disabled')
    ...
    jctanner.network_aci.aci.construct_url(
        ...
        subclass_3=dict(
            jctanner.network_aci.aci_class='fvRsDomAtt',
            jctanner.network_aci.aci_rn='rsdomAtt-[{0}]'.format(epg_domain),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='fvRsDomAtt')
