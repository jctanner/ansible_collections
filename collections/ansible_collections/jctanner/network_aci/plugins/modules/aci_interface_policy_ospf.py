...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_interface_policy_ospf
short_description: Manage OSPF interface policies (ospf:IfPol)
description:
- Manage OSPF interface policies on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.7'
options:
  tenant:
    description:
...
- name: Ensure ospf interface policy exists
  jctanner.network_aci.aci_interface_policy_ospf:
    host: apic
    username: admin
...
def main():
    ...
    prefix_suppression = jctanner.network_aci.aci.boolean(module.params['prefix_suppression'], 'enabled', 'disabled')
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            jctanner.network_aci.aci_class='ospfIfPol',
            jctanner.network_aci.aci_rn='tn-{0}/ospfIfPol-{1}'.format(tenant, ospf),
            module_object=ospf,
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='ospfIfPol')
