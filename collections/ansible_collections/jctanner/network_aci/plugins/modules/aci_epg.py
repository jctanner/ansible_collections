...
description:
- Manage End Point Groups (EPG) on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.4'
options:
  tenant:
    description:
  ...
  preferred_group:
    ...
    - This is very convenient for migration scenarios, or when jctanner.network_jctanner.network_aci.aci.ACI is used for network automation but not for policy.
...
- The C(tenant) and C(app_profile) used must exist before using this module in your playbook.
  The M(jctanner.network_aci.aci_tenant) and M(jctanner.network_aci.aci_ap) modules can be used for this.
seealso:
...
def main():
    ...
    preferred_group = jctanner.network_aci.aci.boolean(module.params['preferred_group'], 'include', 'exclude')
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='fvAEPg')
