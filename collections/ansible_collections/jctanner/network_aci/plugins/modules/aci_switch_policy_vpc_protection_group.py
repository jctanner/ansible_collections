...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_switch_policy_vpc_protection_group
short_description: Manage switch policy explicit vPC protection groups (fabric:ExplicitGEp, fabric:NodePEp).
description:
- Manage switch policy explicit vPC protection groups on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.5'
options:
  protection_group:
    description:
...
- name: Add vPC Protection Group
  jctanner.network_aci.aci_switch_policy_vpc_protection_group:
    host: apic
    username: admin
...
def main():
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            jctanner.network_aci.aci_class='fabricExplicitGEp',
            jctanner.network_aci.aci_rn='fabric/protpol/expgep-{0}'.format(protection_group),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='fabricExplicitGEp')
