...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.mso_schema_site_bd_l3out
short_description: Manage site-local BD l3out's in schema template
description:
- Manage site-local BDs l3out's in schema template on Cisco jctanner.network_jctanner.network_aci.aci.ACI Multi-Site.
author:
...
- name: Add a new site BD l3out
  jctanner.network_aci.mso_schema_site_bd:
...
def main():
    ...
    if l3out is not None and l3out in l3outs:
        ...
        jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['bds'][bd_idx]['l3Outs'][l3out_idx]

    if state == 'query':
        if l3out is None:
            jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['bds'][bd_idx]['l3Outs']
    ...
    if state == 'absent':
        ...
        jctanner.network_aci.mso.sent = l3out
