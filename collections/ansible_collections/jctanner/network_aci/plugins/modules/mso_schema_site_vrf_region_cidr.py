...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.mso_schema_site_vrf_region_cidr
short_description: Manage site-local VRF region CIDRs in schema template
description:
- Manage site-local VRF region CIDRs in schema template on Cisco jctanner.network_jctanner.network_aci.aci.ACI Multi-Site.
author:
...
- module: jctanner.network_aci.mso_schema_site_vrf_region_cidr_subnet
...
- name: Add a new site VRF region CIDR
  jctanner.network_aci.mso_schema_template_vrf_region_cidr:
...
def main():
    ...
    if region not in regions:
        jctanner.network_aci.mso.fail_json(msg="Provided region '{0}' does not exist. Existing regions: {1}".format(region, ', '.join(regions)))
    ...
    if cidr is not None and cidr in cidrs:
        ...
        jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['vrfs'][vrf_idx]['regions'][region_idx]['cidrs'][cidr_idx]

    if state == 'query':
        if cidr is None:
            jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['vrfs'][vrf_idx]['regions'][region_idx]['cidrs']
        elif not jctanner.network_aci.mso.existing:
            jctanner.network_aci.mso.fail_json(msg="CIDR IP '{cidr}' not found".format(cidr=cidr))
    ...
    if state == 'absent':
        ...
        if jctanner.network_aci.mso.existing:
            ops.append(dict(op='replace', path=cidr_path, value=jctanner.network_aci.mso.sent))
        else:
            ops.append(dict(op='add', path=cidrs_path + '/-', value=jctanner.network_aci.mso.sent))
