...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_static_binding_to_epg
short_description: Bind static paths to EPGs (fv:RsPathAtt)
description:
- Bind static paths to EPGs on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.5'
options:
  tenant:
    description:
...
- The C(tenant), C(ap), C(epg) used must exist before using this module in your playbook.
  The M(jctanner.network_aci.aci_tenant), M(jctanner.network_aci.aci_ap), M(jctanner.network_aci.aci_epg) modules can be used for this.
seealso:
...
# TODO: change 'deploy_immediacy' to 'resolution_immediacy' (as seen in jctanner.network_aci.aci_epg_to_domain)?
...
def main():
    ...
    jctanner.network_aci.aci.construct_url(
        ...
        subclass_3=dict(
            jctanner.network_aci.aci_class='fvRsPathAtt',
            jctanner.network_aci.aci_rn='rspathAtt-[{0}]'.format(static_path),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='fvRsPathAtt')
