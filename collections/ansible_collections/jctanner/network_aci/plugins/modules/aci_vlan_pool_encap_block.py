...
description:
- Manage VLAN encap blocks that are assigned to VLAN pools on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.5'
options:
  allocation_mode:
    description:
...
- name: Add a new VLAN encap block
  jctanner.network_aci.aci_vlan_pool_encap_block:
    host: apic
    username: admin
...
def main():
    ...
    jctanner.network_aci.aci_block_mo = 'from-[{0}]-to-[{1}]'.format(encap_start, encap_end)
    pool_name = pool
    ...
    if block_end is not None and block_start is not None:
        ...
        if block_name is None:
            ...
            jctanner.network_aci.aci_block_mo = None

    # jctanner.network_jctanner.network_aci.aci.ACI Pool URL requires the allocation mode (ex: uni/infra/vlanns-[poolname]-static)
    if pool is not None:
        if pool_allocation_mode is not None:
            pool_name = '[{0}]-{1}'.format(pool, pool_allocation_mode)
        else:
            module.fail_json(msg="jctanner.network_jctanner.network_aci.aci.ACI requires the 'pool_allocation_mode' when 'pool' is provided")
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            ...
            jctanner.network_aci.aci_rn='infra/vlanns-{0}'.format(pool_name),
            module_object=pool,
            target_filter={'name': pool},
        ),
        subclass_1=dict(
            jctanner.network_aci.aci_class='fvnsEncapBlk',
            jctanner.network_aci.aci_rn=jctanner.network_aci.aci_block_mo,
            module_object=jctanner.network_aci.aci_block_mo,
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='fvnsEncapBlk')
