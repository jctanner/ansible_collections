...
description:
- Bind Domain to VLAN Pools on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.5'
options:
  domain:
    description:
...
- The C(domain) and C(vlan_pool) parameters should exist before using this module.
  The M(jctanner.network_aci.aci_domain) and M(jctanner.network_aci.aci_vlan_pool) can be used for these.
seealso:
...
- name: Bind a VMM domain to VLAN pool
  jctanner.network_aci.aci_domain_to_vlan_pool:
    host: apic
    username: admin
...
def main():
    ...
    jctanner.network_aci.aci_mo = 'uni/infra/vlanns-{0}'.format(pool_name)
    ...
    if state == 'present':
        jctanner.network_aci.aci.payload(
            ...
            child_configs=[
                {'infraRsVlanNs': {'attributes': {'tDn': jctanner.network_aci.aci_mo}}},
            ]
        )
