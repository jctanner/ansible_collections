...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_bd_subnet
short_description: Manage Subnets (fv:Subnet)
description:
- Manage Subnets on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.4'
options:
  bd:
    description:
  ...
  scope:
    ...
    - The C(public) option allows the Subnet to be advertised outside of the jctanner.network_jctanner.network_aci.aci.ACI Fabric, and allows communication with
      hosts in other VRFs.
...
- The C(tenant) and C(bd) used must exist before using this module in your playbook.
  The M(jctanner.network_aci.aci_tenant) module and M(jctanner.network_aci.aci_bd) can be used for these.
seealso:
...
- name: Create a subnet
  jctanner.network_aci.aci_bd_subnet:
    host: apic
    username: admin
    password: SomeSecretPassword
    tenant: production
...
def main():
    ...
    enable_vip = jctanner.network_aci.aci.boolean(module.params['enable_vip'])
    ...
    preferred = jctanner.network_aci.aci.boolean(module.params['preferred'])
    ...
    jctanner.network_aci.aci.construct_url(
        ...
        subclass_2=dict(
            jctanner.network_aci.aci_class='fvSubnet',
            jctanner.network_aci.aci_rn='subnet-[{0}]'.format(gateway),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='fvSubnet')
