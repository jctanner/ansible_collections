...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_access_sub_port_block_to_access_port
...
description:
- Manage sub port blocks of Fabric interface policy leaf profile interface selectors on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
seealso:
...
- name: Associate an access sub port block (single port) to an interface selector
  jctanner.network_aci.aci_access_sub_port_block_to_access_port:
    host: apic
    username: admin
...
def main():
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            jctanner.network_aci.aci_class='infraAccPortP',
            jctanner.network_aci.aci_rn='infra/accportprof-{0}'.format(leaf_interface_profile),
            module_object=leaf_interface_profile,
            target_filter={'name': leaf_interface_profile},
        ),
        subclass_1=dict(
            jctanner.network_aci.aci_class='infraHPortS',
            # NOTE: normal rn: hports-{name}-typ-{type}, hence here hardcoded to range for purposes of module
            jctanner.network_aci.aci_rn='hports-{0}-typ-range'.format(access_port_selector),
            module_object=access_port_selector,
            target_filter={'name': access_port_selector},
        ),
        subclass_2=dict(
            jctanner.network_aci.aci_class='infraSubPortBlk',
            jctanner.network_aci.aci_rn='subportblk-{0}'.format(leaf_port_blk),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='infraSubPortBlk')
