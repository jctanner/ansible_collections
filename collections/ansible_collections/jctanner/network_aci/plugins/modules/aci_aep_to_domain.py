...
description:
- Bind AEPs to Physical or Virtual Domains on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.5'
options:
  aep:
    description:
...
- The C(aep) and C(domain) parameters should exist before using this module.
  The M(jctanner.network_aci.aci_aep) and M(jctanner.network_aci.aci_domain) can be used for these.
seealso:
- module: jctanner.network_aci.aci_aep
...
- name: Add AEP to domain binding
  jctanner.network_aci.aci_aep_to_domain: &binding_present
    host: apic
    username: admin
...
- name: Remove AEP to domain binding
  jctanner.network_aci.aci_aep_to_domain: &binding_absent
    host: apic
    username: admin
    password: SomeSecretPassword
    aep: test_aep
...
- name: Query our AEP to domain binding
  jctanner.network_aci.aci_aep_to_domain:
    host: apic
    username: admin
    password: SomeSecretPassword
    aep: test_aep
...
- name: Query all AEP to domain bindings
  jctanner.network_aci.aci_aep_to_domain: &binding_query
    host: apic
    username: admin
    password: SomeSecretPassword
    state: query
...
def main():
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            jctanner.network_aci.aci_class='infraAttEntityP',
            jctanner.network_aci.aci_rn='infra/attentp-{0}'.format(aep),
            module_object=aep,
            target_filter={'name': aep},
        ),
        subclass_1=dict(
            jctanner.network_aci.aci_class='infraRsDomP',
            jctanner.network_aci.aci_rn='rsdomP-[{0}]'.format(domain_mo),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='infraRsDomP')
