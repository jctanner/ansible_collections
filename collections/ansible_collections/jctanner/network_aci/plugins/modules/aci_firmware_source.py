...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_firmware_source
short_description: Manage firmware image sources (firmware:OSource)
description:
- Manage firmware image sources on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.5'
options:
  source:
    description:
...
- name: Add firmware source
  jctanner.network_aci.aci_firmware_source:
    host: apic
    username: admin
    password: SomeSecretPassword
    source: jctanner.network_aci.aci-msft-pkg-3.1.1i.zip
    url: foo.bar.cisco.com/download/cisco/jctanner.network_aci.aci/jctanner.network_aci.aci-msft-pkg-3.1.1i.zip
    url_protocol: http
    state: present
...
def main():
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            jctanner.network_aci.aci_class='firmwareOSource',
            jctanner.network_aci.aci_rn='fabric/fwrepop',
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='firmwareOSource')
