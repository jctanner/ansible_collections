...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_tenant_span_src_group_to_dst_group
short_description: Bind SPAN source groups to destination groups (span:SpanLbl)
description:
- Bind SPAN source groups to associated destinaton groups on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.4'
options:
  description:
    description:
...
- The C(tenant), C(src_group), and C(dst_group) must exist before using this module in your playbook.
  The M(jctanner.network_aci.aci_tenant), M(jctanner.network_aci.aci_tenant_span_src_group), and M(jctanner.network_aci.aci_tenant_span_dst_group) modules can be used for this.
seealso:
- module: jctanner.network_aci.aci_tenant
- module: jctanner.network_aci.aci_tenant_span_src_group
- module: jctanner.network_aci.aci_tenant_span_dst_group
...
EXAMPLES = r'''
- jctanner.network_aci.aci_tenant_span_src_group_to_dst_group:
    host: apic
    username: admin
...
def main():
    ...
    jctanner.network_aci.aci.construct_url(
        ...
        subclass_1=dict(
            jctanner.network_aci.aci_class='spanSrcGrp',
            jctanner.network_aci.aci_rn='srcgrp-{0}'.format(src_group),
            module_object=src_group,
            target_filter={'name': src_group},
        ),
        subclass_2=dict(
            jctanner.network_aci.aci_class='spanSpanLbl',
            jctanner.network_aci.aci_rn='spanlbl-{0}'.format(dst_group),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='spanSpanLbl')
