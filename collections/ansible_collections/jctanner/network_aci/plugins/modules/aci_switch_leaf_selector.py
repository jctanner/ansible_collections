...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_switch_leaf_selector
short_description: Bind leaf selectors to switch policy leaf profiles (infra:LeafS, infra:NodeBlk, infra:RsAccNodePGrep)
description:
- Bind leaf selectors (with node block range and policy group) to switch policy leaf profiles on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.5'
options:
  description:
    description:
...
notes:
- This module is to be used with M(jctanner.network_aci.aci_switch_policy_leaf_profile).
...
- name: adding a switch policy leaf profile selector associated Node Block range (w/ policy group)
  jctanner.network_aci.aci_switch_leaf_selector:
    host: apic
    username: admin
...
def main():
    ...
    jctanner.network_aci.aci.construct_url(
        ...
        subclass_1=dict(
            jctanner.network_aci.aci_class='infraLeafS',
            # NOTE: normal rn: leaves-{name}-typ-{type}, hence here hardcoded to range for purposes of module
            jctanner.network_aci.aci_rn='leaves-{0}-typ-range'.format(leaf),
            module_object=leaf,
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='infraLeafS')
