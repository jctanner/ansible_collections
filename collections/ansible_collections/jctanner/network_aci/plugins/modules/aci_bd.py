...
description:
- Manages Bridge Domains (BD) on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.4'
options:
  arp_flooding:
    description:
...
def main():
    ...
    arp_flooding = jctanner.network_aci.aci.boolean(module.params['arp_flooding'])
    bd = module.params['bd']
    ...
    enable_multicast = jctanner.network_aci.aci.boolean(module.params['enable_multicast'])
    enable_routing = jctanner.network_aci.aci.boolean(module.params['enable_routing'])
    endpoint_clear = jctanner.network_aci.aci.boolean(module.params['endpoint_clear'])
    endpoint_move_detect = module.params['endpoint_move_detect']
    if endpoint_move_detect == 'default':
        # the jctanner.network_jctanner.network_aci.aci.ACI default setting is an empty string, but that is not a good input value
        endpoint_move_detect = ''
    ...
    ip_learning = jctanner.network_aci.aci.boolean(module.params['ip_learning'])
    ...
    limit_ip_learn = jctanner.network_aci.aci.boolean(module.params['limit_ip_learn'])
    ...
    # Give warning when fvSubnet parameters are passed as those have been moved to the jctanner.network_aci.aci_subnet module
    if module.params['gateway_ip'] or module.params['subnet_mask'] or module.params['scope']:
        module._warnings = ["The support for managing Subnets has been moved to its own module, jctanner.network_aci.aci_subnet. \
    ...
    jctanner.network_aci.aci.construct_url(
        ...
        subclass_1=dict(
            jctanner.network_aci.aci_class='fvBD',
            jctanner.network_aci.aci_rn='BD-{0}'.format(bd),
            module_object=bd,
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='fvBD')
