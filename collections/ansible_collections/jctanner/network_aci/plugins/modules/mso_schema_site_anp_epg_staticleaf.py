...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.mso_schema_site_anp_epg_staticleaf
short_description: Manage site-local EPG static leafs in schema template
description:
- Manage site-local EPG static leafs in schema template on Cisco jctanner.network_jctanner.network_aci.aci.ACI Multi-Site.
author:
...
- name: Add a new static leaf to a site EPG
  jctanner.network_aci.mso_schema_template_anp_epg_staticleaf:
...
def main():
    ...
    if (leafpath, vlan) in leafs:
        ...
        jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['anps'][anp_idx]['epgs'][epg_idx]['staticLeafs'][leaf_idx]

    if state == 'query':
        if leaf is None or vlan is None:
            jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['anps'][anp_idx]['epgs'][epg_idx]['staticLeafs']
        elif not jctanner.network_aci.mso.existing:
            jctanner.network_aci.mso.fail_json(msg="Static leaf '{leaf}/{vlan}' not found".format(leaf=leaf, vlan=vlan))
    ...
    if state == 'absent':
        ...
        if jctanner.network_aci.mso.existing:
            ops.append(dict(op='replace', path=leaf_path, value=jctanner.network_aci.mso.sent))
        else:
            ops.append(dict(op='add', path=leafs_path + '/-', value=jctanner.network_aci.mso.sent))
