...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.mso_schema_site_bd_subnet
short_description: Manage site-local BD subnets in schema template
description:
- Manage site-local BD subnets in schema template on Cisco jctanner.network_jctanner.network_aci.aci.ACI Multi-Site.
author:
...
seealso:
- module: jctanner.network_aci.mso_schema_site_bd
...
- name: Add a new site BD subnet
  jctanner.network_aci.mso_schema_site_bd_subnet:
...
from ansible_collections.jctanner.network_jctanner.network_aci.aci.plugins.module_utils.network.jctanner.network_aci.aci.jctanner.network_aci.mso import MSOModule, jctanner.network_aci.mso_argument_spec, jctanner.network_aci.mso_subnet_spec
...
def main():
    ...
    # Get BD
    bd_ref = jctanner.network_aci.mso.bd_ref(schema_id=schema_id, template=template, bd=bd)
    ...
    if subnet in subnets:
        ...
        jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['bds'][bd_idx]['subnets'][subnet_idx]

    if state == 'query':
        if subnet is None:
            jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['bds'][bd_idx]['subnets']
