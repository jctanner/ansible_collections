...
DOCUMENTATION = '''
---
module: jctanner.network_aci.aci_firmware_policy
...
EXAMPLES = '''
   - name: firmware policy
     jctanner.network_aci.aci_firmware_policy:
...
def main():
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            jctanner.network_aci.aci_class='firmwareFwP',
            jctanner.network_aci.aci_rn='fabric/fwpol-{0}'.format(name),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='firmwareFwP')
