...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_contract_subject_to_filter
short_description: Bind Contract Subjects to Filters (vz:RsSubjFiltAtt)
description:
- Bind Contract Subjects to Filters on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.4'
options:
  contract:
    description:
...
- The C(tenant), C(contract), C(subject), and C(filter_name) must exist before using this module in your playbook.
  The M(jctanner.network_aci.aci_tenant), M(jctanner.network_aci.aci_contract), M(jctanner.network_aci.aci_contract_subject), and M(jctanner.network_aci.aci_filter) modules can be used for these.
seealso:
...
- name: Add a new contract subject to filer binding
  jctanner.network_aci.aci_contract_subject_to_filter:
    host: apic
    username: admin
...
def main():
    ...
    jctanner.network_aci.aci.construct_url(
        ...
        subclass_3=dict(
            jctanner.network_aci.aci_class='vzRsSubjFiltAtt',
            jctanner.network_aci.aci_rn='rssubjFiltAtt-{0}'.format(filter_name),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='vzRsSubjFiltAtt')
