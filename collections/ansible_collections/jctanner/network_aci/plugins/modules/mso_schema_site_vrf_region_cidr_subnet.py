...
- name: Add a new site VRF region CIDR subnet
  jctanner.network_aci.mso_schema_template_vrf_region_cidr_subnet:
...
def main():
    ...
    if cidr not in cidrs:
        jctanner.network_aci.mso.fail_json(msg="Provided CIDR IP '{0}' does not exist. Existing CIDR IPs: {1}".format(cidr, ', '.join(cidrs)))
    ...
    if subnet is not None and subnet in subnets:
        ...
        jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['vrfs'][vrf_idx]['regions'][region_idx]['cidrs'][cidr_idx]['subnets'][subnet_idx]

    if state == 'query':
        if subnet is None:
            jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['vrfs'][vrf_idx]['regions'][region_idx]['cidrs'][cidr_idx]['subnets']
