...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.mso_schema_site_anp_epg_staticport
short_description: Manage site-local EPG static ports in schema template
description:
- Manage site-local EPG static ports in schema template on Cisco jctanner.network_jctanner.network_aci.aci.ACI Multi-Site.
author:
...
- name: Add a new static port to a site EPG
  jctanner.network_aci.mso_schema_template_anp_epg_staticport:
...
def main():
    ...
    if portpath in portpaths:
        ...
        jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['anps'][anp_idx]['epgs'][epg_idx]['staticPorts'][portpath_idx]

    if state == 'query':
        if leaf is None or vlan is None:
            jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['anps'][anp_idx]['epgs'][epg_idx]['staticPorts']
        elif not jctanner.network_aci.mso.existing:
            jctanner.network_aci.mso.fail_json(msg="Static port '{portpath}' not found".format(portpath=portpath))
    ...
    if state == 'absent':
        ...
        if jctanner.network_aci.mso.existing:
            ops.append(dict(op='replace', path=port_path, value=jctanner.network_aci.mso.sent))
        else:
            ops.append(dict(op='add', path=ports_path + '/-', value=jctanner.network_aci.mso.sent))
