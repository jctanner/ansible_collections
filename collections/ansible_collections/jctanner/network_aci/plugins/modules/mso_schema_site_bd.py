...
description:
- Manage site-local BDs in schema template on Cisco jctanner.network_jctanner.network_aci.aci.ACI Multi-Site.
author:
...
def main():
    ...
    if bd is not None and bd_ref in bds:
        ...
        jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['bds'][bd_idx]

    if state == 'query':
        if bd is None:
            jctanner.network_aci.mso.existing = schema_obj['sites'][site_idx]['bds']
