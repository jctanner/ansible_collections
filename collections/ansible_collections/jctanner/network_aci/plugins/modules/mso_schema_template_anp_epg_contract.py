...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.mso_schema_template_anp_epg_contract
short_description: Manage EPG contracts in schema templates
description:
- Manage EPG contracts in schema templates on Cisco jctanner.network_jctanner.network_aci.aci.ACI Multi-Site.
author:
...
from ansible_collections.jctanner.network_jctanner.network_aci.aci.plugins.module_utils.network.jctanner.network_aci.aci.jctanner.network_aci.mso import MSOModule, jctanner.network_aci.mso_argument_spec, jctanner.network_aci.mso_contractref_spec, issubset
...
def main():
    ...
    argument_spec.update(
        ...
        contract=dict(type='dict', options=jctanner.network_aci.mso_contractref_spec()),
    ...
    if contract:
        ...
        contract['schema_id'] = jctanner.network_aci.mso.lookup_schema(contract['schema'])
    ...
    if epg not in epgs:
        jctanner.network_aci.mso.fail_json(msg="Provided epg '{epg}' does not exist. Existing epgs: {epgs}".format(epg=epg, epgs=', '.join(epgs)))
    ...
    if contract:
        ...
        contract_ref = jctanner.network_aci.mso.contract_ref(**contract)
        if (contract_ref, contract['type']) in contracts:
            ...
            jctanner.network_aci.mso.existing = schema_obj['templates'][template_idx]['anps'][anp_idx]['epgs'][epg_idx]['contractRelationships'][contract_idx]

    if state == 'query':
        if not contract:
            jctanner.network_aci.mso.existing = schema_obj['templates'][template_idx]['anps'][anp_idx]['epgs'][epg_idx]['contractRelationships']
        elif not jctanner.network_aci.mso.existing:
            jctanner.network_aci.mso.fail_json(msg="Contract '{0}' not found".format(contract_ref))
    ...
    if state == 'absent':
        ...
        if jctanner.network_aci.mso.existing:
            ops.append(dict(op='replace', path=contract_path, value=jctanner.network_aci.mso.sent))
        else:
            ops.append(dict(op='add', path=contracts_path + '/-', value=jctanner.network_aci.mso.sent))
