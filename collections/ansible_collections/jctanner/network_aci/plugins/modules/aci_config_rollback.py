...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_config_rollback
short_description: Provides rollback and rollback preview functionality (config:ImportP)
description:
- Provides rollback and rollback preview functionality for Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
- Config Rollbacks are done using snapshots C(jctanner.network_aci.aci_snapshot) with the configImportP class.
version_added: '2.4'
options:
  ...
  snapshot:
    ...
    - The C(jctanner.network_aci.aci_snapshot) module can be used to query the list of available snapshots.
    type: str
    required: yes
  state:
    description:
...
seealso:
- module: jctanner.network_aci.aci_config_snapshot
...
- name: Create a Snapshot
  jctanner.network_aci.aci_config_snapshot:
    host: apic
    username: admin
...
- name: Compare Snapshot Files
  jctanner.network_aci.aci_config_rollback:
    host: apic
    username: admin
    password: SomeSecretPassword
    export_policy: config_backup
...
def main():
    ...
    fail_on_decrypt = jctanner.network_aci.aci.boolean(module.params['fail_on_decrypt'])
    ...
    if state == 'rollback':
        ...
        jctanner.network_aci.aci.construct_url(
            root_class=dict(
                jctanner.network_aci.aci_class='configImportP',
                jctanner.network_aci.aci_rn='fabric/configimp-{0}'.format(import_policy),
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='configImportP')

        jctanner.network_aci.aci.post_config()

    elif state == 'preview':
        jctanner.network_aci.aci.url = '%(protocol)s://%(host)s/mqapi2/snapshots.diff.xml' % module.params
        jctanner.network_aci.aci.filter_string = (
        ...
        get_preview(jctanner.network_aci.aci)

    jctanner.network_aci.aci.exit_json()


def get_preview(jctanner.network_aci.aci):
    '''
