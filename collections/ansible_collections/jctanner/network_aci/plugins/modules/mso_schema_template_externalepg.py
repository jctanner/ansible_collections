...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.mso_schema_template_externalepg
short_description: Manage external EPGs in schema templates
description:
- Manage external EPGs in schema templates on Cisco jctanner.network_jctanner.network_aci.aci.ACI Multi-Site.
author:
...
- name: Add a new external EPG
  jctanner.network_aci.mso_schema_template_externalepg:
...
def main():
    ...
    if externalepg is not None and externalepg in externalepgs:
        ...
        jctanner.network_aci.mso.existing = schema_obj['templates'][template_idx]['externalEpgs'][externalepg_idx]

    if state == 'query':
        if externalepg is None:
            jctanner.network_aci.mso.existing = schema_obj['templates'][template_idx]['externalEpgs']
        elif not jctanner.network_aci.mso.existing:
            jctanner.network_aci.mso.fail_json(msg="External EPG '{externalepg}' not found".format(externalepg=externalepg))
    ...
    if state == 'absent':
        ...
        if jctanner.network_aci.mso.existing:
            ops.append(dict(op='replace', path=eepg_path, value=jctanner.network_aci.mso.sent))
        else:
            ops.append(dict(op='add', path=eepgs_path + '/-', value=jctanner.network_aci.mso.sent))
