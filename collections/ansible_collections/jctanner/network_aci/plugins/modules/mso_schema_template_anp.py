...
description:
- Manage ANPs in schema templates on Cisco jctanner.network_jctanner.network_aci.aci.ACI Multi-Site.
author:
...
EXAMPLES = r'''
- name: Add a new ANP
  jctanner.network_aci.mso_schema_template_anp:
...
def main():
    ...
    if anp is not None and anp in anps:
        ...
        jctanner.network_aci.mso.existing = schema_obj['templates'][template_idx]['anps'][anp_idx]

    if state == 'query':
        if anp is None:
            jctanner.network_aci.mso.existing = schema_obj['templates'][template_idx]['anps']
