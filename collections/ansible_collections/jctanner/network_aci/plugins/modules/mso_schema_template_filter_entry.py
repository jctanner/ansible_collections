...
description:
- Manage filter entries in schema templates on Cisco jctanner.network_jctanner.network_aci.aci.ACI Multi-Site.
author:
...
- name: Add a new filter entry
  jctanner.network_aci.mso_schema_template_filter_entry:
...
def main():
    ...
    if template not in templates:
        jctanner.network_aci.mso.fail_json(msg="Provided template '{template}' does not exist. Existing templates: {templates}".format(template=template,
    ...
    if filter_name in filters:
        ...
        if entry in entries:
            ...
            jctanner.network_aci.mso.existing = schema_obj['templates'][template_idx]['filters'][filter_idx]['entries'][entry_idx]

    if state == 'query':
        if entry is None:
            if filter_idx is None:
                jctanner.network_aci.mso.fail_json(msg="Filter '{filter}' not found".format(filter=filter_name))
            jctanner.network_aci.mso.existing = schema_obj['templates'][template_idx]['filters'][filter_idx]['entries']
        elif not jctanner.network_aci.mso.existing:
            jctanner.network_aci.mso.fail_json(msg="Entry '{entry}' not found".format(entry=entry))
    ...
    if state == 'absent':
        ...
        if filter_idx is None:
            ...
            payload = dict(
                ...
                entries=[jctanner.network_aci.mso.sent],
            )
            ...
            ops.append(dict(op='add', path=entries_path + '/-', value=jctanner.network_aci.mso.sent))

        else:
            # Entry exists, we have to update it
            for (key, value) in jctanner.network_aci.mso.sent.items():
