...
description:
- Manage vlan, vxlan, and vsan pools on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.5'
options:
  description:
    description:
...
- module: jctanner.network_aci.aci_vlan_pool
...
- name: Add a new vlan pool
  jctanner.network_aci.aci_encap_pool:
    host: apic
    username: admin
...
def main():
    ...
    jctanner.network_aci.aci_class = jctanner.network_jctanner.network_aci.aci.ACI_POOL_MAPPING[pool_type]['jctanner.network_aci.aci_class']
    jctanner.network_aci.aci_mo = jctanner.network_jctanner.network_aci.aci.ACI_POOL_MAPPING[pool_type]['jctanner.network_aci.aci_mo']
    pool_name = pool

    # jctanner.network_jctanner.network_aci.aci.ACI Pool URL requires the pool_allocation mode for vlan and vsan pools (ex: uni/infra/vlanns-[poolname]-static)
    if pool_type != 'vxlan' and pool is not None:
        if pool_allocation_mode is not None:
            pool_name = '[{0}]-{1}'.format(pool, pool_allocation_mode)
        else:
            module.fail_json(msg="jctanner.network_jctanner.network_aci.aci.ACI requires parameter 'pool_allocation_mode' for 'pool_type' of 'vlan' and 'vsan' when parameter 'pool' is provided")
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            ...
            jctanner.network_aci.aci_rn='{0}{1}'.format(jctanner.network_aci.aci_mo, pool_name),
            module_object=pool,
