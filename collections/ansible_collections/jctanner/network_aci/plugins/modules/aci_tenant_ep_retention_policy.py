...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_tenant_ep_retention_policy
short_description: Manage End Point (EP) retention protocol policies (fv:EpRetPol)
description:
- Manage End Point (EP) retention protocol policies on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.4'
options:
  tenant:
    description:
...
- name: Add a new EPR policy
  jctanner.network_aci.aci_tenant_ep_retention_policy:
    host: apic
    username: admin
...
def main():
    ...
    jctanner.network_aci.aci.construct_url(
        ...
        subclass_1=dict(
            jctanner.network_aci.aci_class='fvEpRetPol',
            jctanner.network_aci.aci_rn='epRPol-{0}'.format(epr_policy),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='fvEpRetPol')
