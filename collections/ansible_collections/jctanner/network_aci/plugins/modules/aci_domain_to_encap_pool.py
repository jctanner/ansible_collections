...
description:
- Bind Domain to Encap Pools on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
notes:
- The C(domain) and C(encap_pool) parameters should exist before using this module.
  The M(jctanner.network_aci.aci_domain) and M(jctanner.network_aci.aci_encap_pool) can be used for these.
version_added: '2.5'
options:
  domain:
    description:
...
- name: Add domain to VLAN pool binding
  jctanner.network_aci.aci_domain_to_encap_pool:
    host: apic
    username: admin
...
POOL_MAPPING = dict(
    vlan=dict(
        jctanner.network_aci.aci_mo='uni/infra/vlanns-{0}',
        child_class='infraRsVlanNs',
    ),
    vxlan=dict(
        jctanner.network_aci.aci_mo='uni/infra/vxlanns-{0}',
        child_class='vmmRsVxlanNs',
    ),
    vsan=dict(
        jctanner.network_aci.aci_mo='uni/infra/vsanns-{0}',
...
def main():
    ...
    if pool_type != 'vxlan' and pool is not None:
        if pool_allocation_mode is not None:
            ...
            module.fail_json(msg="jctanner.network_jctanner.network_aci.aci.ACI requires the 'pool_allocation_mode' for 'pool_type' of 'vlan' and 'vsan' when 'pool' is provided")
    ...
    pool_mo = POOL_MAPPING[pool_type]['jctanner.network_aci.aci_mo'].format(pool_name)
