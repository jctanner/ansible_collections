...
description:
- Manage contract filters in schema templates on Cisco jctanner.network_jctanner.network_aci.aci.ACI Multi-Site.
author:
...
options:
  ...
  state:
    ...
    default: present
seealso:
- module: jctanner.network_aci.mso_schema_template_filter_entry
notes:
...
- name: Add a new contract filter
  jctanner.network_aci.mso_schema_template_contract_filter:
...
def main():
    ...
    filter_schema_id = jctanner.network_aci.mso.lookup_schema(filter_schema)
    ...
    if contract in contracts:
        ...
        filter_ref = jctanner.network_aci.mso.filter_ref(schema_id=filter_schema_id, template=filter_template, filter=filter_name)
        if filter_ref in filters:
            ...
            jctanner.network_aci.mso.existing = schema_obj['templates'][template_idx]['contracts'][contract_idx][filter_key][filter_idx]

    if state == 'query':
        if contract_idx is None:
            jctanner.network_aci.mso.fail_json(msg="Provided contract '{0}' does not exist. Existing contracts: {1}".format(contract, ', '.join(contracts)))

        if filter_name is None:
            jctanner.network_aci.mso.existing = schema_obj['templates'][template_idx]['contracts'][contract_idx][filter_key]
        elif not jctanner.network_aci.mso.existing:
            jctanner.network_aci.mso.fail_json(msg="FilterRef '{filter_ref}' not found".format(filter_ref=filter_ref))
    ...
    if state == 'absent':
        ...
        jctanner.network_aci.mso.existing = jctanner.network_aci.mso.sent
        ...
        if filter_idx is None:
            ...
            ops.append(dict(op='add', path=filters_path + '/-', value=jctanner.network_aci.mso.sent))

        else:
            # Filter exists, we have to update it
            ops.append(dict(op='replace', path=filter_path, value=jctanner.network_aci.mso.sent))
