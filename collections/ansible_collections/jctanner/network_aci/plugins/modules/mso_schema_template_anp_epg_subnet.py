...
description:
- Manage EPG subnets in schema templates on Cisco jctanner.network_jctanner.network_aci.aci.ACI Multi-Site.
author:
...
- name: Add a new subnet to an EPG
  jctanner.network_aci.mso_schema_template_anp_epg_subnet:
...
def main():
    ...
    if anp not in anps:
        jctanner.network_aci.mso.fail_json(msg="Provided anp '{anp}' does not exist. Existing anps: {anps}".format(anp=anp, anps=', '.join(anps)))
    ...
    if subnet in subnets:
        ...
        jctanner.network_aci.mso.existing = schema_obj['templates'][template_idx]['anps'][anp_idx]['epgs'][epg_idx]['subnets'][subnet_idx]

    if state == 'query':
        if subnet is None:
            jctanner.network_aci.mso.existing = schema_obj['templates'][template_idx]['anps'][anp_idx]['epgs'][epg_idx]['subnets']
