...
description:
- Manage Config Snapshots on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
...
def main():
    ...
    include_secure = jctanner.network_aci.aci.boolean(module.params['include_secure'])
    ...
    if state == 'present':
        jctanner.network_aci.aci.construct_url(
            root_class=dict(
                jctanner.network_aci.aci_class='configExportP',
                jctanner.network_aci.aci_rn='fabric/configexp-{0}'.format(export_policy),
        ...
        jctanner.network_aci.aci.get_diff('configExportP')
        ...
        jctanner.network_aci.aci.construct_url(
            root_class=dict(
                jctanner.network_aci.aci_class='configSnapshotCont',
                jctanner.network_aci.aci_rn='backupst/snapshots-[{0}]'.format(export_policy),
                module_object=export_policy,
                target_filter={'name': export_policy},
            ),
            subclass_1=dict(
                jctanner.network_aci.aci_class='configSnapshot',
                jctanner.network_aci.aci_rn='snapshot-{0}'.format(snapshot),
        ...
        if state == 'absent':
            ...
            if jctanner.network_aci.aci.existing:
                jctanner.network_aci.aci.get_diff('configSnapshot')
