...
DOCUMENTATION = r'''
---
module: jctanner.network_aci.aci_aaa_user_certificate
short_description: Manage AAA user certificates (aaa:UserCert)
description:
- Manage AAA user certificates on Cisco jctanner.network_jctanner.network_aci.aci.ACI fabrics.
version_added: '2.5'
options:
  aaa_user:
    description:
  ...
  certificate_name:
    description:
    - The name of the user certificate entry in jctanner.network_jctanner.network_aci.aci.ACI.
    type: str
    aliases: [ cert_name ]
  state:
    description:
...
- The C(aaa_user) must exist before using this module in your playbook.
  The M(jctanner.network_aci.aci_aaa_user) module can be used for this.
seealso:
- module: jctanner.network_aci.aci_aaa_user
...
jctanner.network_jctanner.network_aci.aci.ACI_MAPPING = dict(
    appuser=dict(
        jctanner.network_aci.aci_class='aaaAppUser',
        jctanner.network_aci.aci_mo='userext/appuser-',
    ),
    user=dict(
        jctanner.network_aci.aci_class='aaaUser',
        jctanner.network_aci.aci_mo='userext/user-',
    ),
)
...
def main():
    ...
    jctanner.network_aci.aci.construct_url(
        root_class=dict(
            jctanner.network_aci.aci_class=jctanner.network_jctanner.network_aci.aci.ACI_MAPPING[aaa_user_type]['jctanner.network_aci.aci_class'],
            jctanner.network_aci.aci_rn=jctanner.network_jctanner.network_aci.aci.ACI_MAPPING[aaa_user_type]['jctanner.network_aci.aci_mo'] + aaa_user,
            module_object=aaa_user,
            target_filter={'name': aaa_user},
        ),
        subclass_1=dict(
            jctanner.network_aci.aci_class='aaaUserCert',
            jctanner.network_aci.aci_rn='usercert-{0}'.format(certificate_name),
    ...
    if state == 'present':
        ...
        jctanner.network_aci.aci.get_diff(jctanner.network_aci.aci_class='aaaUserCert')
