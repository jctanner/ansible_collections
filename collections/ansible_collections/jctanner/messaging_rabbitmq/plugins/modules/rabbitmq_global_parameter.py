...
DOCUMENTATION = '''
---
module: jctanner.messaging_rabbitmq.rabbitmq_global_parameter
...
- jctanner.messaging_rabbitmq.rabbitmq_global_parameter:
    name: cluster_name
...
class RabbitMqGlobalParameter(object):
    ...
    def _exec(self, args, run_in_check_mode=False):
        if not self.module.check_mode or (self.module.check_mode and run_in_check_mode):
            cmd = [self._jctanner.messaging_rabbitmq.rabbitmqctl, '-q', '-n', self.node]
...
def main():
    ...
    jctanner.messaging_rabbitmq.rabbitmq_global_parameter = RabbitMqGlobalParameter(module, name, value, node)

    if jctanner.messaging_rabbitmq.rabbitmq_global_parameter.get():
        if state == 'absent':
            jctanner.messaging_rabbitmq.rabbitmq_global_parameter.delete()
            result['changed'] = True
        else:
            if jctanner.messaging_rabbitmq.rabbitmq_global_parameter.has_modifications():
                jctanner.messaging_rabbitmq.rabbitmq_global_parameter.set()
                result['changed'] = True
    elif state == 'present':
        jctanner.messaging_rabbitmq.rabbitmq_global_parameter.set()
        result['changed'] = True
