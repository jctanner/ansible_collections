...
DOCUMENTATION = '''
---
module: jctanner.messaging_rabbitmq.rabbitmq_policy
...
- name: ensure the default vhost contains the HA policy via a dict
  jctanner.messaging_rabbitmq.rabbitmq_policy:
    name: HA
    pattern: .*
  args:
    tags:
...
class RabbitMqPolicy(object):
    ...
    def _exec(self,
        ...
        if (not self._module.check_mode
            ...
            cmd = [self._jctanner.messaging_rabbitmq.rabbitmqctl, '-q', '-n', self._node]
    ...
    def has_modifications(self):
        ...
        if self._version and self._version >= Version('3.7.0'):
            # Change fields order in jctanner.messaging_rabbitmq.rabbitmqctl output in version 3.7
            return not any(
...
def main():
    ...
    jctanner.messaging_rabbitmq.rabbitmq_policy = RabbitMqPolicy(module, name)

    result = dict(changed=False, name=name, state=state)

    if state == 'present' and jctanner.messaging_rabbitmq.rabbitmq_policy.has_modifications():
        jctanner.messaging_rabbitmq.rabbitmq_policy.set()
        result['changed'] = True
    elif state == 'absent' and jctanner.messaging_rabbitmq.rabbitmq_policy.should_be_deleted():
        jctanner.messaging_rabbitmq.rabbitmq_policy.clear()
        result['changed'] = True
