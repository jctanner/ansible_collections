...
DOCUMENTATION = '''
---
module: jctanner.messaging_rabbitmq.rabbitmq_vhost_limits
...
class RabbitMqVhostLimits(object):
    ...
    def _exec(self, args):
        cmd = [self._jctanner.messaging_rabbitmq.rabbitmqctl, '-q', '-p', self._vhost]
...
def main():
    ...
    jctanner.messaging_rabbitmq.rabbitmq_vhost_limits = RabbitMqVhostLimits(module)
    current_status = jctanner.messaging_rabbitmq.rabbitmq_vhost_limits.list()
    ...
    if current_status != wanted_status:
        ...
        if state == 'present':
            jctanner.messaging_rabbitmq.rabbitmq_vhost_limits.set()
        else:  # state == 'absent'
            jctanner.messaging_rabbitmq.rabbitmq_vhost_limits.clear()
