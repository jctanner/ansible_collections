...
DOCUMENTATION = '''
---
module: jctanner.messaging_rabbitmq.rabbitmq_user
...
- jctanner.messaging_rabbitmq.rabbitmq_user:
    user: joe
    password: changeme
    vhost: /
    configure_priv: .*
...
class RabbitMqUser(object):
    def __init__(self, module, username, password, tags, permissions,
        ...
        self._tags = None
        self._permissions = []
        self._jctanner.messaging_rabbitmq.rabbitmqctl = module.get_bin_path('jctanner.messaging_rabbitmq.rabbitmqctl', True)

    def _exec(self, args, run_in_check_mode=False):
        if not self.module.check_mode or run_in_check_mode:
            cmd = [self._jctanner.messaging_rabbitmq.rabbitmqctl, '-q']
            if self.node:
...
def main():
    ...
    jctanner.messaging_rabbitmq.rabbitmq_user = RabbitMqUser(module, username, password, tags, permissions,
                                 node, bulk_permissions=bulk_permissions)

    result = dict(changed=False, user=username, state=state)
    if jctanner.messaging_rabbitmq.rabbitmq_user.get():
        if state == 'absent':
            jctanner.messaging_rabbitmq.rabbitmq_user.delete()
            result['changed'] = True
        else:
            if force:
                jctanner.messaging_rabbitmq.rabbitmq_user.delete()
                jctanner.messaging_rabbitmq.rabbitmq_user.add()
                jctanner.messaging_rabbitmq.rabbitmq_user.get()
                result['changed'] = True
            elif update_password == 'always':
                if not jctanner.messaging_rabbitmq.rabbitmq_user.check_password():
                    jctanner.messaging_rabbitmq.rabbitmq_user.change_password()
                    result['changed'] = True

            if jctanner.messaging_rabbitmq.rabbitmq_user.has_tags_modifications():
                jctanner.messaging_rabbitmq.rabbitmq_user.set_tags()
                result['changed'] = True

            if jctanner.messaging_rabbitmq.rabbitmq_user.has_permissions_modifications():
                jctanner.messaging_rabbitmq.rabbitmq_user.set_permissions()
                result['changed'] = True
    elif state == 'present':
        jctanner.messaging_rabbitmq.rabbitmq_user.add()
        jctanner.messaging_rabbitmq.rabbitmq_user.set_tags()
