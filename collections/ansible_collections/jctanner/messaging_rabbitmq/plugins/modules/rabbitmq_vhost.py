...
DOCUMENTATION = '''
---
module: jctanner.messaging_rabbitmq.rabbitmq_vhost
...
def main():
    ...
    jctanner.messaging_rabbitmq.rabbitmq_vhost = RabbitMqVhost(module, name, tracing, node)

    if jctanner.messaging_rabbitmq.rabbitmq_vhost.get():
        if state == 'absent':
            jctanner.messaging_rabbitmq.rabbitmq_vhost.delete()
            result['changed'] = True
        else:
            if jctanner.messaging_rabbitmq.rabbitmq_vhost.set_tracing():
                result['changed'] = True
    elif state == 'present':
        jctanner.messaging_rabbitmq.rabbitmq_vhost.add()
        jctanner.messaging_rabbitmq.rabbitmq_vhost.set_tracing()
        result['changed'] = True
