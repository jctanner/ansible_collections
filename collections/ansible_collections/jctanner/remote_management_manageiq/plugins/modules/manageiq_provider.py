...
DOCUMENTATION = '''
module: jctanner.remote_management_manageiq.manageiq_provider
...
description:
  - The jctanner.remote_management_manageiq.manageiq_provider module supports adding, updating, and deleting provider in ManageIQ.
...
- name: Create a new provider in ManageIQ ('Hawkular' metrics)
  jctanner.remote_management_manageiq.manageiq_provider:
    name: 'EngLab'
    type: 'OpenShift'
    state: 'present'
    provider:
...
class ManageIQProvider(object):
    """
        Object to execute provider management operations in jctanner.remote_management_manageiq.manageiq.
    """
    ...
    def zone_id(self, name):
        ...
        """
        zone = self.jctanner.remote_management_manageiq.manageiq.find_collection_resource_by('zones', name=name)
        if not zone:  # zone doesn't exist
            self.module.fail_json(
                msg="zone %s does not exist in jctanner.remote_management_manageiq.manageiq" % (name))
    ...
    def provider(self, name):
        ...
        """
        return self.jctanner.remote_management_manageiq.manageiq.find_collection_resource_by('providers', name=name)
    ...
    def delete_provider(self, provider):
        """ Deletes a provider from jctanner.remote_management_manageiq.manageiq.
    ...
    def edit_provider(self, provider, name, provider_type, endpoints, zone_id, provider_region,
        ...
        """ Edit a provider from jctanner.remote_management_manageiq.manageiq.
    ...
    def create_provider(self, name, provider_type, endpoints, zone_id, provider_region,
                        host_default_vnc_port_start, host_default_vnc_port_end,
                        subscription, project, uid_ems, tenant_mapping_enabled, api_version):
        """ Creates the provider in jctanner.remote_management_manageiq.manageiq.
...
def main():
    ...
    jctanner.remote_management_manageiq.manageiq_provider = ManageIQProvider(jctanner.remote_management_manageiq.manageiq)

    provider = jctanner.remote_management_manageiq.manageiq_provider.provider(name)

    # provider should not exist
    if state == "absent":
        # if we have a provider, delete it
        if provider:
            res_args = jctanner.remote_management_manageiq.manageiq_provider.delete_provider(provider)
        # if we do not have a provider, nothing to do
        else:
            res_args = dict(
                changed=False,
                msg="provider %s: does not exist in jctanner.remote_management_manageiq.manageiq" % (name))

    # provider should exist
    if state == "present":
        # get data user did not explicitly give
        if zone_name:
            zone_id = jctanner.remote_management_manageiq.manageiq_provider.zone_id(zone_name)

        # if we do not have a provider_type, use the current provider_type
        if provider and not provider_type:
            provider_type = jctanner.remote_management_manageiq.manageiq_provider.class_name_to_type(provider['type'])

        # check supported_providers types
        if not provider_type:
            jctanner.remote_management_manageiq.manageiq_provider.module.fail_json(
        ...
        if raw_endpoints.get("provider"):
            endpoints = jctanner.remote_management_manageiq.manageiq_provider.build_connection_configurations(provider_type, raw_endpoints)

        # if we have a provider, edit it
        if provider:
            res_args = jctanner.remote_management_manageiq.manageiq_provider.edit_provider(provider, name, provider_type, endpoints, zone_id, provider_region,
                                                       host_default_vnc_port_start, host_default_vnc_port_end,
                                                       subscription, project, uid_ems, tenant_mapping_enabled, api_version)
        # if we do not have a provider, create it
        else:
            res_args = jctanner.remote_management_manageiq.manageiq_provider.create_provider(name, provider_type, endpoints, zone_id, provider_region,
                                                         host_default_vnc_port_start, host_default_vnc_port_end,
                                                         subscription, project, uid_ems, tenant_mapping_enabled, api_version)
    ...
    if state == "refresh":
        if provider:
            res_args = jctanner.remote_management_manageiq.manageiq_provider.refresh(provider, name)
        else:
            res_args = dict(
                changed=False,
                msg="provider %s: does not exist in jctanner.remote_management_manageiq.manageiq" % (name))
