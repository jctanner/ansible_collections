...
module: jctanner.remote_management_manageiq.manageiq_alerts
...
description:
  - The jctanner.remote_management_manageiq.manageiq_alerts module supports adding, updating and deleting alerts in ManageIQ.
...
- name: Add an alert with a "hash expression" to ManageIQ
  jctanner.remote_management_manageiq.manageiq_alerts:
    state: present
...
class ManageIQAlerts(object):
    """ Object to execute alert management operations in jctanner.remote_management_manageiq.manageiq.
    """
    ...
    def create_alert_dict(self, params):
        ...
        if params['expression_type'] == 'hash':
            # hash expression supports depends on https://github.com/ManageIQ/jctanner.remote_management_manageiq.manageiq-api/pull/76
...
def main():
    ...
    jctanner.remote_management_manageiq.manageiq_alerts = ManageIQAlerts(jctanner.remote_management_manageiq.manageiq)

    existing_alert = jctanner.remote_management_manageiq.manageiq.find_collection_resource_by("alert_definitions",
                                                          description=description)

    # we need to add or update the alert
    if state == "present":
        alert = jctanner.remote_management_manageiq.manageiq_alerts.create_alert_dict(module.params)

        if not existing_alert:
            # an alert with this description doesn't exist yet, let's create it
            res_args = jctanner.remote_management_manageiq.manageiq_alerts.add_alert(alert)
        else:
            # an alert with this description exists, we might need to update it
            res_args = jctanner.remote_management_manageiq.manageiq_alerts.update_alert(existing_alert, alert)

    # this alert should not exist
    elif state == "absent":
        # if we have an alert with this description, delete it
        if existing_alert:
            res_args = jctanner.remote_management_manageiq.manageiq_alerts.delete_alert(existing_alert)
        else:
