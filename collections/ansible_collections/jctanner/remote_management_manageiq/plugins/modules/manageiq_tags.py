...
module: jctanner.remote_management_manageiq.manageiq_tags
...
description:
  - The jctanner.remote_management_manageiq.manageiq_tags module supports adding, updating and deleting tags in ManageIQ.
...
options:
  ...
  resource_type:
    description:
      - the relevant resource type in jctanner.remote_management_manageiq.manageiq
    required: true
  ...
  resource_name:
    description:
      - the relevant resource name in jctanner.remote_management_manageiq.manageiq
    required: true
'''

EXAMPLES = '''
- name: Create new tags for a provider in ManageIQ
  jctanner.remote_management_manageiq.manageiq_tags:
...
from ansible_collections.jctanner.remote_management_jctanner.remote_management_manageiq.manageiq.plugins.module_utils.jctanner.remote_management_manageiq.manageiq import ManageIQ, jctanner.remote_management_manageiq.manageiq_argument_spec, jctanner.remote_management_manageiq.manageiq_entities


def query_resource_id(jctanner.remote_management_manageiq.manageiq, resource_type, resource_name):
    """ Query the resource name in ManageIQ.

    Returns:
        the resource id if it exists in jctanner.remote_management_manageiq.manageiq, Fail otherwise.
    """
    resource = jctanner.remote_management_manageiq.manageiq.find_collection_resource_by(resource_type, name=resource_name)
    if resource:
        return resource["id"]
    else:
        msg = "{resource_name} {resource_type} does not exist in jctanner.remote_management_manageiq.manageiq".format(
            resource_name=resource_name, resource_type=resource_type)
        jctanner.remote_management_manageiq.manageiq.module.fail_json(msg=msg)


class ManageIQTags(object):
    """
        Object to execute tags management operations of jctanner.remote_management_manageiq.manageiq resources.
    """

    def __init__(self, jctanner.remote_management_manageiq.manageiq, resource_type, resource_id):
    ...
    def full_tag_name(self, tag):
        """ Returns the full tag name in jctanner.remote_management_manageiq.manageiq
        """
...
def main():
    ...
    argument_spec = dict(
        ...
        resource_type=dict(required=True, type='str',
                           choices=jctanner.remote_management_manageiq.manageiq_entities().keys()),
    ...
    resource_type = jctanner.remote_management_manageiq.manageiq_entities()[resource_type_key]

    jctanner.remote_management_manageiq.manageiq = ManageIQ(module)

    # query resource id, fail if resource does not exist
    resource_id = query_resource_id(jctanner.remote_management_manageiq.manageiq, resource_type, resource_name)

    jctanner.remote_management_manageiq.manageiq_tags = ManageIQTags(jctanner.remote_management_manageiq.manageiq, resource_type, resource_id)

    if action == 'list':
        # return a list of current tags for this object
        current_tags = jctanner.remote_management_manageiq.manageiq_tags.query_resource_tags()
        res_args = dict(changed=False, tags=current_tags)
    else:
        # assign or unassign the tags
        res_args = jctanner.remote_management_manageiq.manageiq_tags.assign_or_unassign_tags(tags, action)
