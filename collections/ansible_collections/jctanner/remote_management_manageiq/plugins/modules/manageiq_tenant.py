#!/usr/bin/python
#
# (c) 2018, Evert Mulder (base on jctanner.remote_management_manageiq.manageiq_user.py by Daniel Korn <korndaniel1@gmail.com>)
#
...
module: jctanner.remote_management_manageiq.manageiq_tenant
...
description:
  - The jctanner.remote_management_manageiq.manageiq_tenant module supports adding, updating and deleting tenants in ManageIQ.
requirements:
...
- name: Update the root tenant in ManageIQ
  jctanner.remote_management_manageiq.manageiq_tenant:
    name: 'My Company'
...
class ManageIQTenant(object):
    """
        Object to execute tenant management operations in jctanner.remote_management_manageiq.manageiq.
    """
    ...
    def tenant(self, name, parent_id, parent):
        ...
        if parent_id:
            ...
            if not parent_tenant_res:
                self.module.fail_json(msg="Parent tenant with id '%s' not found in jctanner.remote_management_manageiq.manageiq" % str(parent_id))
            ...
            if parent:
                ...
                if not parent_tenant_res:
                    self.module.fail_json(msg="Parent tenant '%s' not found in jctanner.remote_management_manageiq.manageiq" % parent)

                if len(parent_tenant_res) > 1:
                    self.module.fail_json(msg="Multiple parent tenants not found in jctanner.remote_management_manageiq.manageiq with name '%s" % parent)
    ...
    def delete_tenant(self, tenant):
        """ Deletes a tenant from jctanner.remote_management_manageiq.manageiq.
    ...
    def edit_tenant(self, tenant, name, description):
        """ Edit a jctanner.remote_management_manageiq.manageiq tenant.
    ...
    def create_tenant(self, name, description, parent_tenant):
        """ Creates the tenant in jctanner.remote_management_manageiq.manageiq.
    ...
    def update_tenant_quotas(self, tenant, quotas):
        """ Creates the tenant quotas in jctanner.remote_management_manageiq.manageiq.
    ...
    def edit_tenant_quota(self, tenant, current_quota, quota_key, quota_value):
        """ Update the tenant quotas in jctanner.remote_management_manageiq.manageiq.
    ...
    def delete_tenant_quota(self, tenant, quota):
        """ deletes the tenant quotas in jctanner.remote_management_manageiq.manageiq.
    ...
    def create_tenant_response(self, tenant, parent_tenant):
        """ Creates the ansible result object from a jctanner.remote_management_manageiq.manageiq tenant entity
    ...
    def create_tenant_quotas_response(tenant_quotas):
        """ Creates the ansible result object from a jctanner.remote_management_manageiq.manageiq tenant_quotas entity
...
def main():
    ...
    jctanner.remote_management_manageiq.manageiq_tenant = ManageIQTenant(jctanner.remote_management_manageiq.manageiq)

    parent_tenant, tenant = jctanner.remote_management_manageiq.manageiq_tenant.tenant(name, parent_id, parent)

    # tenant should not exist
    if state == "absent":
        # if we have a tenant, delete it
        if tenant:
            res_args = jctanner.remote_management_manageiq.manageiq_tenant.delete_tenant(tenant)
        # if we do not have a tenant, nothing to do
        else:
            if parent_id:
                msg = "tenant '%s' with parent_id %i does not exist in jctanner.remote_management_manageiq.manageiq" % (name, parent_id)
            else:
                msg = "tenant '%s' with parent '%s' does not exist in jctanner.remote_management_manageiq.manageiq" % (name, parent)
    ...
    if state == "present":
        ...
        if tenant:
            res_args = jctanner.remote_management_manageiq.manageiq_tenant.edit_tenant(tenant, name, description)

        # if we do not have a tenant, create it
        else:
            res_args = jctanner.remote_management_manageiq.manageiq_tenant.create_tenant(name, description, parent_tenant)
            tenant = jctanner.remote_management_manageiq.manageiq.client.get_entity('tenants', res_args['tenant_id'])

        # quotas as supplied and we have a tenant
        if quotas:
            tenant_quotas_res = jctanner.remote_management_manageiq.manageiq_tenant.update_tenant_quotas(tenant, quotas)
        ...
        res_args['tenant'] = jctanner.remote_management_manageiq.manageiq_tenant.create_tenant_response(tenant, parent_tenant)
