...
module: jctanner.remote_management_manageiq.manageiq_policies
...
description:
  - The jctanner.remote_management_manageiq.manageiq_policies module supports adding and deleting policy_profiles in ManageIQ.
...
- name: Assign new policy_profile for a provider in ManageIQ
  jctanner.remote_management_manageiq.manageiq_policies:
...
class ManageIQPolicies(object):
    """
        Object to execute policies management operations of jctanner.remote_management_manageiq.manageiq resources.
    """
    ...
    def query_profile_href(self, profile):
        ...
        """
        resource = self.jctanner.remote_management_manageiq.manageiq.find_collection_resource_or_fail(
...
def main():
    ...
    resource_id = jctanner.remote_management_manageiq.manageiq.find_collection_resource_or_fail(resource_type, name=resource_name)['id']

    jctanner.remote_management_manageiq.manageiq_policies = ManageIQPolicies(jctanner.remote_management_manageiq.manageiq, resource_type, resource_id)

    if action == 'list':
        # return a list of current profiles for this object
        current_profiles = jctanner.remote_management_manageiq.manageiq_policies.query_resource_profiles()
        res_args = dict(changed=False, profiles=current_profiles)
    else:
        # assign or unassign the profiles
        res_args = jctanner.remote_management_manageiq.manageiq_policies.assign_or_unassign_profiles(policy_profiles, action)
