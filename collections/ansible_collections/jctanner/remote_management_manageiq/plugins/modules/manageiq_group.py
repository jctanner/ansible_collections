#!/usr/bin/python
#
# (c) 2018, Evert Mulder <evertmulder@gmail.com> (base on jctanner.remote_management_manageiq.manageiq_user.py by Daniel Korn <korndaniel1@gmail.com>)
#
...
module: jctanner.remote_management_manageiq.manageiq_group

short_description: Management of groups in ManageIQ.
extends_documentation_fragment: jctanner.remote_management_manageiq.manageiq
version_added: '2.8'
author: Evert Mulder (@evertmulder)
description:
  - The jctanner.remote_management_manageiq.manageiq_group module supports adding, updating and deleting groups in ManageIQ.
requirements:
- jctanner.remote_management_manageiq.manageiq-client
...
- name: Create a group in ManageIQ with the role EvmRole-user and tenant 'my_tenant'
  jctanner.remote_management_manageiq.manageiq_group:
    description: 'MyGroup-user'
    role: 'EvmRole-user'
    tenant: 'my_tenant'
    jctanner.remote_management_manageiq.manageiq_connection:
      url: 'https://jctanner.remote_management_manageiq.manageiq_server'
      username: 'admin'
      password: 'smartvm'
...
from ansible_collections.jctanner.remote_management_jctanner.remote_management_manageiq.manageiq.plugins.module_utils.jctanner.remote_management_manageiq.manageiq import ManageIQ, jctanner.remote_management_manageiq.manageiq_argument_spec


class ManageIQgroup(object):
    """
        Object to execute group management operations in jctanner.remote_management_manageiq.manageiq.
    """

    def __init__(self, jctanner.remote_management_manageiq.manageiq):
        self.jctanner.remote_management_manageiq.manageiq = jctanner.remote_management_manageiq.manageiq

        self.module = self.jctanner.remote_management_manageiq.manageiq.module
        self.api_url = self.jctanner.remote_management_manageiq.manageiq.api_url
        self.client = self.jctanner.remote_management_manageiq.manageiq.client
    ...
    def tenant(self, tenant_id, tenant_name):
        ...
        if tenant_id:
            ...
            if not tenant:
                self.module.fail_json(msg="Tenant with id '%s' not found in jctanner.remote_management_manageiq.manageiq" % str(tenant_id))
            return tenant
        else:
            if tenant_name:
                tenant_res = self.client.collections.tenants.find_by(name=tenant_name)
                if not tenant_res:
                    self.module.fail_json(msg="Tenant '%s' not found in jctanner.remote_management_manageiq.manageiq" % tenant_name)
                if len(tenant_res) > 1:
                    self.module.fail_json(msg="Multiple tenants found in jctanner.remote_management_manageiq.manageiq with name '%s" % tenant_name)
                tenant = tenant_res[0]
                return tenant
            else:
    ...
    def role(self, role_id, role_name):
        ...
        if role_id:
            ...
            if not role:
                self.module.fail_json(msg="Role with id '%s' not found in jctanner.remote_management_manageiq.manageiq" % str(role_id))
            return role
        else:
            if role_name:
                role_res = self.client.collections.roles.find_by(name=role_name)
                if not role_res:
                    self.module.fail_json(msg="Role '%s' not found in jctanner.remote_management_manageiq.manageiq" % role_name)
                if len(role_res) > 1:
                    self.module.fail_json(msg="Multiple roles found in jctanner.remote_management_manageiq.manageiq with name '%s" % role_name)
                return role_res[0]
            else:
    ...
    def merge_dict_values(norm_current_values, norm_updated_values):
        """ Create an merged update object for jctanner.remote_management_manageiq.manageiq group filters.
    ...
    def delete_group(self, group):
        """ Deletes a group from jctanner.remote_management_manageiq.manageiq.
    ...
    def edit_group(self, group, description, role, tenant, norm_managed_filters, managed_filters_merge_mode,
        ...
        """ Edit a jctanner.remote_management_manageiq.manageiq group.
    ...
    def edit_group_edit_filters(self, current_filters, norm_managed_filters, managed_filters_merge_mode,
                                belongsto_filters, belongsto_filters_merge_mode):
        """ Edit a jctanner.remote_management_manageiq.manageiq group filters.
        ...
        # We normalize the current jctanner.remote_management_manageiq.manageiq array of arrays also to a dict of sorted arrays so we can compare
        norm_current_filters = self.jctanner.remote_management_manageiq.manageiq_filters_to_sorted_dict(current_filters)
    ...
    def create_group(self, description, role, tenant, norm_managed_filters, belongsto_filters):
        """ Creates the group in jctanner.remote_management_manageiq.manageiq.
    ...
    @staticmethod
    def jctanner.remote_management_manageiq.manageiq_filters_to_sorted_dict(current_filters):
    ...
    def create_result_group(group):
        """ Creates the ansible result object from a jctanner.remote_management_manageiq.manageiq group entity
...
def main():
    ...
    # add the jctanner.remote_management_manageiq.manageiq connection arguments to the arguments
    argument_spec.update(jctanner.remote_management_manageiq.manageiq_argument_spec())
    ...
    jctanner.remote_management_manageiq.manageiq = ManageIQ(module)
    jctanner.remote_management_manageiq.manageiq_group = ManageIQgroup(jctanner.remote_management_manageiq.manageiq)

    group = jctanner.remote_management_manageiq.manageiq_group.group(description)

    # group should not exist
    if state == "absent":
        # if we have a group, delete it
        if group:
            res_args = jctanner.remote_management_manageiq.manageiq_group.delete_group(group)
        # if we do not have a group, nothing to do
        else:
            res_args = dict(
                changed=False,
                msg="group '%s' does not exist in jctanner.remote_management_manageiq.manageiq" % description)

    # group should exist
    if state == "present":

        tenant = jctanner.remote_management_manageiq.manageiq_group.tenant(tenant_id, tenant_name)
        role = jctanner.remote_management_manageiq.manageiq_group.role(role_id, role_name)
        norm_managed_filters = jctanner.remote_management_manageiq.manageiq_group.normalize_user_managed_filters_to_sorted_dict(managed_filters, module)
        # if we have a group, edit it
        if group:
            res_args = jctanner.remote_management_manageiq.manageiq_group.edit_group(group, description, role, tenant,
                                                 norm_managed_filters, managed_filters_merge_mode,
                                                 belongsto_filters, belongsto_filters_merge_mode)

        # if we do not have a group, create it
        else:
            res_args = jctanner.remote_management_manageiq.manageiq_group.create_group(description, role, tenant, norm_managed_filters, belongsto_filters)
            group = jctanner.remote_management_manageiq.manageiq.client.get_entity('groups', res_args['group_id'])

        group.reload(expand='resources', attributes=['miq_user_role_name', 'tenant', 'entitlement'])
        res_args['group'] = jctanner.remote_management_manageiq.manageiq_group.create_result_group(group)
