...
module: jctanner.remote_management_manageiq.manageiq_alert_profiles
...
description:
  - The jctanner.remote_management_manageiq.manageiq_alert_profiles module supports adding, updating and deleting alert profiles in ManageIQ.
...
- name: Add an alert profile to ManageIQ
  jctanner.remote_management_manageiq.manageiq_alert_profiles:
    state: present
    name: Test profile
...
class ManageIQAlertProfiles(object):
    """ Object to execute alert profile management operations in jctanner.remote_management_manageiq.manageiq.
    """
    ...
    def get_alerts(self, alert_descriptions):
        ...
        for alert_description in alert_descriptions:
            alert = self.jctanner.remote_management_manageiq.manageiq.find_collection_resource_or_fail("alert_definitions",
...
def main():
    ...
    jctanner.remote_management_manageiq.manageiq_alert_profiles = ManageIQAlertProfiles(jctanner.remote_management_manageiq.manageiq)

    existing_profile = jctanner.remote_management_manageiq.manageiq.find_collection_resource_by("alert_definition_profiles",
                                                            name=name)
    ...
    if state == "present":
        if not existing_profile:
            ...
            res_args = jctanner.remote_management_manageiq.manageiq_alert_profiles.add_profile(module.params)
        else:
            # a profile with this name exists, we might need to update it
            res_args = jctanner.remote_management_manageiq.manageiq_alert_profiles.update_profile(existing_profile, module.params)

    # this alert profile should not exist
    if state == "absent":
        # if we have an alert profile with this name, delete it
        if existing_profile:
            res_args = jctanner.remote_management_manageiq.manageiq_alert_profiles.delete_profile(existing_profile)
        else:
