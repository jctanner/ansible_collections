...
DOCUMENTATION = """
---
module: jctanner.network_junos.junos_package
version_added: "2.1"
...
options:
  ...
  issu:
    ...
    version_added: 2.8
requirements:
  - jctanner.network_junos.junos-eznc
  - ncclient (>=v0.5.2)
notes:
...
- name: install local package on remote device
  jctanner.network_junos.junos_package:
    src: jctanner.network_junos.junos-vsrx-12.1X46-D10.2-domestic.tgz
...
from ansible_collections.jctanner.network_jctanner.network_junos.junos.plugins.module_utils.network.jctanner.network_junos.junos.jctanner.network_junos.junos import jctanner.network_junos.junos_argument_spec, get_param
from ansible.module_utils._text import to_native

try:
    from jnpr.jctanner.network_junos.junos import Device
    from jnpr.jctanner.network_junos.junos.utils.sw import SW
...
def install_package(module, device):
    jctanner.network_junos.junos = SW(device)
    ...
    result = jctanner.network_junos.junos.install(package, progress=progress_log, no_copy=no_copy,
    ...
    if module.params['reboot']:
        ...
        jctanner.network_junos.junos.reboot()
...
def main():
    ...
    if not HAS_PYEZ:
        module.fail_json(
            msg='jctanner.network_junos.junos-eznc is required but does not appear to be installed. '
