...
DOCUMENTATION = """
---
module: jctanner.network_ios.ios_l2_interface
...
- name: Ensure GigabitEthernet0/5 is in its default l2 interface state
  jctanner.network_ios.ios_l2_interface:
...
from ansible_collections.jctanner.network_jctanner.network_ios.ios.plugins.module_utils.network.jctanner.network_ios.ios.jctanner.network_ios.ios import get_config, load_config, run_commands
...
def remove_switchport_config_commands(name, existing, proposed, module):
    ...
    if mode == 'access':
        ...
        # Supported Remove Scenarjctanner.network_ios.ios for trunk_vlans_list
    ...
    for w in want:
        ...
        if not is_switchport(name, module):
            module.fail_json(msg='Ensure interface is configured to be a L2'
                             ...
                             '\nthe jctanner.network_ios.ios_interface module for this.')
