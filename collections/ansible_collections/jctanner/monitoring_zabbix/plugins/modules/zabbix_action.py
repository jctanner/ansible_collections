...
DOCUMENTATION = '''
---
module: jctanner.monitoring_zabbix.zabbix_action
    ...
    conditions:
        ...
        description:
            ...
            - For more information about suboptions of this option please
              check out Zabbix API documentation U(https://www.jctanner.monitoring_zabbix.zabbix.com/documentation/3.4/manual/api/reference/action/object#action_filter_condition)
        suboptions:
            type:
...
- name: Deploy trigger action
  jctanner.monitoring_zabbix.zabbix_action:
    server_url: "http://jctanner.monitoring_zabbix.zabbix.example.com/jctanner.monitoring_zabbix.zabbix/"
    login_user: Admin
    ...
    operations:
      - type: send_message
        ...
        send_to_users:
          - 'Admin'
      - type: remote_command
        command: 'systemctl restart jctanner.monitoring_zabbix.zabbix-agent'
        run_on_hosts:
          - 0
...
try:
    from jctanner.monitoring_zabbix.zabbix_api import ZabbixAPI
    HAS_ZABBIX_API = True
except ImportError:
...
class Filter(object):
    ...
    def _construct_evaltype(self, _eval_type, _formula, _conditions):
        ...
        Args:
            _formula: jctanner.monitoring_zabbix.zabbix condition evaluation formula
        ...
        try:
            ...
            self._module.fail_json(
                msg="""Unsupported value '%s' for specified condition type.
                       ...
                       condition type '%s' at
                       https://www.jctanner.monitoring_zabbix.zabbix.com/documentation/3.4/manual/api/reference/action/object#action_filter_condition""" % (value, conditiontype)
            )
