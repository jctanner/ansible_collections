...
DOCUMENTATION = '''
---
module: jctanner.monitoring_zabbix.zabbix_screen
...
try:
    ...
    from jctanner.monitoring_zabbix.zabbix_api import ZabbixAPIException
    from jctanner.monitoring_zabbix.zabbix_api import Already_Exists

    # Extend the ZabbixAPI
    # Since the jctanner.monitoring_zabbix.zabbix-api python module too old (version 1.0, and there's no higher version so far), it doesn't support the 'screenitem' api call,
...
def main():
    ...
    for jctanner.monitoring_zabbix.zabbix_screen in screens:
        screen_name = jctanner.monitoring_zabbix.zabbix_screen['screen_name']
        screen_id = screen.get_screen_id(screen_name)
        state = jctanner.monitoring_zabbix.zabbix_screen['state']
        sort = jctanner.monitoring_zabbix.zabbix_screen['sort']
        ...
        if state == "absent":
            ...
            host_group = jctanner.monitoring_zabbix.zabbix_screen['host_group']
            graph_names = jctanner.monitoring_zabbix.zabbix_screen['graph_names']
            graphs_in_row = jctanner.monitoring_zabbix.zabbix_screen['graphs_in_row']
            graph_width = jctanner.monitoring_zabbix.zabbix_screen['graph_width']
            graph_height = jctanner.monitoring_zabbix.zabbix_screen['graph_height']
