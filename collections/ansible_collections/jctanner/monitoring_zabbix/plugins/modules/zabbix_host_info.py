...
---
hosts:
  description: List of Zabbix hosts. See https://www.jctanner.monitoring_zabbix.zabbix.com/documentation/3.4/manual/api/reference/host/get for list of host values.
  returned: success
  type: dict
...
DOCUMENTATION = '''
---
module: jctanner.monitoring_zabbix.zabbix_host_info
short_description: Gather information about Zabbix host
description:
   - This module allows you to search for Zabbix host entries.
   - This module was called C(jctanner.monitoring_zabbix.zabbix_host_facts) before Ansible 2.9. The usage did not change.
version_added: "2.7"
author:
    - "Michael Miko (@RedWhiteMiko)"
requirements:
    - "python >= 2.6"
    - jctanner.monitoring_zabbix.zabbix-api
options:
    host_name:
        description:
...
class Host(object):
    ...
    def delete_duplicate_hosts(self, hosts):
        ...
        unique_hosts = []
        listed_hostnames = []
        for jctanner.monitoring_zabbix.zabbix_host in hosts:
            if jctanner.monitoring_zabbix.zabbix_host['name'] in listed_hostnames:
                continue
            unique_hosts.append(jctanner.monitoring_zabbix.zabbix_host)
            listed_hostnames.append(jctanner.monitoring_zabbix.zabbix_host['name'])
        return unique_hosts
...
def main():
    ...
    if module._name == 'jctanner.monitoring_zabbix.zabbix_host_facts':
        module.deprecate("The 'jctanner.monitoring_zabbix.zabbix_host_facts' module has been renamed to 'jctanner.monitoring_zabbix.zabbix_host_info'", version='2.13')
    ...
    if host_name:
        ...
        extended_hosts = []
        for jctanner.monitoring_zabbix.zabbix_host in hosts:
            jctanner.monitoring_zabbix.zabbix_host['hostinterfaces'] = host._zapi.hostinterface.get({
                'output': 'extend', 'hostids': jctanner.monitoring_zabbix.zabbix_host['hostid']
            })
            extended_hosts.append(jctanner.monitoring_zabbix.zabbix_host)
