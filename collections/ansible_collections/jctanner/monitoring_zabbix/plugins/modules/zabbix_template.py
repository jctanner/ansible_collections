...
DOCUMENTATION = '''
---
module: jctanner.monitoring_zabbix.zabbix_template
...
options:
    ...
    clear_templates:
        description:
            ...
            - See templates_clear in https://www.jctanner.monitoring_zabbix.zabbix.com/documentation/3.0/manual/api/reference/template/update
        required: false
    macros:
        description:
...
- name: Create Zabbix template using linked template
  local_action:
    ...
    template_json: "{'jctanner.monitoring_zabbix.zabbix_export': {}}"
    template_groups:
      - Role
      - Role2
    link_templates:
...
- name: Import Zabbix JSON template configuration
  local_action:
    ...
    template_json: "{{ lookup('file', 'jctanner.monitoring_zabbix.zabbix_apache2.json') }}"
    template_groups:
      - Webservers
    state: present
...
- name: Import Zabbix template
  jctanner.monitoring_zabbix.zabbix_template:
    login_user: username
    login_password: password
    server_url: http://127.0.0.1
...
try:
    from jctanner.monitoring_zabbix.zabbix_api import ZabbixAPI, ZabbixAPIException
...
class Template(object):
    ...
    def update_template(self, templateids, template_json,
        ...
        if do_import:
            self.import_template(template_json,
                                 existing_template_json['jctanner.monitoring_zabbix.zabbix_export']['templates'][0]['template'])
    ...
    def diff_template(self, template_json_a, template_json_b):
        # Compare 2 jctanner.monitoring_zabbix.zabbix templates and return True if they differ.
    ...
    def filter_template(self, template_json):
        ...
        unwanted_keys = set(template_json['jctanner.monitoring_zabbix.zabbix_export']) - keep_keys
        for unwanted_key in unwanted_keys:
            del template_json['jctanner.monitoring_zabbix.zabbix_export'][unwanted_key]
        ...
        for template in template_json['jctanner.monitoring_zabbix.zabbix_export']['templates']:
    ...
    def import_template(self, template_json, template_name=None):
        parsed_template_json = self.load_json_template(template_json)
        if template_name != parsed_template_json['jctanner.monitoring_zabbix.zabbix_export']['templates'][0]['template']:
        ...
        try:
            ...
            # updateExisting for application removed from jctanner.monitoring_zabbix.zabbix api after 3.2
...
def main():
    ...
    if template_json and not template_name:
        ...
        template_name = template_loaded['jctanner.monitoring_zabbix.zabbix_export']['templates'][0]['template']
    ...
    if state == "absent":
        ...
        if link_templates is not None:
            ...
            if existing_template_json:
                existing_child_templates = set(list(
                    tmpl['name'] for tmpl in existing_template_json['jctanner.monitoring_zabbix.zabbix_export']['templates'][0]['templates']))
        ...
        if template_groups is not None:
            ...
            if existing_template_json:
                existing_groups = set(list(group['name'] for group in existing_template_json['jctanner.monitoring_zabbix.zabbix_export']['groups']))
        ...
        if template_macros is not None:
            ...
            if existing_template_json:
                existing_macros = existing_template_json['jctanner.monitoring_zabbix.zabbix_export']['templates'][0]['macros']
