...
DOCUMENTATION = '''
---
module: jctanner.cloud_docker.docker_network_info

short_description: Retrieves facts about jctanner.cloud_docker.docker network

description:
  - Retrieves facts about a jctanner.cloud_docker.docker network.
  - Essentially returns the output of C(jctanner.cloud_docker.docker network inspect <name>), similar to what M(jctanner.cloud_docker.docker_network)
...
- name: Get infos on network
  jctanner.cloud_docker.docker_network_info:
    name: mydata
  register: result
...
network:
    description:
      - Facts representing the current state of the network. Matches the jctanner.cloud_docker.docker inspection output.
