...
module: jctanner.cloud_docker.docker_compose

short_description: Manage jctanner.cloud_docker.docker services and containers.
...
description:
  - Consumes jctanner.cloud_docker.docker compose to start, shutdown and scale services.
  - Works with compose versions 1 and 2.
  - Compose can be read from a jctanner.cloud_docker.docker-compose.yml (or .yaml) file or inline using the C(definition) option.
  - See the examples for more details.
  - Supports check mode.
  - This module was called C(jctanner.cloud_docker.docker_service) before Ansible 2.8. The usage did not change.

options:
  project_src:
    description:
      - Path to a directory containing a jctanner.cloud_docker.docker-compose.yml or jctanner.cloud_docker.docker-compose.yaml file.
  ...
  files:
    description:
      - List of file names relative to C(project_src). Overrides jctanner.cloud_docker.docker-compose.yml or jctanner.cloud_docker.docker-compose.yaml.
      - Files are loaded and merged in the order given.
    type: list
  state:
    description:
      - Desired state of the project.
      - Specifying I(present) is the same as running I(jctanner.cloud_docker.docker-compose up).
      - Specifying I(absent) is the same as running I(jctanner.cloud_docker.docker-compose down).
    type: str
    default: present
    choices:
      - absent
  ...
  services:
    description:
      - When C(state) is I(present) run I(jctanner.cloud_docker.docker-compose up) on a subset of services.
    type: list
  scale:
    description:
  ...
  definition:
    description:
      - Provide jctanner.cloud_docker.docker-compose yaml describing one or more services, networks and volumes.
  ...
  build:
    description:
      ...
      - Same as running jctanner.cloud_docker.docker-compose build with the pull option.
  ...
  pull:
    description:
      ...
      - Same as running jctanner.cloud_docker.docker-compose pull.
...
requirements:
  ...
  - "jctanner.cloud_docker.docker-compose >= 1.7.0"
  - "Docker API >= 1.20"
  - "PyYAML >= 3.11"
'''

EXAMPLES = '''
# Examples use the django example at U(https://docs.jctanner.cloud_docker.docker.com/compose/django/). Follow it to create the flask
# directory
...
- name: Run using a project directory
  ...
  gather_facts: no
  tasks:
    - jctanner.cloud_docker.docker_compose:
        project_src: flask
        state: absent
...
AUTH_PARAM_MAPPING = {
    u'jctanner.cloud_docker.docker_host': u'--host',
    u'tls': u'--tls',
...
class ContainerManager(DockerBaseClass):
    ...
    def __init__(self, client):
        ...
        if not HAS_COMPOSE:
            self.client.fail("Unable to load jctanner.cloud_docker.docker-compose. Try `pip install jctanner.cloud_docker.docker-compose`. Error: %s" %
                             HAS_COMPOSE_EXC)

        if LooseVersion(compose_version) < LooseVersion(MINIMUM_COMPOSE_VERSION):
            self.client.fail("Found jctanner.cloud_docker.docker-compose version %s. Minimum required version is %s. "
                             "Upgrade jctanner.cloud_docker.docker-compose to a min version of %s." %
        ...
        if self.definition:
            ...
            compose_file = os.path.join(self.project_src, "jctanner.cloud_docker.docker-compose.yml")
            try:
                self.log('writing: ')
...
def main():
    ...
    if client.module._name == 'jctanner.cloud_docker.docker_service':
        client.module.deprecate("The 'jctanner.cloud_docker.docker_service' module has been renamed to 'jctanner.cloud_docker.docker_compose'.", version='2.12')
