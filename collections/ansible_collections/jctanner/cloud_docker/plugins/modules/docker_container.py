...
DOCUMENTATION = '''
---
module: jctanner.cloud_docker.docker_container

short_description: manage jctanner.cloud_docker.docker containers

description:
  - Manage the life cycle of jctanner.cloud_docker.docker containers.
...
options:
  ...
  log_driver:
    description:
      ...
      - See L(here,https://docs.jctanner.cloud_docker.docker.com/config/containers/logging/configure/) for possible choices.
    type: str
  log_options:
    description:
      - Dictionary of options specific to the chosen log_driver. See https://docs.jctanner.cloud_docker.docker.com/engine/admin/logging/overview/
        for details.
    type: dict
    aliases:
      - log_opt
  ...
  networks:
    description:
      ...
      - Note that as opposed to C(jctanner.cloud_docker.docker run ...), M(jctanner.cloud_docker.docker_container) does not remove the default
  ...
  networks_cli_compatible:
    description:
      - "When networks are provided to the module via the I(networks) option, the module
         behaves differently than C(jctanner.cloud_docker.docker run --network): C(jctanner.cloud_docker.docker run --network other)
      ...
      - "If I(networks_cli_compatible) is set to C(yes), this module will behave as
         C(jctanner.cloud_docker.docker run --network) and will I(not) add the default network if C(networks) is
         specified. If C(networks) is not specified, the default network will be attached."
      - "Note that jctanner.cloud_docker.docker CLI also sets C(network_mode) to the name of the first network
         added if C(--network) is specified. For more compatibility with jctanner.cloud_docker.docker CLI, you
  ...
  pid_mode:
    description:
      ...
      - Note that Docker SDK for Python < 2.0 only supports 'host'. Newer versions of the
        Docker SDK for Python (jctanner.cloud_docker.docker) allow all values supported by the jctanner.cloud_docker.docker daemon.
    type: str
  pids_limit:
    description:
  ...
  published_ports:
    description:
      ...
      - "Use jctanner.cloud_docker.docker CLI syntax: C(8000), C(9000:8000), or C(0.0.0.0:9000:8000), where 8000 is a
      ...
      - "Bind addresses must be either IPv4 or IPv6 addresses. Hostnames are I(not) allowed. This
        is different from the C(jctanner.cloud_docker.docker) command line utility. Use the L(dig lookup,../lookup/dig.html)
        to resolve hostnames."
      - A value of C(all) will publish all exposed container ports to random host ports, ignoring
        any other mappings.
      - If C(networks) parameter is provided, will inspect each network to see if there exists
        a bridge network with optional parameter com.jctanner.cloud_docker.docker.network.bridge.host_binding_ipv4.
        If such a network is found, then published ports where no host IP address is specified
        will be bound to the host IP pointed to by com.jctanner.cloud_docker.docker.network.bridge.host_binding_ipv4.
        Note that the first bridge network with a com.jctanner.cloud_docker.docker.network.bridge.host_binding_ipv4
  ...
  stop_timeout:
    description:
      ...
      - When the container is stopped, will be used as a timeout for stopping the
        ...
        the behavior depends on the version of the jctanner.cloud_docker.docker daemon. New versions of
        the jctanner.cloud_docker.docker daemon will always use the container's configured C(StopTimeout)
  ...
  volumes:
    description:
      ...
      - "Use jctanner.cloud_docker.docker CLI-style syntax: C(/host:/container[:mode])"
      - "Mount modes can be a comma-separated list of various modes such as C(ro), C(rw), C(consistent),
        ...
        Note that the jctanner.cloud_docker.docker daemon might not support all modes and combinations of such modes."
...
- name: Create a data container
  jctanner.cloud_docker.docker_container:
    name: mydata
    image: busybox
    volumes:
      - /data
...
- name: Syslogging output
  jctanner.cloud_docker.docker_container:
    ...
    log_options:
      ...
      # older jctanner.cloud_docker.docker installs, use "syslog-tag" instead
      tag: myservice
...
- name: Create db container and connect to network
  jctanner.cloud_docker.docker_container:
    ...
    networks:
      - name: "{{ jctanner.cloud_docker.docker_network_name }}"
...
container:
    description:
      ...
      - Note that facts are part of the registered vars since Ansible 2.8. For compatibility reasons, the facts
        are also accessible directly as C(jctanner.cloud_docker.docker_container). Note that the returned fact will be removed in Ansible 2.12.
      - Before 2.3 this was C(ansible_jctanner.cloud_docker.docker_container) but was renamed in 2.3 to C(jctanner.cloud_docker.docker_container) due to
...
try:
    from jctanner.cloud_docker.docker import utils
    from ansible_collections.jctanner.cloud_jctanner.cloud_docker.docker.plugins.module_utils.jctanner.cloud_docker.docker.common import jctanner.cloud_docker.docker_version
    if LooseVersion(jctanner.cloud_docker.docker_version) >= LooseVersion('1.10.0'):
        from jctanner.cloud_docker.docker.types import Ulimit, LogConfig
    else:
        from jctanner.cloud_docker.docker.utils.types import Ulimit, LogConfig
    from jctanner.cloud_docker.docker.errors import APIError, NotFound
except Exception:
...
class TaskParameters(DockerBaseClass):
    ...
    def create_parameters(self):
        ...
        if self.client.jctanner.cloud_docker.docker_py_version < LooseVersion('3.0'):
    ...
    def _host_config(self):
        ...
        if self.client.jctanner.cloud_docker.docker_py_version >= LooseVersion('1.9') and self.client.jctanner.cloud_docker.docker_api_version >= LooseVersion('1.22'):
        ...
        for net in self.networks:
            if net.get('name'):
                ...
                if network.get('Driver') == 'bridge' and \
                   network.get('Options', {}).get('com.jctanner.cloud_docker.docker.network.bridge.host_binding_ipv4'):
                    ip = network['Options']['com.jctanner.cloud_docker.docker.network.bridge.host_binding_ipv4']
                    break
        return ip

    def _parse_publish_ports(self):
        '''
        Parse ports from jctanner.cloud_docker.docker CLI syntax
        '''
    ...
    def _parse_exposed_ports(self, published_ports):
        '''
        Parse exposed ports from jctanner.cloud_docker.docker CLI-style ports syntax.
        '''
        exposed = []
...
class Container(DockerBaseClass):
    ...
    def has_different_configuration(self, image):
        ...
        # The previous version of the jctanner.cloud_docker.docker module ignored the detach state by
        ...
        if self.parameters.client.jctanner.cloud_docker.docker_api_version < LooseVersion('1.22'):
            # For jctanner.cloud_docker.docker API < 1.22, update_container() is not supported. Thus
...
class ContainerManager(DockerBaseClass):
    ...
    def __init__(self, client):
        ...
        if self.facts:
            self.results['ansible_facts'] = {'jctanner.cloud_docker.docker_container': self.facts}
    ...
    def present(self, state):
        ...
        if not container.exists:
            ...
            if image_different or different or self.parameters.recreate:
                ...
                self.diff['differences'] = differences.get_legacy_jctanner.cloud_docker.docker_container_diffs()
                if image_different:
                    self.diff['image_different'] = True
                self.log("differences")
                self.log(differences.get_legacy_jctanner.cloud_docker.docker_container_diffs(), pretty_print=True)
    ...
    def update_limits(self, container):
        ...
        if limits_differ:
            ...
            self.log(different_limits.get_legacy_jctanner.cloud_docker.docker_container_diffs(), pretty_print=True)
            ...
            while True:
                try:
                    ...
                    if 'Unpause the container before stopping or killing' in exc.explanation:
                        # New jctanner.cloud_docker.docker daemon versions do not allow containers to be removed
...
class AnsibleDockerClientContainer(AnsibleDockerClient):
    # A list of module options which are not jctanner.cloud_docker.docker container properties
    ...
    def _get_additional_minimal_versions(self):
        stop_timeout_supported = self.jctanner.cloud_docker.docker_api_version >= LooseVersion('1.25')
        ...
        if stop_timeout_supported:
            stop_timeout_supported = self.jctanner.cloud_docker.docker_py_version >= LooseVersion('2.1')
            if stop_timeout_needed_for_update and not stop_timeout_supported:
                ...
                self.module.warn("Docker SDK for Python's version is %s. Minimum version required is 2.1 to update "
                                 ...
                                 "If you use the 'jctanner.cloud_docker.docker-py' module, you have to switch to the 'jctanner.cloud_docker.docker' Python package." % (jctanner.cloud_docker.docker_version,))
        else:
            if stop_timeout_needed_for_update and not stop_timeout_supported:
                # We warn (instead of fail) since in older versions, stop_timeout was not used
                # to update the container's configuration, but only when stopping a container.
                self.module.warn("Docker API version is %s. Minimum version required is 1.25 to set or "
                                 "update the container's stop_timeout configuration." % (self.jctanner.cloud_docker.docker_api_version_str,))
    ...
    def __init__(self, **kwargs):
        option_minimal_versions = dict(
            ...
            name=dict(),
            # normal options
            device_read_bps=dict(jctanner.cloud_docker.docker_py_version='1.9.0', jctanner.cloud_docker.docker_api_version='1.22'),
            device_read_iops=dict(jctanner.cloud_docker.docker_py_version='1.9.0', jctanner.cloud_docker.docker_api_version='1.22'),
            device_write_bps=dict(jctanner.cloud_docker.docker_py_version='1.9.0', jctanner.cloud_docker.docker_api_version='1.22'),
            device_write_iops=dict(jctanner.cloud_docker.docker_py_version='1.9.0', jctanner.cloud_docker.docker_api_version='1.22'),
            dns_opts=dict(jctanner.cloud_docker.docker_api_version='1.21', jctanner.cloud_docker.docker_py_version='1.10.0'),
            ipc_mode=dict(jctanner.cloud_docker.docker_api_version='1.25'),
            mac_address=dict(jctanner.cloud_docker.docker_api_version='1.25'),
            oom_score_adj=dict(jctanner.cloud_docker.docker_api_version='1.22'),
            shm_size=dict(jctanner.cloud_docker.docker_api_version='1.22'),
            stop_signal=dict(jctanner.cloud_docker.docker_api_version='1.21'),
            tmpfs=dict(jctanner.cloud_docker.docker_api_version='1.22'),
            volume_driver=dict(jctanner.cloud_docker.docker_api_version='1.21'),
            memory_reservation=dict(jctanner.cloud_docker.docker_api_version='1.21'),
            kernel_memory=dict(jctanner.cloud_docker.docker_api_version='1.21'),
            auto_remove=dict(jctanner.cloud_docker.docker_py_version='2.1.0', jctanner.cloud_docker.docker_api_version='1.25'),
            healthcheck=dict(jctanner.cloud_docker.docker_py_version='2.0.0', jctanner.cloud_docker.docker_api_version='1.24'),
            init=dict(jctanner.cloud_docker.docker_py_version='2.2.0', jctanner.cloud_docker.docker_api_version='1.25'),
            runtime=dict(jctanner.cloud_docker.docker_py_version='2.4.0', jctanner.cloud_docker.docker_api_version='1.25'),
            sysctls=dict(jctanner.cloud_docker.docker_py_version='1.10.0', jctanner.cloud_docker.docker_api_version='1.24'),
            userns_mode=dict(jctanner.cloud_docker.docker_py_version='1.10.0', jctanner.cloud_docker.docker_api_version='1.23'),
            uts=dict(jctanner.cloud_docker.docker_py_version='3.5.0', jctanner.cloud_docker.docker_api_version='1.25'),
            pids_limit=dict(jctanner.cloud_docker.docker_py_version='1.10.0', jctanner.cloud_docker.docker_api_version='1.23'),
            # specials
            ipvX_address_supported=dict(jctanner.cloud_docker.docker_py_version='1.9.0', detect_usage=detect_ipvX_address_usage,
...
def main():
    ...
    if client.module.params['networks_cli_compatible'] is None and client.module.params['networks']:
        client.module.deprecate(
            'Please note that jctanner.cloud_docker.docker_container handles networks slightly different than jctanner.cloud_docker.docker CLI. '
