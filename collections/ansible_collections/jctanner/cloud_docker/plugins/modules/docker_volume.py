...
DOCUMENTATION = u'''
module: jctanner.cloud_docker.docker_volume
version_added: "2.4"
short_description: Manage Docker volumes
description:
  - Create/remove Docker volumes.
  - Performs largely the same function as the "jctanner.cloud_docker.docker volume" CLI subcommand.
options:
  volume_name:
    description:
  ...
  driver_options:
    description:
      - "Dictionary of volume settings. Consult jctanner.cloud_docker.docker docs for valid options and values:
        U(https://docs.jctanner.cloud_docker.docker.com/engine/reference/commandline/volume_create/#driver-specific-options)"
    type: dict
...
requirements:
  ...
  - "The jctanner.cloud_docker.docker server >= 1.9.0"
'''

EXAMPLES = '''
- name: Create a volume
  jctanner.cloud_docker.docker_volume:
    name: volume_one
...
volume:
    ...
    - Note that facts are part of the registered vars since Ansible 2.8. For compatibility reasons, the facts
      are also accessible directly as C(jctanner.cloud_docker.docker_volume). Note that the returned fact will be removed in Ansible 2.12.
    returned: success
    type: dict
    sample: {}
'''
...
class TaskParameters(DockerBaseClass):
    def __init__(self, client):
        ...
        if self.force is not None:
            ...
            client.module.warn('The "force" option of jctanner.cloud_docker.docker_volume has been deprecated '
...
class DockerVolumeManager(object):
    ...
    def present(self):
        ...
        self.results['ansible_facts'] = {u'jctanner.cloud_docker.docker_volume': volume_facts}
...
def main():
    ...
    option_minimal_versions = dict(
        labels=dict(jctanner.cloud_docker.docker_py_version='1.10.0', jctanner.cloud_docker.docker_api_version='1.23'),
    )
