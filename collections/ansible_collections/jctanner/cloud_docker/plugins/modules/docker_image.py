...
DOCUMENTATION = '''
---
module: jctanner.cloud_docker.docker_image

short_description: Manage jctanner.cloud_docker.docker images.
...
options:
  source:
    description:
      ...
      - "Use C(local) to make sure that the image is already available on the local
         jctanner.cloud_docker.docker daemon, i.e. do not try to build, pull or load the image."
  ...
  build:
    ...
    suboptions:
      cache_from:
        ...
        type: list
      jctanner.cloud_docker.dockerfile:
        description:
      ...
      use_config_proxy:
        description:
          - If set to `yes` and a proxy configuration is specified in the jctanner.cloud_docker.docker client configuration
            (by default C($HOME/.jctanner.cloud_docker.docker/config.json)), the corresponding environment variables will
  ...
  jctanner.cloud_docker.dockerfile:
    description:
      - Use with state C(present) and source C(build) to provide an alternate name for the Dockerfile to use when building an image.
      - This can also include a relative path (relative to I(path)).
      - Please use I(build.jctanner.cloud_docker.dockerfile) instead. This option will be removed in Ansible 2.12.
    type: str
    version_added: "2.0"
  force:
    description:
  ...
  use_tls:
    description:
      - "DEPRECATED. Whether to use tls to connect to the jctanner.cloud_docker.docker daemon. Set to
...
- name: pull an image
  jctanner.cloud_docker.docker_image:
    name: pacur/centos-7
    source: pull

- name: Tag and push to jctanner.cloud_docker.docker hub
  jctanner.cloud_docker.docker_image:
...
from ansible_collections.jctanner.cloud_jctanner.cloud_docker.docker.plugins.module_utils.jctanner.cloud_docker.docker.common import (
    jctanner.cloud_docker.docker_version, AnsibleDockerClient, DockerBaseClass, is_image_name_id,
)
from ansible.module_utils._text import to_native

if jctanner.cloud_docker.docker_version is not None:
    try:
        if LooseVersion(jctanner.cloud_docker.docker_version) >= LooseVersion('2.0.0'):
            from jctanner.cloud_docker.docker.auth import resolve_repository_name
        else:
            from jctanner.cloud_docker.docker.auth.auth import resolve_repository_name
        from jctanner.cloud_docker.docker.utils.utils import parse_repository_tag
    except ImportError:
        # missing Docker SDK for Python handled in module_utils.jctanner.cloud_docker.docker.common
        pass
...
class ImageManager(DockerBaseClass):
    ...
    def __init__(self, client, results):
        ...
        self.jctanner.cloud_docker.dockerfile = build.get('jctanner.cloud_docker.dockerfile')
        ...
        if not self.check_mode:
            ...
            try:
                with open(self.archive_path, 'wb') as fd:
                    if self.client.jctanner.cloud_docker.docker_py_version >= LooseVersion('3.0.0'):
                        for chunk in image:
                            fd.write(chunk)
                    else:
    ...
    def build_image(self):
        ...
        params = dict(
            ...
            pull=self.pull,
            forcerm=self.rm,
            jctanner.cloud_docker.dockerfile=self.jctanner.cloud_docker.dockerfile,
            decode=True,
        )
        if self.client.jctanner.cloud_docker.docker_py_version < LooseVersion('3.0.0'):
            params['stream'] = True
        ...
        if self.use_config_proxy:
            ...
            # Due to a bug in jctanner.cloud_docker.docker-py, it will crash if
...
def main():
    argument_spec = dict(
        ...
        build=dict(type='dict', suboptions=dict(
            ...
            jctanner.cloud_docker.dockerfile=dict(type='str'),
        ...
        jctanner.cloud_docker.dockerfile=dict(type='str', removedin_version='2.12'),
    ...
    option_minimal_versions["build.cache_from"] = dict(jctanner.cloud_docker.docker_py_version='2.1.0', jctanner.cloud_docker.docker_api_version='1.25', detect_usage=detect_build_cache_from)
    option_minimal_versions["build.network"] = dict(jctanner.cloud_docker.docker_py_version='2.4.0', jctanner.cloud_docker.docker_api_version='1.25', detect_usage=detect_build_network)
    option_minimal_versions["build.use_config_proxy"] = dict(jctanner.cloud_docker.docker_py_version='3.7.0', detect_usage=detect_use_config_proxy)
    ...
    build_options = dict(
        ...
        jctanner.cloud_docker.dockerfile='jctanner.cloud_docker.dockerfile',
