...
DOCUMENTATION = '''
---
module: jctanner.cloud_docker.docker_stack
author: "Dario Zanzico (@dariko)"
short_description: jctanner.cloud_docker.docker stack module
description:
  - Manage jctanner.cloud_docker.docker stacks using the 'jctanner.cloud_docker.docker stack' command
...
options:
  ...
  prune:
    description:
      - If true will add the C(--prune) option to the C(jctanner.cloud_docker.docker stack deploy) command.
        This will have jctanner.cloud_docker.docker remove the services not present in the
        current stack definition.
    type: bool
    default: no
  with_registry_auth:
    description:
      - If true will add the C(--with-registry-auth) option to the C(jctanner.cloud_docker.docker stack deploy) command.
        This will have jctanner.cloud_docker.docker send registry authentication details to Swarm agents.
    type: bool
    default: no
  resolve_image:
    description:
      - If set will add the C(--resolve-image) option to the C(jctanner.cloud_docker.docker stack deploy) command.
        This will have jctanner.cloud_docker.docker query the registry to resolve image digest and
        supported platforms. If not set, jctanner.cloud_docker.docker use "always" by default.
    type: str
...
-   name: deploy 'stack1' stack from file
    jctanner.cloud_docker.docker_stack:
        state: present
        name: stack1
        compose:
        -   /opt/stack.compose
...
def jctanner.cloud_docker.docker_stack_services(module, stack_name):
    jctanner.cloud_docker.docker_bin = module.get_bin_path('jctanner.cloud_docker.docker', required=True)
    rc, out, err = module.run_command([jctanner.cloud_docker.docker_bin,
                                       "stack",
                                       "services",
                                       stack_name,
                                       "--format",
...
def jctanner.cloud_docker.docker_service_inspect(module, service_name):
    jctanner.cloud_docker.docker_bin = module.get_bin_path('jctanner.cloud_docker.docker', required=True)
    rc, out, err = module.run_command([jctanner.cloud_docker.docker_bin,
                                       "service",
                                       "inspect",
...
def jctanner.cloud_docker.docker_stack_deploy(module, stack_name, compose_files):
    jctanner.cloud_docker.docker_bin = module.get_bin_path('jctanner.cloud_docker.docker', required=True)
    command = [jctanner.cloud_docker.docker_bin, "stack", "deploy"]
...
def jctanner.cloud_docker.docker_stack_inspect(module, stack_name):
    ret = {}
    for service_name in jctanner.cloud_docker.docker_stack_services(module, stack_name):
        ret[service_name] = jctanner.cloud_docker.docker_service_inspect(module, service_name)
    return ret


def jctanner.cloud_docker.docker_stack_rm(module, stack_name, retries, interval):
    jctanner.cloud_docker.docker_bin = module.get_bin_path('jctanner.cloud_docker.docker', required=True)
    command = [jctanner.cloud_docker.docker_bin, "stack", "rm", stack_name]
...
def main():
    ...
    if state == 'present':
        ...
        before_stack_services = jctanner.cloud_docker.docker_stack_inspect(module, name)

        rc, out, err = jctanner.cloud_docker.docker_stack_deploy(module, name, compose_files)

        after_stack_services = jctanner.cloud_docker.docker_stack_inspect(module, name)

        if rc != 0:
            module.fail_json(msg="jctanner.cloud_docker.docker stack up deploy command failed",
                             out=out,
                             rc=rc, err=err)
        ...
        if jctanner.cloud_docker.docker_stack_services(module, name):
            rc, out, err = jctanner.cloud_docker.docker_stack_rm(module, name, absent_retries, absent_retries_interval)
            if rc != 0:
                module.fail_json(msg="'jctanner.cloud_docker.docker stack down' command failed",
                                 out=out,
                                 rc=rc,
                                 err=err)
            else:
