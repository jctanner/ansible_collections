...
DOCUMENTATION = '''
---
module: jctanner.cloud_docker.docker_swarm_info
...
description:
  ...
  - The output differs depending on API version available on jctanner.cloud_docker.docker host.
  - Must be run on Swarm Manager node; otherwise module fails with error message.
    ...
    the jctanner.cloud_docker.docker daemon can be communicated with, whether it is in Swarm mode, and
...
options:
  ...
  nodes_filters:
    description:
      ...
      - See L(the jctanner.cloud_docker.docker documentation,https://docs.jctanner.cloud_docker.docker.com/engine/reference/commandline/node_ls/#filtering)
  ...
  services_filters:
    description:
      ...
      - See L(the jctanner.cloud_docker.docker documentation,https://docs.jctanner.cloud_docker.docker.com/engine/reference/commandline/service_ls/#filtering)
        for more information on possible filters.
    type: dict
  tasks:
    description:
  ...
  tasks_filters:
    description:
      ...
      - See L(the jctanner.cloud_docker.docker documentation,https://docs.jctanner.cloud_docker.docker.com/engine/reference/commandline/service_ps/#filtering)
        for more information on possible filters.
    type: dict
  unlock_key:
    description:
  ...
  verbose_output:
    description:
      - When set to C(yes) and I(nodes), I(services) or I(tasks) is set to C(yes)
        ...
        For details see the documentation of your version of Docker API at U(https://docs.jctanner.cloud_docker.docker.com/engine/api/).
...
- name: Get info on Docker Swarm
  jctanner.cloud_docker.docker_swarm_info:
  ignore_errors: yes
  register: result
...
- name: Inform about basic flags
  debug:
    msg: |
      Was able to talk to jctanner.cloud_docker.docker daemon: {{ result.can_talk_to_jctanner.cloud_docker.docker }}
      Docker in Swarm mode: {{ result.jctanner.cloud_docker.docker_swarm_active }}
      This is a Manager node: {{ result.jctanner.cloud_docker.docker_swarm_manager }}
...
can_talk_to_jctanner.cloud_docker.docker:
    ...
    type: bool
jctanner.cloud_docker.docker_swarm_active:
    description:
      - Will be C(true) if the module can talk to the jctanner.cloud_docker.docker daemon,
        and the jctanner.cloud_docker.docker daemon is in Swarm mode.
    returned: both on success and on error
    type: bool
jctanner.cloud_docker.docker_swarm_manager:
    description:
      - Will be C(true) if the module can talk to the jctanner.cloud_docker.docker daemon,
        the jctanner.cloud_docker.docker daemon is in Swarm mode, and the current node is
        a manager node.
...
swarm_facts:
    description:
      - Facts representing the basic state of the jctanner.cloud_docker.docker Swarm cluster.
...
nodes:
    description:
      - List of dict objects containing the basic information about each volume.
        Keys matches the C(jctanner.cloud_docker.docker node ls) output unless I(verbose_output=yes).
...
services:
    description:
      - List of dict objects containing the basic information about each volume.
        Keys matches the C(jctanner.cloud_docker.docker service ls) output unless I(verbose_output=yes).
        See description for I(verbose_output).
...
tasks:
    description:
      - List of dict objects containing the basic information about each volume.
        Keys matches the C(jctanner.cloud_docker.docker service ps) output unless I(verbose_output=yes).
        See description for I(verbose_output).
...
try:
    ...
    # missing Docker SDK for Python handled in ansible.module_utils.jctanner.cloud_docker.docker_common
    pass
...
from ansible_collections.jctanner.cloud_jctanner.cloud_docker.docker.plugins.module_utils.jctanner.cloud_docker.docker.common import DockerBaseClass, clean_dict_booleans_for_jctanner.cloud_docker.docker_api
...
class DockerSwarmManager(DockerBaseClass):
    ...
    def __init__(self, client, results):
        ...
        self.results['swarm_facts'] = self.get_jctanner.cloud_docker.docker_swarm_facts()
        ...
        if self.client.module.params['unlock_key']:
            self.results['swarm_unlock_key'] = self.get_jctanner.cloud_docker.docker_swarm_unlock_key()

    def get_jctanner.cloud_docker.docker_swarm_facts(self):
        try:
            return self.client.inspect_swarm()
        except APIError as exc:
            self.client.fail("Error inspecting jctanner.cloud_docker.docker swarm: %s" % to_native(exc))

    def get_jctanner.cloud_docker.docker_items_list(self, jctanner.cloud_docker.docker_object=None, filters=None):
        items = None
        items_list = []

        try:
            if jctanner.cloud_docker.docker_object == 'nodes':
                items = self.client.nodes(filters=filters)
            elif jctanner.cloud_docker.docker_object == 'tasks':
                items = self.client.tasks(filters=filters)
            elif jctanner.cloud_docker.docker_object == 'services':
                items = self.client.services(filters=filters)
        except APIError as exc:
            self.client.fail("Error inspecting jctanner.cloud_docker.docker swarm for object '%s': %s" %
    ...
    def get_jctanner.cloud_docker.docker_swarm_unlock_key(self):
...
def main():
    ...
    option_minimal_versions = dict(
        unlock_key=dict(jctanner.cloud_docker.docker_py_version='2.7.0', jctanner.cloud_docker.docker_api_version='1.25'),
    )
    ...
    client = AnsibleDockerSwarmClient(
        ...
        fail_results=dict(
            ...
            jctanner.cloud_docker.docker_swarm_active=False,
            jctanner.cloud_docker.docker_swarm_manager=False,
        ),
    )
    client.fail_results['can_talk_to_jctanner.cloud_docker.docker'] = True
    client.fail_results['jctanner.cloud_docker.docker_swarm_active'] = client.check_if_swarm_node()
    client.fail_results['jctanner.cloud_docker.docker_swarm_manager'] = client.check_if_swarm_manager()
