...
DOCUMENTATION = '''
module: jctanner.cloud_docker.docker_network
version_added: "2.2"
short_description: Manage Docker networks
description:
  - Create/remove Docker networks and connect containers to them.
  - Performs largely the same function as the "jctanner.cloud_docker.docker network" CLI subcommand.
options:
  name:
    description:
  ...
  driver_options:
    description:
      - Dictionary of network settings. Consult jctanner.cloud_docker.docker docs for valid options and values.
    type: dict
  ...
  ipam_config:
    description:
      - List of IPAM config blocks. Consult
        L(Docker docs,https://docs.jctanner.cloud_docker.docker.com/compose/compose-file/compose-file-v2/#ipam) for valid options and values.
...
requirements:
  ...
  - "The jctanner.cloud_docker.docker server >= 1.10.0"
'''

EXAMPLES = '''
- name: Create a network
  jctanner.cloud_docker.docker_network:
    name: network_one
...
- name: Create a network with driver options
  jctanner.cloud_docker.docker_network:
    name: network_two
    driver_options:
      com.jctanner.cloud_docker.docker.network.bridge.name: net2
...
network:
    ...
    - Note that facts are part of the registered vars since Ansible 2.8. For compatibility reasons, the facts
      are also accessible directly as C(jctanner.cloud_docker.docker_network). Note that the returned fact will be removed in Ansible 2.12.
    returned: success
    type: dict
    sample: {}
'''
...
from ansible_collections.jctanner.cloud_jctanner.cloud_docker.docker.plugins.module_utils.jctanner.cloud_docker.docker.common import (
    AnsibleDockerClient,
    DockerBaseClass,
    jctanner.cloud_docker.docker_version,
    DifferenceTracker,
    clean_dict_booleans_for_jctanner.cloud_docker.docker_api,
)

try:
    from jctanner.cloud_docker.docker import utils
    if LooseVersion(jctanner.cloud_docker.docker_version) >= LooseVersion('2.0.0'):
        from jctanner.cloud_docker.docker.types import IPAMPool, IPAMConfig
except Exception:
...
class DockerNetworkManager(object):
    ...
    def __init__(self, client):
        ...
        if self.parameters.driver_options:
            self.parameters.driver_options = clean_dict_booleans_for_jctanner.cloud_docker.docker_api(self.parameters.driver_options)
    ...
    def present(self):
        ...
        if self.diff or self.check_mode or self.parameters.debug:
            self.diff_result['differences'] = differences.get_legacy_jctanner.cloud_docker.docker_diffs()
        ...
        self.results['ansible_facts'] = {u'jctanner.cloud_docker.docker_network': network_facts}
...
def main():
    ...
    option_minimal_versions = dict(
        scope=dict(jctanner.cloud_docker.docker_py_version='2.6.0', jctanner.cloud_docker.docker_api_version='1.30'),
        attachable=dict(jctanner.cloud_docker.docker_py_version='2.0.0', jctanner.cloud_docker.docker_api_version='1.26'),
        labels=dict(jctanner.cloud_docker.docker_api_version='1.23'),
        ipam_driver_options=dict(jctanner.cloud_docker.docker_py_version='2.0.0'),
    )
    ...
    client = AnsibleDockerClient(
        ...
        min_jctanner.cloud_docker.docker_api_version='1.22',
        # "The jctanner.cloud_docker.docker server >= 1.10.0"
