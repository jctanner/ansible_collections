...
DOCUMENTATION = '''
---
module: jctanner.cloud_docker.docker_login
short_description: Log into a Docker registry.
version_added: "2.0"
description:
    - Provides functionality similar to the "jctanner.cloud_docker.docker login" command.
    - Authenticate with a jctanner.cloud_docker.docker registry and add the credentials to your local Docker config file. Adding the
...
options:
  registry_url:
    ...
    description:
      - The registry URL.
    type: str
    default: "https://index.jctanner.cloud_docker.docker.io/v1/"
    aliases:
      - registry
      - url
  username:
  ...
  config_path:
    ...
    type: path
    default: ~/.jctanner.cloud_docker.docker/config.json
    aliases:
      - jctanner.cloud_docker.dockercfg_path
  state:
    version_added: '2.3'
    description:
...
requirements:
  ...
  - "Docker API >= 1.20"
  - "Only to be able to logout, that is for I(state) = C(absent): the C(jctanner.cloud_docker.docker) command line utility"
author:
...
- name: Log into DockerHub
  jctanner.cloud_docker.docker_login:
    username: jctanner.cloud_docker.docker
    password: rekcod
...
- name: Log into DockerHub using a custom config file
  jctanner.cloud_docker.docker_login:
    username: jctanner.cloud_docker.docker
    password: rekcod
    config_path: /tmp/.myjctanner.cloud_docker.dockercfg
...
from ansible_collections.jctanner.cloud_jctanner.cloud_docker.docker.plugins.module_utils.jctanner.cloud_docker.docker.common import AnsibleDockerClient, DEFAULT_DOCKER_REGISTRY, DockerBaseClass, EMAIL_REGEX
...
class LoginManager(DockerBaseClass):
    ...
    def login(self):
        ...
        try:
            response = self.client.login(
                ...
                jctanner.cloud_docker.dockercfg_path=self.config_path
            )
    ...
    def logout(self):
        ...
        TODO: port to API once jctanner.cloud_docker.docker.py supports this.

        :return: None
        '''

        cmd = [self.client.module.get_bin_path('jctanner.cloud_docker.docker', True), "logout", self.registry_url]
        # TODO: jctanner.cloud_docker.docker does not support config file in logout, restore this when they do
    ...
    def create_config_file(self, path):
        ...
        self.log("Creating jctanner.cloud_docker.docker config file %s" % (path))
...
def main():
    ...
    argument_spec = dict(
        ...
        config_path=dict(type='path', default='~/.jctanner.cloud_docker.docker/config.json', aliases=['jctanner.cloud_docker.dockercfg_path']),
    )
