...
DOCUMENTATION = '''
---
module: jctanner.cloud_docker.docker_prune

short_description: Allows to prune various jctanner.cloud_docker.docker objects

description:
  - Allows to run C(jctanner.cloud_docker.docker container prune), C(jctanner.cloud_docker.docker image prune), C(jctanner.cloud_docker.docker network prune)
    and C(jctanner.cloud_docker.docker volume prune) via the Docker API.
...
- name: Prune containers older than 24h
  jctanner.cloud_docker.docker_prune:
    containers: yes
    containers_filters:
...
try:
    from ansible_collections.jctanner.cloud_jctanner.cloud_docker.docker.plugins.module_utils.jctanner.cloud_docker.docker.common import jctanner.cloud_docker.docker_version, clean_dict_booleans_for_jctanner.cloud_docker.docker_api
...
def main():
    ...
    if client.module.params['builder_cache'] and client.jctanner.cloud_docker.docker_py_version < LooseVersion(cache_min_version):
        msg = "Error: Docker SDK for Python's version is %s. Minimum version required for builds option is %s. Use `pip install --upgrade jctanner.cloud_docker.docker` to upgrade."
        client.fail(msg % (jctanner.cloud_docker.docker_version, cache_min_version))

    result = dict()

    if client.module.params['containers']:
        filters = clean_dict_booleans_for_jctanner.cloud_docker.docker_api(client.module.params.get('containers_filters'))
    ...
    if client.module.params['images']:
        filters = clean_dict_booleans_for_jctanner.cloud_docker.docker_api(client.module.params.get('images_filters'))
    ...
    if client.module.params['networks']:
        filters = clean_dict_booleans_for_jctanner.cloud_docker.docker_api(client.module.params.get('networks_filters'))
        res = client.prune_networks(filters=filters)
        result['networks'] = res.get('NetworksDeleted') or []

    if client.module.params['volumes']:
        filters = clean_dict_booleans_for_jctanner.cloud_docker.docker_api(client.module.params.get('volumes_filters'))
