...
DOCUMENTATION = '''
---
module: jctanner.cloud_docker.docker_node_info

short_description: Retrieves facts about jctanner.cloud_docker.docker swarm node from Swarm Manager

description:
  - Retrieves facts about a jctanner.cloud_docker.docker node.
  - Essentially returns the output of C(jctanner.cloud_docker.docker node inspect <name>).
...
options:
  ...
  self:
    description:
      - If C(true), queries the node (i.e. the jctanner.cloud_docker.docker daemon) the module communicates with.
...
requirements:
  - "L(Docker SDK for Python,https://jctanner.cloud_docker.docker-py.readthedocs.io/en/stable/) >= 2.4.0"
  - "Docker API >= 1.24"
'''

EXAMPLES = '''
- name: Get info on all nodes
  jctanner.cloud_docker.docker_node_info:
  register: result
...
nodes:
    description:
      - Facts representing the current state of the nodes. Matches the C(jctanner.cloud_docker.docker node inspect) output.
...
def main():
    ...
    client = AnsibleDockerSwarmClient(
        ...
        min_jctanner.cloud_docker.docker_version='2.4.0',
