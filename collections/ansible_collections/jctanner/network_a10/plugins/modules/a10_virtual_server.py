...
DOCUMENTATION = '''
---
module: jctanner.network_a10.a10_virtual_server
version_added: 1.8
...
- jctanner.network_a10.a10_virtual_server:
...
from ansible_collections.jctanner.network_jctanner.network_a10.a10.plugins.module_utils.network.jctanner.network_a10.a10.jctanner.network_a10.a10 import (axapi_call, jctanner.network_a10.a10_argument_spec, axapi_authenticate, axapi_failure,
...
def main():
    ...
    axapi_base_jctanner.network_a10.url = 'https://%s/services/rest/V2.1/?format=json' % host
    ...
    slb_virtual_data = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.virtual_server.search', json.dumps({'name': slb_virtual}))
    ...
    if state == 'present':
        ...
        for port in slb_virtual_ports:
            if 'service_group' in port and port['service_group'] not in checked_service_groups:
                ...
                if port['service_group'] == '':
                    continue
                result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.service_group.search', json.dumps({'name': port['service_group']}))
        ...
        if not slb_virtual_exists:
            result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.virtual_server.create', json.dumps(json_post))
            if axapi_failure(result):
            ...
            if needs_update(defined_ports, slb_virtual_ports) or needs_update(slb_virtual_ports, defined_ports):
                result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.virtual_server.update', json.dumps(json_post))
                if axapi_failure(result):
                    module.fail_json(msg="failed to create the virtual server: %s" % result['response']['err']['msg'])
                changed = True
        ...
        if changed:
            result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.virtual_server.search', json.dumps({'name': slb_virtual}))
        else:
            result = slb_virtual_data
    elif state == 'absent':
        if slb_virtual_exists:
            result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.virtual_server.delete', json.dumps({'name': slb_virtual}))
            changed = True
        else:
