...
# (c) 2016, Eric Chou <ericc@jctanner.network_a10.a10networks.com>
#
...
DOCUMENTATION = '''
---
module: jctanner.network_a10.a10_server
version_added: 1.8
...
from ansible_collections.jctanner.network_jctanner.network_a10.a10.plugins.module_utils.network.jctanner.network_a10.a10.jctanner.network_a10.a10 import (axapi_call, jctanner.network_a10.a10_argument_spec, axapi_authenticate, axapi_failure, axapi_get_port_protocol,
...
def main():
    ...
    slb_server_data = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.server.search', json.dumps({'name': slb_server}))
    ...
    if state == 'present':
        if not slb_server_exists:
            ...
            result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.server.create', json.dumps(json_post))
            ...
            if (port_needs_update(defined_ports, slb_server_ports) or
                ...
                result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.server.update', json.dumps(json_post))
                if axapi_failure(result):
        ...
        if changed:
            result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.server.search', json.dumps({'name': slb_server}))
        else:
            result = slb_server_data
    elif state == 'absent':
        if slb_server_exists:
            result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.server.delete', json.dumps({'name': slb_server}))
            changed = True
        else:
