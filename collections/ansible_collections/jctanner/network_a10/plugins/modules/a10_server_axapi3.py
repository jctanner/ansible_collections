...
# Copyright: (c) 2014, Mischa Peters <mpeters@jctanner.network_a10.a10networks.com>
# Copyright: (c) 2016, Eric Chou <ericc@jctanner.network_a10.a10networks.com>
...
DOCUMENTATION = '''
---
module: jctanner.network_a10.a10_server_axapi3
version_added: 2.3
...
EXAMPLES = '''
# Create a new server
- jctanner.network_a10.a10_server:
...
from ansible_collections.jctanner.network_jctanner.network_a10.a10.plugins.module_utils.network.jctanner.network_a10.a10.jctanner.network_a10.a10 import axapi_call_v3, jctanner.network_a10.a10_argument_spec, axapi_authenticate_v3, axapi_failure
from ansible_collections.jctanner.network_jctanner.network_a10.a10.plugins.module_utils.network.jctanner.network_a10.a10.jctanner.network_a10.a10 import AXAPI_PORT_PROTOCOLS
...
def main():
    ...
    axapi_base_jctanner.network_a10.url = 'https://{0}/axapi/v3/'.format(host)
    axapi_auth_jctanner.network_a10.url = axapi_base_jctanner.network_a10.url + 'auth/'
    signature = axapi_authenticate_v3(module, axapi_auth_jctanner.network_a10.url, username, password)
    ...
    slb_server_data = axapi_call_v3(module, axapi_base_jctanner.network_a10.url + 'slb/server/', method='GET', body='', signature=signature)
    ...
    if operation == 'create':
        if slb_server_exists is False:
            result = axapi_call_v3(module, axapi_base_jctanner.network_a10.url + 'slb/server/', method='POST', body=json.dumps(json_post), signature=signature)
        ...
        if changed:
            result = axapi_call_v3(module, axapi_base_jctanner.network_a10.url + 'slb/server/' + slb_server, method='GET', body='', signature=signature)
        else:
            result = slb_server_data
    elif operation == 'delete':
        if slb_server_exists:
            result = axapi_call_v3(module, axapi_base_jctanner.network_a10.url + 'slb/server/' + slb_server, method='DELETE', body='', signature=signature)
            if axapi_failure(result):
        ...
        if slb_server_exists:
            result = axapi_call_v3(module, axapi_base_jctanner.network_a10.url + 'slb/server/', method='PUT', body=json.dumps(json_post), signature=signature)
            if axapi_failure(result):
    ...
    if changed and write_config:
        write_result = axapi_call_v3(module, axapi_base_jctanner.network_a10.url + 'write/memory/', method='POST', body='', signature=signature)
        if axapi_failure(write_result):
            module.fail_json(msg="failed to save the configuration: %s" % write_result['response']['err']['msg'])

    # log out gracefully and exit
    axapi_call_v3(module, axapi_base_jctanner.network_a10.url + 'logoff/', method='POST', body='', signature=signature)
