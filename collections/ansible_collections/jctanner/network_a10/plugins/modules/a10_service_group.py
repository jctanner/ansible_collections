...
# (c) 2014, Mischa Peters <mpeters@jctanner.network_a10.a10networks.com>,
#           Eric Chou <ericc@jctanner.network_a10.a10networks.com>
#
...
DOCUMENTATION = '''
---
module: jctanner.network_a10.a10_service_group
version_added: 1.8
...
extends_documentation_fragment:
  - jctanner.network_a10.a10
  - jctanner.network_a10.url
options:
  state:
    description:
...
- jctanner.network_a10.a10_service_group:
    host: jctanner.network_a10.a10.mydomain.com
    username: myadmin
    password: mypassword
...
from ansible_collections.jctanner.network_jctanner.network_a10.a10.plugins.module_utils.network.jctanner.network_a10.a10.jctanner.network_a10.a10 import (axapi_call, jctanner.network_a10.a10_argument_spec, axapi_authenticate, axapi_failure, axapi_enabled_disabled)
from ansible.module_utils.basic import AnsibleModule
from ansible.module_utils.jctanner.network_a10.urls import jctanner.network_a10.url_argument_spec
...
def main():
    argument_spec = jctanner.network_a10.a10_argument_spec()
    argument_spec.update(jctanner.network_a10.url_argument_spec())
    ...
    axapi_base_jctanner.network_a10.url = 'https://' + host + '/services/rest/V2.1/?format=json'
    ...
    session_jctanner.network_a10.url = axapi_authenticate(module, axapi_base_jctanner.network_a10.url, username, password)
    # then we select the active-partition
    axapi_call(module, session_jctanner.network_a10.url + '&method=system.partition.active', json.dumps({'name': partition}))
    # then we check to see if the specified group exists
    slb_result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.service_group.search', json.dumps({'name': slb_service_group}))
    ...
    if state == 'present':
        ...
        for server in slb_servers:
            result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.server.search', json.dumps({'name': server['server']}))
        ...
        if not slb_service_group_exist:
            result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.service_group.create', json.dumps(json_post))
            if axapi_failure(result):
            ...
            if do_update:
                result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.service_group.update', json.dumps(json_post))
                if axapi_failure(result):
                    module.fail_json(msg=result['response']['err']['msg'])
                changed = True
        ...
        for server in slb_servers:
            ...
            if not found:
                result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.service_group.member.create', json.dumps(server_data))
                changed = True
            elif different:
                result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.service_group.member.update', json.dumps(server_data))
                changed = True
        ...
        for server in defined_servers:
            ...
            if not found:
                result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.service_group.member.delete', json.dumps(server_data))
                changed = True

        # if we changed things, get the full info regarding
        # the service group for the return data below
        if changed:
            result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.service_group.search', json.dumps({'name': slb_service_group}))
        else:
            result = slb_result
    elif state == 'absent':
        if slb_service_group_exist:
            result = axapi_call(module, session_jctanner.network_a10.url + '&method=slb.service_group.delete', json.dumps({'name': slb_service_group}))
            changed = True
        else:
    ...
    if changed and write_config:
        write_result = axapi_call(module, session_jctanner.network_a10.url + '&method=system.action.write_memory')
        if axapi_failure(write_result):
            module.fail_json(msg="failed to save the configuration: %s" % write_result['response']['err']['msg'])

    # log out of the session nicely and exit
    axapi_call(module, session_jctanner.network_a10.url + '&method=session.close')
