...
DOCUMENTATION = '''
---
module: jctanner.network_cnos.cnos_vlag
...
Tasks : The following are examples of using the module jctanner.network_cnos.cnos_vlag. These are
        written in the main.yml file of the tasks directory.
---
- name: Test Vlag  - enable
  jctanner.network_cnos.cnos_vlag:
      deviceType: "{{ hostvars[inventory_hostname]['deviceType']}}"
      outputfile: "./results/jctanner.network_cnos.cnos_vlag_{{ inventory_hostname }}_output.txt"
      vlagArg1: "enable"
...
def vlagConfig(module, prompt, answer):
    ...
    if(vlagArg1 == "enable"):
        ...
        command = command + vlagArg1 + " "
        value = jctanner.network_cnos.cnos.checkSanityofVariable(deviceType, "vlag_priority",
                                           vlagArg2)
        if(value == "ok"):
            command = command + vlagArg2
        else:
        ...
        # debugOutput("tier-id")
        command = command + vlagArg1 + " "
        value = jctanner.network_cnos.cnos.checkSanityofVariable(deviceType, "vlag_tier_id", vlagArg2)
        if(value == "ok"):
            command = command + vlagArg2
        else:
            retVal = "Error-165"
        ...
        command = command + vlagArg1 + " "
        value = jctanner.network_cnos.cnos.checkSanityofVariable(deviceType, "vlag_instance",
                                           vlagArg2)
        if(value == "ok"):
            command = command + vlagArg2
