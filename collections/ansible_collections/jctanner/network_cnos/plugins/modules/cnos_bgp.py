...
DOCUMENTATION = '''
---
module: jctanner.network_cnos.cnos_bgp
...
description:
    - This module allows you to work with Border Gateway Protocol (BGP) related
     ...
     playbook is run.
version_added: "2.3"
extends_documentation_fragment: jctanner.network_cnos.cnos
options:
    asNum:
        description:
            - AS number
...
'''
EXAMPLES = '''
Tasks: The following are examples of using the module jctanner.network_cnos.cnos_bgp. These are
 written in the main.yml file of the tasks directory.
---
- name: Test BGP  - neighbor
  jctanner.network_cnos.cnos_bgp:
...
import re
try:
    from ansible_collections.jctanner.network_jctanner.network_cnos.cnos.plugins.module_utils.network.jctanner.network_cnos.cnos import jctanner.network_cnos.cnos
    HAS_LIB = True
except Exception:
    HAS_LIB = False
...
def bgpNeighborConfig(module, cmd, prompt, answer):
    ...
    if(bgpNeighborArg1 == "address-family"):
        ...
        value = jctanner.network_cnos.cnos.checkSanityofVariable(
    ...
    inner_cmd = [{'command': command, 'prompt': None, 'answer': None}]
    cmd.extend(inner_cmd)
    retVal = retVal + str(jctanner.network_cnos.cnos.run_jctanner.network_cnos.cnos_commands(module, cmd))
    command = "exit \n"
    return retVal
# EOM
...
def bgpAFConfig(module, cmd, prompt, answer):
    ...
    if(bgpAFArg1 == "aggregate-address"):
        ...
        if(bgpAFArg2 == "route-map"):
            ...
            if(value == "ok"):
                ...
                if(bgpAFArg3 is not None):
                    value1 = jctanner.network_cnos.cnos.checkSanityofVariable(
                        deviceType, "start_reuse_route_value", bgpAFArg3)
                    value2 = jctanner.network_cnos.cnos.checkSanityofVariable(
                        deviceType, "start_suppress_route_value", bgpAFArg4)
                    value3 = jctanner.network_cnos.cnos.checkSanityofVariable(
        ...
        command = command + bgpAFArg1 + " "
        value = jctanner.network_cnos.cnos.checkSanityofVariable(deviceType, "maxpath_option", bgpAFArg2)
        if(value == "ok"):
            command = command + bgpAFArg2 + " "
            value = jctanner.network_cnos.cnos.checkSanityofVariable(
...
def bgpConfig(module, cmd, prompt, answer):
    ...
    deviceType = module.params['deviceType']
    # jctanner.network_cnos.cnos.debugOutput(bgpArg1)
    if(bgpArg1 == "address-family"):
        ...
        # debugOutput(bgpArg1)
        command = command + bgpArg1 + " "
        value = jctanner.network_cnos.cnos.checkSanityofVariable(deviceType, "cluster_id_as_ip", bgpArg2)
        if(value == "ok"):
            command = command + bgpArg2
        else:
            value = jctanner.network_cnos.cnos.checkSanityofVariable(
        ...
        # debugOutput(bgpArg1)
        command = command + bgpArg1 + " "
        value = jctanner.network_cnos.cnos.checkSanityofVariable(deviceType, "maxas_limit_as", bgpArg2)
        if(value == "ok"):
            command = command + bgpArg2
        else:
            retVal = "Error-188"
        ...
        # debugOutput(bgpArg1)
        command = command + bgpArg1 + " "
        value = jctanner.network_cnos.cnos.checkSanityofVariable(deviceType, "router_id", bgpArg2)
        if(value == "ok"):
            command = command + bgpArg2
        else:
            retVal = "Error-190"
        ...
        # jctanner.network_cnos.cnos.debugOutput(bgpArg3)
        ...
        if(value == "ok"):
            ...
            return retVal
        if(bgpArg3 is not None):
            value = jctanner.network_cnos.cnos.checkSanityofVariable(deviceType, "bgp_holdtime", bgpArg3)
            if(value == "ok"):
                command = command + " " + bgpArg3
            else:
                retVal = "Error-192"
...
def main():
    ...
    output = ''
    command = 'router bgp '
    value = jctanner.network_cnos.cnos.checkSanityofVariable(deviceType, "bgp_as_number", asNum)
    if(value == "ok"):
    ...
    errorMsg = jctanner.network_cnos.cnos.checkOutputForError(output)
    if(errorMsg is None):
