...
DOCUMENTATION = '''
---
module: jctanner.network_cnos.cnos_rollback
...
- name: Test Rollback of config - Startup config
  jctanner.network_cnos.cnos_rolback:
      ...
      protocol: "sftp"
      serverip: "10.241.106.118"
      rcpath: "/root/jctanner.network_cnos.cnos/G8272-startup-config.txt"
      serverusername: "root"
      serverpassword: "root123"
...
def doConfigRollBack(module, prompt, answer):
    ...
    jctanner.network_cnos.cnos.debugOutput(command + "\n")
    # jctanner.network_cnos.cnos.checkForFirstTimeAccess(module, command, 'yes/no', 'yes')
    cmd = []
    if(protocol == "scp"):
        ...
        sftp_cmd = [{'command': command, 'prompt': 'Password:',
                     'answer': password}]
        cmd.extend(sftp_cmd)
        # jctanner.network_cnos.cnos.debugOutput(configType + "\n")
        if(configType == 'startup-config'):
...
def main():
    ...
    if '/' in outputfile:
        ...
        # jctanner.network_cnos.cnos.debugOutput(path[0])
