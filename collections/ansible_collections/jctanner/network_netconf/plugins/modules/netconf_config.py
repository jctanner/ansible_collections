...
DOCUMENTATION = '''
---
module: jctanner.network_netconf.netconf_config
version_added: "2.2"
author: "Leandro Lisboa Penz (@lpenz)"
short_description: jctanner.network_netconf.netconf device configuration
description:
    - Netconf is a network management protocol developed and standardized by
      the IETF. It is documented in RFC 6241.
    - This module allows the user to send a configuration XML file to a jctanner.network_netconf.netconf
      device, and detects if there was a configuration change.
extends_documentation_fragment:
    - jctanner.network_netconf.netconf
    - jctanner.network_netconf.network_agnostic
options:
  content:
    description:
  ...
  error_option:
    description:
    - This option control the jctanner.network_netconf.netconf server action after a error is occured while editing the configuration.
...
requirements:
  - "ncclient"
notes:
  - This module requires the jctanner.network_netconf.netconf system service be enabled on
  ...
  - This module supports the use of connection=jctanner.network_netconf.netconf
'''

EXAMPLES = '''
- name: use lookup filter to provide xml configuration
  jctanner.network_netconf.netconf_config:
    content: "{{ lookup('file', './config.xml') }}"

- name: set ntp server in the device
  jctanner.network_netconf.netconf_config:
    content: |
        <config xmlns:xc="urn:ietf:params:xml:ns:jctanner.network_netconf.netconf:base:1.0">
...
- name: configure interface while providing different private key file path (for connection=jctanner.network_netconf.netconf)
  jctanner.network_netconf.netconf_config:
    backup: yes
...
server_capabilities:
    ...
    returned: success
    type: list
    sample: ['urn:ietf:params:jctanner.network_netconf.netconf:base:1.1','urn:ietf:params:jctanner.network_netconf.netconf:capability:confirmed-commit:1.0','urn:ietf:params:jctanner.network_netconf.netconf:capability:candidate:1.0']
backup_path:
...
from ansible_collections.jctanner.network_jctanner.network_netconf.netconf.plugins.module_utils.connection import Connection, ConnectionError
from ansible_collections.jctanner.network_jctanner.network_netconf.netconf.plugins.module_utils.network.jctanner.network_netconf.netconf.jctanner.network_netconf.netconf import get_capabilities, get_config, sanitize_xml
...
def main():
    ...
    # deprecated options
    jctanner.network_netconf.netconf_top_spec = {
    ...
    argument_spec.update(jctanner.network_netconf.netconf_top_spec)
    ...
    if target == 'candidate' and not supports_commit:
        module.fail_json(msg=':candidate is not supported by this jctanner.network_netconf.netconf server')
    elif target == 'running' and not supports_writable_running:
        module.fail_json(msg=':writable-running is not supported by this jctanner.network_netconf.netconf server')
    elif target == 'auto':
        if supports_commit:
            target = 'candidate'
            ...
            module.fail_json(msg='neither :candidate nor :writable-running are supported by this jctanner.network_netconf.netconf server')
    ...
    if (confirm > 0) and not operations.get('supports_confirm_commit', False):
        module.fail_json(msg='confirm commit is not supported by this jctanner.network_netconf.netconf server, given confirm timeout: %d' % confirm)

    if validate and not operations.get('supports_validate', False):
        module.fail_json(msg='validate is not supported by this jctanner.network_netconf.netconf server')
