...
version_added: '1.9'
description:
    - Apply patch jctanner.files.files using the GNU patch tool.
short_description: Apply patch jctanner.files.files using the GNU patch tool
options:
  basedir:
    description:
  ...
  dest:
    description:
      ...
      - The names of the jctanner.files.files to be patched are usually taken from the patch
  ...
  src:
    description:
      - Path of the patch file as accepted by the GNU patch tool. If
        ...
        module's I(jctanner.files.files) directory.
    type: path
    required: true
    aliases: [ patchfile ]
  state:
  ...
  binary:
    ...
    description:
      ...
      - If set to C(no), C(patch) will replace CRLF in C(src) jctanner.files.files on POSIX.
    type: bool
    default: no
notes:
...
- name: Apply patch to multiple jctanner.files.files under basedir
  patch:
