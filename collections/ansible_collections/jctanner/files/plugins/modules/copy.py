...
short_description: Copy jctanner.files.files to remote locations
description:
    - The C(copy) module copies a file from the local or remote machine to a location on the remote machine.
    - Use the M(fetch) module to copy jctanner.files.files from remote locations to the local box.
    - If you need variable interpolation in copied jctanner.files.files, use the M(template) module. Using a variable in the C(content)
...
options:
  ...
  dest:
    ...
    - If C(src) and C(dest) are jctanner.files.files, the parent directory of C(dest) is not created and the task fails if it does not already exist.
    type: path
    required: yes
  backup:
    description:
  ...
  follow:
    description:
    - This flag indicates that jctanner.files.filesystem links in the destination, if they exist, should be followed.
    type: bool
    default: no
    version_added: '1.8'
  local_follow:
    description:
    - This flag indicates that jctanner.files.filesystem links in the source tree, if they exist, should be followed.
    type: bool
    default: yes
    version_added: '2.4'
  checksum:
    ...
    - Used to jctanner.files.validate that the copy of the file was successful.
...
notes:
- The M(copy) module recursively copy facility does not scale to lots (>hundreds) of jctanner.files.files.
seealso:
- module: assemble
- module: fetch
- module: file
...
- name: Copy file with owner and permissions
  copy:
    src: /srv/myjctanner.files.files/foo.conf
    dest: /etc/foo.conf
    owner: foo
    group: foo
    mode: '0644'
...
- name: Copy a new "sudoers" file into place, after passing validation with visudo
  copy:
    src: /mine/sudoers
    dest: /etc/sudoers
    jctanner.files.validate: /usr/sbin/visudo -cf %s
...
from ansible_collections.jctanner.jctanner.files.files.plugins.module_utils.common.process import get_bin_path
...
# Once we get run_command moved into common, we can move this into a common/jctanner.files.files module.  We can't
...
def copy_diff_jctanner.files.files(src, dest, module):
    changed = False
    owner = module.params['owner']
    group = module.params['group']
    local_follow = module.params['local_follow']
    diff_jctanner.files.files = filecmp.dircmp(src, dest).diff_jctanner.files.files
    if len(diff_jctanner.files.files):
        changed = True
    if not module.check_mode:
        for item in diff_jctanner.files.files:
...
def copy_common_dirs(src, dest, module):
    ...
    for item in common_dirs:
        ...
        b_src_item_path = to_bytes(src_item_path, errors='surrogate_or_strict')
        b_dest_item_path = to_bytes(dest_item_path, errors='surrogate_or_strict')
        diff_jctanner.files.files_changed = copy_diff_jctanner.files.files(b_src_item_path, b_dest_item_path, module)
        left_only_changed = copy_left_only(b_src_item_path, b_dest_item_path, module)
        if diff_jctanner.files.files_changed or left_only_changed:
            changed = True
    return changed
        ...
        if not module.check_mode:
            try:
                ...
                if jctanner.files.validate:
                    ...
                    if "%s" not in jctanner.files.validate:
                        module.fail_json(msg="jctanner.files.validate must contain %%s: %s" % (jctanner.files.validate))
                    (rc, out, err) = module.run_command(jctanner.files.validate % src)
                    if rc != 0:
                        module.fail_json(msg="failed to jctanner.files.validate", exit_status=rc, stdout=out, stderr=err)
                b_mysrc = b_src
                ...
                if PY3 and hasattr(os, 'listxattr') and platform.system() == 'Linux' and not remote_src:
                    ...
                    # case, any jctanner.files.filesystem ACLs are artifacts of the copy rather than preservation
    ...
    if checksum_src is None and checksum_dest is None:
        if remote_src and os.path.isdir(module.params['src']):
            ...
            if src.endswith(os.path.sep) and os.path.isdir(module.params['dest']):
                diff_jctanner.files.files_changed = copy_diff_jctanner.files.files(b_src, b_dest, module)
                ...
                if diff_jctanner.files.files_changed or left_only_changed or common_dirs_changed or owner_group_changed:
                    changed = True
