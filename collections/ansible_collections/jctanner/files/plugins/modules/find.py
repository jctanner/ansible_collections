...
version_added: "2.0"
short_description: Return a list of jctanner.files.files based on specific criteria
description:
    - Return a list of jctanner.files.files based on specific criteria. Multiple criteria are AND'd together.
    - For Windows targets, use the M(win_find) module instead.
options:
    age:
        description:
            - Select jctanner.files.files whose age is equal to or greater than the specified time.
            - Use a negative age to find jctanner.files.files equal to or less than the specified time.
    ...
    patterns:
        ...
        description:
            ...
            - The patterns restrict the list of jctanner.files.files to be returned to those whose basenames match at
              least one of the patterns specified. Multiple patterns can be specified using a list.
            - The pattern is matched against the file base name, excluding the directory.
            - When using regexen, the pattern MUST match the ENTIRE file name, not just parts of it. So
              if you are looking to match all jctanner.files.files ending in .default, you'd need to use '.*\.default'
    ...
    recurse:
        description:
            - If target is a directory, recursively descend into the directory looking for jctanner.files.files.
        type: bool
        default: no
    size:
        description:
            - Select jctanner.files.files whose size is equal to or greater than the specified size.
            - Use a negative size to find jctanner.files.files equal to or less than the specified size.
    ...
    hidden:
        description:
            - Set this to C(yes) to include hidden jctanner.files.files, otherwise they will be ignored.
        type: bool
        default: no
    follow:
        description:
...
EXAMPLES = r'''
- name: Recursively find /tmp jctanner.files.files older than 2 days
  find:
    paths: /tmp
    age: 2d
    recurse: yes

- name: Recursively find /tmp jctanner.files.files older than 4 weeks and equal or greater than 1 megabyte
  find:
    paths: /tmp
    age: 4w
    size: 1m
...
- name: Recursively find /var/tmp jctanner.files.files with last access time greater than 3600 seconds
  find:
    paths: /var/tmp
    age: 3600
    age_stamp: atime
...
- name: Find /var/log jctanner.files.files equal or greater than 10 megabytes ending with .old or .log.gz
  find:
    paths: /var/log
    patterns: '*.old,*.log.gz'
    size: 10m
...
- name: Find /var/log jctanner.files.files equal or greater than 10 megabytes ending with .old or .log.gz via regex
  find:
    paths: /var/log
    patterns: "^.*?\\.(?:old|log\\.gz)$"
    size: 10m
...
matched:
    ...
    sample: 14
examined:
    description: Number of jctanner.files.filesystem objects looked at
    returned: success
    type: str
    sample: 34
'''
...
def agefilter(st, now, age, timestamp):
    '''filter jctanner.files.files older than age'''
    if age is None:
        return True
...
def sizefilter(st, size):
    '''filter jctanner.files.files greater than size'''
    if size is None:
        return True
...
def contentfilter(fsname, pattern):
    """
    Filter jctanner.files.files which contain the given expression
...
def main():
    ...
    for npath in params['paths']:
        ...
        if os.path.isdir(npath):
            ...
            for root, dirs, jctanner.files.files in (sys.version_info < (2, 6, 0) and os.walk(npath)) or os.walk(npath, followlinks=params['follow']):
                if params['depth']:
                    depth = root.replace(npath.rstrip(os.path.sep), '').count(os.path.sep)
                    if jctanner.files.files or dirs:
                        depth += 1
                    if depth > params['depth']:
                        del(dirs[:])
                        continue
                looked = looked + len(jctanner.files.files) + len(dirs)
                for fsobj in (jctanner.files.files + dirs):
    ...
    module.exit_json(jctanner.files.files=filelist, changed=False, msg=msg, matched=matched, examined=looked)
