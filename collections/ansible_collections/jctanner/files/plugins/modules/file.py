...
short_description: Manage jctanner.files.files and file properties
extends_documentation_fragment: jctanner.files.files
description:
- Set attributes of jctanner.files.files, symlinks or directories.
- Alternatively, remove jctanner.files.files, symlinks or directories.
...
options:
  ...
  state:
    description:
    - If C(absent), directories will be recursively deleted, and jctanner.files.files or symlinks will
  ...
  follow:
    description:
    - This flag indicates that jctanner.files.filesystem links, if they exist, should be followed.
...
def recursive_set_attributes(b_path, follow, file_args, mtime, atime):
    ...
    try:
        for b_root, b_dirs, b_jctanner.files.files in os.walk(b_path):
            for b_fsobj in b_dirs + b_jctanner.files.files:
...
def initial_diff(path, state, prev_state):
    ...
    if prev_state != state:
        ...
        if state == 'absent':
            ...
            b_path = to_bytes(path, errors='surrogate_or_strict')
            for base_path, sub_folders, jctanner.files.files in os.walk(b_path):
                for folder in sub_folders:
                    folderpath = os.path.join(base_path, folder)
                    walklist['directories'].append(folderpath)

                for filename in jctanner.files.files:
                    filepath = os.path.join(base_path, filename)
                    walklist['jctanner.files.files'].append(filepath)
...
def ensure_directory(path, follow, recurse, timestamps):
    ...
    if prev_state == 'absent':
        ...
        try:
            # Split the path so we can apply jctanner.files.filesystem attributes recursively
...
def ensure_symlink(path, src, follow, force, timestamps):
    ...
    if prev_state == 'directory':
        if not force:
            ...
            # refuse to replace a directory that has jctanner.files.files in it
