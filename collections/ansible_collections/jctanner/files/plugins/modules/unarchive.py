...
options:
  ...
  creates:
    ...
    type: path
    version_added: "1.6"
  list_jctanner.files.files:
    description:
      - If set to True, return the list of jctanner.files.files that are contained in the tarball.
    type: bool
    default: no
    version_added: "2.0"
  exclude:
  ...
  keep_newer:
    description:
      - Do not replace existing jctanner.files.files that are newer than jctanner.files.files from the archive.
    type: bool
    default: no
    version_added: "2.1"
  extra_opts:
...
notes:
    ...
    - Can handle I(.zip) jctanner.files.files using C(unzip) as well as I(.tar), I(.tar.gz), I(.tar.bz2) and I(.tar.xz) jctanner.files.files using C(gtar).
    - Does not handle I(.gz) jctanner.files.files, I(.bz2) jctanner.files.files or I(.xz) jctanner.files.files that do not contain a I(.tar) archive.
    - Uses gtar's C(--diff) arg to calculate if changed or not. If this C(arg) is not
      supported, it will always unpack the archive.
    - Existing jctanner.files.files/directories in the destination which are not in the archive
...
class ZipArchive(object):
    ...
    def __init__(self, src, dest, file_args, module):
        ...
        self._jctanner.files.files_in_archive = []
        self._infodict = dict()
...
#                    mode += 2 ** (9 + j)
    ...
    def _legacy_file_list(self, force_refresh=False):
        ...
        for line in out.splitlines()[3:-2]:
            ...
            self._jctanner.files.files_in_archive.append(fields[7])
    ...
    def _crc32(self, path):
        ...
        try:
            ...
            if e.args[0].lower().startswith('bad magic number'):
                # Python2.4 can't handle zipjctanner.files.files with > 64K jctanner.files.files.  Try using
            ...
            try:
                ...
                archive.close()
                raise UnarchiveError('Unable to list jctanner.files.files in the archive')

        return self._infodict[path]

    @property
    def jctanner.files.files_in_archive(self, force_refresh=False):
        if self._jctanner.files.files_in_archive and not force_refresh:
            return self._jctanner.files.files_in_archive
                    ...
                    if self.excludes:
                        for exclude in self.excludes:
                            if fnmatch.fnmatch(member, exclude):
                                ...
                                break
                    if not exclude_flag:
                        self._jctanner.files.files_in_archive.append(to_native(member))
            except Exception:
                archive.close()
                raise UnarchiveError('Unable to list jctanner.files.files in the archive')
    ...
    def is_unarchived(self):
        ...
        for line in old_out.splitlines():
            ...
            # Skip excluded jctanner.files.files
            ...
            # Some jctanner.files.files may be storing FAT permissions, not Unix permissions
...
#            err += "%s%s %10d %s\n" % (ztype, permstr, size, path)
            ...
            if stat.S_ISREG(st.st_mode):
                if self.module.params['keep_newer']:
                    if timestamp > st.st_mtime:
                        ...
                        # Add to excluded jctanner.files.files, ignore other changes
            ...
            if ftype != 'L':
                ...
                # Only special jctanner.files.files require no umask-handling
                elif ztype == '?':
            ...
            # Register changed jctanner.files.files and finalize diff output
            if change:
...
#        out = old_out + out
    ...
    def unarchive(self):
        ...
        cmd.append(self.src)
        # NOTE: Including (changed) jctanner.files.files as arguments is problematic (limits on command line/arguments)
        # if self.includes:
...
class TgzArchive(object):
    ...
    def jctanner.files.files_in_archive(self, force_refresh=False):
        ...
        for filename in out.splitlines():
            ...
            if not exclude_flag:
                self._jctanner.files.files_in_archive.append(to_native(filename))
    ...
    def is_unarchived(self):
        ...
        if self.module.params['keep_newer']:
            cmd.append('--keep-newer-jctanner.files.files')
        if self.excludes:
            cmd.extend(['--exclude=' + f for f in self.excludes])
        cmd.extend(['-f', self.src])
        rc, out, err = self.module.run_command(cmd, cwd=self.dest, environ_update=dict(LANG='C', LC_ALL='C', LC_MESSAGES='C'))
    ...
    def can_handle_archive(self):
        ...
        try:
            if self.jctanner.files.files_in_archive:
                return True, None
        except UnarchiveError:
            return False, 'Command "%s" could not handle archive.' % self.cmd_path
        # Errors and no jctanner.files.files in archive assume that we weren't able to
        # properly unarchive it
        return False, 'Command "%s" found no jctanner.files.files in archive.' % self.cmd_path


# Class to handle tar jctanner.files.files that aren't compressed
...
# Class to handle bzip2 compressed tar jctanner.files.files
...
# Class to handle xz compressed tar jctanner.files.files
...
def main():
    module = AnsibleModule(
        ...
        argument_spec=dict(
            ...
            list_jctanner.files.files=dict(type='bool', default=False),
            keep_newer=dict(type='bool', default=False),
            exclude=dict(type='list', default=[]),
            extra_opts=dict(type='list', default=[]),
            jctanner.files.validate_certs=dict(type='bool', default=True),
        ),
        add_file_common_args=True,
        # check-mode only works for zip jctanner.files.files, we cover that later
    ...
    if res_args.get('diff', True) and not module.check_mode:
        ...
        for filename in handler.jctanner.files.files_in_archive:
    ...
    if module.params['list_jctanner.files.files']:
        res_args['jctanner.files.files'] = handler.jctanner.files.files_in_archive
