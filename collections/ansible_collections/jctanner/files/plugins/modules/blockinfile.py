...
- name: Insert/Update eth0 configuration stanza in /etc/network/interfaces
        (it might be better to copy jctanner.files.files into /etc/network/interfaces.d/)
  blockinfile:
...
- name: Insert/Update configuration using a local file and jctanner.files.validate it
  blockinfile:
...
def write_changes(module, contents, path):
    ...
    valid = not jctanner.files.validate
    if jctanner.files.validate:
        ...
        (rc, out, err) = module.run_command(jctanner.files.validate % tmpfile)
        valid = rc == 0
        if rc != 0:
            module.fail_json(msg='failed to jctanner.files.validate: '
                                 'rc:%s error:%s' % (rc, err))
    if valid:
