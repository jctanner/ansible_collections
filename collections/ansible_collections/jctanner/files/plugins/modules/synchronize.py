...
options:
  ...
  existing_only:
    description:
      - Skip creating new jctanner.files.files on receiver.
    type: bool
    default: no
    version_added: "1.5"
  delete:
    description:
      - Delete jctanner.files.files in C(dest) that don't exist (after transfer, not before) in the C(src) path.
...
notes:
   ...
   - Inspect the verbose output to jctanner.files.validate the destination user/host/path are what was expected.
   - To exclude jctanner.files.files and directories from being synchronized, you may add C(.rsync-filter) jctanner.files.files to the source directory.
   ...
   - The C(synchronize) module forces `--delay-updates` to avoid leaving a destination in a broken in-between state if the underlying rsync process
     encounters an error. Those synchronizing large numbers of jctanner.files.files that are willing to trade safety for performance should call rsync directly.
...
- name: Synchronize and delete jctanner.files.files in dest on the remote host that are not found in src of localhost.
  synchronize:
    src: some/relative/path
    dest: /some/absolute/path
    delete: yes
...
# Hardlink jctanner.files.files if they didn't change
- name: Use hardlinks when synchronizing jctanner.files.filesystems
  synchronize:
