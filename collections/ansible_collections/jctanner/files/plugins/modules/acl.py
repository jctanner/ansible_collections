...
options:
  ...
  recalculate_mask:
    description:
    - Select if and when to recalculate the effective right masks of the jctanner.files.files.
...
notes:
- The C(acl) module requires that ACLs are enabled on the target jctanner.files.filesystem and that the C(setfacl) and C(getfacl) binaries are installed.
