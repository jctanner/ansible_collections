...
module: archive
version_added: '2.3'
short_description: Creates a compressed archive of one or more jctanner.files.files or trees
extends_documentation_fragment: jctanner.files.files
description:
    - Packs an archive.
    ...
    - Source jctanner.files.files can be deleted after archival by specifying I(remove=True).
options:
  path:
    description:
      - Remote absolute path, glob, or list of paths or globs for the file or jctanner.files.files to compress or archive.
    type: list
    required: true
  format:
    description:
  ...
  dest:
    description:
      ...
      - This is required when C(path) refers to multiple jctanner.files.files by either specifying a glob, a directory or multiple paths in a list.
    type: path
  exclude_path:
    description:
      - Remote absolute path, glob, or list of paths or globs for the file or jctanner.files.files to exclude from the archive.
    type: list
    version_added: '2.4'
  force_archive:
    version_added: '2.8'
  ...
  remove:
    description:
      - Remove any added source jctanner.files.files and trees after adding to archive.
    type: bool
    default: no
notes:
    - Requires tarfile, zipfile, gzip and bzip2 packages on target host.
    - Requires lzma or backports.lzma if using xz format.
    - Can produce I(gzip), I(bzip2), I(lzma) and I(zip) compressed jctanner.files.files or archives.
seealso:
- module: unarchive
author:
- Ben Doherty (@bendoh)
...
- name: Create a bz2 archive of multiple jctanner.files.files, rooted at /path
  archive:
    path:
    - /path/to/foo
    - /path/wong/foo
...
state:
    description:
        ...
        If 'absent', then no source jctanner.files.files were found and the archive does not exist.
    ...
    returned: always
missing:
    description: Any jctanner.files.files that were missing from the source.
    type: list
    returned: success
archived:
    description: Any jctanner.files.files that were compressed or added to the archive.
    type: list
    returned: success
arcroot:
...
def main():
    ...
    if not force_archive:
        # If we actually matched multiple jctanner.files.files or TRIED to, then
    ...
    if archive and not dest:
        module.fail_json(dest=dest, path=', '.join(paths), msg='Error, must specify "dest" when archiving multiple jctanner.files.files or trees')
    ...
    for path in expanded_paths:
        # Use the longest common directory name among all the jctanner.files.files
    ...
    # No source jctanner.files.files were found but the named archive exists: are we 'compress' or 'archive' now?
    ...
    # Multiple jctanner.files.files, or globbiness
    elif archive:
        if not archive_paths:
            # No source jctanner.files.files were found, but the archive is there.
            if os.path.lexists(dest):
                state = 'archive'
        elif missing:
            # SOME source jctanner.files.files were found, but not all of them
            state = 'incomplete'
        ...
        if state in ['archive', 'incomplete'] and remove:
            ...
            if errors:
                module.fail_json(dest=dest, msg='Error deleting some source jctanner.files.files: ', jctanner.files.files=errors)
            ...
            if module.check_mode:
                ...
                try:
                    if format == 'zip':
                        ...
                        arcfile.close()
                        state = 'archive'  # because all zip jctanner.files.files are archives
                    elif format == 'tar':
                        arcfile = tarfile.open(dest, 'w')
                        arcfile.add(path)
                        arcfile.close()
