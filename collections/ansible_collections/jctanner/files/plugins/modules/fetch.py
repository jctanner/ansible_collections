...
---
module: fetch
short_description: Fetch jctanner.files.files from remote nodes
description:
- This module works like M(copy), but in reverse.
- It is used for fetching jctanner.files.files from remote machines and storing them locally in a file tree, organized by hostname.
...
options:
  ...
  fail_on_missing:
    ...
    type: bool
    default: yes
  jctanner.files.validate_checksum:
    version_added: '1.4'
    description:
    - Verify that the source and destination checksums match after the jctanner.files.files are fetched.
    type: bool
    default: yes
  flat:
    version_added: '1.2'
