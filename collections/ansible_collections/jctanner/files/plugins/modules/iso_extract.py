...
module: iso_extract
short_description: Extract jctanner.files.files from an ISO image
description:
- This module has two possible ways of operation.
- If 7zip is installed on the system, this module extracts jctanner.files.files from an ISO
  into a temporary directory and copies jctanner.files.files to a given destination,
  if needed.
- If the user has mount-capabilities (CAP_SYS_ADMIN on Linux) this module
  mounts the ISO image to a temporary location, and copies jctanner.files.files to a given
...
options:
  image:
    description:
    - The ISO image to extract jctanner.files.files from.
    type: path
    required: yes
    aliases: [ path, src ]
  dest:
    description:
    - The destination directory to extract jctanner.files.files to.
    type: path
    required: yes
  jctanner.files.files:
    description:
    - A list of jctanner.files.files to extract from the image.
  ...
  executable:
    description:
    - The path to the C(7z) executable to use for extracting jctanner.files.files from the ISO.
    type: path
    default: '7z'
    version_added: '2.4'
notes:
- Only the file checksum (content) is taken into account when extracting jctanner.files.files
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            jctanner.files.files=dict(type='list', required=True),
    ...
    jctanner.files.files = module.params['jctanner.files.files']
    ...
    result['jctanner.files.files'] = []
    extract_jctanner.files.files = list(jctanner.files.files)

    if not force:
        # Check if we have to process any jctanner.files.files based on existence
        for f in jctanner.files.files:
            dest_file = os.path.join(dest, os.path.basename(f))
            if os.path.exists(dest_file):
                result['jctanner.files.files'].append(dict(
                    checksum=None,
                    dest=dest_file,
                    src=f,
                ))
                extract_jctanner.files.files.remove(f)

    if not extract_jctanner.files.files:
    ...
    if binary:
        cmd = '%s x "%s" -o"%s" %s' % (binary, image, tmp_dir, ' '.join([quote(f) for f in extract_jctanner.files.files]))
    else:
    ...
    try:
        for f in extract_jctanner.files.files:
