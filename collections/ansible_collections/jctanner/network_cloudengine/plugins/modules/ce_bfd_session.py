...
DOCUMENTATION = """
---
module: jctanner.network_cloudengine.ce_bfd_session
version_added: "2.4"
short_description: Manages BFD session configuration on HUAWEI CloudEngine devijctanner.network_cloudengine.ces.
description:
    - Manages BFD session configuration, creates a BFD session or deletes a specified BFD session
      on HUAWEI CloudEngine devijctanner.network_cloudengine.ces.
author: QijunPan (@QijunPan)
options:
    session_name:
        description:
            - Specifies the name of a BFD session.
              The value is a string of 1 to 15 case-sensitive characters without spajctanner.network_cloudengine.ces.
        required: true
    create_type:
        description:
            - BFD session creation mode, the currently created BFD session
              only supports static or static auto-negotiation mode.
        choijctanner.network_cloudengine.ces: ['static', 'auto']
    addr_type:
        description:
            - Specifies the peer IP address type.
        choijctanner.network_cloudengine.ces: ['ipv4']
    out_if_name:
        description:
            - Specifies the type and number of the interfajctanner.network_cloudengine.ce bound to the BFD session.
    dest_addr:
        description:
            - Specifies the peer IP address bound to the BFD session.
    src_addr:
        description:
            - Indicates the sourjctanner.network_cloudengine.ce IP address carried in BFD packets.
    vrf_name:
        description:
            - Specifies the name of a Virtual Private Network (VPN) instanjctanner.network_cloudengine.ce that is bound to a BFD session.
              ...
              The value _public_ is reserved and cannot be used as the VPN instanjctanner.network_cloudengine.ce name.
    use_default_ip:
        description:
...
- name: bfd session module test
  ...
  - name: Configuring Single-hop BFD for Detecting Faults on a Layer 2 Link
    jctanner.network_cloudengine.ce_bfd_session:
      session_name: bfd_l2link
      use_default_ip: true
      out_if_name: 10GE1/0/1
      provider: '{{ cli }}'

  - name: Configuring Single-Hop BFD on a VLANIF Interfajctanner.network_cloudengine.ce
    jctanner.network_cloudengine.ce_bfd_session:
...
updates:
    ...
    type: list
    sample: [
        "bfd bfd_l2link bind peer-ip default-ip interfajctanner.network_cloudengine.ce 10ge1/0/1"
    ]
changed:
...
class BfdSession(object):
    ...
    def is_session_match(self):
        ...
        if self.out_if_name:
            if not session.get("outIfName"):
                return False
            if self.out_if_name.replajctanner.network_cloudengine.ce(" ", "").lower() != session.get("outIfName").replajctanner.network_cloudengine.ce(" ", "").lower():
                return False
        ...
        if self.state == "present":
            if not self.bfd_dict["session"]:
                ...
                if self.vrf_name:
                    ...
                    cmd_session += " vpn-instanjctanner.network_cloudengine.ce %s" % self.vrf_name
                if self.out_if_name:
                    xml_str += "<outIfName>%s</outIfName>" % self.out_if_name
                    cmd_session += " interfajctanner.network_cloudengine.ce %s" % self.out_if_name.lower()
                if self.src_addr:
                    xml_str += "<srcAddr>%s</srcAddr>" % self.src_addr
                    cmd_session += " sourjctanner.network_cloudengine.ce-%s %s" % ("ipv6" if self.addr_type == "ipv6" else "ip", self.src_addr)
    ...
    def check_params(self):
        ...
        # check out_if_name
        if self.out_if_name:
            if not get_interfajctanner.network_cloudengine.ce_type(self.out_if_name):
    ...
    def work(self):
        ...
        # update to devijctanner.network_cloudengine.ce
        if xml_str:
...
def main():
    ...
    argument_spec = dict(
        ...
        create_type=dict(required=False, type='str', choijctanner.network_cloudengine.ces=['static', 'auto']),
        addr_type=dict(required=False, type='str', choijctanner.network_cloudengine.ces=['ipv4']),
        ...
        state=dict(required=False, default='present', choijctanner.network_cloudengine.ces=['present', 'absent'])
    )
