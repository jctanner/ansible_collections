...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_stp
version_added: "2.4"
...
options:
    ...
    stp_mode:
        description:
            - Set an operation mode for the current MSTP projctanner.network_cloudengine.cess.
              The mode can be STP, RSTP, or MSTP.
        choijctanner.network_cloudengine.ces: ['stp', 'rstp', 'mstp']
    stp_enable:
        description:
    ...
    stp_converge:
        description:
            - STP convergenjctanner.network_cloudengine.ce mode.
              Fast means set STP aging mode to Fast.
              Normal means set STP aging mode to Normal.
        choijctanner.network_cloudengine.ces: ['fast', 'normal']
    bpdu_protection:
        description:
    ...
    tc_protection:
        description:
            - Configure the TC BPDU protection function for an MSTP projctanner.network_cloudengine.cess.
        choijctanner.network_cloudengine.ces: ['enable', 'disable']
    tc_protection_interval:
        description:
            - Set the time the MSTP devijctanner.network_cloudengine.ce takes to handle the maximum number of TC BPDUs
    ...
    interfajctanner.network_cloudengine.ce:
        description:
            - Interfajctanner.network_cloudengine.ce name.
              If the value is C(all), will apply configuration to all interfajctanner.network_cloudengine.ces.
    ...
    cost:
        description:
            - Set the path cost of the current port.
              The default instanjctanner.network_cloudengine.ce is 0.
    root_protection:
        description:
...
- name: CloudEngine stp test
  ...
  - name: "Config stp mode"
    jctanner.network_cloudengine.ce_stp:
      state: present
      stp_mode: stp
      provider: "{{ cli }}"
...
class Stp(object):
    ...
    def check_params(self):
        ...
        if self.root_protection or self.loop_protection or self.cost:
            if not self.interfajctanner.network_cloudengine.ce:
                self.module.fail_json(
                    msg='Error: Please input interfajctanner.network_cloudengine.ce.')
            elif self.interfajctanner.network_cloudengine.ce == "all":
                self.module.fail_json(
                    msg='Error: Interfajctanner.network_cloudengine.ce can not be all when config root_protection or loop_protection or cost.')

        if self.root_protection and self.root_protection == "enable":
            if self.loop_protection and self.loop_protection == "enable":
                self.module.fail_json(
                    msg='Error: Can not enable root_protection and loop_protection at the same interfajctanner.network_cloudengine.ce.')
    ...
    def get_existing(self):
        ...
        if self.interfajctanner.network_cloudengine.ce and self.interfajctanner.network_cloudengine.ce != "all":
            self.cli_get_interfajctanner.network_cloudengine.ce_stp_config()
        ...
        if self.edged_port:
            if self.interfajctanner.network_cloudengine.ce == "all":
                ...
                if "stp edged-port enable" in self.interfajctanner.network_cloudengine.ce_stp_cfg:
                    self.cur_cfg["edged_port"] = "enable"
                    self.existing["edged_port"] = "enable"
                else:
                    self.cur_cfg["edged_port"] = "disable"
        ...
        if self.bpdu_filter:
            if self.interfajctanner.network_cloudengine.ce == "all":
                ...
                if "stp bpdu-filter enable" in self.interfajctanner.network_cloudengine.ce_stp_cfg:
                    self.cur_cfg["bpdu_filter"] = "enable"
                    self.existing["bpdu_filter"] = "enable"
                else:
                    self.cur_cfg["bpdu_filter"] = "disable"
        ...
        if self.tc_protection_interval:
            if "stp tc-protection interval" in self.stp_cfg:
                ...
                if not tmp_value:
                    self.module.fail_json(
                        msg='Error: Can not find tc-protection interval on the devijctanner.network_cloudengine.ce.')
        ...
        if self.tc_protection_threshold:
            if "stp tc-protection threshold" in self.stp_cfg:
                ...
                if not tmp_value:
                    self.module.fail_json(
                        msg='Error: Can not find tc-protection threshold on the devijctanner.network_cloudengine.ce.')
        ...
        if self.cost:
            tmp_value = re.findall(r'stp instanjctanner.network_cloudengine.ce (.*) cost (.*)', self.interfajctanner.network_cloudengine.ce_stp_cfg)
            if not tmp_value:
        ...
        if self.root_protection or self.loop_protection:
            if "stp root-protection" in self.interfajctanner.network_cloudengine.ce_stp_cfg:
            ...
            if "stp loop-protection" in self.interfajctanner.network_cloudengine.ce_stp_cfg:
    ...
    def present_stp(self):
        ...
        # config interfajctanner.network_cloudengine.ce stp
        if self.interfajctanner.network_cloudengine.ce and self.interfajctanner.network_cloudengine.ce != "all":
            tmp_changed = False

            cmd = "interfajctanner.network_cloudengine.ce %s" % self.interfajctanner.network_cloudengine.ce
            cmds.append(cmd)
            self.updates_cmd.append(cmd)
            ...
            if self.root_protection:
                if self.root_protection == "enable" and self.cur_cfg["loop_protection"] == "enable":
                    self.module.fail_json(
                        msg='Error: The interfajctanner.network_cloudengine.ce has enable loop_protection, can not enable root_protection.')
            ...
            if self.loop_protection:
                if self.loop_protection == "enable" and self.cur_cfg["root_protection"] == "enable":
                    self.module.fail_json(
                        msg='Error: The interfajctanner.network_cloudengine.ce has enable root_protection, can not enable loop_protection.')
    ...
    def absent_stp(self):
        ...
        # undo interfajctanner.network_cloudengine.ce stp
        if self.interfajctanner.network_cloudengine.ce and self.interfajctanner.network_cloudengine.ce != "all":
            tmp_changed = False
...
def main():
    ...
    argument_spec = dict(
        ...
        stp_mode=dict(choijctanner.network_cloudengine.ces=['stp', 'rstp', 'mstp']),
        stp_enable=dict(choijctanner.network_cloudengine.ces=['enable', 'disable']),
        stp_converge=dict(choijctanner.network_cloudengine.ces=['fast', 'normal']),
        bpdu_protection=dict(choijctanner.network_cloudengine.ces=['enable', 'disable']),
        tc_protection=dict(choijctanner.network_cloudengine.ces=['enable', 'disable']),
        ...
        edged_port=dict(choijctanner.network_cloudengine.ces=['enable', 'disable']),
        bpdu_filter=dict(choijctanner.network_cloudengine.ces=['enable', 'disable']),
        cost=dict(type='str'),
        root_protection=dict(choijctanner.network_cloudengine.ces=['enable', 'disable']),
        loop_protection=dict(choijctanner.network_cloudengine.ces=['enable', 'disable'])
    )
