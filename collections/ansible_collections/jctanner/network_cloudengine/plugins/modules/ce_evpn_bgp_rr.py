...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_evpn_bgp_rr
version_added: "2.4"
...
options:
    ...
    bgp_instanjctanner.network_cloudengine.ce:
        description:
            - Specifies the name of a BGP instanjctanner.network_cloudengine.ce.
              The value of instanjctanner.network_cloudengine.ce-name can be an integer 1 or a string of 1 to 31.
    bgp_evpn_enable:
        description:
    ...
    peer_type:
        ...
        choijctanner.network_cloudengine.ces: ['group_name','ipv4_address']
    peer:
        description:
            - Specifies the IPv4 address or the group name of a peer.
    reflect_client:
        description:
            - Configure the local devijctanner.network_cloudengine.ce as the route reflector and the peer or peer group as the client of the route reflector.
        choijctanner.network_cloudengine.ces: ['enable','disable']
    policy_vpn_target:
        description:
...
- name: BGP RR test
  ...
  - name: "Configure BGP-EVPN address family view and ensure that BGP view has existed."
    jctanner.network_cloudengine.ce_evpn_bgp_rr:
      as_number: 20
...
proposed:
    ...
    sample: {
                "as_number": "20",
                "bgp_evpn_enable": "enable",
                "bgp_instanjctanner.network_cloudengine.ce": null,
                "peer": "192.8.3.3",
                "peer_type": "ipv4_address",
                "policy_vpn_target": "disable",
                "reflect_client": "enable"
...
class EvpnBgpRr(object):
    ...
    def is_bgp_view_exist(self):
        ...
        if self.bgp_instanjctanner.network_cloudengine.ce:
            view_cmd = "bgp %s instanjctanner.network_cloudengine.ce %s" % (
                self.as_number, self.bgp_instanjctanner.network_cloudengine.ce)
        else:
    ...
    def get_existing(self):
        ...
        self.existing = dict(as_number=self.as_number,
                             bgp_instanjctanner.network_cloudengine.ce=self.bgp_instanjctanner.network_cloudengine.ce,
    ...
    def check_params(self):
        ...
        if self.bgp_instanjctanner.network_cloudengine.ce:
            if len(self.bgp_instanjctanner.network_cloudengine.ce) < 1 or len(self.bgp_instanjctanner.network_cloudengine.ce) > 31:
                self.module.fail_json(
                    msg="Error: The length of BGP instanjctanner.network_cloudengine.ce-name must be between 1 or a string of 1 to and 31.")
...
def main():
    ...
    argument_spec = dict(
        ...
        bgp_instanjctanner.network_cloudengine.ce=dict(required=False, type='str'),
        bgp_evpn_enable=dict(required=False, type='str',
                             default='enable', choijctanner.network_cloudengine.ces=['enable', 'disable']),
        peer_type=dict(required=False, type='str', choijctanner.network_cloudengine.ces=[
            'group_name', 'ipv4_address']),
        peer=dict(required=False, type='str'),
        reflect_client=dict(required=False, type='str',
                            choijctanner.network_cloudengine.ces=['enable', 'disable']),
        policy_vpn_target=dict(required=False, choijctanner.network_cloudengine.ces=['enable', 'disable']),
    )
