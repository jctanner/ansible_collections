...
DOCUMENTATION = """
---
module: jctanner.network_cloudengine.ce_netstream_aging
version_added: "2.4"
...
options:
    ...
    timeout_type:
        ...
        choijctanner.network_cloudengine.ces: ['active', 'inactive', 'tcp-session', 'manual']
    manual_slot:
        description:
...
- name: netstream aging module test
  ...
  - name: Configure netstream ip timeout active interval , the interval is 40 minutes.
    jctanner.network_cloudengine.ce_netstream_aging:
      timeout_interval: 40
      type: ip
      timeout_type: active
      state: present
...
def main():
    ...
    argument_spec = dict(
        ...
        type=dict(required=False, choijctanner.network_cloudengine.ces=['ip', 'vxlan']),
        state=dict(required=False, choijctanner.network_cloudengine.ces=['present', 'absent'], default='present'),
        timeout_type=dict(required=False, choijctanner.network_cloudengine.ces=['active', 'inactive', 'tcp-session', 'manual']),
