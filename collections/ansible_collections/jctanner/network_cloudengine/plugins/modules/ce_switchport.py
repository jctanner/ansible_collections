...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_switchport
version_added: "2.4"
short_description: Manages Layer 2 switchport interfajctanner.network_cloudengine.ces on HUAWEI CloudEngine switches.
description:
    - Manages Layer 2 switchport interfajctanner.network_cloudengine.ces on HUAWEI CloudEngine switches.
author: QijunPan (@QijunPan)
notes:
    - When C(state=absent), VLANs can be added/removed from trunk links and
      the existing acjctanner.network_cloudengine.cess VLAN can be 'unconfigured' to just having VLAN 1
      on that interfajctanner.network_cloudengine.ce.
    - When working with trunks VLANs the keywords add/remove are always sent
      ...
      commands sent.
    - When C(state=unconfigured), the interfajctanner.network_cloudengine.ce will result with having a default
      Layer 2 interfajctanner.network_cloudengine.ce, i.e. vlan 1 in acjctanner.network_cloudengine.cess mode.
options:
    interfajctanner.network_cloudengine.ce:
        description:
            - Full name of the interfajctanner.network_cloudengine.ce, i.e. 40GE1/0/22.
        required: true
    mode:
        description:
            - The link type of an interfajctanner.network_cloudengine.ce.
        choijctanner.network_cloudengine.ces: ['acjctanner.network_cloudengine.cess','trunk']
    acjctanner.network_cloudengine.cess_vlan:
        description:
            - If C(mode=acjctanner.network_cloudengine.cess), used as the acjctanner.network_cloudengine.cess VLAN ID, in the range from 1 to 4094.
    native_vlan:
        description:
    ...
    state:
        ...
        default: present
        choijctanner.network_cloudengine.ces: ['present', 'absent', 'unconfigured']
'''
...
- name: switchport module test
  ...
  - name: Ensure 10GE1/0/22 is in its default switchport state
    jctanner.network_cloudengine.ce_switchport:
      interfajctanner.network_cloudengine.ce: 10GE1/0/22
      state: unconfigured
      provider: '{{ cli }}'

  - name: Ensure 10GE1/0/22 is configured for acjctanner.network_cloudengine.cess vlan 20
    jctanner.network_cloudengine.ce_switchport:
      interfajctanner.network_cloudengine.ce: 10GE1/0/22
      mode: acjctanner.network_cloudengine.cess
      acjctanner.network_cloudengine.cess_vlan: 20
      provider: '{{ cli }}'
...
proposed:
    ...
    returned: always
    type: dict
    sample: {"acjctanner.network_cloudengine.cess_vlan": "20", "interfajctanner.network_cloudengine.ce": "10GE1/0/22", "mode": "acjctanner.network_cloudengine.cess"}
existing:
    description: k/v pairs of existing switchport
    returned: always
    type: dict
    sample: {"acjctanner.network_cloudengine.cess_vlan": "10", "interfajctanner.network_cloudengine.ce": "10GE1/0/22",
             "mode": "acjctanner.network_cloudengine.cess", "switchport": "enable"}
end_state:
    description: k/v pairs of switchport after module execution
    returned: always
    type: dict
    sample: {"acjctanner.network_cloudengine.cess_vlan": "20", "interfajctanner.network_cloudengine.ce": "10GE1/0/22",
             "mode": "acjctanner.network_cloudengine.cess", "switchport": "enable"}
updates:
    description: command string sent to the devijctanner.network_cloudengine.ce
    returned: always
    type: list
   ...
   <ethernet xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0">
    <ethernetIfs>
        <ethernetIf operation="merge">
            <ifName>%s</ifName>
            <l2Attribute>
                <linkType>acjctanner.network_cloudengine.cess</linkType>
                <pvid>%s</pvid>
                <trunkVlans></trunkVlans>
...
class SwitchPort(object):
    ...
    def merge_acjctanner.network_cloudengine.cess_vlan(self, ifname, acjctanner.network_cloudengine.cess_vlan):
        """Merge acjctanner.network_cloudengine.cess interfajctanner.network_cloudengine.ce vlan"""
        ...
        if self.state == "present":
            if self.intf_info["linkType"] == "acjctanner.network_cloudengine.cess":
                if acjctanner.network_cloudengine.cess_vlan and self.intf_info["pvid"] != acjctanner.network_cloudengine.cess_vlan:
                    self.updates_cmd.append(
                        "port default vlan %s" % acjctanner.network_cloudengine.cess_vlan)
                    conf_str = CE_NC_SET_ACCESS_PORT % (ifname, acjctanner.network_cloudengine.cess_vlan)
                    change = True
            else:  # not acjctanner.network_cloudengine.cess
                self.updates_cmd.append("port link-type acjctanner.network_cloudengine.cess")
                if acjctanner.network_cloudengine.cess_vlan:
                    self.updates_cmd.append(
                        "port default vlan %s" % acjctanner.network_cloudengine.cess_vlan)
                    conf_str = CE_NC_SET_ACCESS_PORT % (ifname, acjctanner.network_cloudengine.cess_vlan)
                else:
            ...
            if self.intf_info["linkType"] == "acjctanner.network_cloudengine.cess":
                if acjctanner.network_cloudengine.cess_vlan and self.intf_info["pvid"] == acjctanner.network_cloudengine.cess_vlan and acjctanner.network_cloudengine.cess_vlan != "1":
                    self.updates_cmd.append(
                        "undo port default vlan %s" % acjctanner.network_cloudengine.cess_vlan)
                    conf_str = CE_NC_SET_ACCESS_PORT % (ifname, "1")
                    change = True
            else:  # not acjctanner.network_cloudengine.cess
                self.updates_cmd.append("port link-type acjctanner.network_cloudengine.cess")
        ...
        if not change:
            self.updates_cmd.pop()   # remove interfajctanner.network_cloudengine.ce
            return
    ...
    def check_params(self):
        ...
        if not self.intf_type or not is_portswitch_enalbed(self.intf_type):
            self.module.fail_json(msg='Error: Interfajctanner.network_cloudengine.ce %s is error.')

        # check acjctanner.network_cloudengine.cess_vlan
        if self.acjctanner.network_cloudengine.cess_vlan:
            if not self.acjctanner.network_cloudengine.cess_vlan.isdigit():
                self.module.fail_json(msg='Error: Acjctanner.network_cloudengine.cess vlan id is invalid.')
            if int(self.acjctanner.network_cloudengine.cess_vlan) <= 0 or int(self.acjctanner.network_cloudengine.cess_vlan) > 4094:
                self.module.fail_json(
                    msg='Error: Acjctanner.network_cloudengine.cess vlan id is not in the range from 1 to 4094.')
        ...
        if not self.is_l2switchport():
            self.module.fail_json(
                msg='Error: Interfajctanner.network_cloudengine.ce is not layer2 swtich port.')
    ...
    def get_proposed(self):
        ...
        self.proposed['acjctanner.network_cloudengine.cess_vlan'] = self.acjctanner.network_cloudengine.cess_vlan
    ...
    def get_existing(self):
        ...
        if self.intf_info:
            ...
            self.existing['acjctanner.network_cloudengine.cess_vlan'] = self.intf_info["pvid"]
    ...
    def get_end_state(self):
        ...
        if self.intf_info:
            ...
            if end_info:
                ...
                self.end_state['acjctanner.network_cloudengine.cess_vlan'] = end_info["pvid"]
    ...
    def work(self):
        ...
        if self.state == "present" or self.state == "absent":
            if self.mode == "acjctanner.network_cloudengine.cess":
                self.merge_acjctanner.network_cloudengine.cess_vlan(self.interfajctanner.network_cloudengine.ce, self.acjctanner.network_cloudengine.cess_vlan)
            elif self.mode == "trunk":
                self.merge_trunk_vlan(
                    self.interfajctanner.network_cloudengine.ce, self.native_vlan, self.trunk_vlans)
        # unconfigured
        else:
            self.default_switchport(self.interfajctanner.network_cloudengine.ce)
...
def main():
    ...
    argument_spec = dict(
        ...
        mode=dict(choijctanner.network_cloudengine.ces=['acjctanner.network_cloudengine.cess', 'trunk'], required=False),
        acjctanner.network_cloudengine.cess_vlan=dict(type='str', required=False),
        native_vlan=dict(type='str', required=False),
        trunk_vlans=dict(type='str', required=False),
        state=dict(choijctanner.network_cloudengine.ces=['absent', 'present', 'unconfigured'],
                   default='present')
    )
