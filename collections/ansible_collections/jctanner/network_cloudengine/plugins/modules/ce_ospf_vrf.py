...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_ospf_vrf
version_added: "2.4"
short_description: Manages configuration of an OSPF VPN instanjctanner.network_cloudengine.ce on HUAWEI CloudEngine switches.
description:
    - Manages configuration of an OSPF VPN instanjctanner.network_cloudengine.ce on HUAWEI CloudEngine switches.
author: Yang yang (@QijunPan)
options:
    ospf:
        description:
            - The ID of the ospf projctanner.network_cloudengine.cess.
    ...
    vrf:
        description:
            - Specifies the vpn instanjctanner.network_cloudengine.ce which use ospf,length is 1 - 31.
              Valid values are a string.
        default: _public_
    description:
        description:
            - Specifies the description information of ospf projctanner.network_cloudengine.cess.
    bandwidth:
        description:
            - Specifies the referenjctanner.network_cloudengine.ce bandwidth used to assign ospf cost.
    ...
    lsaointervalflag:
        description:
            - Specifies whether canjctanner.network_cloudengine.cel the interval of LSA originate or not.
              If set the parameter but noe specifies value, the default will be used.
              true:canjctanner.network_cloudengine.cel the interval of LSA originate, the interval is 0.
              false:do not canjctanner.network_cloudengine.cel the interval of LSA originate.
        type: bool
        default: 'no'
    lsaointerval:
        description:
    ...
    spfintervaltype:
        ...
        choijctanner.network_cloudengine.ces: ['intelligent-timer','timer','millisecond']
...
- name: ospf vrf module test
  ...
  - name: Configure ospf route id
    jctanner.network_cloudengine.ce_ospf_vrf:
      ospf: 2
      route_id: 2.2.2.2
...
proposed:
    ...
    sample: {
        ...
        "projctanner.network_cloudengine.cess_id": "2",
        "route_id": "2.2.2.2",
...
existing:
    ...
    type: dict
    sample: {
                "bandwidthReferenjctanner.network_cloudengine.ce": "100",
                "description": null,
                ...
                "projctanner.network_cloudengine.cessId": "2",
                "routerId": "2.2.2.2",
    ...
    <filter type="subtree">
      <ospfv2 xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0">
        <ospfv2comm>
          <ospfSites>
            <ospfSite>
              <projctanner.network_cloudengine.cessId></projctanner.network_cloudengine.cessId>
              <routerId></routerId>
              <vrfName></vrfName>
              <description></description>
              <bandwidthReferenjctanner.network_cloudengine.ce></bandwidthReferenjctanner.network_cloudengine.ce>
...
%s
              ...
              <bandwidthReferenjctanner.network_cloudengine.ce>%s</bandwidthReferenjctanner.network_cloudengine.ce>
...
class OspfVrf(object):
    ...
    def is_valid_ospf_projctanner.network_cloudengine.cess_id(self):
        """check whether the input ospf projctanner.network_cloudengine.cess id is valid"""
    ...
    def is_valid_bandwidth(self):
        """check whether the input ospf bandwidth referenjctanner.network_cloudengine.ce is valid"""
    ...
    def is_route_id_exist(self):
        ...
        for ospf_site in self.ospf_info["ospfsite"]:
            if ospf_site["projctanner.network_cloudengine.cessId"] != self.ospf:
                continue
    ...
    def get_exist_ospf_id(self):
        """get exist ospf projctanner.network_cloudengine.cess id"""
        ...
        for ospf_site in self.ospf_info["ospfsite"]:
            if ospf_site["projctanner.network_cloudengine.cessId"] == self.ospf:
                return ospf_site["projctanner.network_cloudengine.cessId"]
            else:
                continue
        return None
    ...
    def get_exist_bandwidth(self):
        ...
        for ospf_site in self.ospf_info["ospfsite"]:
            if ospf_site["projctanner.network_cloudengine.cessId"] == self.ospf:
                return ospf_site["bandwidthReferenjctanner.network_cloudengine.ce"]
            else:
                continue
        return None
    ...
    def check_params(self):
        ...
        if self.ospf == '':
            self.module.fail_json(
                msg='Error: The ospf projctanner.network_cloudengine.cess id should not be null.')
        if self.ospf:
            if not self.is_valid_ospf_projctanner.network_cloudengine.cess_id():
                self.module.fail_json(
                    msg='Error: The ospf projctanner.network_cloudengine.cess id should between 1 - 4294967295.')
        if self.route_id == '':
            self.module.fail_json(
        ...
        if self.vrf == '':
            self.module.fail_json(
                msg='Error: The ospf vpn instanjctanner.network_cloudengine.ce length should not be null.')
        if self.vrf:
            if not self.is_valid_vrf_name():
                self.module.fail_json(
                    msg='Error: The ospf vpn instanjctanner.network_cloudengine.ce length should between 0 - 31,but can not contain " " or "?".')
        if self.description == '':
            self.module.fail_json(
        ...
        if self.bandwidth == '':
            self.module.fail_json(
                msg='Error: The ospf bandwidth referenjctanner.network_cloudengine.ce should not be null.')
        if self.bandwidth:
            if not self.is_valid_bandwidth():
                self.module.fail_json(
                    msg='Error: The ospf bandwidth referenjctanner.network_cloudengine.ce should between 1 - 2147483648.')
        ...
        if ospf_sites:
            for ospf_site in ospf_sites:
                ...
                for ospf_site_ele in ospf_site:
                    if ospf_site_ele.tag in ["projctanner.network_cloudengine.cessId", "routerId", "vrfName", "bandwidthReferenjctanner.network_cloudengine.ce",
                ...
                if ospf_ele_info["projctanner.network_cloudengine.cessId"] == self.ospf:
                    self.ospf_info["ospfsite"].append(ospf_ele_info)

    def get_proposed(self):
        """get proposed info"""

        self.proposed["projctanner.network_cloudengine.cess_id"] = self.ospf
    ...
    def operate_ospf_info(self):
        ...
        if description is None:
            description = ''
        bandwidth_referenjctanner.network_cloudengine.ce = self.get_exist_bandwidth()
        if bandwidth_referenjctanner.network_cloudengine.ce is None:
            bandwidth_referenjctanner.network_cloudengine.ce = '100'
        ...
        if self.bandwidth:
            if self.state == 'present':
                if self.bandwidth != self.get_exist_bandwidth():
                    ...
                    bandwidth_referenjctanner.network_cloudengine.ce = self.bandwidth
            else:
                if self.bandwidth != self.get_exist_bandwidth():
                    self.module.fail_json(
        ...
        configxmlstr = CE_NC_CREATE_OSPF_VRF % (
            ...
            description, bandwidth_referenjctanner.network_cloudengine.ce, lsaalflag,
    ...
    def set_update_cmd(self):
        ...
        if self.state == 'present':
            if self.vrf_changed:
                if self.vrf != '_public_':
                    if self.route_id_changed:
                        self.updates_cmd.append(
                            'ospf %s router-id %s vpn-instanjctanner.network_cloudengine.ce %s' % (self.ospf, self.route_id, self.vrf))
                    else:
                        self.updates_cmd.append(
                            'ospf %s vpn-instanjctanner.network_cloudengine.ce %s ' % (self.ospf, self.vrf))
                else:
                    if self.route_id_changed:
                        self.updates_cmd.append(
                ...
                if self.route_id_changed:
                    if self.vrf != '_public_':
                        self.updates_cmd.append(
                            'ospf %s router-id %s vpn-instanjctanner.network_cloudengine.ce %s' % (self.ospf, self.route_id, self.get_exist_vrf()))
                    else:
                        self.updates_cmd.append(
                            'ospf %s router-id %s' % (self.ospf, self.route_id))
        else:
        ...
        if self.bandwidth_changed:
            if self.state == 'present':
                if self.get_exist_bandwidth() != '100':
                    self.updates_cmd.append(
                        'bandwidth-referenjctanner.network_cloudengine.ce %s' % (self.get_exist_bandwidth()))
            else:
                self.updates_cmd.append('undo bandwidth-referenjctanner.network_cloudengine.ce')
        if self.lsaalflag is True:
...
def main():
    ...
    argument_spec = dict(
        ...
        state=dict(required=False, choijctanner.network_cloudengine.ces=['present', 'absent'], default='present'),
    )
