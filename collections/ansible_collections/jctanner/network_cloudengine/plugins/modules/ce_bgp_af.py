...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_bgp_af
version_added: "2.4"
...
options:
    ...
    vrf_name:
        description:
            - Name of a BGP instanjctanner.network_cloudengine.ce. The name is a case-sensitive string of characters.
              The BGP instanjctanner.network_cloudengine.ce can be used only after the corresponding VPN instanjctanner.network_cloudengine.ce is created.
              The value is a string of 1 to 31 case-sensitive characters.
        required: true
    af_type:
        description:
            - Address family type of a BGP instanjctanner.network_cloudengine.ce.
        required: true
        choijctanner.network_cloudengine.ces: ['ipv4uni','ipv4multi', 'ipv4vpn', 'ipv6uni', 'ipv6vpn', 'evpn']
    max_load_ibgp_num:
        description:
    ...
    ibgp_ecmp_nexthop_changed:
        description:
            - If the value is true, the next hop of an advertised route is changed to the advertiser itself in IBGP
              load-balancing sjctanner.network_cloudengine.cenarios.
              If the value is false, the next hop of an advertised route is not changed to the advertiser itself in
              IBGP load-balancing sjctanner.network_cloudengine.cenarios.
    ...
    ebgp_ecmp_nexthop_changed:
        description:
            - If the value is true, the next hop of an advertised route is changed to the advertiser itself in EBGP
              load-balancing sjctanner.network_cloudengine.cenarios.
              If the value is false, the next hop of an advertised route is not changed to the advertiser itself in
              EBGP load-balancing sjctanner.network_cloudengine.cenarios.
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
        default: no_use
    maximum_load_balanjctanner.network_cloudengine.ce:
        description:
    ...
    ecmp_nexthop_changed:
        description:
            - If the value is true, the next hop of an advertised route is changed to the advertiser itself in BGP
              ...
              in BGP load-balancing sjctanner.network_cloudengine.cenarios.
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
        default: no_use
    default_local_pref:
        description:
            - Set the Local-Preferenjctanner.network_cloudengine.ce attribute. The value is an integer.
    ...
    vrf_rid_auto_sel:
        description:
            - If the value is true, VPN BGP instanjctanner.network_cloudengine.ces are enabled to automatically select router IDs.
              If the value is false, VPN BGP instanjctanner.network_cloudengine.ces are disabled from automatically selecting router IDs.
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
        default: no_use
    nexthop_third_party:
        description:
    ...
    determin_med:
        ...
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
        default: no_use
    preferenjctanner.network_cloudengine.ce_external:
        description:
            - Set the protocol priority of EBGP routes.
              The value is an integer ranging from 1 to 255.
    preferenjctanner.network_cloudengine.ce_internal:
        description:
            - Set the protocol priority of IBGP routes.
              The value is an integer ranging from 1 to 255.
    preferenjctanner.network_cloudengine.ce_local:
        description:
            - Set the protocol priority of a local BGP route.
              The value is an integer ranging from 1 to 255.
    prefrenjctanner.network_cloudengine.ce_policy_name:
        description:
    ...
    reflector_cluster_id:
        description:
            - Set a cluster ID. Configuring multiple RRs in a cluster can enhanjctanner.network_cloudengine.ce the stability of the network.
    ...
    policy_vpn_target:
        description:
            - If the value is true, VPN-Target filtering function is performed for rejctanner.network_cloudengine.ceived VPN routes.
              If the value is false, VPN-Target filtering function is not performed for rejctanner.network_cloudengine.ceived VPN routes.
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
        default: no_use
    next_hop_sel_depend_type:
        description:
            - Next hop select depend type.
        choijctanner.network_cloudengine.ces: ['default','dependTunnel', 'dependIp']
        default: default
    ...
    ebgp_if_sensitive:
        description:
            - If the value is true, after the fast EBGP interfajctanner.network_cloudengine.ce awareness function is enabled,
              EBGP sessions on an interfajctanner.network_cloudengine.ce are deleted immediately when the interfajctanner.network_cloudengine.ce goes Down.
              If the value is false, after the fast EBGP interfajctanner.network_cloudengine.ce awareness function is enabled,
              EBGP sessions on an interfajctanner.network_cloudengine.ce are not deleted immediately when the interfajctanner.network_cloudengine.ce goes Down.
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
        default: no_use
    reflect_chg_path:
        description:
    ...
    lowest_priority:
        description:
            - If the value is true, enable redujctanner.network_cloudengine.ce priority to advertise route.
              If the value is false, disable redujctanner.network_cloudengine.ce priority to advertise route.
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
        default: no_use
    relay_delay_enable:
        description:
    ...
    import_protocol:
        ...
        choijctanner.network_cloudengine.ces: ['direct', 'ospf', 'isis', 'static', 'rip', 'ospfv3', 'ripng']
    import_projctanner.network_cloudengine.cess_id:
        description:
            - Projctanner.network_cloudengine.cess ID of an imported routing protocol.
              The value is an integer ranging from 0 to 4294967295.
    network_address:
        description:
...
- name: CloudEngine BGP address family test
  ...
  - name: "Config BGP Address_Family"
    jctanner.network_cloudengine.ce_bgp_af:
      state: present
      vrf_name: js
      af_type: ipv4uni
      provider: "{{ cli }}"
  ...
  - name: "Config import route"
    jctanner.network_cloudengine.ce_bgp_af:
      ...
      import_projctanner.network_cloudengine.cess_id: 123
      provider: "{{ cli }}"
...
updates:
    ...
    returned: always
    type: list
    sample: ["ipv4-family vpn-instanjctanner.network_cloudengine.ce js"]
'''

import re
from ansible.module_utils.basic import AnsibleModule
from ansible_collections.jctanner.network_cloudengine.plugins.module_utils.network.cloudengine.jctanner.network_cloudengine.ce import get_nc_config, set_nc_config, jctanner.network_cloudengine.ce_argument_spec, check_ip_addr
                ...
                <bgpVrfAF>
                  ...
                  <importRoutes>
                    <importRoute>
                      ...
                      <importProjctanner.network_cloudengine.cessId></importProjctanner.network_cloudengine.cessId>
                    </importRoute>
                  </importRoutes>
                  <networkRoutes>
                    <networkRoute>
                ...
                <bgpVrfAF>
                  ...
                  <importRoutes>
                    <importRoute operation="merge">
                      ...
                      <importProjctanner.network_cloudengine.cessId>%s</importProjctanner.network_cloudengine.cessId>
"""
...
class BgpAf(object):
    ...
    def check_bgp_af_other_args(self, **kwargs):
        ...
        maximum_load_balanjctanner.network_cloudengine.ce = module.params['maximum_load_balanjctanner.network_cloudengine.ce']
        if maximum_load_balanjctanner.network_cloudengine.ce:
            if int(maximum_load_balanjctanner.network_cloudengine.ce) > 65535 or int(maximum_load_balanjctanner.network_cloudengine.ce) < 1:
                module.fail_json(
                    msg='Error: The value of maximum_load_balanjctanner.network_cloudengine.ce %s is out of [1 - 65535].' % maximum_load_balanjctanner.network_cloudengine.ce)

            conf_str = CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name + \
                "<maximumLoadBalanjctanner.network_cloudengine.ce></maximumLoadBalanjctanner.network_cloudengine.ce>" + CE_GET_BGP_ADDRESS_FAMILY_TAIL
            recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
            ...
            if "<data/>" in recv_xml:
                ...
                re_find = re.findall(
                    r'.*<maximumLoadBalanjctanner.network_cloudengine.ce>(.*)</maximumLoadBalanjctanner.network_cloudengine.ce>.*', recv_xml)

                if re_find:
                    result["maximum_load_balanjctanner.network_cloudengine.ce"] = re_find
                    result["vrf_name"] = vrf_name
                    if re_find[0] != maximum_load_balanjctanner.network_cloudengine.ce:
                        need_cfg = True
                else:
                    need_cfg = True
        ...
        preferenjctanner.network_cloudengine.ce_external = module.params['preferenjctanner.network_cloudengine.ce_external']
        if preferenjctanner.network_cloudengine.ce_external:
            if int(preferenjctanner.network_cloudengine.ce_external) > 255 or int(preferenjctanner.network_cloudengine.ce_external) < 1:
                module.fail_json(
                    msg='Error: The value of preferenjctanner.network_cloudengine.ce_external %s is out of [1 - 255].' % preferenjctanner.network_cloudengine.ce_external)

            conf_str = CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name + \
                "<preferenjctanner.network_cloudengine.ceExternal></preferenjctanner.network_cloudengine.ceExternal>" + CE_GET_BGP_ADDRESS_FAMILY_TAIL
            recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
            ...
            if "<data/>" in recv_xml:
                ...
                re_find = re.findall(
                    r'.*<preferenjctanner.network_cloudengine.ceExternal>(.*)</preferenjctanner.network_cloudengine.ceExternal>.*', recv_xml)

                if re_find:
                    result["preferenjctanner.network_cloudengine.ce_external"] = re_find
                    result["vrf_name"] = vrf_name
                    if re_find[0] != preferenjctanner.network_cloudengine.ce_external:
                        need_cfg = True
                else:
                    need_cfg = True

        preferenjctanner.network_cloudengine.ce_internal = module.params['preferenjctanner.network_cloudengine.ce_internal']
        if preferenjctanner.network_cloudengine.ce_internal:
            if int(preferenjctanner.network_cloudengine.ce_internal) > 255 or int(preferenjctanner.network_cloudengine.ce_internal) < 1:
                module.fail_json(
                    msg='Error: The value of preferenjctanner.network_cloudengine.ce_internal %s is out of [1 - 255].' % preferenjctanner.network_cloudengine.ce_internal)

            conf_str = CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name + \
                "<preferenjctanner.network_cloudengine.ceInternal></preferenjctanner.network_cloudengine.ceInternal>" + CE_GET_BGP_ADDRESS_FAMILY_TAIL
            recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
            ...
            if "<data/>" in recv_xml:
                ...
                re_find = re.findall(
                    r'.*<preferenjctanner.network_cloudengine.ceInternal>(.*)</preferenjctanner.network_cloudengine.ceInternal>.*', recv_xml)

                if re_find:
                    result["preferenjctanner.network_cloudengine.ce_internal"] = re_find
                    result["vrf_name"] = vrf_name
                    if re_find[0] != preferenjctanner.network_cloudengine.ce_internal:
                        need_cfg = True
                else:
                    need_cfg = True

        preferenjctanner.network_cloudengine.ce_local = module.params['preferenjctanner.network_cloudengine.ce_local']
        if preferenjctanner.network_cloudengine.ce_local:
            if int(preferenjctanner.network_cloudengine.ce_local) > 255 or int(preferenjctanner.network_cloudengine.ce_local) < 1:
                module.fail_json(
                    msg='Error: The value of preferenjctanner.network_cloudengine.ce_local %s is out of [1 - 255].' % preferenjctanner.network_cloudengine.ce_local)

            conf_str = CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name + \
                "<preferenjctanner.network_cloudengine.ceLocal></preferenjctanner.network_cloudengine.ceLocal>" + CE_GET_BGP_ADDRESS_FAMILY_TAIL
            recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
            ...
            if "<data/>" in recv_xml:
                ...
                re_find = re.findall(
                    r'.*<preferenjctanner.network_cloudengine.ceLocal>(.*)</preferenjctanner.network_cloudengine.ceLocal>.*', recv_xml)

                if re_find:
                    result["preferenjctanner.network_cloudengine.ce_local"] = re_find
                    result["vrf_name"] = vrf_name
                    if re_find[0] != preferenjctanner.network_cloudengine.ce_local:
                        need_cfg = True
                else:
                    need_cfg = True

        prefrenjctanner.network_cloudengine.ce_policy_name = module.params['prefrenjctanner.network_cloudengine.ce_policy_name']
        if prefrenjctanner.network_cloudengine.ce_policy_name:
            if len(prefrenjctanner.network_cloudengine.ce_policy_name) > 40 or len(prefrenjctanner.network_cloudengine.ce_policy_name) < 1:
                module.fail_json(
                    msg='Error: The len of prefrenjctanner.network_cloudengine.ce_policy_name %s is out of [1 - 40].' % prefrenjctanner.network_cloudengine.ce_policy_name)

            conf_str = CE_GET_BGP_ADDRESS_FAMILY_HEADER % vrf_name + \
                "<prefrenjctanner.network_cloudengine.cePolicyName></prefrenjctanner.network_cloudengine.cePolicyName>" + CE_GET_BGP_ADDRESS_FAMILY_TAIL
            recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
            ...
            if "<data/>" in recv_xml:
                ...
                re_find = re.findall(
                    r'.*<prefrenjctanner.network_cloudengine.cePolicyName>(.*)</prefrenjctanner.network_cloudengine.cePolicyName>.*', recv_xml)

                if re_find:
                    result["prefrenjctanner.network_cloudengine.ce_policy_name"] = re_find
                    result["vrf_name"] = vrf_name
                    if re_find[0] != prefrenjctanner.network_cloudengine.ce_policy_name:
                        need_cfg = True
                else:
                    need_cfg = True
    ...
    def check_bgp_import_network_route(self, **kwargs):
        ...
        import_projctanner.network_cloudengine.cess_id = module.params['import_projctanner.network_cloudengine.cess_id']

        if import_protocol and (import_protocol != "direct" and import_protocol != "static"):
            if not import_projctanner.network_cloudengine.cess_id:
                module.fail_json(
                    msg='Error: Please input import_protocol and import_projctanner.network_cloudengine.cess_id value at the same time.')
            else:
                if int(import_projctanner.network_cloudengine.cess_id) < 0:
                    module.fail_json(
                        msg='Error: The value of import_projctanner.network_cloudengine.cess_id %s is out of [0 - 4294967295].' % import_projctanner.network_cloudengine.cess_id)

        if import_projctanner.network_cloudengine.cess_id:
        ...
        if import_protocol:
            ...
            if import_protocol == "direct" or import_protocol == "static":
                import_projctanner.network_cloudengine.cess_id = "0"
            else:
                if not import_projctanner.network_cloudengine.cess_id or import_projctanner.network_cloudengine.cess_id == "0":
                    module.fail_json(
                        msg='Error: Please input import_projctanner.network_cloudengine.cess_id not 0 when import_protocol is '
                            '[ospf, isis, rip, ospfv3, ripng].')

            bgp_import_route_new = (import_protocol, import_projctanner.network_cloudengine.cess_id)
            ...
            if state == "present":
                if "<data/>" in recv_xml:
                    ...
                    re_find = re.findall(
                        r'.*<importProtocol>(.*)</importProtocol>.*\s.*<importProjctanner.network_cloudengine.cessId>(.*)</importProjctanner.network_cloudengine.cessId>.*',
                        recv_xml)
    ...
    def merge_bgp_af(self, **kwargs):
        ...
        cmd = "ipv4-family vpn-instanjctanner.network_cloudengine.ce %s" % vrf_name
        cmds.append(cmd)
        ...
        if af_type == "ipv4multi":
            ...
            cmd = "ipv6-family vpn-instanjctanner.network_cloudengine.ce %s" % vrf_name
        cmds.append(cmd)
    ...
    def delete_bgp_af(self, **kwargs):
        ...
        cmd = "undo ipv4-family vpn-instanjctanner.network_cloudengine.ce %s" % vrf_name
        cmds.append(cmd)
        ...
        if af_type == "ipv4multi":
            ...
            cmd = "undo ipv6-family vpn-instanjctanner.network_cloudengine.ce %s" % vrf_name
        cmds.append(cmd)
    ...
    def merge_bgp_af_other(self, **kwargs):
        ...
        maximum_load_balanjctanner.network_cloudengine.ce = module.params['maximum_load_balanjctanner.network_cloudengine.ce']
        if maximum_load_balanjctanner.network_cloudengine.ce:
            conf_str += "<maximumLoadBalanjctanner.network_cloudengine.ce>%s</maximumLoadBalanjctanner.network_cloudengine.ce>" % maximum_load_balanjctanner.network_cloudengine.ce

            cmd = "maximum load-balancing %s" % maximum_load_balanjctanner.network_cloudengine.ce
            cmds.append(cmd)
        ...
        if ecmp_nexthop_changed != 'no_use':
            ...
            if ecmp_nexthop_changed == "true":
                cmd = "maximum load-balancing %s ecmp-nexthop-changed" % maximum_load_balanjctanner.network_cloudengine.ce
            else:
                cmd = "undo maximum load-balancing %s ecmp-nexthop-changed" % maximum_load_balanjctanner.network_cloudengine.ce
            cmds.append(cmd)
        ...
        if default_local_pref:
            ...
            cmd = "default local-preferenjctanner.network_cloudengine.ce %s" % default_local_pref
            cmds.append(cmd)
        ...
        if vrf_rid_auto_sel != 'no_use':
            ...
            if vrf_rid_auto_sel == "true":
                cmd = "router-id %s vpn-instanjctanner.network_cloudengine.ce auto-select" % router_id
            else:
                cmd = "undo router-id %s vpn-instanjctanner.network_cloudengine.ce auto-select" % router_id
            cmds.append(cmd)
        ...
        preferenjctanner.network_cloudengine.ce_external = module.params['preferenjctanner.network_cloudengine.ce_external']
        if preferenjctanner.network_cloudengine.ce_external:
            conf_str += "<preferenjctanner.network_cloudengine.ceExternal>%s</preferenjctanner.network_cloudengine.ceExternal>" % preferenjctanner.network_cloudengine.ce_external

        preferenjctanner.network_cloudengine.ce_internal = module.params['preferenjctanner.network_cloudengine.ce_internal']
        if preferenjctanner.network_cloudengine.ce_internal:
            conf_str += "<preferenjctanner.network_cloudengine.ceInternal>%s</preferenjctanner.network_cloudengine.ceInternal>" % preferenjctanner.network_cloudengine.ce_internal

        preferenjctanner.network_cloudengine.ce_local = module.params['preferenjctanner.network_cloudengine.ce_local']
        if preferenjctanner.network_cloudengine.ce_local:
            conf_str += "<preferenjctanner.network_cloudengine.ceLocal>%s</preferenjctanner.network_cloudengine.ceLocal>" % preferenjctanner.network_cloudengine.ce_local

            cmd = "preferenjctanner.network_cloudengine.ce %s %s %s" % (
                preferenjctanner.network_cloudengine.ce_external, preferenjctanner.network_cloudengine.ce_internal, preferenjctanner.network_cloudengine.ce_local)
            cmds.append(cmd)

        prefrenjctanner.network_cloudengine.ce_policy_name = module.params['prefrenjctanner.network_cloudengine.ce_policy_name']
        if prefrenjctanner.network_cloudengine.ce_policy_name:
            conf_str += "<prefrenjctanner.network_cloudengine.cePolicyName>%s</prefrenjctanner.network_cloudengine.cePolicyName>" % prefrenjctanner.network_cloudengine.ce_policy_name

            cmd = "preferenjctanner.network_cloudengine.ce route-policy %s" % prefrenjctanner.network_cloudengine.ce_policy_name
            cmds.append(cmd)
        ...
        if ebgp_if_sensitive != 'no_use':
            ...
            if ebgp_if_sensitive == "true":
                cmd = "ebgp-interfajctanner.network_cloudengine.ce-sensitive"
            else:
                cmd = "undo ebgp-interfajctanner.network_cloudengine.ce-sensitive"
            cmds.append(cmd)
    ...
    def merge_bgp_import_route(self, **kwargs):
        ...
        conf_str = CE_MERGE_BGP_IMPORT_ROUTE_HEADER % (
            vrf_name, af_type, import_protocol, import_projctanner.network_cloudengine.cess_id) + CE_MERGE_BGP_ADDRESS_FAMILY_TAIL
        ...
        cmds = []
        cmd = "import-route %s %s" % (import_protocol, import_projctanner.network_cloudengine.cess_id)
        cmds.append(cmd)
    ...
    def create_bgp_import_route(self, **kwargs):
        ...
        conf_str = CE_CREATE_BGP_IMPORT_ROUTE % (
            vrf_name, af_type, import_protocol, import_projctanner.network_cloudengine.cess_id)
    ...
    def delete_bgp_import_route(self, **kwargs):
        ...
        cmds = []
        cmd = "undo import-route %s %s" % (import_protocol, import_projctanner.network_cloudengine.cess_id)
        cmds.append(cmd)
...
def main():
    ...
    argument_spec = dict(
        ...
        af_type=dict(choijctanner.network_cloudengine.ces=['ipv4uni', 'ipv4multi', 'ipv4vpn',
                              'ipv6uni', 'ipv6vpn', 'evpn'], required=True),
        max_load_ibgp_num=dict(type='str'),
        ibgp_ecmp_nexthop_changed=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        max_load_ebgp_num=dict(type='str'),
        ebgp_ecmp_nexthop_changed=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        maximum_load_balanjctanner.network_cloudengine.ce=dict(type='str'),
        ecmp_nexthop_changed=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        default_local_pref=dict(type='str'),
        default_med=dict(type='str'),
        default_rt_import_enable=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        router_id=dict(type='str'),
        vrf_rid_auto_sel=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        nexthop_third_party=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        summary_automatic=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        auto_frr_enable=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        load_balancing_as_path_ignore=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        rib_only_enable=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        rib_only_policy_name=dict(type='str'),
        active_route_advertise=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        as_path_neglect=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        med_none_as_maximum=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        router_id_neglect=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        igp_metric_ignore=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        always_compare_med=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        determin_med=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        preferenjctanner.network_cloudengine.ce_external=dict(type='str'),
        preferenjctanner.network_cloudengine.ce_internal=dict(type='str'),
        preferenjctanner.network_cloudengine.ce_local=dict(type='str'),
        prefrenjctanner.network_cloudengine.ce_policy_name=dict(type='str'),
        reflect_between_client=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        reflector_cluster_id=dict(type='str'),
        reflector_cluster_ipv4=dict(type='str'),
        rr_filter_number=dict(type='str'),
        policy_vpn_target=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        next_hop_sel_depend_type=dict(
            choijctanner.network_cloudengine.ces=['default', 'dependTunnel', 'dependIp']),
        nhp_relay_route_policy_name=dict(type='str'),
        ebgp_if_sensitive=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        reflect_chg_path=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        add_path_sel_num=dict(type='str'),
        route_sel_delay=dict(type='str'),
        allow_invalid_as=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        policy_ext_comm_enable=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        supernet_uni_adv=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        supernet_label_adv=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        ingress_lsp_policy_name=dict(type='str'),
        originator_prior=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        lowest_priority=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        relay_delay_enable=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        import_protocol=dict(
            choijctanner.network_cloudengine.ces=['direct', 'ospf', 'isis', 'static', 'rip', 'ospfv3', 'ripng']),
        import_projctanner.network_cloudengine.cess_id=dict(type='str'),
    ...
    jctanner.network_cloudengine.ce_bgp_af_obj = BgpAf()

    if not jctanner.network_cloudengine.ce_bgp_af_obj:
    ...
    if maximum_load_balanjctanner.network_cloudengine.ce:
        proposed["maximum_load_balanjctanner.network_cloudengine.ce"] = maximum_load_balanjctanner.network_cloudengine.ce
    if ecmp_nexthop_changed != 'no_use':
    ...
    if preferenjctanner.network_cloudengine.ce_external:
        proposed["preferenjctanner.network_cloudengine.ce_external"] = preferenjctanner.network_cloudengine.ce_external
    if preferenjctanner.network_cloudengine.ce_internal:
        proposed["preferenjctanner.network_cloudengine.ce_internal"] = preferenjctanner.network_cloudengine.ce_internal
    if preferenjctanner.network_cloudengine.ce_local:
        proposed["preferenjctanner.network_cloudengine.ce_local"] = preferenjctanner.network_cloudengine.ce_local
    if prefrenjctanner.network_cloudengine.ce_policy_name:
        proposed["prefrenjctanner.network_cloudengine.ce_policy_name"] = prefrenjctanner.network_cloudengine.ce_policy_name
    if reflect_between_client != 'no_use':
    ...
    if import_projctanner.network_cloudengine.cess_id:
        proposed["import_projctanner.network_cloudengine.cess_id"] = import_projctanner.network_cloudengine.cess_id
    if network_address:
        proposed["network_address"] = network_address
    if mask_len:
        proposed["mask_len"] = mask_len

    bgp_af_rst = jctanner.network_cloudengine.ce_bgp_af_obj.check_bgp_af_args(module=module)
    bgp_af_other_rst = jctanner.network_cloudengine.ce_bgp_af_obj.check_bgp_af_other_args(module=module)
    bgp_af_other_can_del_rst = jctanner.network_cloudengine.ce_bgp_af_obj.check_bgp_af_other_can_del(
        module=module)
    bgp_import_network_route_rst = jctanner.network_cloudengine.ce_bgp_af_obj.check_bgp_import_network_route(
        module=module)
    ...
    if state == "present":
        if bgp_af_rst["need_cfg"] and bgp_import_network_route_rst["import_need_cfg"] and \
            ...
            if "bgp_import_route" in bgp_import_network_route_rst.keys():
                conf_str += CE_BGP_MERGE_IMPORT_UNIT % (
                    import_protocol, import_projctanner.network_cloudengine.cess_id)
            else:
            ...
            conf_str += CE_MERGE_BGP_ADDRESS_FAMILY_TAIL
            recv_xml = jctanner.network_cloudengine.ce_bgp_af_obj.netconf_set_config(
            ...
            if bgp_af_rst["need_cfg"]:
                if "af_type" in bgp_af_rst.keys():
                    cmd = jctanner.network_cloudengine.ce_bgp_af_obj.merge_bgp_af(module=module)
                    changed = True
                    for item in cmd:
                        updates.append(item)
                else:
                    cmd = jctanner.network_cloudengine.ce_bgp_af_obj.create_bgp_af(module=module)
                    changed = True
                    for item in cmd:
                        updates.append(item)

            if bgp_af_other_rst["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_bgp_af_obj.merge_bgp_af_other(module=module)
                changed = True
                for item in cmd:
                    updates.append(item)
            ...
            if bgp_import_network_route_rst["import_need_cfg"]:
                if "bgp_import_route" in bgp_import_network_route_rst.keys():
                    cmd = jctanner.network_cloudengine.ce_bgp_af_obj.merge_bgp_import_route(module=module)
                    changed = True
                    for item in cmd:
                        updates.append(item)
                else:
                    cmd = jctanner.network_cloudengine.ce_bgp_af_obj.create_bgp_import_route(module=module)
                    changed = True
                    for item in cmd:
                        updates.append(item)
            ...
            if bgp_import_network_route_rst["network_need_cfg"]:
                if "bgp_network_route" in bgp_import_network_route_rst.keys():
                    cmd = jctanner.network_cloudengine.ce_bgp_af_obj.merge_bgp_network_route(module=module)
                    changed = True
                    for item in cmd:
                        updates.append(item)
                else:
                    cmd = jctanner.network_cloudengine.ce_bgp_af_obj.create_bgp_network_route(module=module)
                    changed = True
                    for item in cmd:
                        updates.append(item)
        ...
        if bgp_import_network_route_rst["import_need_cfg"] and bgp_import_network_route_rst["network_need_cfg"]:
            ...
            if bgp_import_network_route_rst["import_need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_bgp_af_obj.delete_bgp_import_route(module=module)
                changed = True
                for item in cmd:
                    updates.append(item)

            if bgp_import_network_route_rst["network_need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_bgp_af_obj.delete_bgp_network_route(module=module)
                changed = True
                for item in cmd:
                    updates.append(item)

        if bgp_af_other_can_del_rst["need_cfg"]:
            cmd = jctanner.network_cloudengine.ce_bgp_af_obj.delete_bgp_af_other(module=module)
            changed = True
            for item in cmd:
                updates.append(item)

        if bgp_af_rst["need_cfg"] and not bgp_af_other_can_del_rst["need_cfg"]:
            cmd = jctanner.network_cloudengine.ce_bgp_af_obj.delete_bgp_af(module=module)
            changed = True
            for item in cmd:
                updates.append(item)
