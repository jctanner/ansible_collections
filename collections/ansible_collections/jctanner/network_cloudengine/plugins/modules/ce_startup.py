...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_startup
version_added: "2.4"
...
options:
    ...
    slot:
        description:
            - Position of the devijctanner.network_cloudengine.ce.The value is a string of 1 to 32 characters.
              The possible value of slot is all, slave-board, or the specific slotID.
    action:
        description:
            - Display the startup information.
        choijctanner.network_cloudengine.ces: ['display']
...
- name: startup module test
  ...
  - name: Display startup information
    jctanner.network_cloudengine.ce_startup:
      action: display
      provider: "{{ cli }}"
...
from ansible_collections.jctanner.network_cloudengine.plugins.module_utils.network.cloudengine.jctanner.network_cloudengine.ce import get_nc_config, jctanner.network_cloudengine.ce_argument_spec, run_commands
...
class StartUp(object):
    ...
    def get_pat_filename_type(self, filename):
        ...
        if ' ' in filename:
            self.module.fail_json(
                msg='Error: Patch file name include spajctanner.network_cloudengine.ces.')
    ...
    def get_software_filename_type(self, filename):
        ...
        if ' ' in filename:
            self.module.fail_json(
                msg='Error: Software file name include spajctanner.network_cloudengine.ces.')
...
def main():
    ...
    argument_spec = dict(
        ...
        slot=dict(type='str'),
        action=dict(type='str', choijctanner.network_cloudengine.ces=['display'])
    )
