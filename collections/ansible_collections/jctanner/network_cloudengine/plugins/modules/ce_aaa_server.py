...
DOCUMENTATION = r'''
---
module: jctanner.network_cloudengine.ce_aaa_server
version_added: "2.4"
...
options:
    state:
        ...
        type: str
        choijctanner.network_cloudengine.ces: [ absent, present ]
        default: present
    authen_scheme_name:
        description:
    ...
    first_authen_mode:
        ...
        type: str
        choijctanner.network_cloudengine.ces: ['invalid', 'local', 'hwtacacs', 'radius', 'none']
        default: local
    author_scheme_name:
        description:
    ...
    first_author_mode:
        ...
        type: str
        choijctanner.network_cloudengine.ces: ['invalid', 'local', 'hwtacacs', 'if-authenticated', 'none']
        default: local
    acct_scheme_name:
        description:
    ...
    accounting_mode:
        description:
            - Accounting Mode.
        type: str
        choijctanner.network_cloudengine.ces: ['invalid', 'hwtacacs', 'radius', 'none']
        default: none
    domain_name:
        description:
            - Name of a domain.
...
- name: AAA server test
  ...
  - name: "Radius authentication Server Basic settings"
    jctanner.network_cloudengine.ce_aaa_server:
      state: present
...
def main():
    ...
    argument_spec = dict(
        ...
        first_authen_mode=dict(default='local', choijctanner.network_cloudengine.ces=['invalid', 'local', 'hwtacacs', 'radius', 'none']),
        author_scheme_name=dict(type='str'),
        first_author_mode=dict(default='local', choijctanner.network_cloudengine.ces=['invalid', 'local', 'hwtacacs', 'if-authenticated', 'none']),
        acct_scheme_name=dict(type='str'),
        accounting_mode=dict(default='none', choijctanner.network_cloudengine.ces=['invalid', 'hwtacacs', 'radius', 'none']),
    ...
    jctanner.network_cloudengine.ce_aaa_server = AaaServer()

    if not jctanner.network_cloudengine.ce_aaa_server:
    ...
    if authen_scheme_name:
        ...
        scheme_exist = jctanner.network_cloudengine.ce_aaa_server.get_authentication_scheme(module=module)
        ...
        if state == "present":
            ...
            if len(scheme_exist) == 0:
                cmd = jctanner.network_cloudengine.ce_aaa_server.create_authentication_scheme(
                    module=module,
                ...
                cmd = jctanner.network_cloudengine.ce_aaa_server.merge_authentication_scheme(
                    module=module,
                    authen_scheme_name=authen_scheme_name,
                    first_authen_mode=first_authen_mode)
                updates.append(cmd)
            ...
            if domain_name:
                domain_exist = jctanner.network_cloudengine.ce_aaa_server.get_authentication_domain(
                    module=module)
                domain_new = (domain_name.lower(), authen_scheme_name.lower())

                if len(domain_exist) == 0:
                    cmd = jctanner.network_cloudengine.ce_aaa_server.create_authentication_domain(
                        module=module,
                    ...
                    cmd = jctanner.network_cloudengine.ce_aaa_server.merge_authentication_domain(
                        module=module,
                        domain_name=domain_name,
                        authen_scheme_name=authen_scheme_name)
                    updates.append(cmd)
            ...
            if len(scheme_exist) == 0:
                ...
                cmd = jctanner.network_cloudengine.ce_aaa_server.delete_authentication_scheme(
                    module=module,
                    authen_scheme_name=authen_scheme_name,
                    first_authen_mode=first_authen_mode)
                updates.append(cmd)
            ...
            if domain_name:
                ...
                if len(domain_exist) == 0:
                    ...
                    cmd = jctanner.network_cloudengine.ce_aaa_server.delete_authentication_domain(
                        module=module,
                        domain_name=domain_name,
                        authen_scheme_name=authen_scheme_name)
                    updates.append(cmd)
        ...
        scheme_end = jctanner.network_cloudengine.ce_aaa_server.get_authentication_scheme(module=module)
        end_state["authentication scheme"] = scheme_end

    # authorization
    if author_scheme_name:

        scheme_exist = jctanner.network_cloudengine.ce_aaa_server.get_authorization_scheme(module=module)
        ...
        if state == "present":
            ...
            if len(scheme_exist) == 0:
                cmd = jctanner.network_cloudengine.ce_aaa_server.create_authorization_scheme(
                    module=module,
                ...
                cmd = jctanner.network_cloudengine.ce_aaa_server.merge_authorization_scheme(
                    module=module,
                    author_scheme_name=author_scheme_name,
                    first_author_mode=first_author_mode)
                updates.append(cmd)
            ...
            if domain_name:
                domain_exist = jctanner.network_cloudengine.ce_aaa_server.get_authorization_domain(
                    module=module)
                domain_new = (domain_name.lower(), author_scheme_name.lower())

                if len(domain_exist) == 0:
                    cmd = jctanner.network_cloudengine.ce_aaa_server.create_authorization_domain(
                        module=module,
                        domain_name=domain_name,
                        author_scheme_name=author_scheme_name)
                    updates.append(cmd)
                    ...
                    cmd = jctanner.network_cloudengine.ce_aaa_server.merge_authorization_domain(
                        module=module,
                        domain_name=domain_name,
                        author_scheme_name=author_scheme_name)
                    updates.append(cmd)
            ...
            if len(scheme_exist) == 0:
                ...
                cmd = jctanner.network_cloudengine.ce_aaa_server.delete_authorization_scheme(
                    module=module,
                    author_scheme_name=author_scheme_name,
                    first_author_mode=first_author_mode)
                updates.append(cmd)
            ...
            if domain_name:
                ...
                if len(domain_exist) == 0:
                    ...
                    cmd = jctanner.network_cloudengine.ce_aaa_server.delete_authorization_domain(
                        module=module,
                        domain_name=domain_name,
                        author_scheme_name=author_scheme_name)
                    updates.append(cmd)
        ...
        scheme_end = jctanner.network_cloudengine.ce_aaa_server.get_authorization_scheme(module=module)
        end_state["authorization scheme"] = scheme_end

    # accounting
    if acct_scheme_name:

        scheme_exist = jctanner.network_cloudengine.ce_aaa_server.get_accounting_scheme(module=module)
        ...
        if state == "present":
            ...
            if len(scheme_exist) == 0:
                cmd = jctanner.network_cloudengine.ce_aaa_server.create_accounting_scheme(
                    module=module,
                ...
                cmd = jctanner.network_cloudengine.ce_aaa_server.merge_accounting_scheme(
                    module=module,
                    acct_scheme_name=acct_scheme_name,
                    accounting_mode=accounting_mode)
                updates.append(cmd)
            ...
            if domain_name:
                domain_exist = jctanner.network_cloudengine.ce_aaa_server.get_accounting_domain(
                    module=module)
                domain_new = (domain_name.lower(), acct_scheme_name.lower())

                if len(domain_exist) == 0:
                    cmd = jctanner.network_cloudengine.ce_aaa_server.create_accounting_domain(
                        module=module,
                        domain_name=domain_name,
                        acct_scheme_name=acct_scheme_name)
                    updates.append(cmd)
                    ...
                    cmd = jctanner.network_cloudengine.ce_aaa_server.merge_accounting_domain(
                        module=module,
                        domain_name=domain_name,
                        acct_scheme_name=acct_scheme_name)
                    updates.append(cmd)
            ...
            if len(scheme_exist) == 0:
                ...
                cmd = jctanner.network_cloudengine.ce_aaa_server.delete_accounting_scheme(
                    module=module,
                    acct_scheme_name=acct_scheme_name,
                    accounting_mode=accounting_mode)
                updates.append(cmd)
            ...
            if domain_name:
                ...
                if len(domain_exist) == 0:
                    ...
                    cmd = jctanner.network_cloudengine.ce_aaa_server.delete_accounting_domain(
                        module=module,
                        domain_name=domain_name,
                        acct_scheme_name=acct_scheme_name)
                    updates.append(cmd)
        ...
        scheme_end = jctanner.network_cloudengine.ce_aaa_server.get_accounting_scheme(module=module)
    ...
    if (authen_scheme_name and first_authen_mode.lower() == "radius") \
        ...
        rds_template_exist = jctanner.network_cloudengine.ce_aaa_server.get_radius_template(module=module)
        rds_template_new = (radius_server_group)

        rds_enable_exist = jctanner.network_cloudengine.ce_aaa_server.get_radius_client(module=module)
        ...
        if state == "present":
            ...
            if len(rds_template_exist) == 0:
                cmd = jctanner.network_cloudengine.ce_aaa_server.create_radius_template(
                    module=module, radius_server_group=radius_server_group)
                updates.append(cmd)
                changed = True
            elif rds_template_new not in rds_template_exist:
                cmd = jctanner.network_cloudengine.ce_aaa_server.merge_radius_template(
                    module=module, radius_server_group=radius_server_group)
                updates.append(cmd)
                changed = True
            ...
            if rds_enable_new not in rds_enable_exist:
                cmd = jctanner.network_cloudengine.ce_aaa_server.merge_radius_client(
            ...
            if len(rds_template_exist) == 0:
                ...
                cmd = jctanner.network_cloudengine.ce_aaa_server.delete_radius_template(
                    module=module, radius_server_group=radius_server_group)
                updates.append(cmd)
                changed = True
        ...
        rds_template_end = jctanner.network_cloudengine.ce_aaa_server.get_radius_template(module=module)
        end_state["radius template"] = rds_template_end

        rds_enable_end = jctanner.network_cloudengine.ce_aaa_server.get_radius_client(module=module)
    ...
    if (authen_scheme_name and first_authen_mode.lower() == "hwtacacs") \
        ...
        hwtacacs_exist = jctanner.network_cloudengine.ce_aaa_server.get_hwtacacs_template(module=module)
        hwtacacs_new = (hwtacas_template)

        hwtacacs_enbale_exist = jctanner.network_cloudengine.ce_aaa_server.get_hwtacacs_global_cfg(
            module=module)
        ...
        if state == "present":
            ...
            if len(hwtacacs_exist) == 0:
                cmd = jctanner.network_cloudengine.ce_aaa_server.create_hwtacacs_template(
                    module=module, hwtacas_template=hwtacas_template)
                updates.append(cmd)
                changed = True
            elif hwtacacs_new not in hwtacacs_exist:
                cmd = jctanner.network_cloudengine.ce_aaa_server.merge_hwtacacs_template(
                    module=module, hwtacas_template=hwtacas_template)
                updates.append(cmd)
                changed = True
            ...
            if hwtacacs_enbale_new not in hwtacacs_enbale_exist:
                cmd = jctanner.network_cloudengine.ce_aaa_server.merge_hwtacacs_global_cfg(
                    module=module, isEnable="true")
                updates.append(cmd)
                changed = True
            ...
            if len(hwtacacs_exist) == 0:
                ...
                cmd = jctanner.network_cloudengine.ce_aaa_server.delete_hwtacacs_template(
                    module=module, hwtacas_template=hwtacas_template)
                updates.append(cmd)
                changed = True
        ...
        hwtacacs_end = jctanner.network_cloudengine.ce_aaa_server.get_hwtacacs_template(module=module)
        end_state["hwtacacs template"] = hwtacacs_end

        hwtacacs_enable_end = jctanner.network_cloudengine.ce_aaa_server.get_hwtacacs_global_cfg(
            module=module)
    ...
    if local_user_group:
        ...
        user_group_exist = jctanner.network_cloudengine.ce_aaa_server.get_local_user_group(module=module)
        ...
        if state == "present":
            ...
            if len(user_group_exist) == 0:
                cmd = jctanner.network_cloudengine.ce_aaa_server.merge_local_user_group(
            ...
            if len(user_group_exist) == 0:
                ...
                cmd = jctanner.network_cloudengine.ce_aaa_server.delete_local_user_group(
                    module=module, local_user_group=local_user_group)
                updates.append(cmd)
                changed = True

        user_group_end = jctanner.network_cloudengine.ce_aaa_server.get_local_user_group(module=module)
