...
DOCUMENTATION = """
---
module: jctanner.network_cloudengine.ce_sflow
version_added: "2.4"
short_description: Manages sFlow configuration on HUAWEI CloudEngine switches.
description:
    - Configure Sampled Flow (sFlow) to monitor traffic on an interfajctanner.network_cloudengine.ce in real time,
      detect abnormal traffic, and locate the sourjctanner.network_cloudengine.ce of attack traffic,
...
options:
    ...
    sourjctanner.network_cloudengine.ce_ip:
        description:
            - Specifies the sourjctanner.network_cloudengine.ce IPv4/IPv6 address of sFlow packets.
    collector_id:
        description:
            - Specifies the ID of an sFlow collector. This ID is used when you specify
              the collector in subsequent sFlow configuration.
        choijctanner.network_cloudengine.ces: ['1', '2']
    collector_ip:
        description:
    ...
    collector_meth:
        description:
            - Configures the devijctanner.network_cloudengine.ce to send sFlow packets through servijctanner.network_cloudengine.ce interfajctanner.network_cloudengine.ces,
              enhancing the sFlow packet forwarding capability.
              The enhanjctanner.network_cloudengine.ced parameter is optional. No matter whether you configure the enhanjctanner.network_cloudengine.ced mode,
              the switch determines to send sFlow packets through servijctanner.network_cloudengine.ce cards or management port
              based on the routing information on the collector.
              When the value is meth, the devijctanner.network_cloudengine.ce forwards sFlow packets at the control plane.
              When the value is enhanjctanner.network_cloudengine.ced, the devijctanner.network_cloudengine.ce forwards sFlow packets at the forwarding plane to
              enhanjctanner.network_cloudengine.ce the sFlow packet forwarding capacity.
        choijctanner.network_cloudengine.ces: ['meth', 'enhanjctanner.network_cloudengine.ced']
    collector_description:
        description:
            - Specifies the description of an sFlow collector.
              The value is a string of 1 to 255 case-sensitive characters without spajctanner.network_cloudengine.ces.
    sflow_interfajctanner.network_cloudengine.ce:
        description:
            - Full name of interfajctanner.network_cloudengine.ce for Flow Sampling or Counter.
              It must be a physical interfajctanner.network_cloudengine.ce, Eth-Trunk, or Layer 2 subinterfajctanner.network_cloudengine.ce.
    sample_collector:
        description:
    ...
    sample_direction:
        ...
        choijctanner.network_cloudengine.ces: ['inbound', 'outbound', 'both']
    counter_collector:
        description:
    ...
    forward_enp_slot:
        description:
            - Enable the Embedded Network Projctanner.network_cloudengine.cessor (ENP) chip function.
...
- name: sflow module test
  ...
  - name: Configuring sFlow Agent
    jctanner.network_cloudengine.ce_sflow:
      agent_ip: 6.6.6.6
      provider: '{{ cli }}'
  ...
  - name: Configure flow sampling.
    jctanner.network_cloudengine.ce_sflow:
      sflow_interfajctanner.network_cloudengine.ce: 10GE2/0/2
      sample_collector: 1
...
<sflow xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0">
    <sourjctanner.network_cloudengine.ces>
        <sourjctanner.network_cloudengine.ce>
            <family></family>
            <ipv4Addr></ipv4Addr>
            <ipv6Addr></ipv6Addr>
        </sourjctanner.network_cloudengine.ce>
...
def get_interfajctanner.network_cloudengine.ce_type(interfajctanner.network_cloudengine.ce):
    """get the type of interfajctanner.network_cloudengine.ce, such as 10GE, ETH-TRUNK, VLANIF..."""
...
class Sflow(object):
    ...
    def __init__(self, argument_spec):
        ...
        self.sourjctanner.network_cloudengine.ce_ip = self.module.params['sourjctanner.network_cloudengine.ce_ip']
        self.sourjctanner.network_cloudengine.ce_version = None
        ...
        self.sflow_interfajctanner.network_cloudengine.ce = self.module.params['sflow_interfajctanner.network_cloudengine.ce']
    ...
    def get_sflow_dict(self):
        ...
        sflow_dict = dict(sourjctanner.network_cloudengine.ce=list(), agent=dict(), collector=list(),
                          sampling=dict(), counter=dict(), export=dict())
        conf_str = CE_NC_GET_SFLOW % (
            self.sflow_interfajctanner.network_cloudengine.ce, self.sflow_interfajctanner.network_cloudengine.ce)

        if not self.collector_meth:
            conf_str = conf_str.replajctanner.network_cloudengine.ce("<meth></meth>", "")
        ...
        # get sourjctanner.network_cloudengine.ce info
        srcs = root.findall("data/sflow/sourjctanner.network_cloudengine.ces/sourjctanner.network_cloudengine.ce")
        if srcs:
            for src in srcs:
                attrs = dict()
                for attr in src:
                ...
                sflow_dict["sourjctanner.network_cloudengine.ce"].append(attrs)
    ...
    def config_sourjctanner.network_cloudengine.ce(self):
        """configures the sourjctanner.network_cloudengine.ce IP address for sFlow packets"""

        xml_str = ''
        if not self.sourjctanner.network_cloudengine.ce_ip:
            return xml_str

        self.sourjctanner.network_cloudengine.ce_version = get_ip_version(self.sourjctanner.network_cloudengine.ce_ip)
        if not self.sourjctanner.network_cloudengine.ce_version:
            self.module.fail_json(msg="Error: sourjctanner.network_cloudengine.ce_ip is invalid.")

        src_dict = dict()
        for src in self.sflow_dict["sourjctanner.network_cloudengine.ce"]:
            if src.get("family") == self.sourjctanner.network_cloudengine.ce_version:
                src_dict = src
                break

        if self.state == "present":
            if self.sourjctanner.network_cloudengine.ce_ip != src_dict.get("ipv4Addr") \
                    and self.sourjctanner.network_cloudengine.ce_ip != src_dict.get("ipv6Addr"):
                xml_str += '<sourjctanner.network_cloudengine.ces><sourjctanner.network_cloudengine.ce operation="merge">'
                xml_str += '<family>%s</family>' % self.sourjctanner.network_cloudengine.ce_version
                if self.sourjctanner.network_cloudengine.ce_version == "ipv4":
                    xml_str += '<ipv4Addr>%s</ipv4Addr>' % self.sourjctanner.network_cloudengine.ce_ip
                    self.updates_cmd.append("sflow sourjctanner.network_cloudengine.ce ip %s" % self.sourjctanner.network_cloudengine.ce_ip)
                else:
                    xml_str += '<ipv6Addr>%s</ipv6Addr>' % self.sourjctanner.network_cloudengine.ce_ip
                    self.updates_cmd.append(
                        "sflow sourjctanner.network_cloudengine.ce ipv6 %s" % self.sourjctanner.network_cloudengine.ce_ip)
                xml_str += '</sourjctanner.network_cloudengine.ce ></sourjctanner.network_cloudengine.ces>'
        else:
            if self.sourjctanner.network_cloudengine.ce_ip == src_dict.get("ipv4Addr"):
                xml_str += '<sourjctanner.network_cloudengine.ces><sourjctanner.network_cloudengine.ce operation="delete"><family>ipv4</family></sourjctanner.network_cloudengine.ce ></sourjctanner.network_cloudengine.ces>'
                self.updates_cmd.append("undo sflow sourjctanner.network_cloudengine.ce ip %s" % self.sourjctanner.network_cloudengine.ce_ip)
            elif self.sourjctanner.network_cloudengine.ce_ip == src_dict.get("ipv6Addr"):
                xml_str += '<sourjctanner.network_cloudengine.ces><sourjctanner.network_cloudengine.ce operation="delete"><family>ipv6</family></sourjctanner.network_cloudengine.ce ></sourjctanner.network_cloudengine.ces>'
                self.updates_cmd.append("undo sflow sourjctanner.network_cloudengine.ce ipv6 %s" % self.sourjctanner.network_cloudengine.ce_ip)
    ...
    def config_collector(self):
        ...
        if self.state == "absent":
            ...
            if self.collector_ip_vpn:
                cmd += " vpn-instanjctanner.network_cloudengine.ce %s" % self.collector_ip_vpn
            ...
            if self.collector_meth:
                if self.collector_meth == "enhanjctanner.network_cloudengine.ced":
                    cmd += " enhanjctanner.network_cloudengine.ced"
    ...
    def config_sampling(self):
        """configure sflow sampling on an interfajctanner.network_cloudengine.ce"""

        xml_str = ''
        if not self.sflow_interfajctanner.network_cloudengine.ce:
            return xml_str

        if not self.sflow_dict["sampling"] and self.state == "absent":
            return xml_str

        self.updates_cmd.append("interfajctanner.network_cloudengine.ce %s" % self.sflow_interfajctanner.network_cloudengine.ce)
        if self.state == "present":
            xml_str += '<samplings><sampling operation="merge"><ifName>%s</ifName>' % self.sflow_interfajctanner.network_cloudengine.ce
        else:
            xml_str += '<samplings><sampling operation="delete"><ifName>%s</ifName>' % self.sflow_interfajctanner.network_cloudengine.ce
    ...
    def config_counter(self):
        """configures sflow counter on an interfajctanner.network_cloudengine.ce"""
        ...
        self.updates_cmd.append("interfajctanner.network_cloudengine.ce %s" % self.sflow_interfajctanner.network_cloudengine.ce)
        if self.state == "present":
            xml_str += '<counters><counter operation="merge"><ifName>%s</ifName>' % self.sflow_interfajctanner.network_cloudengine.ce
        else:
            xml_str += '<counters><counter operation="delete"><ifName>%s</ifName>' % self.sflow_interfajctanner.network_cloudengine.ce
    ...
    def check_params(self):
        ...
        # check sourjctanner.network_cloudengine.ce_ip
        if self.sourjctanner.network_cloudengine.ce_ip:
            self.sourjctanner.network_cloudengine.ce_ip = self.sourjctanner.network_cloudengine.ce_ip.upper()
        ...
        if self.collector_id:
            ...
            if self.collector_description:
                if self.collector_description.count(" "):
                    self.module.fail_json(
                        msg="Error: collector_description should without spajctanner.network_cloudengine.ces.")
        ...
        # check sflow_interfajctanner.network_cloudengine.ce
        if self.sflow_interfajctanner.network_cloudengine.ce:
            intf_type = get_interfajctanner.network_cloudengine.ce_type(self.sflow_interfajctanner.network_cloudengine.ce)
            if not intf_type:
                self.module.fail_json(msg="Error: intf_type is invalid.")
            if intf_type not in ['ge', '10ge', '25ge', '4x10ge', '40ge', '100ge', 'eth-trunk']:
                self.module.fail_json(
                    msg="Error: interfajctanner.network_cloudengine.ce %s is not support sFlow." % self.sflow_interfajctanner.network_cloudengine.ce)
    ...
    def get_proposed(self):
        ...
        if self.sflow_interfajctanner.network_cloudengine.ce:
            self.proposed["sflow_interfajctanner.network_cloudengine.ce"] = self.sflow_interfajctanner.network_cloudengine.ce
            if self.sample_collector:
    ...
    def get_existing(self):
        ...
        if self.sourjctanner.network_cloudengine.ce_ip:
            self.existing["sourjctanner.network_cloudengine.ce"] = self.sflow_dict["sourjctanner.network_cloudengine.ce"]
        if self.collector_id:
    ...
    def get_end_state(self):
        ...
        if self.sourjctanner.network_cloudengine.ce_ip:
            self.end_state["sourjctanner.network_cloudengine.ce"] = sflow_dict["sourjctanner.network_cloudengine.ce"]
        if self.collector_id:
    ...
    def work(self):
        ...
        if self.sourjctanner.network_cloudengine.ce_ip:
            xml_str += self.config_sourjctanner.network_cloudengine.ce()
...
def main():
    ...
    argument_spec = dict(
        ...
        sourjctanner.network_cloudengine.ce_ip=dict(required=False, type='str'),
        ...
        collector_id=dict(required=False, type='str', choijctanner.network_cloudengine.ces=['1', '2']),
        ...
        sflow_interfajctanner.network_cloudengine.ce=dict(required=False, type='str'),
