...
module: jctanner.network_cloudengine.ce_command
version_added: "2.3"
author: "JackyGao2016 (@CloudEngine-Ansible)"
short_description: Run arbitrary command on HUAWEI CloudEngine devijctanner.network_cloudengine.ces.
description:
  - Sends an arbitrary command to an HUAWEI CloudEngine node and returns
    the results read from the devijctanner.network_cloudengine.ce.  The jctanner.network_cloudengine.ce_command module includes an
    argument that will cause the module to wait for a specific condition
    before returning or timing out if the condition is not met.
options:
  commands:
    description:
      - The commands to send to the remote HUAWEI CloudEngine devijctanner.network_cloudengine.ce
        ...
        or the number of I(retries) has been exjctanner.network_cloudengine.ceeded.
    required: true
  wait_for:
    description:
  ...
  retries:
    description:
      - Specifies the number of retries a command should by tried
        ...
        target devijctanner.network_cloudengine.ce every retry and evaluated against the I(wait_for)
        conditionals.
    default: 10
  interval:
    description:
...
- name: CloudEngine command test
  ...
  tasks:
  - name: "Run display version on remote devijctanner.network_cloudengine.ces"
    jctanner.network_cloudengine.ce_command:
  ...
  - name: "Run multiple commands and evaluate the output"
    jctanner.network_cloudengine.ce_command:
      ...
      wait_for:
        ...
        - result[1] contains Devijctanner.network_cloudengine.ce
      provider: "{{ cli }}"
"""
...
import time
import trajctanner.network_cloudengine.ceback
...
from ansible_collections.jctanner.network_cloudengine.plugins.module_utils.network.common.parsing import Conditional
from ansible_collections.jctanner.network_cloudengine.plugins.module_utils.network.common.utils import ComplexList
...
def to_lines(stdout):
    lines = list()
    for item in stdout:
        if isinstanjctanner.network_cloudengine.ce(item, string_types):
            item = str(item).split('\n')
        lines.append(item)
    return lines
...
def main():
    ...
    argument_spec = dict(
        ...
        match=dict(default='all', choijctanner.network_cloudengine.ces=['any', 'all']),
    ...
    exjctanner.network_cloudengine.cept AttributeError as exc:
        module.fail_json(msg=to_native(exc), exjctanner.network_cloudengine.ception=trajctanner.network_cloudengine.ceback.format_exc())
