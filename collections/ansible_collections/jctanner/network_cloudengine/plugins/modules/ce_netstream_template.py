...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_netstream_template
version_added: "2.4"
...
options:
    ...
    match:
        ...
        choijctanner.network_cloudengine.ces: ['destination-address', 'destination-port', 'tos', 'protocol', 'sourjctanner.network_cloudengine.ce-address', 'sourjctanner.network_cloudengine.ce-port']
    collect_counter:
        description:
            - Configure the number of packets and bytes that are included in the flexible flow statistics sent to NSC.
        choijctanner.network_cloudengine.ces: ['bytes', 'packets']
    collect_interfajctanner.network_cloudengine.ce:
        description:
            - Configure the input or output interfajctanner.network_cloudengine.ce that are included in the flexible flow statistics sent to NSC.
        choijctanner.network_cloudengine.ces: ['input', 'output']
    description:
        description:
...
- name: netstream template module test
  ...
  - name: Config ipv4 netstream record
    jctanner.network_cloudengine.ce_netstream_template:
      state: present
      type: ip
      record_name: test
      provider: "{{ cli }}"
...
class NetstreamTemplate(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.collect_interfajctanner.network_cloudengine.ce = self.module.params['collect_interfajctanner.network_cloudengine.ce'] or None
    ...
    def get_proposed(self):
        ...
        if self.collect_interfajctanner.network_cloudengine.ce:
            self.proposed["collect_interfajctanner.network_cloudengine.ce"] = self.collect_interfajctanner.network_cloudengine.ce
        if self.description:
    ...
    def get_existing(self):
        ...
        if self.netstream_cfg:
            ...
            if self.collect_interfajctanner.network_cloudengine.ce:
                tmp_value = re.findall(r'collect interfajctanner.network_cloudengine.ce (.*)', self.netstream_cfg)
                if tmp_value:
                    self.existing["collect_interfajctanner.network_cloudengine.ce"] = tmp_value
    ...
    def get_end_state(self):
        ...
        if self.netstream_cfg:
            ...
            if self.collect_interfajctanner.network_cloudengine.ce:
                tmp_value = re.findall(r'collect interfajctanner.network_cloudengine.ce (.*)', self.netstream_cfg)
                if tmp_value:
                    self.end_state["collect_interfajctanner.network_cloudengine.ce"] = tmp_value
    ...
    def present_netstream(self):
        ...
        if self.collect_interfajctanner.network_cloudengine.ce:
            cmd = "collect interfajctanner.network_cloudengine.ce %s" % self.collect_interfajctanner.network_cloudengine.ce
            if not self.netstream_cfg or cmd not in self.netstream_cfg:
                cmds.append(cmd)
                self.updates_cmd.append(cmd)
    ...
    def absent_netstream(self):
        ...
        if self.description or self.match or self.collect_counter or self.collect_interfajctanner.network_cloudengine.ce:
        ...
        if absent_netstream_attr:
            ...
            if self.collect_interfajctanner.network_cloudengine.ce:
                cfg = "collect interfajctanner.network_cloudengine.ce %s" % self.collect_interfajctanner.network_cloudengine.ce
                if self.netstream_cfg and cfg in self.netstream_cfg:
                    cmd = "undo collect interfajctanner.network_cloudengine.ce %s" % self.collect_interfajctanner.network_cloudengine.ce
                    cmds.append(cmd)
                    self.updates_cmd.append(cmd)
...
def main():
    ...
    argument_spec = dict(
        ...
        type=dict(choijctanner.network_cloudengine.ces=['ip', 'vxlan'], required=True),
        record_name=dict(type='str'),
        match=dict(choijctanner.network_cloudengine.ces=['destination-address', 'destination-port',
                            'tos', 'protocol', 'sourjctanner.network_cloudengine.ce-address', 'sourjctanner.network_cloudengine.ce-port']),
        collect_counter=dict(choijctanner.network_cloudengine.ces=['bytes', 'packets']),
        collect_interfajctanner.network_cloudengine.ce=dict(choijctanner.network_cloudengine.ces=['input', 'output']),
