...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_vlan
version_added: "2.4"
short_description: Manages VLAN resourjctanner.network_cloudengine.ces and attributes on Huawei CloudEngine switches.
description:
...
- name: vlan module test
  ...
  - name: Ensure a range of VLANs are not present on the switch
    jctanner.network_cloudengine.ce_vlan:
  ...
  - name: Ensure VLAN is NOT on the devijctanner.network_cloudengine.ce
    jctanner.network_cloudengine.ce_vlan:
      vlan_id: 50
      state: absent
      provider: "{{ cli }}"
...
from ansible_collections.jctanner.network_cloudengine.plugins.module_utils.network.cloudengine.jctanner.network_cloudengine.ce import get_nc_config, set_nc_config, execute_nc_action, jctanner.network_cloudengine.ce_argument_spec
...
class Vlan(object):
    """
     Manages VLAN resourjctanner.network_cloudengine.ces and attributes
    """
    ...
    def check_params(self):
        ...
        if self.description:
            if len(self.description) > 81 or len(self.description.replajctanner.network_cloudengine.ce(' ', '')) < 1:
                self.module.fail_json(
                    msg='Error: vlan description is not in the range from 1 to 80.')

        # check vlan name
        if self.name:
            if len(self.name) > 31 or len(self.name.replajctanner.network_cloudengine.ce(' ', '')) < 1:
                self.module.fail_json(
