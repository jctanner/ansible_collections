...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_vxlan_tunnel
version_added: "2.4"
short_description: Manages VXLAN tunnel configuration on HUAWEI CloudEngine devijctanner.network_cloudengine.ces.
description:
    - This module offers the ability to set the VNI and mapped to the BD,
      and configure an ingress replication list on HUAWEI CloudEngine devijctanner.network_cloudengine.ces.
author:
    - Li Yanfeng (@QijunPan)
options:
    bridge_domain_id:
    ...
    nve_name:
        description:
            - Specifies the number of an NVE interfajctanner.network_cloudengine.ce. The value ranges from 1 to 2.
    nve_mode:
        description:
            - Specifies the working mode of an NVE interfajctanner.network_cloudengine.ce.
        choijctanner.network_cloudengine.ces: ['mode-l2','mode-l3']
    peer_list_ip:
        description:
    ...
    protocol_type:
        ...
        choijctanner.network_cloudengine.ces: ['bgp','null']
    sourjctanner.network_cloudengine.ce_ip:
        description:
            - Specifies an IP address for a sourjctanner.network_cloudengine.ce VTEP. The value is in dotted decimal notation.
    state:
        description:
...
- name: vxlan tunnel module test
  ...
  - name: Make sure nve_name is exist, ensure vni_id and protocol_type is configured on Nve1 interfajctanner.network_cloudengine.ce.
    jctanner.network_cloudengine.ce_vxlan_tunnel:
      nve_name: Nve1
      vni_id: 100
      protocol_type: bgp
      state: present
...
proposed:
    ...
    returned: always
    type: dict
    sample: {nve_interfajctanner.network_cloudengine.ce_name": "Nve1", nve_mode": "mode-l2", "sourjctanner.network_cloudengine.ce_ip": "0.0.0.0"}
existing:
    description:
        - k/v pairs of existing rollback
    returned: always
    type: dict
    sample: {nve_interfajctanner.network_cloudengine.ce_name": "Nve1", nve_mode": "mode-l3", "sourjctanner.network_cloudengine.ce_ip": "0.0.0.0"}

updates:
    description: command sent to the devijctanner.network_cloudengine.ce
    returned: always
    type: list
    sample: ["interfajctanner.network_cloudengine.ce Nve1",
             "mode l3"]
changed:
...
from ansible_collections.jctanner.network_cloudengine.plugins.module_utils.network.cloudengine.jctanner.network_cloudengine.ce import get_nc_config, set_nc_config, get_config, jctanner.network_cloudengine.ce_argument_spec
...
class VxlanTunnel(object):
    ...
    def check_nve_interfajctanner.network_cloudengine.ce(self, nve_name):
        """is nve interfajctanner.network_cloudengine.ce exist"""
    ...
    def check_nve_name(self):
        """Gets Nve interfajctanner.network_cloudengine.ce name"""
    ...
    def is_nve_mode_exist(self, nve_name, mode):
        """is nve interfajctanner.network_cloudengine.ce mode exist"""
    ...
    def is_nve_mode_change(self, nve_name, mode):
        """is nve interfajctanner.network_cloudengine.ce mode change"""
    ...
    def is_nve_sourjctanner.network_cloudengine.ce_ip_exist(self, nve_name, sourjctanner.network_cloudengine.ce_ip):
        """is vni to bridge-domain-id exist"""

        if not self.nve_info:
            return False

        if self.nve_info["ifName"] == nve_name and self.nve_info["srcAddr"] == sourjctanner.network_cloudengine.ce_ip:
            return True
        return False

    def is_nve_sourjctanner.network_cloudengine.ce_ip_change(self, nve_name, sourjctanner.network_cloudengine.ce_ip):
        """is vni to bridge-domain-id change"""
    ...
    def config_merge_sourjctanner.network_cloudengine.ce_ip(self, nve_name, sourjctanner.network_cloudengine.ce_ip):
        """config nve sourjctanner.network_cloudengine.ce ip"""

        if self.is_nve_sourjctanner.network_cloudengine.ce_ip_change(nve_name, sourjctanner.network_cloudengine.ce_ip):
            cfg_xml = CE_NC_MERGE_NVE_SOURCE_IP_PROTOCOL % (
                nve_name, sourjctanner.network_cloudengine.ce_ip)
            recv_xml = set_nc_config(self.module, cfg_xml)
            self.check_response(recv_xml, "MERGE_SOURCE_IP")
            self.updates_cmd.append("interfajctanner.network_cloudengine.ce %s" % nve_name)
            self.updates_cmd.append("sourjctanner.network_cloudengine.ce %s" % sourjctanner.network_cloudengine.ce_ip)
            self.changed = True
    ...
    def config_delete_sourjctanner.network_cloudengine.ce_ip(self, nve_name, sourjctanner.network_cloudengine.ce_ip):
        """nve sourjctanner.network_cloudengine.ce ip"""

        if not self.is_nve_sourjctanner.network_cloudengine.ce_ip_exist(nve_name, sourjctanner.network_cloudengine.ce_ip):
            return
        ipaddr = "0.0.0.0"
        cfg_xml = CE_NC_MERGE_NVE_SOURCE_IP_PROTOCOL % (
            nve_name, ipaddr)
        ...
        self.updates_cmd.append("interfajctanner.network_cloudengine.ce %s" % nve_name)
        self.updates_cmd.append("undo sourjctanner.network_cloudengine.ce %s" % sourjctanner.network_cloudengine.ce_ip)
        self.changed = True
    ...
    def check_params(self):
        ...
        if self.nve_name:
            if not self.check_nve_name():
                self.module.fail_json(
                    msg='Error: Error: NVE interfajctanner.network_cloudengine.ce %s is invalid.' % self.nve_name)
        ...
        if self.sourjctanner.network_cloudengine.ce_ip:
            if not is_valid_address(self.sourjctanner.network_cloudengine.ce_ip):
                self.module.fail_json(
                    msg='Error: The ip address %s is invalid.' % self.sourjctanner.network_cloudengine.ce_ip)
    ...
    def get_existing(self):
        ...
        if self.nve_info:
            self.existing["nve_interfajctanner.network_cloudengine.ce_name"] = self.nve_info["ifName"]
            self.existing["sourjctanner.network_cloudengine.ce_ip"] = self.nve_info["srcAddr"]
    ...
    def get_end_state(self):
        ...
        if nve_info:
            self.end_state["nve_interfajctanner.network_cloudengine.ce_name"] = nve_info["ifName"]
            self.end_state["sourjctanner.network_cloudengine.ce_ip"] = nve_info["srcAddr"]
    ...
    def work(self):
        ...
        if self.state == "present":
            ...
            if self.nve_name:
                if self.check_nve_interfajctanner.network_cloudengine.ce(self.nve_name):
                    if self.nve_mode:
                        self.config_merge_mode(self.nve_name, self.nve_mode)
                    if self.sourjctanner.network_cloudengine.ce_ip:
                        self.config_merge_sourjctanner.network_cloudengine.ce_ip(
                            self.nve_name, self.sourjctanner.network_cloudengine.ce_ip)
                    ...
                    self.module.fail_json(
                        msg='Error: Nve interfajctanner.network_cloudengine.ce %s does not exist.' % self.nve_name)
            ...
            if self.nve_name:
                if self.check_nve_interfajctanner.network_cloudengine.ce(self.nve_name):
                    ...
                    if self.sourjctanner.network_cloudengine.ce_ip:
                        self.config_delete_sourjctanner.network_cloudengine.ce_ip(
                            self.nve_name, self.sourjctanner.network_cloudengine.ce_ip)
                    if self.vni_id and self.peer_list_ip:
...
def main():
    ...
    argument_spec = dict(
        ...
        nve_mode=dict(required=False, choijctanner.network_cloudengine.ces=['mode-l2', 'mode-l3']),
        peer_list_ip=dict(required=False, type='list'),
        protocol_type=dict(required=False, type='str', choijctanner.network_cloudengine.ces=[
            'bgp', 'null']),

        sourjctanner.network_cloudengine.ce_ip=dict(required=False),
