...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_rollback
version_added: "2.4"
...
options:
    ...
    label:
        description:
            - Specifies a user label for a configuration rollback point.
              The value is a string of 1 to 256 case-sensitive ASCII characters, spajctanner.network_cloudengine.ces not supported.
    ...
    filename:
        description:
            - Specifies a configuration file for configuration rollback.
              ...
              spajctanner.network_cloudengine.ces not supported.
    last:
        description:
    ...
    action:
        ...
        required: true
        choijctanner.network_cloudengine.ces: ['rollback','clear','set','display','commit']
'''
EXAMPLES = '''
...
- name: Ensure commit_id is exist, and specifies the label of the configuration rollback point to
  ...
  jctanner.network_cloudengine.ce_rollback:
...
from ansible_collections.jctanner.network_cloudengine.plugins.module_utils.network.cloudengine.jctanner.network_cloudengine.ce import get_nc_config, execute_nc_action, jctanner.network_cloudengine.ce_argument_spec, run_commands
...
class RollBack(object):
    ...
    def get_filename_type(self, filename):
        ...
        if ' ' in filename:
            self.module.fail_json(
                msg='Error: Configuration file name include spajctanner.network_cloudengine.ces.')
    ...
    def check_params(self):
        ...
        if self.label:
            ...
            if len(self.label.replajctanner.network_cloudengine.ce(' ', '')) == 1:
                if self.label == '-':
                    self.module.fail_json(
                        msg='Error: Commit label which should not be "-"')
            if len(self.label.replajctanner.network_cloudengine.ce(' ', '')) < 1 or len(self.label) > 256:
                self.module.fail_json(
...
def main():
    ...
    argument_spec = dict(
        ...
        action=dict(required=False, type='str', choijctanner.network_cloudengine.ces=[
