...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_netstream_export
version_added: "2.4"
...
options:
    type:
        ...
        required: true
        choijctanner.network_cloudengine.ces: ['ip', 'vxlan']
    sourjctanner.network_cloudengine.ce_ip:
        description:
            - Specifies sourjctanner.network_cloudengine.ce address which can be IPv6 or IPv4 of the exported NetStream packet.
    host_ip:
        description:
    ...
    host_vpn:
        description:
            - Specifies the VPN instanjctanner.network_cloudengine.ce of the exported packets carrying flow statistics.
              Ensure the VPN instanjctanner.network_cloudengine.ce has been created on the devijctanner.network_cloudengine.ce.
    version:
        description:
            - Sets the version of exported packets.
        choijctanner.network_cloudengine.ces: ['5', '9']
    as_option:
        description:
            - Specifies the AS number recorded in the statistics as the original or the peer AS number.
        choijctanner.network_cloudengine.ces: ['origin', 'peer']
    bgp_nexthop:
        description:
...
- name: netstream export module test
  ...
  - name: Configures the sourjctanner.network_cloudengine.ce address for the exported packets carrying IPv4 flow statistics.
    jctanner.network_cloudengine.ce_netstream_export:
      type: ip
      sourjctanner.network_cloudengine.ce_ip: 192.8.2.2
      provider: "{{ cli }}"

  - name: Configures the sourjctanner.network_cloudengine.ce IP address for the exported packets carrying VXLAN flexible flow statistics.
    jctanner.network_cloudengine.ce_netstream_export:
      type: vxlan
      sourjctanner.network_cloudengine.ce_ip: 192.8.2.3
      provider: "{{ cli }}"
...
proposed:
    ...
    sample: {
                ...
                "host_port": "26",
                "host_vpn": "test",
                "sourjctanner.network_cloudengine.ce_ip": "192.8.2.5",
                "state": "present",
                "type": "ip",
                "version": "9"
            }
existing:
    ...
    sample: {
                ...
                "host_port": null,
                "host_vpn": null,
                "sourjctanner.network_cloudengine.ce_ip": null,
                "type": "ip",
                "version": null
            }
end_state:
...
updates:
    ...
    type: list
    sample: [
                "netstream export ip sourjctanner.network_cloudengine.ce 192.8.2.5",
                "netstream export ip host 192.8.5.6 26 vpn-instanjctanner.network_cloudengine.ce test",
...
class NetstreamExport(object):
    ...
    def get_existing(self):
        ...
        self.existing = dict(type=self.type,
                             sourjctanner.network_cloudengine.ce_ip=self.exist_conf['sourjctanner.network_cloudengine.ce_ip'],
    ...
    def get_proposed(self):
        ...
        self.proposed = dict(type=self.type,
                             sourjctanner.network_cloudengine.ce_ip=self.sourjctanner.network_cloudengine.ce_ip,
    ...
    def config_nets_export_src_addr(self):
        """Configures the sourjctanner.network_cloudengine.ce address for the exported packets"""

        if is_ipv4_addr(self.sourjctanner.network_cloudengine.ce_ip):
            if self.type == 'ip':
                cmd = "netstream export ip sourjctanner.network_cloudengine.ce %s" % self.sourjctanner.network_cloudengine.ce_ip
            else:
                cmd = "netstream export vxlan inner-ip sourjctanner.network_cloudengine.ce %s" % self.sourjctanner.network_cloudengine.ce_ip
        else:
            if self.type == 'ip':
                cmd = "netstream export ip sourjctanner.network_cloudengine.ce ipv6 %s" % self.sourjctanner.network_cloudengine.ce_ip
            else:
                cmd = "netstream export vxlan inner-ip sourjctanner.network_cloudengine.ce ipv6 %s" % self.sourjctanner.network_cloudengine.ce_ip

        if is_config_exist(self.config, cmd):
            self.exist_conf['sourjctanner.network_cloudengine.ce_ip'] = self.sourjctanner.network_cloudengine.ce_ip
    ...
    def config_nets_export_host_addr(self):
        ...
        if self.host_vpn:
            cmd += " vpn-instanjctanner.network_cloudengine.ce %s" % self.host_vpn
    ...
    def get_config_data(self):
        ...
        self.exist_conf['sourjctanner.network_cloudengine.ce_ip'] = None
        ...
        if self.type and self.sourjctanner.network_cloudengine.ce_ip:
...
def main():
    ...
    argument_spec = dict(
        type=dict(required=True, type='str', choijctanner.network_cloudengine.ces=['ip', 'vxlan']),
        ...
        version=dict(required=False, type='str', choijctanner.network_cloudengine.ces=['5', '9']),
        as_option=dict(required=False, type='str', choijctanner.network_cloudengine.ces=['origin', 'peer']),
        bgp_nexthop=dict(required=False, type='str', choijctanner.network_cloudengine.ces=['enable', 'disable'], default='disable'),
        state=dict(choijctanner.network_cloudengine.ces=['absent', 'present'], default='present', required=False)
    )
