...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_bgp_neighbor
version_added: "2.4"
...
options:
    ...
    local_if_name:
        description:
            - Name of a sourjctanner.network_cloudengine.ce interfajctanner.network_cloudengine.ce that sends BGP packets.
    ...
    pswd_type:
        ...
        choijctanner.network_cloudengine.ces: ['null','cipher','simple']
    pswd_cipher_text:
        description:
            - The character string in a password identifies the contents of the password, spajctanner.network_cloudengine.ces not supported.
    ...
    prepend_fake_as:
        description:
            - Add the Fake AS number to rejctanner.network_cloudengine.ceived Update packets.
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
        default: no_use
    is_bfd_block:
        description:
    ...
    rx_interval:
        description:
            - Specify the minimum interval at which BFD packets are rejctanner.network_cloudengine.ceived.
...
- name: CloudEngine BGP neighbor test
  ...
  - name: "Config bgp peer"
    jctanner.network_cloudengine.ce_bgp_neighbor:
      state: present
      vrf_name: js
...
class BgpNeighbor(object):
    ...
    def merge_bgp_peer_other(self, **kwargs):
        ...
        if local_if_name:
            ...
            cmd = "peer %s connect-interfajctanner.network_cloudengine.ce local_if_name" % peer_addr
            cmds.append(cmd)
...
def main():
    ...
    argument_spec = dict(
        ...
        dual_as=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        conventional=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        route_refresh=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        four_byte_as=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        is_ignore=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        local_if_name=dict(type='str'),
        ebgp_max_hop=dict(type='str'),
        valid_ttl_hops=dict(type='str'),
        connect_mode=dict(choijctanner.network_cloudengine.ces=['listenOnly', 'connectOnly', 'null']),
        is_log_change=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        pswd_type=dict(choijctanner.network_cloudengine.ces=['null', 'cipher', 'simple']),
        ...
        mpls_local_ifnet_disable=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        prepend_global_as=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        prepend_fake_as=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        is_bfd_block=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        multiplier=dict(type='str'),
        is_bfd_enable=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        rx_interval=dict(type='str'),
        tx_interval=dict(type='str'),
        is_single_hop=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']))
    ...
    jctanner.network_cloudengine.ce_bgp_peer_obj = BgpNeighbor()
    ...
    if not jctanner.network_cloudengine.ce_bgp_peer_obj:
        module.fail_json(msg='Error: Init module failed.')

    need_bgp_peer_enable = jctanner.network_cloudengine.ce_bgp_peer_obj.check_bgp_peer_args(module=module)
    need_bgp_peer_other_rst = jctanner.network_cloudengine.ce_bgp_peer_obj.check_bgp_peer_other_args(
        module=module)
    need_peer_bfd_merge_rst = jctanner.network_cloudengine.ce_bgp_peer_obj.check_peer_bfd_merge_args(
        module=module)
    need_peer_bfd_del_rst = jctanner.network_cloudengine.ce_bgp_peer_obj.check_peer_bfd_delete_args(
        module=module)
    ...
    if need_bgp_peer_enable["need_cfg"]:
        ...
        if state == "present":
            ...
            if remote_as:
                ...
                bgp_peer_exist = jctanner.network_cloudengine.ce_bgp_peer_obj.get_bgp_peer(module=module)
                existing["bgp peer"] = bgp_peer_exist

                bgp_peer_new = (peer_addr, remote_as)

                if len(bgp_peer_exist) == 0:
                    cmd = jctanner.network_cloudengine.ce_bgp_peer_obj.create_bgp_peer(module=module)
                    changed = True
                    for item in cmd:
                        updates.append(item)
                    ...
                    cmd = jctanner.network_cloudengine.ce_bgp_peer_obj.merge_bgp_peer(module=module)
                    changed = True
                    for item in cmd:
                        updates.append(item)

                bgp_peer_end = jctanner.network_cloudengine.ce_bgp_peer_obj.get_bgp_peer(module=module)
                end_state["bgp peer"] = bgp_peer_end

        else:

            bgp_peer_exist = jctanner.network_cloudengine.ce_bgp_peer_obj.get_bgp_del_peer(module=module)
            existing["bgp peer"] = bgp_peer_exist
            ...
            if len(bgp_peer_exist) == 0:
                ...
                cmd = jctanner.network_cloudengine.ce_bgp_peer_obj.delete_bgp_peer(module=module)
                changed = True
                for item in cmd:
                    updates.append(item)

            bgp_peer_end = jctanner.network_cloudengine.ce_bgp_peer_obj.get_bgp_del_peer(module=module)
            end_state["bgp peer"] = bgp_peer_end
    ...
    if need_bgp_peer_other_rst["need_cfg"]:
        ...
        if state == "present":
            cmd = jctanner.network_cloudengine.ce_bgp_peer_obj.merge_bgp_peer_other(module=module)
            changed = True
            for item in cmd:
                updates.append(item)
    ...
    if state == "present":
        ...
        if need_peer_bfd_merge_rst["need_cfg"]:
            cmd = jctanner.network_cloudengine.ce_bgp_peer_obj.merge_peer_bfd(module=module)
            changed = True
            for item in cmd:
                updates.append(item)
