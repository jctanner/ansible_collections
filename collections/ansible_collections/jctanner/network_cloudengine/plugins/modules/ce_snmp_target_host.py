...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_snmp_target_host
version_added: "2.4"
...
options:
    version:
        ...
        choijctanner.network_cloudengine.ces: ['none', 'v1', 'v2c', 'v3', 'v1v2c', 'v1v3', 'v2cv3', 'all']
    connect_port:
        description:
    ...
    notify_type:
        ...
        choijctanner.network_cloudengine.ces: ['trap','inform']
    vpn_name:
        description:
            - VPN instanjctanner.network_cloudengine.ce Name.
    recv_port:
        description:
            - UDP Port number used by network management to rejctanner.network_cloudengine.ceive alarm messages.
    security_model:
        description:
            - Security Model.
        choijctanner.network_cloudengine.ces: ['v1','v2c', 'v3']
    security_name:
        description:
            - Security Name.
    security_name_v3:
    ...
    security_level:
        ...
        choijctanner.network_cloudengine.ces: ['noAuthNoPriv','authentication', 'privacy']
    is_public_net:
        description:
            - To enable or disable Public Net-manager for target Host.
        default: no_use
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
    interfajctanner.network_cloudengine.ce_name:
        description:
            - Name of the interfajctanner.network_cloudengine.ce to send the trap message.
'''
...
- name: CloudEngine snmp target host test
  ...
  - name: "Config SNMP version"
    jctanner.network_cloudengine.ce_snmp_target_host:
      state: present
      version: v2cv3
      provider: "{{ cli }}"
...
end_state:
    ...
    sample: {"target host info": [{"address": "10.135.182.158", "domain": "snmpUDPDomain",
                                   ...
                                   "securityNameV3": "wdz", "vpnInstanjctanner.network_cloudengine.ceName": "js"}]}
updates:
    description: command sent to the devijctanner.network_cloudengine.ce
    returned: always
    type: list
    sample: ["snmp-agent target-host host-name test2 trap address udp-domain 10.135.182.158 vpn-instanjctanner.network_cloudengine.ce js params securityname wdz v3 authentication"]
'''
...
from ansible_collections.jctanner.network_cloudengine.plugins.module_utils.network.cloudengine.jctanner.network_cloudengine.ce import get_nc_config, set_nc_config, \
    jctanner.network_cloudengine.ce_argument_spec, get_config, load_config, check_ip_addr
...
class SnmpTargetHost(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.interfajctanner.network_cloudengine.ce_name = self.module.params['interfajctanner.network_cloudengine.ce_name']
    ...
    def check_netconf_args(self, result):
        ...
        if self.host_name:
            ...
            if self.vpn_name:
                ...
                conf_str += "<vpnInstanjctanner.network_cloudengine.ceName></vpnInstanjctanner.network_cloudengine.ceName>"
            ...
            if self.interfajctanner.network_cloudengine.ce_name:
                if len(self.interfajctanner.network_cloudengine.ce_name) > 63 or len(self.interfajctanner.network_cloudengine.ce_name) < 1:
                    self.module.fail_json(
                        msg='Error: The len of interfajctanner.network_cloudengine.ce_name is out of [1 - 63].')

                find_flag = False
                for item in INTERFACE_TYPE:
                    if item in self.interfajctanner.network_cloudengine.ce_name:
                        find_flag = True
                        break
                if not find_flag:
                    self.module.fail_json(
                        msg='Error: Please input full name of interfajctanner.network_cloudengine.ce_name.')

                conf_str += "<interfajctanner.network_cloudengine.ce-name></interfajctanner.network_cloudengine.ce-name>"
            ...
            if "<data/>" in recv_xml:
                ...
                if target_host_info:
                    for tmp in target_host_info:
                        tmp_dict = dict()
                        for site in tmp:
                            if site.tag in ["nmsName", "domain", "address", "notifyType", "vpnInstanjctanner.network_cloudengine.ceName",
                                            "portNumber", "securityModel", "securityName", "securityNameV3",
                                            "securityLevel", "isPublicNet", "interfajctanner.network_cloudengine.ce-name"]:
                ...
                if result["target_host_info"]:
                    for tmp in result["target_host_info"]:
                        ...
                        if "vpnInstanjctanner.network_cloudengine.ceName" in tmp.keys():
                            if tmp["vpnInstanjctanner.network_cloudengine.ceName"] != self.vpn_name:
                                same_flag = False
                        ...
                        if "interfajctanner.network_cloudengine.ce-name" in tmp.keys():
                            if tmp["interfajctanner.network_cloudengine.ce-name"] != self.interfajctanner.network_cloudengine.ce_name:
                                same_flag = False
    ...
    def get_proposed(self):
        ...
        if self.interfajctanner.network_cloudengine.ce_name:
            self.proposed["interfajctanner.network_cloudengine.ce_name"] = self.interfajctanner.network_cloudengine.ce_name
    ...
    def merge_snmp_target_host(self):
        ...
        if self.vpn_name:
            conf_str += "<vpnInstanjctanner.network_cloudengine.ceName>%s</vpnInstanjctanner.network_cloudengine.ceName>" % self.vpn_name
        if self.recv_port:
        ...
        if self.interfajctanner.network_cloudengine.ce_name:
            conf_str += "<interfajctanner.network_cloudengine.ce-name>%s</interfajctanner.network_cloudengine.ce-name>" % self.interfajctanner.network_cloudengine.ce_name
        ...
        if self.interfajctanner.network_cloudengine.ce_name:
            cmd += "sourjctanner.network_cloudengine.ce %s " % self.interfajctanner.network_cloudengine.ce_name
        if self.vpn_name:
            cmd += "vpn-instanjctanner.network_cloudengine.ce %s " % self.vpn_name
        if self.is_public_net == "true":
            cmd += "public-net "
...
def main():
    ...
    argument_spec = dict(
        ...
        version=dict(choijctanner.network_cloudengine.ces=['none', 'v1', 'v2c', 'v3',
                              'v1v2c', 'v1v3', 'v2cv3', 'all']),
        connect_port=dict(type='str'),
        host_name=dict(type='str'),
        address=dict(type='str'),
        notify_type=dict(choijctanner.network_cloudengine.ces=['trap', 'inform']),
        vpn_name=dict(type='str'),
        recv_port=dict(type='str'),
        security_model=dict(choijctanner.network_cloudengine.ces=['v1', 'v2c', 'v3']),
        security_name=dict(type='str', no_log=True),
        security_name_v3=dict(type='str'),
        security_level=dict(
            choijctanner.network_cloudengine.ces=['noAuthNoPriv', 'authentication', 'privacy']),
        is_public_net=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        interfajctanner.network_cloudengine.ce_name=dict(type='str')
    )
