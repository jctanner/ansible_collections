...
module: jctanner.network_cloudengine.ce_ntp_auth
version_added: "2.4"
...
notes:
    - If C(state=absent), the module will attempt to remove the given key configuration.
      If a matching key configuration isn't found on the devijctanner.network_cloudengine.ce, the module will fail.
...
options:
    ...
    auth_mode:
        ...
        choijctanner.network_cloudengine.ces: ['hmac-sha256', 'md5']
    auth_type:
        description:
            - Whether the given password is in cleartext or
              has been encrypted. If in cleartext, the devijctanner.network_cloudengine.ce
              will encrypt it before storing it.
        default: encrypt
        choijctanner.network_cloudengine.ces: ['text', 'encrypt']
    trusted_key:
        description:
            - Whether the given key is required to be supplied by a time sourjctanner.network_cloudengine.ce
              for the devijctanner.network_cloudengine.ce to synchronize to the time sourjctanner.network_cloudengine.ce.
        default: 'disable'
...
- name: NTP AUTH test
  ...
  - name: "Configure ntp authentication key-id"
    jctanner.network_cloudengine.ce_ntp_auth:
      key_id: 32
      auth_mode: md5
...
from ansible_collections.jctanner.network_cloudengine.plugins.module_utils.network.cloudengine.jctanner.network_cloudengine.ce import jctanner.network_cloudengine.ce_argument_spec, load_config, get_nc_config, set_nc_config
...
def main():
    ...
    argument_spec = dict(
        ...
        auth_mode=dict(choijctanner.network_cloudengine.ces=['md5', 'hmac-sha256'], type='str'),
        auth_type=dict(choijctanner.network_cloudengine.ces=['text', 'encrypt'], default='encrypt'),
        trusted_key=dict(choijctanner.network_cloudengine.ces=['enable', 'disable'], default='disable'),
        authentication=dict(choijctanner.network_cloudengine.ces=['enable', 'disable']),
