...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_aaa_server_host
version_added: "2.4"
...
options:
    ...
    local_servijctanner.network_cloudengine.ce_type:
        description:
    ...
    radius_server_type:
        ...
        choijctanner.network_cloudengine.ces: ['Authentication', 'Accounting']
    radius_server_ip:
        description:
    ...
    radius_server_mode:
        ...
        choijctanner.network_cloudengine.ces: ['Secondary-server', 'Primary-server']
    radius_vpn_name:
        description:
            - Set VPN instanjctanner.network_cloudengine.ce.
    ...
    hwtacacs_server_type:
        ...
        choijctanner.network_cloudengine.ces: ['Authentication', 'Authorization', 'Accounting', 'Common']
...
- name: AAA server host test
  ...
  - name: "Config local user when use local scheme"
    jctanner.network_cloudengine.ce_aaa_server_host:
      state: present
...
updates:
    ...
    sample: ["hwtacacs server template test",
             "hwtacacs server authorization 10.135.182.157 vpn-instanjctanner.network_cloudengine.ce test_vpn public-net"]
'''
...
class AaaServerHost(object):
    ...
    def get_local_user_info(self, **kwargs):
        ...
        local_servijctanner.network_cloudengine.ce_type = module.params['local_servijctanner.network_cloudengine.ce_type']
        ...
        if local_servijctanner.network_cloudengine.ce_type:
            if local_servijctanner.network_cloudengine.ce_type == "none":
                conf_str += "<servijctanner.network_cloudengine.ceTerminal></servijctanner.network_cloudengine.ceTerminal>"
                conf_str += "<servijctanner.network_cloudengine.ceTelnet></servijctanner.network_cloudengine.ceTelnet>"
                conf_str += "<servijctanner.network_cloudengine.ceFtp></servijctanner.network_cloudengine.ceFtp>"
                conf_str += "<servijctanner.network_cloudengine.ceSsh></servijctanner.network_cloudengine.ceSsh>"
                conf_str += "<servijctanner.network_cloudengine.ceSnmp></servijctanner.network_cloudengine.ceSnmp>"
                conf_str += "<servijctanner.network_cloudengine.ceDot1x></servijctanner.network_cloudengine.ceDot1x>"
            elif local_servijctanner.network_cloudengine.ce_type == "dot1x":
                conf_str += "<servijctanner.network_cloudengine.ceDot1x></servijctanner.network_cloudengine.ceDot1x>"
            else:
                option = local_servijctanner.network_cloudengine.ce_type.split(" ")
                for tmp in option:
                    if tmp == "dot1x":
                        module.fail_json(
                            msg='Error: Do not input dot1x with other servijctanner.network_cloudengine.ce type.')
                    elif tmp == "none":
                        module.fail_json(
                            msg='Error: Do not input none with other servijctanner.network_cloudengine.ce type.')
                    elif tmp == "ftp":
                        conf_str += "<servijctanner.network_cloudengine.ceFtp></servijctanner.network_cloudengine.ceFtp>"
                    elif tmp == "snmp":
                        conf_str += "<servijctanner.network_cloudengine.ceSnmp></servijctanner.network_cloudengine.ceSnmp>"
                ...
                for tmp in local_user_info:
                    ...
                    for site in tmp:
                        if site.tag in ["userName", "password", "userLevel", "ftpDir", "userGroupName",
                                        "servijctanner.network_cloudengine.ceTerminal", "servijctanner.network_cloudengine.ceTelnet", "servijctanner.network_cloudengine.ceFtp", "servijctanner.network_cloudengine.ceSsh",
                                        "servijctanner.network_cloudengine.ceSnmp", "servijctanner.network_cloudengine.ceDot1x"]:
                    ...
                    for tmp in result["local_user_info"]:
                        if "userName" in tmp.keys():
                            if tmp["userName"] == local_user_name:
                                ...
                                if not local_servijctanner.network_cloudengine.ce_type and not local_user_level \
                                ...
                                if local_servijctanner.network_cloudengine.ce_type:
                                    if local_servijctanner.network_cloudengine.ce_type == "none":
                                        if tmp.get("servijctanner.network_cloudengine.ceTerminal") == "true" or \
                                                tmp.get("servijctanner.network_cloudengine.ceTelnet") == "true" or \
                                                tmp.get("servijctanner.network_cloudengine.ceFtp") == "true" or \
                                                tmp.get("servijctanner.network_cloudengine.ceSsh") == "true" or \
                                                tmp.get("servijctanner.network_cloudengine.ceSnmp") == "true" or \
                                                tmp.get("servijctanner.network_cloudengine.ceDot1x") == "true":
                                            need_cfg = True
                                    elif local_servijctanner.network_cloudengine.ce_type == "dot1x":
                                        if tmp.get("servijctanner.network_cloudengine.ceDot1x") == "true":
                                            need_cfg = True
                                    elif tmp == "ftp":
                                        if tmp.get("servijctanner.network_cloudengine.ceFtp") == "true":
                                            need_cfg = True
                                    elif tmp == "snmp":
                                        if tmp.get("servijctanner.network_cloudengine.ceSnmp") == "true":
                                            need_cfg = True
                                    elif tmp == "ssh":
                                        if tmp.get("servijctanner.network_cloudengine.ceSsh") == "true":
                                            need_cfg = True
                                    elif tmp == "telnet":
                                        if tmp.get("servijctanner.network_cloudengine.ceTelnet") == "true":
                                            need_cfg = True
                                    elif tmp == "terminal":
                                        if tmp.get("servijctanner.network_cloudengine.ceTerminal") == "true":
                                            need_cfg = True
    ...
    def merge_local_user_info(self, **kwargs):
        ...
        if local_servijctanner.network_cloudengine.ce_type:
            if local_servijctanner.network_cloudengine.ce_type == "none":
                conf_str += "<servijctanner.network_cloudengine.ceTerminal>false</servijctanner.network_cloudengine.ceTerminal>"
                conf_str += "<servijctanner.network_cloudengine.ceTelnet>false</servijctanner.network_cloudengine.ceTelnet>"
                conf_str += "<servijctanner.network_cloudengine.ceFtp>false</servijctanner.network_cloudengine.ceFtp>"
                conf_str += "<servijctanner.network_cloudengine.ceSsh>false</servijctanner.network_cloudengine.ceSsh>"
                conf_str += "<servijctanner.network_cloudengine.ceSnmp>false</servijctanner.network_cloudengine.ceSnmp>"
                conf_str += "<servijctanner.network_cloudengine.ceDot1x>false</servijctanner.network_cloudengine.ceDot1x>"

                cmd = "local-user %s servijctanner.network_cloudengine.ce-type none" % local_user_name
                cmds.append(cmd)

            elif local_servijctanner.network_cloudengine.ce_type == "dot1x":
                if state == "present":
                    conf_str += "<servijctanner.network_cloudengine.ceDot1x>true</servijctanner.network_cloudengine.ceDot1x>"
                    cmd = "local-user %s servijctanner.network_cloudengine.ce-type dot1x" % local_user_name
                else:
                    conf_str += "<servijctanner.network_cloudengine.ceDot1x>false</servijctanner.network_cloudengine.ceDot1x>"
                    cmd = "undo local-user %s servijctanner.network_cloudengine.ce-type" % local_user_name
                ...
                for tmp in option:
                    ...
                    if state == "present":
                        if tmp == "ftp":
                            conf_str += "<servijctanner.network_cloudengine.ceFtp>true</servijctanner.network_cloudengine.ceFtp>"
                            cmd = "local-user %s servijctanner.network_cloudengine.ce-type ftp" % local_user_name
                        elif tmp == "snmp":
                            conf_str += "<servijctanner.network_cloudengine.ceSnmp>true</servijctanner.network_cloudengine.ceSnmp>"
                            cmd = "local-user %s servijctanner.network_cloudengine.ce-type snmp" % local_user_name
                        elif tmp == "ssh":
                            conf_str += "<servijctanner.network_cloudengine.ceSsh>true</servijctanner.network_cloudengine.ceSsh>"
                            cmd = "local-user %s servijctanner.network_cloudengine.ce-type ssh" % local_user_name
                        elif tmp == "telnet":
                            conf_str += "<servijctanner.network_cloudengine.ceTelnet>true</servijctanner.network_cloudengine.ceTelnet>"
                            cmd = "local-user %s servijctanner.network_cloudengine.ce-type telnet" % local_user_name
                        elif tmp == "terminal":
                            conf_str += "<servijctanner.network_cloudengine.ceTerminal>true</servijctanner.network_cloudengine.ceTerminal>"
                            cmd = "local-user %s servijctanner.network_cloudengine.ce-type terminal" % local_user_name
    ...
    def merge_radius_server_cfg_ipv4(self, **kwargs):
        ...
        if radius_server_type == "Authentication":
            ...
            if radius_vpn_name and radius_vpn_name != "_public_":
                cmd += " vpn-instanjctanner.network_cloudengine.ce %s" % radius_vpn_name
    ...
    def merge_hwtacacs_server_cfg_ipv4(self, **kwargs):
        ...
        if hwtacacs_server_type == "Authentication":
            ...
            if hwtacacs_vpn_name and hwtacacs_vpn_name != "_public_":
                cmd += " vpn-instanjctanner.network_cloudengine.ce %s" % hwtacacs_vpn_name
            if hwtacacs_is_public_net:
                cmd += " public-net"
            if hwtacacs_is_secondary_server:
                cmd += " secondary"
...
def check_module_argument(**kwargs):
    ...
    if radius_vpn_name:
        ...
        if ' ' in radius_vpn_name:
            module.fail_json(
                msg='Error: The radius_vpn_name %s include spajctanner.network_cloudengine.ce.' % radius_vpn_name)
    ...
    if radius_server_name:
        ...
        if ' ' in radius_server_name:
            module.fail_json(
                msg='Error: The radius_server_name %s include spajctanner.network_cloudengine.ce.' % radius_server_name)
    ...
    if hwtacacs_vpn_name:
        ...
        if ' ' in hwtacacs_vpn_name:
            module.fail_json(
                msg='Error: The hwtacacs_vpn_name %s include spajctanner.network_cloudengine.ce.' % hwtacacs_vpn_name)
    ...
    if hwtacacs_server_host_name:
        ...
        if ' ' in hwtacacs_server_host_name:
            module.fail_json(
                msg='Error: The hwtacacs_server_host_name %s include spajctanner.network_cloudengine.ce.' % hwtacacs_server_host_name)
...
def main():
    ...
    argument_spec = dict(
        ...
        local_servijctanner.network_cloudengine.ce_type=dict(type='str'),
        local_ftp_dir=dict(type='str'),
        local_user_level=dict(type='str'),
        local_user_group=dict(type='str'),
        radius_group_name=dict(type='str'),
        radius_server_type=dict(choijctanner.network_cloudengine.ces=['Authentication', 'Accounting']),
    ...
    jctanner.network_cloudengine.ce_aaa_server_host = AaaServerHost()

    if not jctanner.network_cloudengine.ce_aaa_server_host:
        module.fail_json(msg='Error: Construct jctanner.network_cloudengine.ce_aaa_server failed.')
    ...
    if local_servijctanner.network_cloudengine.ce_type:
        proposed["local_servijctanner.network_cloudengine.ce_type"] = local_servijctanner.network_cloudengine.ce_type
    if local_ftp_dir:
    ...
    if local_user_name:
        ...
        local_user_result = jctanner.network_cloudengine.ce_aaa_server_host.get_local_user_info(
            module=module)
        ...
        if state == "present":
            ...
            if local_user_result["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_aaa_server_host.merge_local_user_info(module=module)
            ...
            # absent local user
            if local_user_result["need_cfg"]:
                if not local_servijctanner.network_cloudengine.ce_type and not local_ftp_dir and not local_user_level and not local_user_group:
                    cmd = jctanner.network_cloudengine.ce_aaa_server_host.delete_local_user_info(
                        module=module)
                else:
                    cmd = jctanner.network_cloudengine.ce_aaa_server_host.merge_local_user_info(
                        module=module)
    ...
    if radius_group_name:
        ...
        if radius_server_ip:
            rds_server_ipv4_result = jctanner.network_cloudengine.ce_aaa_server_host.get_radius_server_cfg_ipv4(
                module=module)
        if radius_server_ipv6:
            rds_server_ipv6_result = jctanner.network_cloudengine.ce_aaa_server_host.get_radius_server_cfg_ipv6(
                module=module)
        if radius_server_name:
            rds_server_name_result = jctanner.network_cloudengine.ce_aaa_server_host.get_radius_server_name(
                module=module)
        ...
        if state == "present":
            if radius_server_ip and rds_server_ipv4_result["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_aaa_server_host.merge_radius_server_cfg_ipv4(
                    module=module)
                changed = True
                updates.append(cmd)

            if radius_server_ipv6 and rds_server_ipv6_result["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_aaa_server_host.merge_radius_server_cfg_ipv6(
                    module=module)
                changed = True
                updates.append(cmd)

            if radius_server_name and rds_server_name_result["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_aaa_server_host.merge_radius_server_name(
                    module=module)
                changed = True
                updates.append(cmd)
        else:
            if radius_server_ip and rds_server_ipv4_result["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_aaa_server_host.delete_radius_server_cfg_ipv4(
                    module=module)
                changed = True
                updates.append(cmd)

            if radius_server_ipv6 and rds_server_ipv6_result["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_aaa_server_host.delete_radius_server_cfg_ipv6(
                    module=module)
                changed = True
                updates.append(cmd)

            if radius_server_name and rds_server_name_result["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_aaa_server_host.delete_radius_server_name(
                    module=module)
                changed = True
                updates.append(cmd)
    ...
    if hwtacacs_template:
        ...
        if hwtacacs_server_ip:
            hwtacacs_server_ipv4_result = jctanner.network_cloudengine.ce_aaa_server_host.get_hwtacacs_server_cfg_ipv4(
                module=module)
        if hwtacacs_server_ipv6:
            hwtacacs_server_ipv6_result = jctanner.network_cloudengine.ce_aaa_server_host.get_hwtacacs_server_cfg_ipv6(
                module=module)
        if hwtacacs_server_host_name:
            hwtacacs_host_name_result = jctanner.network_cloudengine.ce_aaa_server_host.get_hwtacacs_host_server_cfg(
                module=module)
        ...
        if state == "present":
            if hwtacacs_server_ip and hwtacacs_server_ipv4_result["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_aaa_server_host.merge_hwtacacs_server_cfg_ipv4(
                    module=module)
                changed = True
                updates.append(cmd)

            if hwtacacs_server_ipv6 and hwtacacs_server_ipv6_result["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_aaa_server_host.merge_hwtacacs_server_cfg_ipv6(
                    module=module)
                changed = True
                updates.append(cmd)

            if hwtacacs_server_host_name and hwtacacs_host_name_result["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_aaa_server_host.merge_hwtacacs_host_server_cfg(
                    module=module)
                changed = True
                updates.append(cmd)
            ...
            if hwtacacs_server_ip and hwtacacs_server_ipv4_result["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_aaa_server_host.delete_hwtacacs_server_cfg_ipv4(
                    module=module)
                changed = True
                updates.append(cmd)

            if hwtacacs_server_ipv6 and hwtacacs_server_ipv6_result["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_aaa_server_host.delete_hwtacacs_server_cfg_ipv6(
                    module=module)
                changed = True
                updates.append(cmd)

            if hwtacacs_server_host_name and hwtacacs_host_name_result["need_cfg"]:
                cmd = jctanner.network_cloudengine.ce_aaa_server_host.delete_hwtacacs_host_server_cfg(
                    module=module)
                changed = True
                updates.append(cmd)
