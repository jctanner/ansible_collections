...
DOCUMENTATION = """
---
module: jctanner.network_cloudengine.ce_netstream_global
version_added: "2.4"
...
options:
    ...
    interfajctanner.network_cloudengine.ce:
        description:
            - Netstream global interfajctanner.network_cloudengine.ce.
        required: true
    sampler_interval:
        description:
    ...
    index_switch:
        ...
        choijctanner.network_cloudengine.ces: ['16', '32']
        default: '16'
"""
...
- name: netstream global module test
  ...
  - name: Configure a netstream sampler at interfajctanner.network_cloudengine.ce 10ge1/0/2, direction is outbound,interval is 30.
    jctanner.network_cloudengine.ce_netstream_global:
      interfajctanner.network_cloudengine.ce: 10ge1/0/2
      type: ip
      sampler_interval: 30
      sampler_direction: outbound
      state: present
      provider: "{{ cli }}"
  - name: Configure a netstream flexible statistic at interfajctanner.network_cloudengine.ce 10ge1/0/2, record is test1, type is ip.
    jctanner.network_cloudengine.ce_netstream_global:
      type: ip
      interfajctanner.network_cloudengine.ce: 10ge1/0/2
...
updates:
    ...
    returned: always
    type: list
    sample: ["interfajctanner.network_cloudengine.ce 10ge1/0/2",
...
def get_interfajctanner.network_cloudengine.ce_type(interfajctanner.network_cloudengine.ce):
    """Gets the type of interfajctanner.network_cloudengine.ce, such as 10GE, ETH-TRUNK..."""
    ...
    if interfajctanner.network_cloudengine.ce.upper().startswith('GE'):
        ...
        iftype = 'eth-trunk'
    elif interfajctanner.network_cloudengine.ce.upper().startswith('ALL'):
        iftype = 'all'
    else:
        return None
...
class NetStreamGlobal(object):
    ...
    def check_params(self):
        ...
        if self.interfajctanner.network_cloudengine.ce == "all":
            if self.statistics_record or self.statistics_direction:
                self.module.fail_json(
                    msg="Error: Statistic function should be used at interfajctanner.network_cloudengine.ce.")
    ...
    def get_existing(self):
        ...
        self.get_exist_interfajctanner.network_cloudengine.ce_statistic()
        self.get_exist_statistic_record()
        self.get_exist_index_switch()

        if self.state == "present":
            for sampler_tmp in self.existing["sampler"]:
                if self.interfajctanner.network_cloudengine.ce == str(sampler_tmp["interfajctanner.network_cloudengine.ce"]):
                    temp = True
            ...
            for statistic_tmp in self.existing["statistic"]:
                if str(statistic_tmp["interfajctanner.network_cloudengine.ce"]) == self.interfajctanner.network_cloudengine.ce and self.interfajctanner.network_cloudengine.ce != "all":
                    if self.type == "vxlan":
            ...
            for statistic_tmp1 in self.existing["flexible_statistic"]:
                if self.interfajctanner.network_cloudengine.ce != "all" \
                        ...
                        and self.interfajctanner.network_cloudengine.ce == str(statistic_tmp1["interfajctanner.network_cloudengine.ce"]):
    ...
    def get_end_state(self):
        ...
        self.get_end_interfajctanner.network_cloudengine.ce_statistic()
...
def main():
    ...
    argument_spec = dict(
        type=dict(required=False, choijctanner.network_cloudengine.ces=['ip', 'vxlan'], default='ip'),
        interfajctanner.network_cloudengine.ce=dict(required=True, type='str'),
        sampler_interval=dict(required=False, type='str'),
        sampler_direction=dict(required=False, choijctanner.network_cloudengine.ces=['inbound', 'outbound']),
        statistics_direction=dict(required=False, choijctanner.network_cloudengine.ces=['inbound', 'outbound']),
        statistics_record=dict(required=False, type='str'),
        index_switch=dict(required=False, choijctanner.network_cloudengine.ces=['16', '32'], default='16'),
