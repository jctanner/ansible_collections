...
DOCUMENTATION = """
---
module: jctanner.network_cloudengine.ce_vxlan_arp
version_added: "2.4"
short_description: Manages ARP attributes of VXLAN on HUAWEI CloudEngine devijctanner.network_cloudengine.ces.
description:
    - Manages ARP attributes of VXLAN on HUAWEI CloudEngine devijctanner.network_cloudengine.ces.
...
options:
    ...
    evn_sourjctanner.network_cloudengine.ce_ip:
        description:
            - Specifies the sourjctanner.network_cloudengine.ce address of an EVN BGP peer.
    ...
    evn_server:
        description:
            - Configures the local devijctanner.network_cloudengine.ce as the router reflector (RR) on the EVN network.
        choijctanner.network_cloudengine.ces: ['enable', 'disable']
    evn_reflect_client:
        description:
            - Configures the local devijctanner.network_cloudengine.ce as the route reflector (RR) and its peer as the client.
        choijctanner.network_cloudengine.ces: ['enable', 'disable']
    vbdif_name:
        description:
    ...
    host_collect_protocol:
        ...
        choijctanner.network_cloudengine.ces: ['bgp','none']
    bridge_domain_id:
        description:
...
- name: vxlan arp module test
  ...
  - name: Configure EVN BGP on Layer 2 and Layer 3 VXLAN gateways to establish EVN BGP peer relationships.
    jctanner.network_cloudengine.ce_vxlan_arp:
      evn_bgp: enable
      evn_sourjctanner.network_cloudengine.ce_ip: 6.6.6.6
      evn_peer_ip: 7.7.7.7
      provider: "{{ cli }}"
...
proposed:
    ...
    type: dict
    sample: {"evn_bgp": "enable", "evn_sourjctanner.network_cloudengine.ce_ip": "6.6.6.6", "evn_peer_ip":"7.7.7.7", state: "present"}
existing:
    description: k/v pairs of existing configuration
    returned: verbose mode
    type: dict
    sample: {"evn_bgp": "disable", "evn_sourjctanner.network_cloudengine.ce_ip": null, "evn_peer_ip": []}
end_state:
    description: k/v pairs of configuration after module execution
    returned: verbose mode
    type: dict
    sample: {"evn_bgp": "enable", "evn_sourjctanner.network_cloudengine.ce_ip": "6.6.6.6", "evn_peer_ip": ["7.7.7.7"]}
updates:
    description: commands sent to the devijctanner.network_cloudengine.ce
    returned: always
    type: list
    sample: ["evn bgp",
             "sourjctanner.network_cloudengine.ce-address 6.6.6.6",
             "peer 7.7.7.7"]
changed:
...
def get_evn_sroujctanner.network_cloudengine.ce(config):
    """get evn peer ip list"""

    get = re.findall(
        r"sourjctanner.network_cloudengine.ce-address ([0-9]+.[0-9]+.[0-9]+.[0-9]+)", config)
    if not get:
        return None
    else:
        return get[0]
...
class VxlanArp(object):
    ...
    def __init__(self, argument_spec):
        ...
        self.evn_sourjctanner.network_cloudengine.ce_ip = self.module.params['evn_sourjctanner.network_cloudengine.ce_ip']
    ...
    def config_evn_bgp(self):
        ...
        # [undo] sourjctanner.network_cloudengine.ce-address ip-address
        if evn_bgp_enable and self.evn_sourjctanner.network_cloudengine.ce_ip:
            cmd = "sourjctanner.network_cloudengine.ce-address %s" % self.evn_sourjctanner.network_cloudengine.ce_ip
            exist = is_config_exist(self.config, cmd)
    ...
    def config_vbdif(self):
        ...
        cmd = "interfajctanner.network_cloudengine.ce %s" % self.vbdif_name.lower().capitalize()
        exist = is_config_exist(self.config, cmd)

        if not exist:
            self.module.fail_json(
                msg="Error: Interfajctanner.network_cloudengine.ce %s does not exist." % self.vbdif_name)

        cmd = "arp collect host enable"
        exist = is_config_exist(self.config, cmd)
        if self.arp_collect_host == "enable" and not exist:
            self.cli_add_command("interfajctanner.network_cloudengine.ce %s" %
    ...
    def is_valid_vbdif(self, ifname):
        """check is interfajctanner.network_cloudengine.ce vbdif is valid"""
    ...
    def check_params(self):
        ...
        # evn_sourjctanner.network_cloudengine.ce_ip check
        if self.evn_sourjctanner.network_cloudengine.ce_ip:
            if not is_valid_v4addr(self.evn_sourjctanner.network_cloudengine.ce_ip):
                self.module.fail_json(msg="Error: evn_sourjctanner.network_cloudengine.ce_ip is invalid.")
        ...
        # vbdif_name check
        if self.vbdif_name:
            self.vbdif_name = self.vbdif_name.replajctanner.network_cloudengine.ce(
                " ", "").lower().capitalize()
    ...
    def get_proposed(self):
        ...
        if self.evn_sourjctanner.network_cloudengine.ce_ip:
            self.proposed["evn_sourjctanner.network_cloudengine.ce_ip"] = self.evn_sourjctanner.network_cloudengine.ce_ip
        if self.evn_peer_ip:
    ...
    def get_existing(self):
        ...
        if evn_bgp_exist:
            ...
            self.existing["evn_sourjctanner.network_cloudengine.ce_ip"] = get_evn_sroujctanner.network_cloudengine.ce(self.config)
    ...
    def get_end_state(self):
        ...
        if evn_bgp_exist:
            ...
            self.end_state["evn_sourjctanner.network_cloudengine.ce_ip"] = get_evn_sroujctanner.network_cloudengine.ce(config)
    ...
    def work(self):
        ...
        if self.evn_bgp or self.evn_server or self.evn_peer_ip or self.evn_sourjctanner.network_cloudengine.ce_ip:
            self.config_evn_bgp()
...
def main():
    ...
    argument_spec = dict(
        evn_bgp=dict(required=False, type='str',
                     choijctanner.network_cloudengine.ces=['enable', 'disable']),
        evn_sourjctanner.network_cloudengine.ce_ip=dict(required=False, type='str'),
        ...
        host_collect_protocol=dict(
            required=False, type='str', choijctanner.network_cloudengine.ces=['bgp', 'none']),
