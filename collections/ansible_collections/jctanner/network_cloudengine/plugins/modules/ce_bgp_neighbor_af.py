...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_bgp_neighbor_af
version_added: "2.4"
...
options:
    ...
    keep_all_routes:
        description:
            - If the value is true, the system stores all route update messages rejctanner.network_cloudengine.ceived from all peers (groups)
              after BGP connection setup.
              If the value is false, the system stores only BGP update messages that are rejctanner.network_cloudengine.ceived from peers
    ...
    nexthop_configure:
        description:
            - null, The next hop is not changed.
              ...
              invariable, Prevent the devijctanner.network_cloudengine.ce from changing the next hop of each imported IGP route
              when advertising it to its BGP peers.
        choijctanner.network_cloudengine.ces: ['null', 'local', 'invariable']
    preferred_value:
        description:
    ...
    public_as_only:
        ...
        default: no_use
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
    public_as_only_forjctanner.network_cloudengine.ce:
        description:
            - If the value is true, sent BGP update messages carry only the public AS number but do not carry
              private AS numbers.
              If the value is false, sent BGP update messages can carry private AS numbers.
    ...
    public_as_only_limited:
        ...
        default: no_use
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
    public_as_only_replajctanner.network_cloudengine.ce:
        description:
            - Private as replajctanner.network_cloudengine.ced by public as number.
        default: no_use
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
    ...
    route_limit:
        description:
            - Configure the maximum number of routes that can be acjctanner.network_cloudengine.cepted from a peer.
              The value is an integer ranging from 1 to 4294967295.
    route_limit_perjctanner.network_cloudengine.cent:
        description:
            - Specify the perjctanner.network_cloudengine.centage of routes when a router starts to generate an alarm.
              The value is an integer ranging from 1 to 100.
    route_limit_type:
        description:
            - Noparameter, After the number of rejctanner.network_cloudengine.ceived routes exjctanner.network_cloudengine.ceeds the threshold and the timeout
              timer expires,no action.
              AlertOnly, An alarm is generated and no additional routes will be acjctanner.network_cloudengine.cepted if the maximum
              number of routes allowed have been rejctanner.network_cloudengine.ceived.
              IdleForever, The connection that is interrupted is not automatically re-established if the
              maximum number of routes allowed have been rejctanner.network_cloudengine.ceived.
              IdleTimeout, After the number of rejctanner.network_cloudengine.ceived routes exjctanner.network_cloudengine.ceeds the threshold and the timeout timer
              expires, the connection that is interrupted is automatically re-established.
        choijctanner.network_cloudengine.ces: ['noparameter', 'alertOnly', 'idleForever', 'idleTimeout']
    route_limit_idle_timeout:
        description:
            - Specify the value of the idle-timeout timer to automatically reestablish the connections after
              they are cut off when the number of routes exjctanner.network_cloudengine.ceeds the set threshold.
    ...
    reflect_client:
        description:
            - If the value is true, the local devijctanner.network_cloudengine.ce functions as the route reflector and a peer functions
    ...
    substitute_as_enable:
        description:
            - If the value is true, the function to replajctanner.network_cloudengine.ce a specified peer's AS number in the AS-Path attribute with
              the local AS number is enabled.
              If the value is false, the function to replajctanner.network_cloudengine.ce a specified peer's AS number in the AS-Path attribute with
    ...
    import_pref_filt_name:
        description:
            - Specify the IPv4 filtering policy applied to the routes rejctanner.network_cloudengine.ceived from a specified peer.
    ...
    import_as_path_filter:
        description:
            - Apply an AS_Path-based filtering policy to the routes rejctanner.network_cloudengine.ceived from a specified peer.
    ...
    import_as_path_name_or_num:
        description:
            - A routing strategy based on the AS path list for routing rejctanner.network_cloudengine.ceived by a designated peer.
    ...
    import_acl_name_or_num:
        description:
            - Apply an IPv4 ACL-based filtering policy to the routes rejctanner.network_cloudengine.ceived from a specified peer.
    ...
    orf_mode:
        description:
            - ORF mode.
              null, Default value.
              rejctanner.network_cloudengine.ceive, ORF for incoming packets.
              send, ORF for outgoing packets.
              both, ORF for incoming and outgoing packets.
        choijctanner.network_cloudengine.ces: ['null', 'rejctanner.network_cloudengine.ceive', 'send', 'both']
    soostring:
        description:
    ...
    default_rt_match_mode:
        ...
        choijctanner.network_cloudengine.ces: ['null', 'matchall', 'matchany']
    add_path_mode:
        description:
            - null, Null.
              rejctanner.network_cloudengine.ceive, Support rejctanner.network_cloudengine.ceiving Add-Path routes.
              send, Support sending Add-Path routes.
              both, Support rejctanner.network_cloudengine.ceiving and sending Add-Path routes.
        choijctanner.network_cloudengine.ces: ['null', 'rejctanner.network_cloudengine.ceive', 'send', 'both']
    adv_add_path_num:
        description:
    ...
    origin_as_valid:
        description:
            - If the value is true, Application results of route announjctanner.network_cloudengine.cement.
    ...
    update_pkt_standard_compatible:
        description:
            - If the value is true, When the vpnv4 multicast neighbor rejctanner.network_cloudengine.ceives and updates the message,
              the message has no label.
              If the value is false, When the vpnv4 multicast neighbor rejctanner.network_cloudengine.ceives and updates the message,
...
- name: CloudEngine BGP neighbor address family test
  ...
  - name: "Config BGP peer Address_Family"
    jctanner.network_cloudengine.ce_bgp_neighbor_af:
      state: present
      vrf_name: js
      af_type: ipv4uni
...
class BgpNeighborAf(object):
    ...
    def check_bgp_neighbor_af_other(self, **kwargs):
        ...
        public_as_only_forjctanner.network_cloudengine.ce = module.params['public_as_only_forjctanner.network_cloudengine.ce']
        if public_as_only_forjctanner.network_cloudengine.ce != 'no_use':

            conf_str = CE_GET_BGP_PEER_AF_HEADER % (
                vrf_name, af_type) + "<publicAsOnlyForjctanner.network_cloudengine.ce></publicAsOnlyForjctanner.network_cloudengine.ce>" + CE_GET_BGP_PEER_AF_TAIL
            recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
            ...
            if "<data/>" in recv_xml:
                ...
                re_find = re.findall(
                    r'.*<publicAsOnlyForjctanner.network_cloudengine.ce>(.*)</publicAsOnlyForjctanner.network_cloudengine.ce>.*', recv_xml)

                if re_find:
                    result["public_as_only_forjctanner.network_cloudengine.ce"] = re_find
                    result["vrf_name"] = vrf_name
                    result["af_type"] = af_type
                    if re_find[0] != public_as_only_forjctanner.network_cloudengine.ce:
                        need_cfg = True
                else:
                    need_cfg = True
        ...
        public_as_only_replajctanner.network_cloudengine.ce = module.params['public_as_only_replajctanner.network_cloudengine.ce']
        if public_as_only_replajctanner.network_cloudengine.ce != 'no_use':

            conf_str = CE_GET_BGP_PEER_AF_HEADER % (
                vrf_name, af_type) + "<publicAsOnlyReplajctanner.network_cloudengine.ce></publicAsOnlyReplajctanner.network_cloudengine.ce>" + CE_GET_BGP_PEER_AF_TAIL
            recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
            ...
            if "<data/>" in recv_xml:
                ...
                re_find = re.findall(
                    r'.*<publicAsOnlyReplajctanner.network_cloudengine.ce>(.*)</publicAsOnlyReplajctanner.network_cloudengine.ce>.*', recv_xml)

                if re_find:
                    result["public_as_only_replajctanner.network_cloudengine.ce"] = re_find
                    result["vrf_name"] = vrf_name
                    result["af_type"] = af_type
                    if re_find[0] != public_as_only_replajctanner.network_cloudengine.ce:
                        need_cfg = True
                else:
                    need_cfg = True
        ...
        route_limit_perjctanner.network_cloudengine.cent = module.params['route_limit_perjctanner.network_cloudengine.cent']
        if route_limit_perjctanner.network_cloudengine.cent:

            if int(route_limit_perjctanner.network_cloudengine.cent) < 1 or int(route_limit_perjctanner.network_cloudengine.cent) > 100:
                module.fail_json(
                    msg='Error: The value of route_limit_perjctanner.network_cloudengine.cent %s is out of [1 - 100].' % route_limit_perjctanner.network_cloudengine.cent)

            conf_str = CE_GET_BGP_PEER_AF_HEADER % (
                vrf_name, af_type) + "<routeLimitPerjctanner.network_cloudengine.cent></routeLimitPerjctanner.network_cloudengine.cent>" + CE_GET_BGP_PEER_AF_TAIL
            recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
            ...
            if "<data/>" in recv_xml:
                ...
                re_find = re.findall(
                    r'.*<routeLimitPerjctanner.network_cloudengine.cent>(.*)</routeLimitPerjctanner.network_cloudengine.cent>.*', recv_xml)

                if re_find:
                    result["route_limit_perjctanner.network_cloudengine.cent"] = re_find
                    result["vrf_name"] = vrf_name
                    result["af_type"] = af_type
                    if re_find[0] != route_limit_perjctanner.network_cloudengine.cent:
                        need_cfg = True
                else:
                    need_cfg = True
        ...
        if route_limit_idle_timeout:
            ...
            conf_str = CE_GET_BGP_PEER_AF_HEADER % (
                vrf_name, af_type) + "<routeLimitIdleTimeout></routeLimitPerjctanner.network_cloudengine.cent>" + CE_GET_BGP_PEER_AF_TAIL
            recv_xml = self.netconf_get_config(module=module, conf_str=conf_str)
    ...
    def merge_bgp_peer_af_other(self, **kwargs):
        ...
        public_as_only_forjctanner.network_cloudengine.ce = module.params['public_as_only_forjctanner.network_cloudengine.ce']
        if public_as_only_forjctanner.network_cloudengine.ce != 'no_use':
            conf_str += "<publicAsOnlyForjctanner.network_cloudengine.ce>%s</publicAsOnlyForjctanner.network_cloudengine.ce>" % public_as_only_forjctanner.network_cloudengine.ce

            if public_as_only_forjctanner.network_cloudengine.ce == "true":
                cmd = "peer %s public-as-only forjctanner.network_cloudengine.ce" % remote_address
            else:
                cmd = "undo peer %s public-as-only forjctanner.network_cloudengine.ce" % remote_address
            cmds.append(cmd)
        ...
        public_as_only_replajctanner.network_cloudengine.ce = module.params['public_as_only_replajctanner.network_cloudengine.ce']
        if public_as_only_replajctanner.network_cloudengine.ce != 'no_use':
            conf_str += "<publicAsOnlyReplajctanner.network_cloudengine.ce>%s</publicAsOnlyReplajctanner.network_cloudengine.ce>" % public_as_only_replajctanner.network_cloudengine.ce

            if public_as_only_replajctanner.network_cloudengine.ce == "true":
                cmd = "peer %s public-as-only forjctanner.network_cloudengine.ce replajctanner.network_cloudengine.ce" % remote_address
            else:
                cmd = "undo peer %s public-as-only forjctanner.network_cloudengine.ce replajctanner.network_cloudengine.ce" % remote_address
            cmds.append(cmd)
        ...
        if public_as_only_skip_peer_as != 'no_use':
            ...
            if public_as_only_skip_peer_as == "true":
                cmd = "peer %s public-as-only forjctanner.network_cloudengine.ce include-peer-as" % remote_address
            else:
                cmd = "undo peer %s public-as-only forjctanner.network_cloudengine.ce include-peer-as" % remote_address
            cmds.append(cmd)
        ...
        route_limit_perjctanner.network_cloudengine.cent = module.params['route_limit_perjctanner.network_cloudengine.cent']
        if route_limit_perjctanner.network_cloudengine.cent:
            conf_str += "<routeLimitPerjctanner.network_cloudengine.cent>%s</routeLimitPerjctanner.network_cloudengine.cent>" % route_limit_perjctanner.network_cloudengine.cent

            cmd = "peer %s route-limit %s %s" % (remote_address, route_limit, route_limit_perjctanner.network_cloudengine.cent)
            cmds.append(cmd)
        ...
        if route_limit_type:
            ...
            if route_limit_type == "alertOnly":
                cmd = "peer %s route-limit %s %s alert-only" % (remote_address, route_limit, route_limit_perjctanner.network_cloudengine.cent)
                cmds.append(cmd)
            elif route_limit_type == "idleForever":
                cmd = "peer %s route-limit %s %s idle-forever" % (remote_address, route_limit, route_limit_perjctanner.network_cloudengine.cent)
                cmds.append(cmd)
            elif route_limit_type == "idleTimeout":
                cmd = "peer %s route-limit %s %s idle-timeout" % (remote_address, route_limit, route_limit_perjctanner.network_cloudengine.cent)
                cmds.append(cmd)
        ...
        if route_limit_idle_timeout:
            ...
            cmd = "peer %s route-limit %s %s idle-timeout %s" % (remote_address, route_limit,
                                                                 route_limit_perjctanner.network_cloudengine.cent, route_limit_idle_timeout)
            cmds.append(cmd)
...
def main():
    ...
    argument_spec = dict(
        ...
        advertise_irb=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        advertise_arp=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        advertise_remote_nexthop=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        advertise_community=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        advertise_ext_community=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        discard_ext_community=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        allow_as_loop_enable=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        allow_as_loop_limit=dict(type='str'),
        keep_all_routes=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        nexthop_configure=dict(choijctanner.network_cloudengine.ces=['null', 'local', 'invariable']),
        preferred_value=dict(type='str'),
        public_as_only=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        public_as_only_forjctanner.network_cloudengine.ce=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        public_as_only_limited=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        public_as_only_replajctanner.network_cloudengine.ce=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        public_as_only_skip_peer_as=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        route_limit=dict(type='str'),
        route_limit_perjctanner.network_cloudengine.cent=dict(type='str'),
        route_limit_type=dict(
            choijctanner.network_cloudengine.ces=['noparameter', 'alertOnly', 'idleForever', 'idleTimeout']),
        route_limit_idle_timeout=dict(type='str'),
        rt_updt_interval=dict(type='str'),
        redirect_ip=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        redirect_ip_vaildation=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        reflect_client=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        substitute_as_enable=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        ...
        ipprefix_orf_enable=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        is_nonstd_ipprefix_mod=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        orftype=dict(type='str'),
        orf_mode=dict(choijctanner.network_cloudengine.ces=['null', 'rejctanner.network_cloudengine.ceive', 'send', 'both']),
        soostring=dict(type='str'),
        default_rt_adv_enable=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        default_rt_adv_policy=dict(type='str'),
        default_rt_match_mode=dict(choijctanner.network_cloudengine.ces=['null', 'matchall', 'matchany']),
        add_path_mode=dict(choijctanner.network_cloudengine.ces=['null', 'rejctanner.network_cloudengine.ceive', 'send', 'both']),
        adv_add_path_num=dict(type='str'),
        origin_as_valid=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        vpls_enable=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        vpls_ad_disable=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        update_pkt_standard_compatible=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']))
    ...
    jctanner.network_cloudengine.ce_bgp_peer_af_obj = BgpNeighborAf()
    ...
    if public_as_only_forjctanner.network_cloudengine.ce != 'no_use':
        proposed["public_as_only_forjctanner.network_cloudengine.ce"] = public_as_only_forjctanner.network_cloudengine.ce
    if public_as_only_limited != 'no_use':
        proposed["public_as_only_limited"] = public_as_only_limited
    if public_as_only_replajctanner.network_cloudengine.ce != 'no_use':
        proposed["public_as_only_replajctanner.network_cloudengine.ce"] = public_as_only_replajctanner.network_cloudengine.ce
    if public_as_only_skip_peer_as != 'no_use':
        proposed["public_as_only_skip_peer_as"] = public_as_only_skip_peer_as
    if route_limit:
        proposed["route_limit"] = route_limit
    if route_limit_perjctanner.network_cloudengine.cent:
        proposed["route_limit_perjctanner.network_cloudengine.cent"] = route_limit_perjctanner.network_cloudengine.cent
    if route_limit_type:
    ...
    if not jctanner.network_cloudengine.ce_bgp_peer_af_obj:
        module.fail_json(msg='Error: Init module failed.')

    bgp_peer_af_rst = jctanner.network_cloudengine.ce_bgp_peer_af_obj.check_bgp_neighbor_af_args(
        module=module)
    bgp_peer_af_other_rst = jctanner.network_cloudengine.ce_bgp_peer_af_obj.check_bgp_neighbor_af_other(
        module=module)
    ...
    if state == "present":
        if bgp_peer_af_rst["need_cfg"]:
            if "remote_address" in bgp_peer_af_rst.keys():
                cmd = jctanner.network_cloudengine.ce_bgp_peer_af_obj.merge_bgp_peer_af(module=module)
                changed = True
                for item in cmd:
                    updates.append(item)
            else:
                cmd = jctanner.network_cloudengine.ce_bgp_peer_af_obj.create_bgp_peer_af(module=module)
                changed = True
                for item in cmd:
                    updates.append(item)

        if bgp_peer_af_other_rst["need_cfg"]:
            cmd = jctanner.network_cloudengine.ce_bgp_peer_af_obj.merge_bgp_peer_af_other(module=module)
            changed = True
            for item in cmd:
                updates.append(item)
        ...
        if bgp_peer_af_rst["need_cfg"]:
            cmd = jctanner.network_cloudengine.ce_bgp_peer_af_obj.delete_bgp_peer_af(module=module)
            changed = True
            for item in cmd:
                updates.append(item)
