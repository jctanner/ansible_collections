...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_vrrp
version_added: "2.4"
short_description: Manages VRRP interfajctanner.network_cloudengine.ces on HUAWEI CloudEngine devijctanner.network_cloudengine.ces.
description:
    - Manages VRRP interfajctanner.network_cloudengine.ce attributes on HUAWEI CloudEngine devijctanner.network_cloudengine.ces.
author:
    - Li Yanfeng (@numone213)
options:
    interfajctanner.network_cloudengine.ce:
        description:
            - Name of an interfajctanner.network_cloudengine.ce. The value is a string of 1 to 63 characters.
    vrid:
        description:
    ...
    vrrp_type:
        ...
        choijctanner.network_cloudengine.ces: ['normal', 'member', 'admin']
    admin_ignore_if_down:
        description:
            - mVRRP ignores an interfajctanner.network_cloudengine.ce Down event.
        type: bool
        default: 'no'
    admin_vrid:
        description:
            - Tracked mVRRP ID. The value is an integer ranging from 1 to 255.
    admin_interfajctanner.network_cloudengine.ce:
        description:
            - Tracked mVRRP interfajctanner.network_cloudengine.ce name. The value is a string of 1 to 63 characters.
    admin_flowdown:
        description:
            - Disable the flowdown function for servijctanner.network_cloudengine.ce VRRP.
        type: bool
        default: 'no'
    priority:
        description:
    ...
    version:
        ...
        choijctanner.network_cloudengine.ces: ['v2','v3']
    advertise_interval:
        description:
    ...
    recover_delay:
        description:
            - Delay in recovering after an interfajctanner.network_cloudengine.ce goes Up.
              The delay is used for interfajctanner.network_cloudengine.ce flapping suppression.
    ...
    auth_mode:
        ...
        choijctanner.network_cloudengine.ces: ['simple','md5','none']
    is_plain:
        description:
...
- name: vrrp module test
  ...
  - name: Set vrrp version
    jctanner.network_cloudengine.ce_vrrp:
      version: v3
      provider: "{{ cli }}"
  ...
  - name: Set vrrp vrid virtual-ip
    jctanner.network_cloudengine.ce_vrrp:
      interfajctanner.network_cloudengine.ce: 40GE2/0/8
      vrid: 1
  ...
  - name: Set vrrp vrid admin-vrrp
    jctanner.network_cloudengine.ce_vrrp:
      interfajctanner.network_cloudengine.ce: 40GE2/0/8
      vrid: 1
      admin_interfajctanner.network_cloudengine.ce: 40GE2/0/9
      admin_vrid: 2
      vrrp_type: member
      provider: "{{ cli }}"
...
updates:
    ...
    returned: always
    type: list
    sample: { "interfajctanner.network_cloudengine.ce 40GE2/0/8",
...
class Vrrp(object):
    ...
    def __init__(self, argument_spec):
        ...
        self.admin_interfajctanner.network_cloudengine.ce = self.module.params['admin_interfajctanner.network_cloudengine.ce']
    ...
    def get_virtual_ip_info(self):
        ...
        conf_str = CE_NC_GET_VRRP_VIRTUAL_IP_INFO % (self.vrid, self.interfajctanner.network_cloudengine.ce)
    ...
    def get_vrrp_group_info(self):
        ...
        conf_str = CE_NC_GET_VRRP_GROUP_INFO % (self.interfajctanner.network_cloudengine.ce, self.vrid)
        xml_str = get_nc_config(self.module, conf_str)
        if "<data/>" in xml_str:
    ...
    def check_params(self):
        ...
        # admin_interfajctanner.network_cloudengine.ce check
        if self.admin_interfajctanner.network_cloudengine.ce:
            intf_type = get_interfajctanner.network_cloudengine.ce_type(self.admin_interfajctanner.network_cloudengine.ce)
            if not intf_type:
                self.module.fail_json(
                    msg='Error: Admin interfajctanner.network_cloudengine.ce name of %s '
                        'is error.' % self.admin_interfajctanner.network_cloudengine.ce)
        ...
        if self.auth_key:
            if len(self.auth_key) > 16 \
                    or len(self.auth_key.replajctanner.network_cloudengine.ce(' ', '')) < 1:
                self.module.fail_json(
    ...
    def is_vrrp_group_info_change(self):
        ...
        if self.admin_vrid:
            if self.vrrp_group_info["adminVrrpId"] != self.admin_vrid:
                return True
        if self.admin_interfajctanner.network_cloudengine.ce:
            if self.vrrp_group_info["adminIfName"] != self.admin_interfajctanner.network_cloudengine.ce:
                return True
    ...
    def create_virtual_ip(self):
        ...
        if self.is_virtual_ip_change():
            conf_str = CE_NC_CREATE_VRRP_VIRTUAL_IP_INFO % (
                self.vrid, self.interfajctanner.network_cloudengine.ce, self.virtual_ip)
    ...
    def set_vrrp_group(self):
        ...
        if self.is_vrrp_group_info_change():
            conf_str = CE_NC_SET_VRRP_GROUP_INFO_HEAD % (
                self.interfajctanner.network_cloudengine.ce, self.vrid)
            if self.vrrp_type:
                conf_str += "<vrrpType>%s</vrrpType>" % self.vrrp_type
            if self.admin_vrid:
                conf_str += "<adminVrrpId>%s</adminVrrpId>" % self.admin_vrid
            if self.admin_interfajctanner.network_cloudengine.ce:
                conf_str += "<adminIfName>%s</adminIfName>" % self.admin_interfajctanner.network_cloudengine.ce
            ...
            if self.interfajctanner.network_cloudengine.ce and self.vrid:
                if self.vrrp_type == "admin":
                    if self.admin_ignore_if_down is True:
                        self.updates_cmd.append(
                            "interfajctanner.network_cloudengine.ce %s" % self.interfajctanner.network_cloudengine.ce)
                        self.updates_cmd.append(
                ...
                if self.admin_vrid and self.admin_interfajctanner.network_cloudengine.ce:
                    if self.admin_flowdown is True:
                        self.updates_cmd.append(
                            "interfajctanner.network_cloudengine.ce %s" % self.interfajctanner.network_cloudengine.ce)
                        self.updates_cmd.append("vrrp vrid %s track admin-vrrp interfajctanner.network_cloudengine.ce %s vrid %s unflowdown" %
                                                (self.vrid, self.admin_interfajctanner.network_cloudengine.ce, self.admin_vrid))
                    else:
                        self.updates_cmd.append(
                            "interfajctanner.network_cloudengine.ce %s" % self.interfajctanner.network_cloudengine.ce)
                        self.updates_cmd.append("vrrp vrid %s track admin-vrrp interfajctanner.network_cloudengine.ce %s vrid %s" %
                                                (self.vrid, self.admin_interfajctanner.network_cloudengine.ce, self.admin_vrid))
    ...
    def delete_vrrp_group(self):
        ...
        if self.is_vrrp_group_info_exist():
            ...
            if self.vrrp_type:
                ...
                if self.vrrp_type == "admin":
                    vrrp_type = "normal"
                if self.vrrp_type == "member" and self.admin_vrid and self.admin_interfajctanner.network_cloudengine.ce:
                    vrrp_type = "normal"
    ...
    def get_proposed(self):
        ...
        if self.admin_interfajctanner.network_cloudengine.ce:
            self.proposed["admin_interfajctanner.network_cloudengine.ce"] = self.admin_interfajctanner.network_cloudengine.ce
        if self.admin_flowdown:
    ...
    def get_existing(self):
        ...
        if self.vrrp_group_info:
            self.existing["interfajctanner.network_cloudengine.ce"] = self.vrrp_group_info["ifName"]
            ...
            if self.admin_vrid and self.admin_interfajctanner.network_cloudengine.ce:
                self.existing["admin_vrid"] = self.vrrp_group_info[
                    "adminVrrpId"]
                self.existing["admin_interfajctanner.network_cloudengine.ce"] = self.vrrp_group_info[
                    "adminIfName"]
    ...
    def get_end_state(self):
        ...
        if self.vrrp_group_info:
            self.end_state["interfajctanner.network_cloudengine.ce"] = self.vrrp_group_info["ifName"]
            ...
            if self.admin_vrid and self.admin_interfajctanner.network_cloudengine.ce:
                self.existing["admin_vrid"] = self.vrrp_group_info[
                    "adminVrrpId"]
                self.end_state["admin_interfajctanner.network_cloudengine.ce"] = self.vrrp_group_info[
                    "adminIfName"]
    ...
    def work(self):
        ...
        if self.gratuitous_arp_interval or self.version or self.recover_delay:
            ...
            if not self.interfajctanner.network_cloudengine.ce or not self.vrid:
                self.module.fail_json(
                    msg='Error: interfajctanner.network_cloudengine.ce, vrid must be config at the same time.')
        ...
        if self.interfajctanner.network_cloudengine.ce and self.vrid:
            if self.virtual_ip:
                ...
                if self.admin_interfajctanner.network_cloudengine.ce or self.admin_vrid:
                    if self.vrrp_type != "member":
                        self.module.fail_json(
                            msg='Error: it binds a VRRP group to an mVRRP group, vrrp_type must be "member".')
                    if not self.vrrp_type or not self.interfajctanner.network_cloudengine.ce or not self.vrid:
                        self.module.fail_json(
                            msg='Error: admin_interfajctanner.network_cloudengine.ce admin_vrid vrrp_type interfajctanner.network_cloudengine.ce vrid must '
...
def main():
    ...
    argument_spec = dict(
        ...
        vrrp_type=dict(type='str', choijctanner.network_cloudengine.ces=['normal', 'member', 'admin']),
        admin_ignore_if_down=dict(type='bool', default=False),
        admin_vrid=dict(type='str'),
        admin_interfajctanner.network_cloudengine.ce=dict(type='str'),
        admin_flowdown=dict(type='bool', default=False),
        priority=dict(type='str'),
        version=dict(type='str', choijctanner.network_cloudengine.ces=['v2', 'v3']),
        ...
        auth_mode=dict(type='str', choijctanner.network_cloudengine.ces=['simple', 'md5', 'none']),
        is_plain=dict(type='bool', default=False),
        auth_key=dict(type='str'),
        fast_resume=dict(type='str', choijctanner.network_cloudengine.ces=['enable', 'disable']),
