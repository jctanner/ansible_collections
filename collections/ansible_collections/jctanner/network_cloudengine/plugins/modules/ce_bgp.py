...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_bgp
version_added: "2.4"
...
options:
    ...
    grajctanner.network_cloudengine.ceful_restart:
        description:
    ...
    confed_nonstanded:
        description:
            - Configure the devijctanner.network_cloudengine.ce to be compatible with devijctanner.network_cloudengine.ces in a nonstandard confederation.
        default: no_use
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
    bgp_rid_auto_sel:
        description:
            - The function to automatically select router IDs for all VPN BGP instanjctanner.network_cloudengine.ces is enabled.
        default: no_use
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
    keep_all_routes:
        description:
            - If the value is true, the system stores all route update messages rejctanner.network_cloudengine.ceived from all peers (groups) after
              BGP connection setup.
              If the value is false, the system stores only BGP update messages that are rejctanner.network_cloudengine.ceived from peers and pass
    ...
    ebgp_if_sensitive:
        description:
            - If the value is true, After the fast EBGP interfajctanner.network_cloudengine.ce awareness function is enabled, EBGP sessions on
              an interfajctanner.network_cloudengine.ce are deleted immediately when the interfajctanner.network_cloudengine.ce goes Down.
              If the value is  false, After the fast EBGP interfajctanner.network_cloudengine.ce awareness function is enabled, EBGP sessions
              on an interfajctanner.network_cloudengine.ce are not deleted immediately when the interfajctanner.network_cloudengine.ce goes Down.
        default: no_use
        choijctanner.network_cloudengine.ces: ['no_use','true','false']
    default_af_type:
        description:
        ...
        choijctanner.network_cloudengine.ces: ['ipv4uni','ipv6uni']
'''
...
- name: CloudEngine BGP test
  ...
  - name: "Enable BGP"
    jctanner.network_cloudengine.ce_bgp:
      state: present
      as_number: 100
...
SUCCESS = """sucjctanner.network_cloudengine.cess"""
FAILED = """failed"""
...
# get bgp instanjctanner.network_cloudengine.ce
CE_GET_BGP_INSTANCE = """
    <filter type="subtree">
      <bgp xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0">
        <bgpcomm>
...
# merge bgp instanjctanner.network_cloudengine.ce
...
# create bgp instanjctanner.network_cloudengine.ce
...
# delete bgp instanjctanner.network_cloudengine.ce
...
class Bgp(object):
    ...
    def check_bgp_enable_other_args(self, **kwargs):
        ...
        grajctanner.network_cloudengine.ceful_restart = module.params['grajctanner.network_cloudengine.ceful_restart']
        if grajctanner.network_cloudengine.ceful_restart != 'no_use':

            conf_str = CE_GET_BGP_ENABLE_HEADER + \
                "<grajctanner.network_cloudengine.cefulRestart></grajctanner.network_cloudengine.cefulRestart>" + CE_GET_BGP_ENABLE_TAIL
            ...
            if "<data/>" in recv_xml:
                ...
                re_find = re.findall(
                    r'.*<grajctanner.network_cloudengine.cefulRestart>(.*)</grajctanner.network_cloudengine.cefulRestart>.*', recv_xml)

                if re_find:
                    result["grajctanner.network_cloudengine.ceful_restart"] = re_find
                    if re_find[0] != grajctanner.network_cloudengine.ceful_restart:
                        need_cfg = True
                else:
                    need_cfg = True
    ...
    def check_bgp_instanjctanner.network_cloudengine.ce_args(self, **kwargs):
        """ check_bgp_instanjctanner.network_cloudengine.ce_args """
    ...
    def check_bgp_instanjctanner.network_cloudengine.ce_other_args(self, **kwargs):
        """ check_bgp_instanjctanner.network_cloudengine.ce_other_args """
    ...
    def merge_bgp_enable_other(self, **kwargs):
        ...
        grajctanner.network_cloudengine.ceful_restart = module.params['grajctanner.network_cloudengine.ceful_restart']
        if grajctanner.network_cloudengine.ceful_restart != 'no_use':
            conf_str += "<grajctanner.network_cloudengine.cefulRestart>%s</grajctanner.network_cloudengine.cefulRestart>" % grajctanner.network_cloudengine.ceful_restart

            if grajctanner.network_cloudengine.ceful_restart == "true":
                cmd = "grajctanner.network_cloudengine.ceful-restart"
            else:
                cmd = "undo grajctanner.network_cloudengine.ceful-restart"
            cmds.append(cmd)
        ...
        if time_wait_for_rib:
            ...
            cmd = "grajctanner.network_cloudengine.ceful-restart timer wait-for-rib %s" % time_wait_for_rib
            cmds.append(cmd)
        ...
        if bgp_rid_auto_sel != 'no_use':
            ...
            if bgp_rid_auto_sel == "true":
                cmd = "router-id vpn-instanjctanner.network_cloudengine.ce auto-select"
            else:
                cmd = "undo router-id"
            cmds.append(cmd)
        ...
        if gr_peer_reset != 'no_use':
            ...
            if gr_peer_reset == "true":
                cmd = "grajctanner.network_cloudengine.ceful-restart peer-reset"
            else:
                cmd = "undo grajctanner.network_cloudengine.ceful-restart peer-reset"
            cmds.append(cmd)
    ...
    def delete_bgp_enable_other(self, **kwargs):
        ...
        if time_wait_for_rib:
            ...
            cmd = "undo grajctanner.network_cloudengine.ceful-restart timer wait-for-rib"
            cmds.append(cmd)
    ...
    def get_bgp_instanjctanner.network_cloudengine.ce(self, **kwargs):
        """ get_bgp_instanjctanner.network_cloudengine.ce """
    ...
    def merge_bgp_instanjctanner.network_cloudengine.ce(self, **kwargs):
        """ merge_bgp_instanjctanner.network_cloudengine.ce """
        ...
        if "<ok/>" not in recv_xml:
            module.fail_json(msg='Error: Merge bgp instanjctanner.network_cloudengine.ce failed.')

    def create_bgp_instanjctanner.network_cloudengine.ce(self, **kwargs):
        """ create_bgp_instanjctanner.network_cloudengine.ce """
        ...
        if "<ok/>" not in recv_xml:
            module.fail_json(msg='Error: Create bgp instanjctanner.network_cloudengine.ce failed.')
    ...
    def delete_bgp_instanjctanner.network_cloudengine.ce(self, **kwargs):
        """ delete_bgp_instanjctanner.network_cloudengine.ce """
        ...
        if "<ok/>" not in recv_xml:
            module.fail_json(msg='Error: Delete bgp instanjctanner.network_cloudengine.ce failed.')
    ...
    def merge_bgp_instanjctanner.network_cloudengine.ce_other(self, **kwargs):
        """ merge_bgp_instanjctanner.network_cloudengine.ce_other """
        ...
        if vrf_rid_auto_sel != 'no_use':
            ...
            if vrf_rid_auto_sel == "true":
                ...
                cmd = "undo router-id vpn-instanjctanner.network_cloudengine.ce auto-select"
            cmds.append(cmd)
        ...
        if "<ok/>" not in recv_xml:
            module.fail_json(msg='Error: Merge bgp instanjctanner.network_cloudengine.ce other failed.')

        return cmds

    def delete_bgp_instanjctanner.network_cloudengine.ce_other_comm(self, **kwargs):
        """ delete_bgp_instanjctanner.network_cloudengine.ce_other_comm """
        ...
        if "<ok/>" not in recv_xml:
            module.fail_json(
                msg='Error: Delete common vpn bgp instanjctanner.network_cloudengine.ce other args failed.')

        return cmds

    def delete_instanjctanner.network_cloudengine.ce_other_public(self, **kwargs):
        """ delete_instanjctanner.network_cloudengine.ce_other_public """
        ...
        if "<ok/>" not in recv_xml:
            module.fail_json(
                msg='Error: Delete default vpn bgp instanjctanner.network_cloudengine.ce other args failed.')
...
def main():
    ...
    argument_spec = dict(
        ...
        grajctanner.network_cloudengine.ceful_restart=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        time_wait_for_rib=dict(type='str'),
        as_path_limit=dict(type='str'),
        check_first_as=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        confed_id_number=dict(type='str'),
        confed_nonstanded=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        bgp_rid_auto_sel=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        keep_all_routes=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        memory_limit=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        gr_peer_reset=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        is_shutdown=dict(type='str', default='no_use', choijctanner.network_cloudengine.ces=['no_use', 'true', 'false']),
        ...
        default_af_type=dict(type='str', choijctanner.network_cloudengine.ces=['ipv4uni', 'ipv6uni'])
    )
    ...
    jctanner.network_cloudengine.ce_bgp_obj = Bgp()

    if not jctanner.network_cloudengine.ce_bgp_obj:
    ...
    if grajctanner.network_cloudengine.ceful_restart != 'no_use':
        proposed["grajctanner.network_cloudengine.ceful_restart"] = grajctanner.network_cloudengine.ceful_restart
    if time_wait_for_rib:
    ...
    need_bgp_enable_other_rst = jctanner.network_cloudengine.ce_bgp_obj.check_bgp_enable_other_args(
        module=module)
    need_bgp_confed = check_bgp_confed_args(module=module)
    need_bgp_instanjctanner.network_cloudengine.ce = jctanner.network_cloudengine.ce_bgp_obj.check_bgp_instanjctanner.network_cloudengine.ce_args(module=module)
    need_bgp_instanjctanner.network_cloudengine.ce_other_rst = jctanner.network_cloudengine.ce_bgp_obj.check_bgp_instanjctanner.network_cloudengine.ce_other_args(
        module=module)

    # bgp enable/disable
    if need_bgp_enable:

        bgp_enable_exist = jctanner.network_cloudengine.ce_bgp_obj.get_bgp_enable(module=module)
        ...
        if state == "present":
            ...
            if bgp_enable_new in bgp_enable_exist:
                ...
                cmd = jctanner.network_cloudengine.ce_bgp_obj.merge_bgp_enable(module=module)
                changed = True
                for item in cmd:
                    updates.append(item)

        else:
            if need_bgp_enable_other_rst["need_cfg"] or need_bgp_confed or need_bgp_instanjctanner.network_cloudengine.ce_other_rst["need_cfg"]:
                pass
        ...
        bgp_enable_end = jctanner.network_cloudengine.ce_bgp_obj.get_bgp_enable(module=module)
    ...
    if need_bgp_enable_other_rst["need_cfg"]:
        if state == "present":
            cmd = jctanner.network_cloudengine.ce_bgp_obj.merge_bgp_enable_other(module=module)
            changed = True
            for item in cmd:
                updates.append(item)
        else:
            cmd = jctanner.network_cloudengine.ce_bgp_obj.delete_bgp_enable_other(module=module)
            changed = True
            for item in cmd:
                updates.append(item)
    ...
    if need_bgp_confed:
        confed_exist = jctanner.network_cloudengine.ce_bgp_obj.get_bgp_confed_peer_as(module=module)
        existing["confederation peer as"] = confed_exist
        confed_new = (confed_peer_as_num)

        if state == "present":
            if len(confed_exist) == 0:
                cmd = jctanner.network_cloudengine.ce_bgp_obj.create_bgp_confed_peer_as(module=module)
                changed = True
                for item in cmd:
                    updates.append(item)

            elif confed_new not in confed_exist:
                cmd = jctanner.network_cloudengine.ce_bgp_obj.merge_bgp_confed_peer_as(module=module)
                changed = True
                for item in cmd:
                    updates.append(item)
            ...
            if len(confed_exist) == 0:
                ...
                cmd = jctanner.network_cloudengine.ce_bgp_obj.delete_bgp_confed_peer_as(module=module)
                changed = True
                for item in cmd:
                    updates.append(item)

        confed_end = jctanner.network_cloudengine.ce_bgp_obj.get_bgp_confed_peer_as(module=module)
        end_state["confederation peer as"] = confed_end

    # bgp instanjctanner.network_cloudengine.ce
    router_id_exist = jctanner.network_cloudengine.ce_bgp_obj.get_bgp_instanjctanner.network_cloudengine.ce(module=module)
    existing["bgp instanjctanner.network_cloudengine.ce"] = router_id_exist
    if need_bgp_instanjctanner.network_cloudengine.ce:
        router_id_new = (vrf_name)

        if state == "present":
            if len(router_id_exist) == 0:
                cmd = jctanner.network_cloudengine.ce_bgp_obj.create_bgp_instanjctanner.network_cloudengine.ce(module=module)
                changed = True
                updates.append(cmd)
            elif router_id_new not in router_id_exist:
                jctanner.network_cloudengine.ce_bgp_obj.merge_bgp_instanjctanner.network_cloudengine.ce(module=module)
                changed = True

        else:
            if not need_bgp_instanjctanner.network_cloudengine.ce_other_rst["need_cfg"]:
                if vrf_name != "_public_":
                    if len(router_id_exist) == 0:
                        pass
                    elif router_id_new not in router_id_exist:
                        ...
                        cmd = jctanner.network_cloudengine.ce_bgp_obj.delete_bgp_instanjctanner.network_cloudengine.ce(module=module)
                        changed = True
                        for item in cmd:
                            updates.append(item)

    router_id_end = jctanner.network_cloudengine.ce_bgp_obj.get_bgp_instanjctanner.network_cloudengine.ce(module=module)
    end_state["bgp instanjctanner.network_cloudengine.ce"] = router_id_end

    # bgp instanjctanner.network_cloudengine.ce other
    exist_tmp = dict()
    for item in need_bgp_instanjctanner.network_cloudengine.ce_other_rst:
        if item != "need_cfg":
            exist_tmp[item] = need_bgp_instanjctanner.network_cloudengine.ce_other_rst[item]

    if exist_tmp:
        existing["bgp instanjctanner.network_cloudengine.ce other"] = exist_tmp

    if need_bgp_instanjctanner.network_cloudengine.ce_other_rst["need_cfg"]:
        if state == "present":
            cmd = jctanner.network_cloudengine.ce_bgp_obj.merge_bgp_instanjctanner.network_cloudengine.ce_other(module=module)
            changed = True
            for item in cmd:
                updates.append(item)
            ...
            if vrf_name == "_public_":
                cmd = jctanner.network_cloudengine.ce_bgp_obj.delete_instanjctanner.network_cloudengine.ce_other_public(
                    module=module)
                changed = True
                for item in cmd:
                    updates.append(item)
            else:
                cmd = jctanner.network_cloudengine.ce_bgp_obj.delete_bgp_instanjctanner.network_cloudengine.ce_other_comm(module=module)
                changed = True
                for item in cmd:
                    updates.append(item)
    ...
    for item in need_bgp_instanjctanner.network_cloudengine.ce_other_rst:
        if item != "need_cfg":
            end_tmp[item] = need_bgp_instanjctanner.network_cloudengine.ce_other_rst[item]

    if end_tmp:
        end_state["bgp instanjctanner.network_cloudengine.ce other"] = end_tmp
