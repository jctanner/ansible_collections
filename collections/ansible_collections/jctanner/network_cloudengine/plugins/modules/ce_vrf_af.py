...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_vrf_af
version_added: "2.4"
short_description: Manages VPN instanjctanner.network_cloudengine.ce address family on HUAWEI CloudEngine switches.
description:
    - Manages VPN instanjctanner.network_cloudengine.ce address family of HUAWEI CloudEngine switches.
...
options:
    vrf:
        description:
            - VPN instanjctanner.network_cloudengine.ce.
        required: true
    vrf_aftype:
        description:
            - VPN instanjctanner.network_cloudengine.ce address family.
        choijctanner.network_cloudengine.ces: ['v4','v6']
        default: v4
    route_distinguisher:
        description:
            - VPN instanjctanner.network_cloudengine.ce route distinguisher,the RD used to distinguish same route prefix from different vpn.
    ...
    vpn_target_type:
        description:
            - VPN instanjctanner.network_cloudengine.ce vpn target type.
        choijctanner.network_cloudengine.ces: ['export_extcommunity', 'import_extcommunity']
    vpn_target_value:
        description:
            - VPN instanjctanner.network_cloudengine.ce target value. Such as X.X.X.X:number<0-65535> or number<0-65535>:number<0-4294967295>
...
- name: vrf af module test
  ...
  - name: Config vpna, set address family is ipv4
    jctanner.network_cloudengine.ce_vrf_af:
      vrf: vpna
      vrf_aftype: v4
      state: present
      provider: "{{ cli }}"
...
updates:
    ...
    type: list
    sample: [
        "ip vpn-instanjctanner.network_cloudengine.ce vpna",
...
def main():
    ...
    argument_spec = dict(
        ...
        vrf_aftype=dict(choijctanner.network_cloudengine.ces=['v4', 'v6'],
        ...
        vpn_target_type=dict(
            choijctanner.network_cloudengine.ces=['export_extcommunity', 'import_extcommunity'], required=False),
        vpn_target_value=dict(required=False, type='str'),
        vpn_target_state=dict(choijctanner.network_cloudengine.ces=['absent', 'present'], required=False),
    ...
    interfajctanner.network_cloudengine.ce = VrfAf(argument_spec)
