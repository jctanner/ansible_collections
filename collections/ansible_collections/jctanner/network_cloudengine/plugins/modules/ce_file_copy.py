...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_file_copy
version_added: "2.4"
short_description: Copy a file to a remote cloudengine devijctanner.network_cloudengine.ce over SCP on HUAWEI CloudEngine switches.
description:
    - Copy a file to a remote cloudengine devijctanner.network_cloudengine.ce over SCP on HUAWEI CloudEngine switches.
author:
    - Zhou Zhijin (@QijunPan)
notes:
    - The feature must be enabled with feature scp-server.
    - If the file is already present, no transfer will take plajctanner.network_cloudengine.ce.
requirements:
    - paramiko
options:
    local_file:
    ...
    file_system:
        description:
            - The remote file system of the devijctanner.network_cloudengine.ce. If omitted,
              devijctanner.network_cloudengine.ces that support a I(file_system) parameter will use
              ...
              3) C(chassis ID/slot number#flash) is root directory of the flash memory on
                 a devijctanner.network_cloudengine.ce in a stack. For example, C(1/5#flash) indicates the flash memory
...
- name: File copy test
  ...
  - name: "Copy a local file to remote devijctanner.network_cloudengine.ce"
    jctanner.network_cloudengine.ce_file_copy:
...
transfer_result:
    ...
    returned: always
    type: str
    sample: 'The local file has been sucjctanner.network_cloudengine.cessfully transferred to the devijctanner.network_cloudengine.ce.'
local_file:
...
from ansible_collections.jctanner.network_cloudengine.plugins.module_utils.network.cloudengine.jctanner.network_cloudengine.ce import jctanner.network_cloudengine.ce_argument_spec, run_commands, get_nc_config

try:
    import paramiko
    HAS_PARAMIKO = True
exjctanner.network_cloudengine.cept ImportError:
    HAS_PARAMIKO = False
...
def get_cli_exjctanner.network_cloudengine.ception(exc=None):
    """Get cli exjctanner.network_cloudengine.ception message"""
    ...
    if exc:
        ...
        for err in errs:
            ...
            if " at '^' position" in err:
                err = err.replajctanner.network_cloudengine.ce(" at '^' position", "")
            if err.replajctanner.network_cloudengine.ce(" ", "") == "^":
                continue
        ...
        msg = ["Error: Fail to get cli exjctanner.network_cloudengine.ception message."]
...
class FileCopy(object):
    ...
    def remote_file_exists(self, dst, file_system='flash:'):
        ...
        xml_str = ret_xml.replajctanner.network_cloudengine.ce('\r', '').replajctanner.network_cloudengine.ce('\n', '').\
        ...
        for eles in topo:
            if eles.tag in ["DirSize"]:
                return True, int(eles.text.replajctanner.network_cloudengine.ce(',', ''))
    ...
    def enough_spajctanner.network_cloudengine.ce(self):
        """Whether devijctanner.network_cloudengine.ce has enough spajctanner.network_cloudengine.ce"""
        ...
        kbytes_free = kbytes_free.replajctanner.network_cloudengine.ce(',', '')
    ...
    def work(self):
        ...
        if not remote_exists:
            ...
            file_exists = True
            self.transfer_result = 'The local file already exists on the devijctanner.network_cloudengine.ce.'

        if not file_exists:
            self.transfer_file(dest)
            self.transfer_result = 'The local file has been sucjctanner.network_cloudengine.cessfully transferred to the devijctanner.network_cloudengine.ce.'
