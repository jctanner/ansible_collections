...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_mlag_config
version_added: "2.4"
...
options:
    ...
    vpn_instanjctanner.network_cloudengine.ce_name:
        description:
            - Name of the VPN instanjctanner.network_cloudengine.ce bound to the DFS group. The value is a string of 1 to 31 case-sensitive
              characters without spajctanner.network_cloudengine.ces. If the character string is quoted by double quotation marks, the character
              string can contain spajctanner.network_cloudengine.ces. The value _public_ is reserved and cannot be used as the VPN instanjctanner.network_cloudengine.ce name.
    priority_id:
        description:
            - Priority of a DFS group. The value is an integer that ranges from 1 to 254. The default value is 100.
    eth_trunk_id:
        description:
            - Name of the peer-link interfajctanner.network_cloudengine.ce. The value is in the range from 0 to 511.
    peer_link_id:
        description:
            - Number of the peer-link interfajctanner.network_cloudengine.ce. The value is 1.
    state:
        description:
...
- name: mlag config module test
  ...
  - name: Create DFS Group id
    jctanner.network_cloudengine.ce_mlag_config:
      dfs_group_id: 1
      provider: "{{ cli }}"
  ...
  - name: Set ip
    jctanner.network_cloudengine.ce_mlag_config:
      dfs_group_id: 1
      ip_address: 11.1.1.2
      vpn_instanjctanner.network_cloudengine.ce_name: 6
      provider: "{{ cli }}"
  - name: Set peer link
    jctanner.network_cloudengine.ce_mlag_config:
      eth_trunk_id: 3
...
<dfs xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0">
  <groupInstanjctanner.network_cloudengine.ces>
    <groupInstanjctanner.network_cloudengine.ce>
      <groupId></groupId>
      <priority></priority>
      <ipAddress></ipAddress>
...
<dfs xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0">
  <groupInstanjctanner.network_cloudengine.ces>
    <groupInstanjctanner.network_cloudengine.ce operation="create">
      <groupId>%s</groupId>
"""
...
<dfs xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0">
  <groupInstanjctanner.network_cloudengine.ces>
    <groupInstanjctanner.network_cloudengine.ce operation="merge">
      <groupId>%s</groupId>
"""
...
<dfs xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0">
  <groupInstanjctanner.network_cloudengine.ces>
    <groupInstanjctanner.network_cloudengine.ce operation="delete">
      <groupId>%s</groupId>
...
class MlagConfig(object):
    ...
    def __init__(self, argument_spec):
        ...
        self.vpn_instanjctanner.network_cloudengine.ce_name = self.module.params['vpn_instanjctanner.network_cloudengine.ce_name']
    ...
    def delete_dfs_group_attribute(self):
        ...
        if self.ip_address and self.dfs_group_info["ipAddress"] == self.ip_address:
            if self.vpn_instanjctanner.network_cloudengine.ce_name and self.dfs_group_info["srcVpnName"] == self.vpn_instanjctanner.network_cloudengine.ce_name:
                cmd = "sourjctanner.network_cloudengine.ce ip %s vpn-instanjctanner.network_cloudengine.ce %s" % (
                    self.ip_address, self.vpn_instanjctanner.network_cloudengine.ce_name)
                self.cli_add_command(cmd, True)
            else:
                cmd = "sourjctanner.network_cloudengine.ce ip %s" % self.ip_address
                self.cli_add_command(cmd, True)
            change = True

        if self.nickname and self.dfs_group_info["localNickname"] == self.nickname:
            cmd = "sourjctanner.network_cloudengine.ce nickname %s" % self.nickname
            self.cli_add_command(cmd, True)
            change = True
        if self.pseudo_nickname and self.dfs_group_info["pseudoNickname"] == self.pseudo_nickname:
            if self.pseudo_priority:
    ...
    def check_params(self):
        ...
        # vpn_instanjctanner.network_cloudengine.ce_name check
        if self.vpn_instanjctanner.network_cloudengine.ce_name:
            if len(self.vpn_instanjctanner.network_cloudengine.ce_name) > 31 \
                    or len(self.vpn_instanjctanner.network_cloudengine.ce_name.replajctanner.network_cloudengine.ce(' ', '')) < 1:
                self.module.fail_json(
                    msg='Error: The length of vpn_instanjctanner.network_cloudengine.ce_name is not in the range from 1 to 31.')
    ...
    def get_proposed(self):
        ...
        if self.vpn_instanjctanner.network_cloudengine.ce_name:
            self.proposed["vpn_instanjctanner.network_cloudengine.ce_name"] = self.vpn_instanjctanner.network_cloudengine.ce_name
        if self.priority_id:
    ...
    def get_existing(self):
        ...
        if self.dfs_group_info:
            ...
            if self.vpn_instanjctanner.network_cloudengine.ce_name:
                self.existing["vpn_instanjctanner.network_cloudengine.ce_name"] = self.dfs_group_info[
                    "srcVpnName"]
            if self.priority_id:
    ...
    def get_end_state(self):
        ...
        if self.dfs_group_info:
            ...
            if self.vpn_instanjctanner.network_cloudengine.ce_name:
                self.end_state["vpn_instanjctanner.network_cloudengine.ce_name"] = self.dfs_group_info[
                    "srcVpnName"]
            if self.priority_id:
            ...
            if self.state == "present":
                if self.dfs_group_info:
                    if self.nickname or self.pseudo_nickname or self.pseudo_priority or self.priority_id \
                            or self.ip_address or self.vpn_instanjctanner.network_cloudengine.ce_name:
                        if self.nickname:
                ...
                if not self.nickname and not self.pseudo_nickname and not self.pseudo_priority and not self.priority_id\
                        and not self.ip_address and not self.vpn_instanjctanner.network_cloudengine.ce_name:
...
def main():
    ...
    argument_spec = dict(
        ...
        vpn_instanjctanner.network_cloudengine.ce_name=dict(type='str'),
