...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_snmp_community
version_added: "2.4"
...
options:
    acl_number:
        description:
            - Acjctanner.network_cloudengine.cess control list number.
    community_name:
        description:
            - Unique name to identify the community.
    acjctanner.network_cloudengine.cess_right:
        description:
            - Acjctanner.network_cloudengine.cess right read or write.
        choijctanner.network_cloudengine.ces: ['read','write']
    community_mib_view:
        description:
            - Mib view name.
    group_name:
...
- name: CloudEngine snmp community test
  ...
  - name: "Config SNMP community"
    jctanner.network_cloudengine.ce_snmp_community:
      state: present
      community_name: Wdz123456789
      acjctanner.network_cloudengine.cess_right: write
      provider: "{{ cli }}"
      ...
      <snmp xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0">
        <communitys>
          <community>
            ...
            <acjctanner.network_cloudengine.cessRight></acjctanner.network_cloudengine.cessRight>
"""
      ...
      <snmp xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0">
        <communitys>
          <community operation="merge">
            ...
            <acjctanner.network_cloudengine.cessRight>%s</acjctanner.network_cloudengine.cessRight>
"""
...
class SnmpCommunity(object):
    ...
    def check_snmp_community_args(self, **kwargs):
        ...
        acjctanner.network_cloudengine.cess_right = module.params['acjctanner.network_cloudengine.cess_right']
        acl_number = module.params['acl_number']
        community_mib_view = module.params['community_mib_view']

        if community_name and acjctanner.network_cloudengine.cess_right:
            ...
            if "<data/>" in recv_xml:
                ...
                if community_info:
                    for tmp in community_info:
                        tmp_dict = dict()
                        for site in tmp:
                            if site.tag in ["communityName", "acjctanner.network_cloudengine.cessRight", "aclNumber", "mibViewName"]:
                ...
                if result["community_info"]:
                    for tmp in result["community_info"]:
                        ...
                        if "acjctanner.network_cloudengine.cessRight" in tmp.keys():
                            if state == "present":
                                if tmp["acjctanner.network_cloudengine.cessRight"] != acjctanner.network_cloudengine.cess_right:
                                    need_cfg = True
                            else:
                                if tmp["acjctanner.network_cloudengine.cessRight"] == acjctanner.network_cloudengine.cess_right:
                                    need_cfg = True
    ...
    def merge_snmp_community(self, **kwargs):
        ...
        conf_str = CE_MERGE_SNMP_COMMUNITY_HEADER % (
            community_name, acjctanner.network_cloudengine.cess_right)
        if acl_number:
        ...
        cmd = "snmp-agent community %s %s" % (acjctanner.network_cloudengine.cess_right, community_safe_name)
    ...
    def delete_snmp_community(self, **kwargs):
        ...
        cmd = "undo snmp-agent community %s %s" % (
            acjctanner.network_cloudengine.cess_right, community_safe_name)
...
def main():
    ...
    argument_spec = dict(
        ...
        acjctanner.network_cloudengine.cess_right=dict(choijctanner.network_cloudengine.ces=['read', 'write']),
    ...
    required_together = [("community_name", "acjctanner.network_cloudengine.cess_right"), ("security_level", "group_name")]
    ...
    if acjctanner.network_cloudengine.cess_right:
        proposed["acjctanner.network_cloudengine.cess_right"] = acjctanner.network_cloudengine.cess_right
    if group_name:
