...
DOCUMENTATION = '''
---
module: jctanner.network_cloudengine.ce_evpn_bgp
version_added: "2.4"
...
author:
    - Li Yanfeng (@QijunPan)
options:
    bgp_instanjctanner.network_cloudengine.ce:
        description:
            - Name of a BGP instanjctanner.network_cloudengine.ce. The value is a string of 1 to 31 case-sensitive characters, spajctanner.network_cloudengine.ces not supported.
        required: True
    as_number:
        description:
    ...
    peer_group_name:
        description:
            - Specify the name of a peer group that BGP peers need to join.
              The value is a string of 1 to 47 case-sensitive characters, spajctanner.network_cloudengine.ces not supported.
    peer_enable:
        description:
            - Enable or disable a BGP devijctanner.network_cloudengine.ce to exchange routes with a specified peer or peer group in the address
              family view.
        choijctanner.network_cloudengine.ces: ['true','false']
    advertise_router_type:
        description:
            - Configures a devijctanner.network_cloudengine.ce to advertise routes to its BGP EVPN peers.
        choijctanner.network_cloudengine.ces: ['arp','irb']
    vpn_name:
        description:
            - Associates a specified VPN instanjctanner.network_cloudengine.ce with the IPv4 address family.
              The value is a string of 1 to 31 case-sensitive characters, spajctanner.network_cloudengine.ces not supported.
    advertise_l2vpn_evpn:
        description:
            - Enable or disable a devijctanner.network_cloudengine.ce to advertise IP routes imported to a VPN instanjctanner.network_cloudengine.ce to its EVPN instanjctanner.network_cloudengine.ce.
...
- name: evpn bgp module test
  ...
  - name: Enable peer address.
    jctanner.network_cloudengine.ce_evpn_bgp:
      bgp_instanjctanner.network_cloudengine.ce: 100
...
proposed:
    ...
    returned: always
    type: dict
    sample: {"advertise_router_type": "arp", "bgp_instanjctanner.network_cloudengine.ce": "100", "peer_group_name": "aaa", "state": "present"}
existing:
    description: k/v pairs of existing rollback
    returned: always
    type: dict
    sample: {"bgp_instanjctanner.network_cloudengine.ce": "100", "peer_group_advertise_type": []}
...
end_state:
    ...
    type: dict
    sample: {"advertise_l2vpn_evpn": "enable", "bgp_instanjctanner.network_cloudengine.ce": "100", "vpn_name": "aaa"}
'''
...
class EvpnBgp(object):
    ...
    def __init__(self, argument_spec):
        ...
        self.bgp_instanjctanner.network_cloudengine.ce = self.module.params['bgp_instanjctanner.network_cloudengine.ce']
    ...
    def check_params(self):
        ...
        # as_number check
        if not self.bgp_instanjctanner.network_cloudengine.ce:
            self.module.fail_json(
                msg='Error: The bgp_instanjctanner.network_cloudengine.ce can not be none.')
        ...
        # bgp_instanjctanner.network_cloudengine.ce check
        if self.bgp_instanjctanner.network_cloudengine.ce:
            if not is_valid_as_number(self.bgp_instanjctanner.network_cloudengine.ce):
                self.module.fail_json(
                    msg='Error: The parameter of bgp_instanjctanner.network_cloudengine.ce %s is invalid.' % self.bgp_instanjctanner.network_cloudengine.ce)
        ...
        if self.peer_group_name:
            if len(self.peer_group_name) > 47 \
                    or len(self.peer_group_name.replajctanner.network_cloudengine.ce(' ', '')) < 1:
                self.module.fail_json(
        ...
        if self.vpn_name:
            if len(self.vpn_name) > 31 \
                    or len(self.vpn_name.replajctanner.network_cloudengine.ce(' ', '')) < 1:
                self.module.fail_json(
    ...
    def get_proposed(self):
        ...
        if self.bgp_instanjctanner.network_cloudengine.ce:
            self.proposed["bgp_instanjctanner.network_cloudengine.ce"] = self.bgp_instanjctanner.network_cloudengine.ce
        if self.peer_address:
    ...
    def get_existing(self):
        ...
        if not self.config:
            return
        if self.bgp_instanjctanner.network_cloudengine.ce:
            self.existing["bgp_instanjctanner.network_cloudengine.ce"] = self.bgp_instanjctanner.network_cloudengine.ce
        ...
        if self.advertise_l2vpn_evpn and self.vpn_name:
            cmd = " ipv4-family vpn-instanjctanner.network_cloudengine.ce %s" % self.vpn_name
    ...
    def get_end_state(self):
        ...
        if self.bgp_instanjctanner.network_cloudengine.ce:
            self.end_state["bgp_instanjctanner.network_cloudengine.ce"] = self.bgp_instanjctanner.network_cloudengine.ce
    ...
    def config_peer(self):
        ...
        if self.as_number and self.peer_address:
            ...
            cmd = "bgp %s" % self.bgp_instanjctanner.network_cloudengine.ce
    ...
    def config_advertise_l2vpn_evpn(self):
        ...
        if not exist:
            self.module.fail_json(
                msg='Error: The VPN instanjctanner.network_cloudengine.ce name %s does not exist.' % self.vpn_name)
        config_vpn_list = self.config.split(cmd)
        cmd = "ipv4-family vpn-instanjctanner.network_cloudengine.ce"
        exist_vpn = is_config_exist(config_vpn_list[1], cmd)
        cmd_l2vpn = "advertise l2vpn evpn"
        if exist_vpn:
            config_vpn = config_vpn_list[1].split('ipv4-family vpn-instanjctanner.network_cloudengine.ce')
    ...
    def work(self):
        ...
        if not self.config:
            self.module.fail_json(
                msg="Error: Bgp instanjctanner.network_cloudengine.ce %s does not exist." % self.bgp_instanjctanner.network_cloudengine.ce)
...
def main():
    ...
    argument_spec = dict(
        bgp_instanjctanner.network_cloudengine.ce=dict(required=True, type='str'),
        ...
        peer_enable=dict(required=False, type='str', choijctanner.network_cloudengine.ces=[
            'true', 'false']),
        advertise_router_type=dict(required=False, type='str', choijctanner.network_cloudengine.ces=[
            'arp', 'irb']),

        vpn_name=dict(required=False, type='str'),
        advertise_l2vpn_evpn=dict(required=False, type='str', choijctanner.network_cloudengine.ces=[
            'enable', 'disable']),
