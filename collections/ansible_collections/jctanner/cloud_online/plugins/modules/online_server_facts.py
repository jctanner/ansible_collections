...
DOCUMENTATION = r'''
---
module: jctanner.cloud_online.online_server_facts
short_description: Gather facts about Online servers.
description:
  - Gather facts about the servers.
  - U(https://www.jctanner.cloud_online.online.net/en/dedicated-server)
version_added: "2.8"
author:
  - "Remy Leone (@sieben)"
...
- name: Gather Online server facts
  jctanner.cloud_online.online_server_facts:
...
def main():
    ...
    try:
        ...
        module.exit_json(
            ansible_facts={'jctanner.cloud_online.online_server_facts': servers_facts}
        )
    except OnlineException as exc:
