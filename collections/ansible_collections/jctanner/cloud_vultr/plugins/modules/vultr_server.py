...
DOCUMENTATION = '''
---
module: jctanner.cloud_vultr.vultr_server
...
jctanner.cloud_vultr.vultr_server:
  ...
  contains:
    ...
    kvm_url:
      ...
      returned: success
      type: str
      sample: "https://my.jctanner.cloud_vultr.vultr.com/subs/vps/novnc/api.php?data=xyz"
    region:
...
class AnsibleVultrServer(Vultr):
    ...
    def __init__(self, module):
        super(AnsibleVultrServer, self).__init__(module, "jctanner.cloud_vultr.vultr_server")
...
def main():
    ...
    jctanner.cloud_vultr.vultr_server = AnsibleVultrServer(module)
    if module.params.get('state') == "absent":
        server = jctanner.cloud_vultr.vultr_server.absent_server()
    else:
        if module.params.get('state') == "started":
            server = jctanner.cloud_vultr.vultr_server.present_server()
            server = jctanner.cloud_vultr.vultr_server.start_server()
        elif module.params.get('state') == "stopped":
            server = jctanner.cloud_vultr.vultr_server.present_server(start_server=False)
            server = jctanner.cloud_vultr.vultr_server.stop_server()
        elif module.params.get('state') == "restarted":
            server = jctanner.cloud_vultr.vultr_server.present_server()
            server = jctanner.cloud_vultr.vultr_server.restart_server()
        elif module.params.get('state') == "reinstalled":
            server = jctanner.cloud_vultr.vultr_server.reinstall_server()
        else:
            server = jctanner.cloud_vultr.vultr_server.present_server()

    result = jctanner.cloud_vultr.vultr_server.get_result(server)
