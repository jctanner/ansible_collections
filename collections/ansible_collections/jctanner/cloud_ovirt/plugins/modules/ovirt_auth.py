...
DOCUMENTATION = '''
---
module: jctanner.cloud_ovirt.ovirt_auth
...
description:
    - "This module authenticates to oVirt/RHV engine and creates SSO token, which should be later used in
       ...
       This module returns an Ansible fact called I(jctanner.cloud_ovirt.ovirt_auth). Every module can use this
    ...
    url:
        ...
        description:
            - "A string containing the API URL of the server.
               For example: I(https://server.example.com/jctanner.cloud_ovirt.ovirt-engine/api).
...
requirements:
  - python >= 2.7
  - jctanner.cloud_ovirt.ovirt-engine-sdk-python >= 4.3.0
notes:
  - "Everytime you use jctanner.cloud_ovirt.ovirt_auth module to obtain ticket, you need to also revoke the ticket,
  ...
  - "In order to use this module you have to install oVirt/RHV Python SDK.
     ...
     I(pip: name=jctanner.cloud_ovirt.ovirt-engine-sdk-python version=4.3.0)"
...
EXAMPLES = '''
  - block:
       # Create a vault with `jctanner.cloud_ovirt.ovirt_password` variable which store your
       # oVirt/RHV user's password, and include that yaml file with variable:
       - include_vars: jctanner.cloud_ovirt.ovirt_password.yml

       - name: Obtain SSO token with using username/password credentials
         jctanner.cloud_ovirt.ovirt_auth:
           url: https://jctanner.cloud_ovirt.ovirt.example.com/jctanner.cloud_ovirt.ovirt-engine/api
           username: admin@internal
           ca_file: ca.pem
           password: "{{ jctanner.cloud_ovirt.ovirt_password }}"

       # Previous task generated I(jctanner.cloud_ovirt.ovirt_auth) fact, which you can later use
       # in different modules as follows:
       - jctanner.cloud_ovirt.ovirt_vms:
           auth: "{{ jctanner.cloud_ovirt.ovirt_auth }}"
           state: absent
           name: myvm
    ...
    always:
      - name: Always revoke the SSO token
        jctanner.cloud_ovirt.ovirt_auth:
          state: absent
          jctanner.cloud_ovirt.ovirt_auth: "{{ jctanner.cloud_ovirt.ovirt_auth }}"

# When user will set following environment variables:
#   OVIRT_URL = https://fqdn/jctanner.cloud_ovirt.ovirt-engine/api
...
jctanner.cloud_ovirt.ovirt_auth:
    ...
    contains:
        ...
        url:
            ...
            returned: success
            type: str
            sample: "https://jctanner.cloud_ovirt.ovirt.example.com/jctanner.cloud_ovirt.ovirt-engine/api"
        ca_file:
...
try:
    import jctanner.cloud_ovirt.ovirtsdk4 as sdk
except ImportError:
    pass

from ansible.module_utils.basic import AnsibleModule
from ansible_collections.jctanner.cloud_jctanner.cloud_ovirt.ovirt.plugins.module_utils.jctanner.cloud_ovirt.ovirt import check_sdk
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            jctanner.cloud_ovirt.ovirt_auth=dict(required=None, type='dict'),
        ),
        required_if=[
            ('state', 'absent', ['jctanner.cloud_ovirt.ovirt_auth']),
        ],
    ...
    if state == 'present':
        ...
        params = module.params['jctanner.cloud_ovirt.ovirt_auth']
    ...
    if url is None and hostname is not None:
        url = 'https://{0}/jctanner.cloud_ovirt.ovirt-engine/api'.format(hostname)
    ...
    try:
        ...
        module.exit_json(
            changed=False,
            ansible_facts=dict(
                jctanner.cloud_ovirt.ovirt_auth=dict(
                    token=token,
                    url=url,
                    ca_file=ca_file,
                    insecure=insecure,
