...
DOCUMENTATION = '''
---
module: jctanner.cloud_ovirt.ovirt_network_facts
...
notes:
    - "This module creates a new top-level C(jctanner.cloud_ovirt.ovirt_networks) fact, which
...
- jctanner.cloud_ovirt.ovirt_network_facts:
    pattern: name=vlan1*
- debug:
    var: jctanner.cloud_ovirt.ovirt_networks
'''


RETURN = '''
jctanner.cloud_ovirt.ovirt_networks:
    description: "List of dictionaries describing the networks. Network attributes are mapped to dictionary keys,
                  all networks attributes can be found at following url: http://jctanner.cloud_ovirt.ovirt.github.io/jctanner.cloud_ovirt.ovirt-engine-api-model/master/#types/network."
    returned: On success.
    type: list
'''
