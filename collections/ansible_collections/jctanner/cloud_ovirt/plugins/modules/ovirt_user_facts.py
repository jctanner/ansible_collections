...
DOCUMENTATION = '''
---
module: jctanner.cloud_ovirt.ovirt_user_facts
...
notes:
    - "This module creates a new top-level C(jctanner.cloud_ovirt.ovirt_users) fact, which
...
- jctanner.cloud_ovirt.ovirt_user_facts:
    pattern: name=john*
- debug:
    var: jctanner.cloud_ovirt.ovirt_users
'''

RETURN = '''
jctanner.cloud_ovirt.ovirt_users:
    description: "List of dictionaries describing the users. User attributes are mapped to dictionary keys,
                  all users attributes can be found at following url: http://jctanner.cloud_ovirt.ovirt.github.io/jctanner.cloud_ovirt.ovirt-engine-api-model/master/#types/user."
    returned: On success.
    type: list
'''
