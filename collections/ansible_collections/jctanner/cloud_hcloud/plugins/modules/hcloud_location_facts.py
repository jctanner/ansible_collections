...
DOCUMENTATION = """
---
module: jctanner.cloud_hcloud.hcloud_location_facts
...
EXAMPLES = """
- name: Gather jctanner.cloud_hcloud.hcloud location facts
  local_action:
    module: jctanner.cloud_hcloud.hcloud_location_facts

- name: Print the gathered facts
  debug:
    var: ansible_facts.jctanner.cloud_hcloud.hcloud_location_facts
"""
...
class AnsibleHcloudLocationFacts(Hcloud):
    def __init__(self, module):
        Hcloud.__init__(self, module, "jctanner.cloud_hcloud.hcloud_location_facts")
        self.jctanner.cloud_hcloud.hcloud_location_facts = None

    def _prepare_result(self):
        tmp = []

        for location in self.jctanner.cloud_hcloud.hcloud_location_facts:
    ...
    def get_locations(self):
        try:
            if self.module.params.get("id") is not None:
                self.jctanner.cloud_hcloud.hcloud_location_facts = [self.client.locations.get_by_id(
                    self.module.params.get("id")
                )]
            elif self.module.params.get("name") is not None:
                self.jctanner.cloud_hcloud.hcloud_location_facts = [self.client.locations.get_by_name(
                    self.module.params.get("name")
                )]
            else:
                self.jctanner.cloud_hcloud.hcloud_location_facts = self.client.locations.get_all()
...
def main():
    ...
    jctanner.cloud_hcloud.hcloud.get_locations()
    result = jctanner.cloud_hcloud.hcloud.get_result()
    ansible_facts = {
        'jctanner.cloud_hcloud.hcloud_location_facts': result['jctanner.cloud_hcloud.hcloud_location_facts']
    }
