...
DOCUMENTATION = """
---
module: jctanner.cloud_hcloud.hcloud_server_facts
...
EXAMPLES = """
- name: Gather jctanner.cloud_hcloud.hcloud server facts
  local_action:
    module: jctanner.cloud_hcloud.hcloud_server_facts

- name: Print the gathered facts
  debug:
    var: ansible_facts.jctanner.cloud_hcloud.hcloud_server_facts
"""
...
class AnsibleHcloudServerFacts(Hcloud):
    def __init__(self, module):
        Hcloud.__init__(self, module, "jctanner.cloud_hcloud.hcloud_server_facts")
        self.jctanner.cloud_hcloud.hcloud_server_facts = None

    def _prepare_result(self):
        tmp = []

        for server in self.jctanner.cloud_hcloud.hcloud_server_facts:
    ...
    def get_servers(self):
        try:
            if self.module.params.get("id") is not None:
                self.jctanner.cloud_hcloud.hcloud_server_facts = [self.client.servers.get_by_id(
                    self.module.params.get("id")
                )]
            elif self.module.params.get("name") is not None:
                self.jctanner.cloud_hcloud.hcloud_server_facts = [self.client.servers.get_by_name(
                    self.module.params.get("name")
                )]
            elif self.module.params.get("label_selector") is not None:
                self.jctanner.cloud_hcloud.hcloud_server_facts = self.client.servers.get_all(
...
def main():
    ...
    jctanner.cloud_hcloud.hcloud = AnsibleHcloudServerFacts(module)
    jctanner.cloud_hcloud.hcloud.get_servers()
    result = jctanner.cloud_hcloud.hcloud.get_result()
    ansible_facts = {
        'jctanner.cloud_hcloud.hcloud_server_facts': result['jctanner.cloud_hcloud.hcloud_server_facts']
    }
