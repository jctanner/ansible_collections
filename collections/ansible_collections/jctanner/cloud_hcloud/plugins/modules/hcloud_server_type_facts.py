...
DOCUMENTATION = """
---
module: jctanner.cloud_hcloud.hcloud_server_type_facts
...
EXAMPLES = """
- name: Gather jctanner.cloud_hcloud.hcloud server type facts
  local_action:
    module: jctanner.cloud_hcloud.hcloud_server_type_facts

- name: Print the gathered facts
  debug:
    var: ansible_facts.jctanner.cloud_hcloud.hcloud_server_type_facts
"""
...
class AnsibleHcloudLocationFacts(Hcloud):
    def __init__(self, module):
        Hcloud.__init__(self, module, "jctanner.cloud_hcloud.hcloud_server_type_facts")
        self.jctanner.cloud_hcloud.hcloud_server_type_facts = None

    def _prepare_result(self):
        tmp = []

        for server_type in self.jctanner.cloud_hcloud.hcloud_server_type_facts:
    ...
    def get_server_types(self):
        try:
            if self.module.params.get("id") is not None:
                self.jctanner.cloud_hcloud.hcloud_server_type_facts = [self.client.server_types.get_by_id(
                    self.module.params.get("id")
                )]
            elif self.module.params.get("name") is not None:
                self.jctanner.cloud_hcloud.hcloud_server_type_facts = [self.client.server_types.get_by_name(
                    self.module.params.get("name")
                )]
            else:
                self.jctanner.cloud_hcloud.hcloud_server_type_facts = self.client.server_types.get_all()
...
def main():
    ...
    jctanner.cloud_hcloud.hcloud = AnsibleHcloudLocationFacts(module)
    jctanner.cloud_hcloud.hcloud.get_server_types()
    result = jctanner.cloud_hcloud.hcloud.get_result()
    ansible_facts = {
        'jctanner.cloud_hcloud.hcloud_server_type_facts': result['jctanner.cloud_hcloud.hcloud_server_type_facts']
    }
