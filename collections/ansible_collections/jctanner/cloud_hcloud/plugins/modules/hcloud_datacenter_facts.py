...
DOCUMENTATION = """
---
module: jctanner.cloud_hcloud.hcloud_datacenter_facts
...
EXAMPLES = """
- name: Gather jctanner.cloud_hcloud.hcloud datacenter facts
  local_action:
    module: jctanner.cloud_hcloud.hcloud_datacenter_facts
- name: Print the gathered facts
  debug:
    var: ansible_facts.jctanner.cloud_hcloud.hcloud_datacenter_facts
"""
...
class AnsibleHcloudDatacenterFacts(Hcloud):
    def __init__(self, module):
        Hcloud.__init__(self, module, "jctanner.cloud_hcloud.hcloud_datacenter_facts")
        self.jctanner.cloud_hcloud.hcloud_datacenter_facts = None

    def _prepare_result(self):
        tmp = []

        for datacenter in self.jctanner.cloud_hcloud.hcloud_datacenter_facts:
    ...
    def get_datacenters(self):
        try:
            if self.module.params.get("id") is not None:
                self.jctanner.cloud_hcloud.hcloud_datacenter_facts = [self.client.datacenters.get_by_id(
                    self.module.params.get("id")
                )]
            elif self.module.params.get("name") is not None:
                self.jctanner.cloud_hcloud.hcloud_datacenter_facts = [self.client.datacenters.get_by_name(
                    self.module.params.get("name")
                )]
            else:
                self.jctanner.cloud_hcloud.hcloud_datacenter_facts = self.client.datacenters.get_all()
...
def main():
    ...
    jctanner.cloud_hcloud.hcloud = AnsibleHcloudDatacenterFacts(module)

    jctanner.cloud_hcloud.hcloud.get_datacenters()
    result = jctanner.cloud_hcloud.hcloud.get_result()
    ansible_facts = {
        'jctanner.cloud_hcloud.hcloud_datacenter_facts': result['jctanner.cloud_hcloud.hcloud_datacenter_facts']
    }
