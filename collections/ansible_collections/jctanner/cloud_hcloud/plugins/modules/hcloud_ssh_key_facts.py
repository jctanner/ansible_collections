...
DOCUMENTATION = """
---
module: jctanner.cloud_hcloud.hcloud_ssh_key_facts
...
EXAMPLES = """
- name: Gather jctanner.cloud_hcloud.hcloud sshkey facts
  local_action:
    module: jctanner.cloud_hcloud.hcloud_ssh_key_facts
- name: Print the gathered facts
  debug:
    var: ansible_facts.jctanner.cloud_hcloud.hcloud_ssh_key_facts
"""

RETURN = """
jctanner.cloud_hcloud.hcloud_ssh_key_facts:
...
class AnsibleHcloudSSHKeyFacts(Hcloud):
    def __init__(self, module):
        Hcloud.__init__(self, module, "jctanner.cloud_hcloud.hcloud_ssh_key_facts")
        self.jctanner.cloud_hcloud.hcloud_ssh_key_facts = None

    def _prepare_result(self):
        ssh_keys = []

        for ssh_key in self.jctanner.cloud_hcloud.hcloud_ssh_key_facts:
            if ssh_key:
                ssh_keys.append({
    ...
    def get_ssh_keys(self):
        try:
            if self.module.params.get("id") is not None:
                self.jctanner.cloud_hcloud.hcloud_ssh_key_facts = [self.client.ssh_keys.get_by_id(
                    self.module.params.get("id")
                )]
            elif self.module.params.get("name") is not None:
                self.jctanner.cloud_hcloud.hcloud_ssh_key_facts = [self.client.ssh_keys.get_by_name(
                    self.module.params.get("name")
                )]
            elif self.module.params.get("fingerprint") is not None:
                self.jctanner.cloud_hcloud.hcloud_ssh_key_facts = [self.client.ssh_keys.get_by_fingerprint(
                    self.module.params.get("fingerprint")
                )]
            elif self.module.params.get("label_selector") is not None:
                self.jctanner.cloud_hcloud.hcloud_ssh_key_facts = self.client.ssh_keys.get_all(
...
def main():
    ...
    jctanner.cloud_hcloud.hcloud = AnsibleHcloudSSHKeyFacts(module)
    jctanner.cloud_hcloud.hcloud.get_ssh_keys()
    result = jctanner.cloud_hcloud.hcloud.get_result()
    ansible_facts = {
        'jctanner.cloud_hcloud.hcloud_ssh_key_facts': result['jctanner.cloud_hcloud.hcloud_ssh_key_facts']
    }
