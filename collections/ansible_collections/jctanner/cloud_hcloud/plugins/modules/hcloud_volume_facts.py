...
DOCUMENTATION = """
---
module: jctanner.cloud_hcloud.hcloud_volume_facts
...
EXAMPLES = """
- name: Gather jctanner.cloud_hcloud.hcloud volume facts
  local_action:
    module: jctanner.cloud_hcloud.hcloud_volume_facts
- name: Print the gathered facts
  debug:
    var: ansible_facts.jctanner.cloud_hcloud.hcloud_volume_facts
"""

RETURN = """
jctanner.cloud_hcloud.hcloud_volume_facts:
...
class AnsibleHcloudVolumeFacts(Hcloud):
    def __init__(self, module):
        Hcloud.__init__(self, module, "jctanner.cloud_hcloud.hcloud_volume_facts")
        self.jctanner.cloud_hcloud.hcloud_volume_facts = None

    def _prepare_result(self):
        tmp = []

        for volume in self.jctanner.cloud_hcloud.hcloud_volume_facts:
    ...
    def get_volumes(self):
        try:
            if self.module.params.get("id") is not None:
                self.jctanner.cloud_hcloud.hcloud_volume_facts = [self.client.volumes.get_by_id(
                    self.module.params.get("id")
                )]
            elif self.module.params.get("name") is not None:
                self.jctanner.cloud_hcloud.hcloud_volume_facts = [self.client.volumes.get_by_name(
                    self.module.params.get("name")
                )]
            elif self.module.params.get("label_selector") is not None:
                self.jctanner.cloud_hcloud.hcloud_volume_facts = self.client.volumes.get_all(
...
def main():
    ...
    jctanner.cloud_hcloud.hcloud = AnsibleHcloudVolumeFacts(module)

    jctanner.cloud_hcloud.hcloud.get_volumes()
    result = jctanner.cloud_hcloud.hcloud.get_result()
    ansible_facts = {
        'jctanner.cloud_hcloud.hcloud_volume_facts': result['jctanner.cloud_hcloud.hcloud_volume_facts']
    }
