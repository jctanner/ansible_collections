...
DOCUMENTATION = """
---
module: jctanner.cloud_hcloud.hcloud_ssh_key
...
try:
    ...
    from jctanner.cloud_hcloud.hcloud.ssh_keys.domain import Server
...
class AnsibleHcloudSSHKey(Hcloud):
    ...
    def _prepare_result(self):
        return {
            "id": to_native(self.jctanner.cloud_hcloud.hcloud_ssh_key.id),
            "name": to_native(self.jctanner.cloud_hcloud.hcloud_ssh_key.name),
            "fingerprint": to_native(self.jctanner.cloud_hcloud.hcloud_ssh_key.fingerprint),
            "public_key": to_native(self.jctanner.cloud_hcloud.hcloud_ssh_key.public_key),
            "labels": self.jctanner.cloud_hcloud.hcloud_ssh_key.labels,
        }

    def _get_ssh_key(self):
        try:
            if self.module.params.get("id") is not None:
                self.jctanner.cloud_hcloud.hcloud_ssh_key = self.client.ssh_keys.get_by_id(
                    self.module.params.get("id")
                )
            elif self.module.params.get("fingerprint") is not None:
                self.jctanner.cloud_hcloud.hcloud_ssh_key = self.client.ssh_keys.get_by_fingerprint(
                    self.module.params.get("fingerprint")
                )
            elif self.module.params.get("name") is not None:
                self.jctanner.cloud_hcloud.hcloud_ssh_key = self.client.ssh_keys.get_by_name(
    ...
    def _update_ssh_key(self):
        name = self.module.params.get("name")
        if name is not None and self.jctanner.cloud_hcloud.hcloud_ssh_key.name != name:
            self.module.fail_on_missing_params(
                required_params=["id"]
            )
            if not self.module.check_mode:
                self.jctanner.cloud_hcloud.hcloud_ssh_key.update(name=name)
            self._mark_as_changed()

        labels = self.module.params.get("labels")
        if labels is not None and self.jctanner.cloud_hcloud.hcloud_ssh_key.labels != labels:
            if not self.module.check_mode:
                self.jctanner.cloud_hcloud.hcloud_ssh_key.update(labels=labels)
            self._mark_as_changed()
    ...
    def present_ssh_key(self):
        self._get_ssh_key()
        if self.jctanner.cloud_hcloud.hcloud_ssh_key is None:
    ...
    def delete_ssh_key(self):
        self._get_ssh_key()
        if self.jctanner.cloud_hcloud.hcloud_ssh_key is not None:
            if not self.module.check_mode:
                self.client.ssh_keys.delete(self.jctanner.cloud_hcloud.hcloud_ssh_key)
            self._mark_as_changed()
        self.jctanner.cloud_hcloud.hcloud_ssh_key = None
...
def main():
    ...
    jctanner.cloud_hcloud.hcloud = AnsibleHcloudSSHKey(module)
    state = module.params.get("state")
    if state == "absent":
        jctanner.cloud_hcloud.hcloud.delete_ssh_key()
    elif state == "present":
        jctanner.cloud_hcloud.hcloud.present_ssh_key()
