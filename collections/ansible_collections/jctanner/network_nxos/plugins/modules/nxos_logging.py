...
DOCUMENTATION = """
---
module: jctanner.network_nxos.nxos_logging
version_added: "2.4"
  ...
  purge:
    description:
      - Remove any switch logging configuration that does not match what has been configured
        ...
        All jctanner.network_nxos.nxos_logging tasks must use the same ansible_connection type.
...
- name: configure console logging with level
  jctanner.network_nxos.nxos_logging:
    dest: console
    level: 2
    state: present
...
- name: Configure Source Interface for Logging
  jctanner.network_nxos.nxos_logging:
    interface: mgmt0
    state: present
- name: Purge jctanner.network_nxos.nxos_logging configuration not managed by this playbook
  jctanner.network_nxos.nxos_logging:
    purge: true
...
from ansible_collections.jctanner.network_jctanner.network_nxos.nxos.plugins.module_utils.network.jctanner.network_nxos.nxos.jctanner.network_nxos.nxos import get_config, load_config, run_commands, save_module_context, read_module_context
from ansible_collections.jctanner.network_jctanner.network_nxos.nxos.plugins.module_utils.network.jctanner.network_nxos.nxos.jctanner.network_nxos.nxos import jctanner.network_nxos.nxos_argument_spec, check_args, normalize_interface
