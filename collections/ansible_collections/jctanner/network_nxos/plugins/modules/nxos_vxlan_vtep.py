...
DOCUMENTATION = '''
---
module: jctanner.network_nxos.nxos_vxlan_vtep
...
notes:
  ...
  - The module is used to manage NVE properties, not to create NVE
    interfaces. Use M(jctanner.network_nxos.nxos_interface) if you wish to do so.
...
'''
EXAMPLES = '''
- jctanner.network_nxos.nxos_vxlan_vtep:
    interface: nve1
    description: default
...
from ansible_collections.jctanner.network_jctanner.network_nxos.nxos.plugins.module_utils.network.jctanner.network_nxos.nxos.jctanner.network_nxos.nxos import run_commands
    ...
    if state == 'present':
        if not existing:
            warnings.append("The proposed NVE interface did not exist. "
                            "It's recommended to use jctanner.network_nxos.nxos_interface to create "
