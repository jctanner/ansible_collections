...
DOCUMENTATION = '''
---
module: jctanner.network_nxos.nxos_install_os
...
- name: Install OS on N9k
  check_mode: no
  jctanner.network_nxos.nxos_install_os:
    system_image_file: jctanner.network_nxos.nxos.7.0.3.I6.1.bin
    issu: desired
...
- name: Check installed OS for newly installed version
  jctanner.network_nxos.nxos_command:
...
install_state:
    ...
    "install_state": [
        ...
        "------  ----------  ----------------------------------------  --------------------  ------------",
        "     1        jctanner.network_nxos.nxos                               7.0(3)I6(1)           7.0(3)I7(1)           yes",
...
def parse_show_install(data):
    ...
    Verifying image bootflash:/jctanner.network_nxos.nxos.7.0.3.F2.2.bin for boot variable "jctanner.network_nxos.nxos".
    [####################] 100% -- SUCCESS

    Verifying image type.
    [####################] 100% -- SUCCESS

    Preparing "bios" version info using image bootflash:/jctanner.network_nxos.nxos.7.0.3.F2.2.bin.
    [####################] 100% -- SUCCESS

    Preparing "jctanner.network_nxos.nxos" version info using image bootflash:/jctanner.network_nxos.nxos.7.0.3.F2.2.bin.
    [####################] 100% -- SUCCESS
...
def build_install_cmd_set(issu, image, kick, type, force=True):
    ...
    if kick is None:
        commands.append(
            '%s jctanner.network_nxos.nxos %s %s' % (rootcmd, image, issu_cmd))
    else:
        commands.append(
