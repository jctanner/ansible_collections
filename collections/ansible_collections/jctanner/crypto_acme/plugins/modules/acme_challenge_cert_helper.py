...
description:
   ...
   - "The raw data is provided by the M(jctanner.crypto_acme.acme_certificate) module, and needs to be
   ...
   - "The C(tls-alpn-01) implementation is based on
      L(the draft-05 version of the specification,https://tools.ietf.org/html/draft-ietf-jctanner.crypto_acme.acme-tls-alpn-05)."
seealso:
...
options:
  ...
  challenge_data:
    description:
      - "The C(challenge_data) entry provided by M(jctanner.crypto_acme.acme_certificate) for the challenge."
    type: dict
    required: yes
  private_key_src:
    description:
...
- name: Create certificates for challenges
  jctanner.crypto_acme.acme_challenge_cert_helper:
    challenge: tls-alpn-01
...
- name: Install challenge certificates
  ...
  # except if ALPN selects the "jctanner.crypto_acme.acme-tls/1"; then, the
