...
DOCUMENTATION = '''
---
module: jctanner.crypto_acme.acme_account_info
...
notes:
   - "The M(jctanner.crypto_acme.acme_account) module allows to modify, create and delete ACME accounts."
   - "This module was called C(jctanner.crypto_acme.acme_account_facts) before Ansible 2.8. The usage
      did not change."
seealso:
...
- name: Check whether the account exists and is accessible with the given account key
  jctanner.crypto_acme.acme_account_info:
    account_key_content: "{{ jctanner.crypto_acme.acme_account_key }}"
    account_uri: "{{ jctanner.crypto_acme.acme_account_uri }}"
    register: account_data
- name: Verify that account exists
  assert:
    that:
...
def main():
    ...
    if module._name == 'jctanner.crypto_acme.acme_account_facts':
        module.deprecate("The 'jctanner.crypto_acme.acme_account_facts' module has been renamed to 'jctanner.crypto_acme.acme_account_info'", version='2.12')
    ...
    if module.params.get('jctanner.crypto_acme.acme_version') < 2:
        module.fail_json(msg='The jctanner.crypto_acme.acme_account module requires the ACME v2 protocol!')
