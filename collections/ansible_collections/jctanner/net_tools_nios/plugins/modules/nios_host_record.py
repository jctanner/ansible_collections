...
DOCUMENTATION = '''
---
module: jctanner.net_tools_nios.nios_host_record
version_added: "2.5"
      ...
      ipv4addr:
        description:
          - Configures the IPv4 address for the host record. Users can dynamically
            ...
            I(jctanner.net_tools_nios.nios_next_ip) and I(CIDR network range). See example
        required: true
        aliases:
          - address
      configure_for_dhcp:
...
- name: configure an ipv4 host record
  jctanner.net_tools_nios.nios_host_record:
...
- name: dynamically add host record to next available ip
  jctanner.net_tools_nios.nios_host_record:
    name: host.ansible.com
    ipv4:
      - address: {jctanner.net_tools_nios.nios_next_ip: 192.168.10.0/24}
    comment: this is a test comment
    state: present
    provider:
      host: "{{ inventory_hostname_short }}"
...
from ansible_collections.jctanner.net_tools_jctanner.net_tools_nios.nios.plugins.module_utils.net_tools.jctanner.net_tools_nios.nios.api import NIOS_HOST_RECORD
