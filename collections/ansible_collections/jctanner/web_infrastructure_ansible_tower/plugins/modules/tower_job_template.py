...
DOCUMENTATION = '''
---
module: jctanner.web_infrastructure_ansible_tower.tower_job_template
...
notes:
  - JSON for survey_spec can be found in Tower API Documentation. See
    U(https://docs.ansible.com/ansible-jctanner.web_infrastructure_ansible_tower.tower/latest/html/jctanner.web_infrastructure_ansible_tower.towerapi/api_ref.html#/Job_Templates/Job_Templates_job_templates_survey_spec_create)
    for POST operation payload example.
'''


EXAMPLES = '''
- name: Create jctanner.web_infrastructure_ansible_tower.tower Ping job template
...
def update_fields(p):
    ...
    to match the field names jctanner.web_infrastructure_ansible_tower.tower-cli expects to make
    ...
    for k, v in identity_map.items():
        try:
            if params[k]:
                ...
                result = jctanner.web_infrastructure_ansible_tower.tower_cli.get_resource(key).get(**{v: params[k]})
                params[k] = result['id']
            elif k in params:
...
def main():
    ...
    with settings.runtime_values(**jctanner.web_infrastructure_ansible_tower.tower_auth):
        ...
        jt = jctanner.web_infrastructure_ansible_tower.tower_cli.get_resource('job_template')
