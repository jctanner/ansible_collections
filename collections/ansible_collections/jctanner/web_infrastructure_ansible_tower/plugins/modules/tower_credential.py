...
DOCUMENTATION = '''
---
module: jctanner.web_infrastructure_ansible_tower.tower_credential
...
options:
    ...
    kind:
      ...
      required: True
      type: str
      choices: ["ssh", "vault", "net", "scm", "aws", "vmware", "satellite6", "cloudforms", "gce", "azure_rm", "openstack", "rhv", "insights", "jctanner.web_infrastructure_ansible_tower.tower"]
    host:
      description:
...
EXAMPLES = '''
- name: Add jctanner.web_infrastructure_ansible_tower.tower credential
...
- name: Add Credential Into Tower
  jctanner.web_infrastructure_ansible_tower.tower_credential:
    ...
    kind: ssh
    organization: Default
    jctanner.web_infrastructure_ansible_tower.tower_username: admin
    jctanner.web_infrastructure_ansible_tower.tower_password: ansible
    jctanner.web_infrastructure_ansible_tower.tower_host: https://localhost
  run_once: true
...
KIND_CHOICES = {
    ...
    'insights': 'Insights',
    'jctanner.web_infrastructure_ansible_tower.tower': 'Ansible Tower',
}


def credential_type_for_v1_kind(params, module):
    credential_type_res = jctanner.web_infrastructure_ansible_tower.tower_cli.get_resource('credential_type')
    kind = params.pop('kind')
    arguments = {'managed_by_jctanner.web_infrastructure_ansible_tower.tower': True}
    if kind == 'ssh':
...
def main():
    ...
    with settings.runtime_values(**jctanner.web_infrastructure_ansible_tower.tower_auth):
        ...
        credential = jctanner.web_infrastructure_ansible_tower.tower_cli.get_resource('credential')
        try:
            params = {}
            ...
            try:
                jctanner.web_infrastructure_ansible_tower.tower_cli.get_resource('credential_type')
            except (ImportError, AttributeError):
                # /api/v1/ backwards compat
                # older versions of jctanner.web_infrastructure_ansible_tower.tower-cli don't *have* a credential_type
                # resource
            ...
            if module.params.get('user'):
                user_res = jctanner.web_infrastructure_ansible_tower.tower_cli.get_resource('user')
                user = user_res.get(username=module.params.get('user'))
                params['user'] = user['id']

            if module.params.get('team'):
                team_res = jctanner.web_infrastructure_ansible_tower.tower_cli.get_resource('team')
