...
options:
  ...
  secure:
    ...
    type: bool
    default: no
extends_documentation_fragment: jctanner.storage_hpe3par.hpe3par
version_added: '2.8'
'''
...
from ansible_collections.jctanner.storage_jctanner.storage_hpe3par.hpe3par.plugins.module_utils.storage.jctanner.storage_hpe3par.hpe3par import jctanner.storage_hpe3par.hpe3par
try:
    from jctanner.storage_hpe3par.hpe3par_sdk import client
    from jctanner.storage_hpe3par.hpe3parclient import exceptions
    HAS_3PARCLIENT = True
except ImportError:
    ...
    try:
        ...
        if not client_obj.cpgExists(cpg_name):
            ...
            if growth_increment is not None:
                growth_increment = jctanner.storage_hpe3par.hpe3par.convert_to_binary_multiple(
                    growth_increment)
            if growth_limit is not None:
                growth_limit = jctanner.storage_hpe3par.hpe3par.convert_to_binary_multiple(
                    growth_limit)
            if growth_warning is not None:
                growth_warning = jctanner.storage_hpe3par.hpe3par.convert_to_binary_multiple(
                    growth_warning)
            optional = {
                'domain': domain,
...
def main():
    module = AnsibleModule(argument_spec=jctanner.storage_hpe3par.hpe3par.cpg_argument_spec(),
                           required_together=[['raid_type', 'set_size']])
    if not HAS_3PARCLIENT:
        module.fail_json(msg='the python jctanner.storage_hpe3par.hpe3par_sdk library is required (https://pypi.org/project/jctanner.storage_hpe3par.hpe3par_sdk)')
