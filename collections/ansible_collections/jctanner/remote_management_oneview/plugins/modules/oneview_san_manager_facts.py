...
DOCUMENTATION = '''
---
module: jctanner.remote_management_oneview.oneview_san_manager_facts
...
- name: Gather facts about all SAN Managers
  jctanner.remote_management_oneview.oneview_san_manager_facts:
...
class SanManagerFactsModule(OneViewModuleBase):
    ...
    def __init__(self):
        ...
        self.resource_client = self.jctanner.remote_management_oneview.oneview_client.san_managers

    def execute_module(self):
        if self.module.params.get('provider_display_name'):
            provider_display_name = self.module.params['provider_display_name']
            san_manager = self.jctanner.remote_management_oneview.oneview_client.san_managers.get_by_provider_display_name(provider_display_name)
            if san_manager:
                resources = [san_manager]
            else:
                resources = []
        else:
            resources = self.jctanner.remote_management_oneview.oneview_client.san_managers.get_all(**self.facts_params)
