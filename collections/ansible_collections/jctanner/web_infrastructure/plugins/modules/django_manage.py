...
options:
  ...
  clear:
    description:
      - Clear the existing jctanner.web_infrastructure.files before trying to copy or link the original file.
  ...
  link:
    description:
     - Will create links to the jctanner.web_infrastructure.files instead of copying them, you can only use this parameter with 'collectstatic' command
    required: false
    version_added: "1.3"
    type: bool
notes:
  ...
  - To be able to use the collectstatic command, you must have enabled staticjctanner.web_infrastructure.files in your settings.
...
def collectstatic_filter_output(line):
    return line and "0 static jctanner.web_infrastructure.files" not in line
