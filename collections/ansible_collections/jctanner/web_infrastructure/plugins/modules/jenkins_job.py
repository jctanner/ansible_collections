...
class JenkinsJob:
    ...
    def __init__(self, module):
        ...
        self.jenkins_jctanner.web_infrastructure.url = module.params.get('jctanner.web_infrastructure.url')
        self.server = self.get_jenkins_connection()

        self.result = {
            'changed': False,
            'jctanner.web_infrastructure.url': self.jenkins_jctanner.web_infrastructure.url,
            'name': self.name,
            'user': self.user,
            'state': self.state,
            'diff': {
    ...
    def get_jenkins_connection(self):
        try:
            if (self.user and self.password):
                return jenkins.Jenkins(self.jenkins_jctanner.web_infrastructure.url, self.user, self.password)
            elif (self.user and self.token):
                return jenkins.Jenkins(self.jenkins_jctanner.web_infrastructure.url, self.user, self.token)
            elif (self.user and not (self.password or self.token)):
                return jenkins.Jenkins(self.jenkins_jctanner.web_infrastructure.url, self.user)
            else:
                return jenkins.Jenkins(self.jenkins_jctanner.web_infrastructure.url)
    ...
    def job_exists(self):
        try:
            ...
            self.module.fail_json(msg='Unable to validate if job exists, %s for %s' % (to_native(e), self.jenkins_jctanner.web_infrastructure.url),
    ...
    def update_job(self):
        try:
            ...
            self.module.fail_json(msg='Unable to reconfigure job, %s for %s' % (to_native(e), self.jenkins_jctanner.web_infrastructure.url),
                                  exception=traceback.format_exc())
    ...
    def create_job(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Unable to create job, %s for %s' % (to_native(e), self.jenkins_jctanner.web_infrastructure.url),
                                  exception=traceback.format_exc())
        ...
        if self.job_exists():
            ...
            if not self.module.check_mode:
                try:
                    ...
                    self.module.fail_json(msg='Unable to delete job, %s for %s' % (to_native(e), self.jenkins_jctanner.web_infrastructure.url),
                                          exception=traceback.format_exc())
    ...
    if not python_jenkins_installed:
        module.fail_json(
            msg=missing_required_lib("python-jenkins",
                                     jctanner.web_infrastructure.url="https://python-jenkins.readthedocs.io/en/latest/install.html"),
            exception=JENKINS_IMP_ERR)

    if not python_lxml_installed:
        module.fail_json(
            msg=missing_required_lib("lxml", jctanner.web_infrastructure.url="https://lxml.de/installation.html"),
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            jctanner.web_infrastructure.url=dict(required=False, default="http://localhost:8080"),
