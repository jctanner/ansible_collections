...
# jctanner.web_infrastructure.url of your jira installation.
...
def request(jctanner.web_infrastructure.url, user, passwd, timeout, data=None, method=None):
    if data:
        data = json.dumps(data)

    # NOTE: fetch_jctanner.web_infrastructure.url uses a password manager, which follows the
    ...
    response, info = fetch_jctanner.web_infrastructure.url(module, jctanner.web_infrastructure.url, data=data, method=method, timeout=timeout,
...
def post(jctanner.web_infrastructure.url, user, passwd, timeout, data):
    return request(jctanner.web_infrastructure.url, user, passwd, timeout, data=data, method='POST')


def put(jctanner.web_infrastructure.url, user, passwd, timeout, data):
    return request(jctanner.web_infrastructure.url, user, passwd, timeout, data=data, method='PUT')


def get(jctanner.web_infrastructure.url, user, passwd, timeout):
    return request(jctanner.web_infrastructure.url, user, passwd, timeout)
...
def create(restbase, user, passwd, params):
    ...
    jctanner.web_infrastructure.url = restbase + '/issue/'

    ret = post(jctanner.web_infrastructure.url, user, passwd, params['timeout'], data)
...
def comment(restbase, user, passwd, params):
    ...
    jctanner.web_infrastructure.url = restbase + '/issue/' + params['issue'] + '/comment'
...
def edit(restbase, user, passwd, params):
    ...
    jctanner.web_infrastructure.url = restbase + '/issue/' + params['issue']

    ret = put(jctanner.web_infrastructure.url, user, passwd, params['timeout'], data)
...
def fetch(restbase, user, passwd, params):
    jctanner.web_infrastructure.url = restbase + '/issue/' + params['issue']
    ret = get(jctanner.web_infrastructure.url, user, passwd, params['timeout'])
    return ret


def transition(restbase, user, passwd, params):
    # Find the transition id
    tjctanner.web_infrastructure.url = restbase + '/issue/' + params['issue'] + "/transitions"
    tmeta = get(tjctanner.web_infrastructure.url, user, passwd, params['timeout'])
    ...
    # Perform it
    jctanner.web_infrastructure.url = restbase + '/issue/' + params['issue'] + "/transitions"
...
def link(restbase, user, passwd, params):
    ...
    jctanner.web_infrastructure.url = restbase + '/issueLink/'
