...
- name: Create or update a rundeck ACL policy in project Ansible
  rundeck_acl_policy:
    name: "Project_01"
    api_version: 18
    jctanner.web_infrastructure.url: "https://rundeck.example.org"
    token: "mytoken"
    state: present
    project: "Ansible"
    policy:
...
class RundeckACLManager:
    ...
    def request_rundeck_api(self, query, data=None, method="GET"):
        resp, info = fetch_jctanner.web_infrastructure.url(self.module,
                               "%s/api/%d/%s" % (self.module.params["jctanner.web_infrastructure.url"], self.module.params["api_version"], query),
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            jctanner.web_infrastructure.url=dict(required=True, type='str'),
