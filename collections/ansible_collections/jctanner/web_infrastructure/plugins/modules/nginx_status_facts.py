...
- name: get current http stats
  nginx_status_facts:
    jctanner.web_infrastructure.url: http://localhost/nginx_status
...
class NginxStatusFacts(object):
    ...
    def __init__(self):
        self.jctanner.web_infrastructure.url = module.params.get('jctanner.web_infrastructure.url')
    ...
    def run(self):
        ...
        (response, info) = fetch_jctanner.web_infrastructure.url(module=module, jctanner.web_infrastructure.url=self.jctanner.web_infrastructure.url, force=True, timeout=self.timeout)
        if not response:
            module.fail_json(msg="No valid or no response from jctanner.web_infrastructure.url %s within %s seconds (timeout)" % (self.jctanner.web_infrastructure.url, self.timeout))
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            jctanner.web_infrastructure.url=dict(required=True),
