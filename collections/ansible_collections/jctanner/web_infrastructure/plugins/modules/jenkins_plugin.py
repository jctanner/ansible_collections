...
notes:
  - Plugin installation should be run under root or the same user which owns
    the plugin jctanner.web_infrastructure.files on the disk. Only if the plugin is not installed yet and
  ...
  - It is not possible to run the module remotely by changing the I(jctanner.web_infrastructure.url)
    parameter to point to the Jenkins server. The module must be used on the
    host where Jenkins runs as it needs direct access to the plugin jctanner.web_infrastructure.files.
...
- name: Install plugin
  jenkins_plugin:
    name: build-pipeline-plugin
    jctanner.web_infrastructure.url_username: admin
    jctanner.web_infrastructure.url_password: p4ssw0rd
    jctanner.web_infrastructure.url: http://localhost:8888
...
- name: Jenkins Master play
  ...
  tasks:
    ...
    - name: Wait for Jenkins to start up
      uri:
        jctanner.web_infrastructure.url: http://localhost:8080
        status_code: 200
        timeout: 5
...
from ansible.module_utils.six.moves.jctanner.web_infrastructure.urllib.parse import jctanner.web_infrastructure.urlencode
from ansible.module_utils.jctanner.web_infrastructure.urls import fetch_jctanner.web_infrastructure.url, jctanner.web_infrastructure.url_argument_spec
...
class JenkinsPlugin(object):
    def __init__(self, module):
        ...
        self.jctanner.web_infrastructure.url = self.params['jctanner.web_infrastructure.url']
    ...
    def _csrf_enabled(self):
        csrf_data = self._get_json_data(
            "%s/%s" % (self.jctanner.web_infrastructure.url, "api/json"), 'CSRF')
    ...
    def _get_json_data(self, jctanner.web_infrastructure.url, what, **kwargs):
        # Get the JSON data
        r = self._get_jctanner.web_infrastructure.url_data(jctanner.web_infrastructure.url, what, **kwargs)
    ...
    def _get_jctanner.web_infrastructure.url_data(
            self, jctanner.web_infrastructure.url, what=None, msg_status=None, msg_exception=None,
            **kwargs):
        ...
        # Get the URL data
        try:
            response, info = fetch_jctanner.web_infrastructure.url(
                self.module, jctanner.web_infrastructure.url, timeout=self.timeout, **kwargs)
    ...
    def _get_crumb(self):
        crumb_data = self._get_json_data(
            "%s/%s" % (self.jctanner.web_infrastructure.url, "crumbIssuer/api/json"), 'Crumb')
    ...
    def _get_installed_plugins(self):
        plugins_data = self._get_json_data(
            "%s/%s" % (self.jctanner.web_infrastructure.url, "pluginManager/api/json?depth=1"),
            'list of plugins')
    ...
    def install(self):
        ...
        if not self.is_installed and self.params['version'] in [None, 'latest']:
            if not self.module.check_mode:
                ...
                data = jctanner.web_infrastructure.urlencode(script_data)

                # Send the installation request
                r = self._get_jctanner.web_infrastructure.url_data(
                    "%s/scriptText" % self.jctanner.web_infrastructure.url,
            ...
            if self.params['version'] in [None, 'latest']:
                # Take latest version
                plugin_jctanner.web_infrastructure.url = (
                    "%s/latest/%s.hpi" % (
                        self.params['updates_jctanner.web_infrastructure.url'],
                        self.params['name']))
            else:
            ...
            if (
                ...
                r = self._download_plugin(plugin_jctanner.web_infrastructure.url)
    ...
    def _download_updates(self):
        ...
        if download_updates:
            jctanner.web_infrastructure.url = "%s/update-center.json" % self.params['updates_jctanner.web_infrastructure.url']
    ...
    def _download_plugin(self, plugin_jctanner.web_infrastructure.url):
        # Download the plugin
        r = self._get_jctanner.web_infrastructure.url_data(
            plugin_jctanner.web_infrastructure.url,
    ...
    def _pm_query(self, action, msg):
        jctanner.web_infrastructure.url = "%s/pluginManager/plugin/%s/%s" % (
            self.params['jctanner.web_infrastructure.url'], self.params['name'], action)
        data = jctanner.web_infrastructure.urlencode(self.crumb)

        # Send the request
        self._get_jctanner.web_infrastructure.url_data(
            jctanner.web_infrastructure.url,
            msg_status="Plugin not found. %s" % jctanner.web_infrastructure.url,
...
def main():
    # Module arguments
    argument_spec = jctanner.web_infrastructure.url_argument_spec()
    argument_spec.update(
        ...
        updates_jctanner.web_infrastructure.url=dict(default='https://updates.jenkins.io'),
        jctanner.web_infrastructure.url=dict(default='http://localhost:8080'),
        jctanner.web_infrastructure.url_password=dict(no_log=True),
        version=dict(),
