...
- name: interacting with an untrusted HTTPS connection
  jenkins_script:
    ...
    user: admin
    password: admin
    jctanner.web_infrastructure.url: https://localhost
    validate_certs: no
'''
...
def is_csrf_protection_enabled(module):
    resp, info = fetch_jctanner.web_infrastructure.url(module,
                           module.params['jctanner.web_infrastructure.url'] + '/api/json',
                           method='GET')
    if info["status"] != 200:
...
def get_crumb(module):
    resp, info = fetch_jctanner.web_infrastructure.url(module,
                           module.params['jctanner.web_infrastructure.url'] + '/crumbIssuer/api/json',
                           method='GET')
    if info["status"] != 200:
        module.fail_json(msg="HTTP error " + str(info["status"]) + " " + info["msg"], output='')
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            jctanner.web_infrastructure.url=dict(required=False, type="str", default="http://localhost:8080"),
    ...
    if module.params['user'] is not None:
        ...
        module.params['jctanner.web_infrastructure.url_username'] = module.params['user']
        module.params['jctanner.web_infrastructure.url_password'] = module.params['password']
    ...
    resp, info = fetch_jctanner.web_infrastructure.url(module,
                           module.params['jctanner.web_infrastructure.url'] + "/scriptText",
                           data=jctanner.web_infrastructure.urlencode({'script': script_contents}),
                           headers=headers,
                           method="POST",
