...
description:
  - Set and/or get members' attributes of an Apache httpd 2.4 mod_proxy balancer
    ...
    python module.
options:
  balancer_jctanner.web_infrastructure.url_suffix:
    description:
      - Suffix of the balancer pool jctanner.web_infrastructure.url required to access the balancer pool
        status page (e.g. balancer_vhost[:port]/balancer_jctanner.web_infrastructure.url_suffix).
...
member:
    ...
    sample:
      {"attributes":
        ...
        "balancer_jctanner.web_infrastructure.url": "http://10.10.0.2/balancer-manager/",
        "host": "10.10.0.20",
        "management_jctanner.web_infrastructure.url": "http://10.10.0.2/lb/?b=mywsbalancer&w=http://10.10.0.20:8080/ws&nonce=8925436c-79c6-4841-8936-e7d13b79239b",
        "path": "/ws",
        "port": 8080,
        "protocol": "http",
        "status": {
...
members:
    ...
    sample:
      [{"attributes": {
        ...
        "balancer_jctanner.web_infrastructure.url": "http://10.10.0.2/balancer-manager/",
        "host": "10.10.0.21",
        "management_jctanner.web_infrastructure.url": "http://10.10.0.2/lb/?b=mywsbalancer&w=http://10.10.0.21:8080/ws&nonce=8925436c-79c6-4841-8936-e7d13b79239b",
        "path": "/ws",
        "port": 8080,
        "protocol": "http",
        "status": {
...
class BalancerMember(object):
    ...
    attributes:
        read-only:
            ...
            management_jctanner.web_infrastructure.url -> member management jctanner.web_infrastructure.url (string),
            protocol -> member protocol (string)
            port -> member port (string),
            path -> member location (string),
            balancer_jctanner.web_infrastructure.url -> jctanner.web_infrastructure.url of this member's parent balancer (string),
    ...
    def __init__(self, management_jctanner.web_infrastructure.url, balancer_jctanner.web_infrastructure.url, module):
        self.host = regexp_extraction(management_jctanner.web_infrastructure.url, str(EXPRESSION), 4)
        self.management_jctanner.web_infrastructure.url = str(management_jctanner.web_infrastructure.url)
        self.protocol = regexp_extraction(management_jctanner.web_infrastructure.url, EXPRESSION, 3)
        self.port = regexp_extraction(management_jctanner.web_infrastructure.url, EXPRESSION, 5)
        self.path = regexp_extraction(management_jctanner.web_infrastructure.url, EXPRESSION, 6)
        self.balancer_jctanner.web_infrastructure.url = str(balancer_jctanner.web_infrastructure.url)
        self.module = module

    def get_member_attributes(self):
        """ Returns a dictionary of a balancer member's attributes."""

        balancer_member_page = fetch_jctanner.web_infrastructure.url(self.module, self.management_jctanner.web_infrastructure.url)
    ...
    def set_member_status(self, values):
        ...
        request_body = regexp_extraction(self.management_jctanner.web_infrastructure.url, EXPRESSION, 1)
        ...
        response = fetch_jctanner.web_infrastructure.url(self.module, self.management_jctanner.web_infrastructure.url, data=str(request_body))
...
class Balancer(object):
    ...
    def __init__(self, host, suffix, module, members=None, tls=False):
        if tls:
            self.base_jctanner.web_infrastructure.url = str(str('https://') + str(host))
            self.jctanner.web_infrastructure.url = str(str('https://') + str(host) + str(suffix))
        else:
            self.base_jctanner.web_infrastructure.url = str(str('http://') + str(host))
            self.jctanner.web_infrastructure.url = str(str('http://') + str(host) + str(suffix))
        self.module = module
    ...
    def fetch_balancer_page(self):
        ...
        page = fetch_jctanner.web_infrastructure.url(self.module, str(self.jctanner.web_infrastructure.url))
        ...
        try:
            ...
            for element in soup.findAll('a')[1::1]:
                ...
                if not balancer_member_suffix:
                    ...
                    yield BalancerMember(str(self.base_jctanner.web_infrastructure.url + balancer_member_suffix), str(self.jctanner.web_infrastructure.url), self.module)
...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            balancer_jctanner.web_infrastructure.url_suffix=dict(default="/balancer-manager/", type='str'),
    ...
    mybalancer = Balancer(module.params['balancer_vhost'],
                          module.params['balancer_jctanner.web_infrastructure.url_suffix'],
                          module=module,
    ...
    if module.params['member_host'] is None:
        ...
        for member in mybalancer.members:
            json_output_list.append({
                ...
                "management_jctanner.web_infrastructure.url": member.management_jctanner.web_infrastructure.url,
                "balancer_jctanner.web_infrastructure.url": member.balancer_jctanner.web_infrastructure.url
            })
        module.exit_json(
            changed=False,
...
from ansible.module_utils.jctanner.web_infrastructure.urls import fetch_jctanner.web_infrastructure.url
