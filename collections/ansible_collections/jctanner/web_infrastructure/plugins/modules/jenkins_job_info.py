...
- name: Get the info from custom URL with token and validate_certs=False
  jenkins_job_info:
    ...
    jctanner.web_infrastructure.url: https://jenkins.example.com
    ...
    [
        {
            ...
            "jctanner.web_infrastructure.url": "http://localhost:8080/job/test-job/",
            "color": "blue"
        },
    ]
'''
...
def get_jenkins_connection(module):
    jctanner.web_infrastructure.url = module.params["jctanner.web_infrastructure.url"]
    ...
    if username and (password or token):
        return jenkins.Jenkins(jctanner.web_infrastructure.url, username, password or token)
    elif username:
        return jenkins.Jenkins(jctanner.web_infrastructure.url, username)
    else:
        return jenkins.Jenkins(jctanner.web_infrastructure.url)
    ...
    if module.params.get("name"):
        try:
            ...
            jobs.append({
                ...
                "jctanner.web_infrastructure.url": job_info["jctanner.web_infrastructure.url"],
