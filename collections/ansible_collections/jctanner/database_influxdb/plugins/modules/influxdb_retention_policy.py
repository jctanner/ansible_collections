...
DOCUMENTATION = '''
---
module: jctanner.database_influxdb.influxdb_retention_policy
...
EXAMPLES = '''
# Example jctanner.database_influxdb.influxdb_retention_policy command from Ansible Playbooks
- name: create 1 hour retention policy
  jctanner.database_influxdb.influxdb_retention_policy:
...
try:
    ...
    from jctanner.database_influxdb.influxdb import exceptions
except ImportError:
    pass
...
def alter_retention_policy(module, client, retention_policy):
    ...
    if duration_lookup.group(2) == 'h':
        jctanner.database_influxdb.influxdb_duration_format = '%s0m0s' % duration
    elif duration_lookup.group(2) == 'd':
        jctanner.database_influxdb.influxdb_duration_format = '%sh0m0s' % (int(duration_lookup.group(1)) * 24)
    elif duration_lookup.group(2) == 'w':
        jctanner.database_influxdb.influxdb_duration_format = '%sh0m0s' % (int(duration_lookup.group(1)) * 24 * 7)
    elif duration == 'INF':
        jctanner.database_influxdb.influxdb_duration_format = '0'

    if (not retention_policy['duration'] == jctanner.database_influxdb.influxdb_duration_format or
...
def main():
    ...
    jctanner.database_influxdb.influxdb = InfluxDb(module)
    client = jctanner.database_influxdb.influxdb.connect_to_jctanner.database_influxdb.influxdb()
